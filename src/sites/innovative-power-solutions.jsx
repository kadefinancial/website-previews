// Layout: D | Industry: Electrician | City: North Little Rock
import { useState, useEffect, useRef } from "react";

const PHONE = "(501) 851-9990";
const TEL = "tel:+15018519990";
...
const Icons = {
  ...
  gauge: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/><path d="M16.24 7.76l-1.41 1.41"/><path d="M18 12h-2"/></svg>,
...
const services = [
  { name: "Commercial Electrical Contracting", desc: "Full-service electrical contracting for commercial and industrial facilities. From new construction to tenant improvements, IPS delivers code-compliant installations on schedule and on budget.", icon: Icons.building },
  { name: "Energy Management & Audits", desc: "Comprehensive energy audits and management solutions that reduce operating costs. We analyze your facility's electrical consumption and implement strategies that deliver measurable savings.", icon: Icons.gauge },
  { name: "Lighting Retrofits & Upgrades", desc: "LED conversions, high-bay replacements, and intelligent lighting controls for warehouses, offices, and production floors. Reduce energy costs while improving visibility and safety.", icon: Icons.bulb },
  { name: "EV Charger Installation", desc: "Commercial EV charging station design and installation for parking structures, fleet facilities, and corporate campuses. Future-proof your property with Level 2 and DC fast charging.", icon: Icons.plug },
  { name: "Emergency Standby Power Systems", desc: "Generator installation, automatic transfer switches, and UPS systems that keep your operations running when the grid goes down. Designed for mission-critical uptime.", icon: Icons.zap },
  { name: "Voice, Communications & Security", desc: "Low-voltage cabling, structured data networks, access control, and security systems. Integrated solutions that keep your facility connected and protected.", icon: Icons.shield },
];
...
const areas = ["North Little Rock", "Little Rock", "Central Arkansas", "Southeast US"];

const reviews = [
  { text: "Great professional work and friendly people. Would highly recommend.", name: "Jack Stillblud", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function InnovativePowerSolutions() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("ips-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="roof"){sessionStorage.setItem("ips-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "Commercial Electrician North Little Rock AR | Innovative Power Solutions";
    const els = [];
    ...
    add("meta",{name:"description",content:"Commercial & industrial electrical contractor in North Little Rock, AR. Serving Fortune 500 clients since 2003. Call (501) 851-9990."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"Innovative Power Solutions",telephone:"+15018519990",
      address:{"@type":"PostalAddress",streetAddress:"4101 Richards Rd",addressLocality:"North Little Rock",addressRegion:"AR",postalCode:"72113",addressCountry:"US"},
      ...
      founder:{"@type":"Person",name:"Joe Rice",jobTitle:"Owner & President"},
      foundingDate:"2003",
      description:"Innovative Power Solutions is a full-service commercial and industrial electrical contractor in North Little Rock, AR. Founded in 2003, serving Fortune 500 clients across the Southeast US.",
      areaServed:areas.map(a=>({"@type":"City",name:a})),
      hasOfferCatalog:{"@type":"OfferCatalog",name:"Commercial Electrical Services",itemListElement:allServiceNames.map(s=>({"@type":"Offer",itemOffered:{"@type":"Service",name:s}}))},
      memberOf:[],sameAs:[],
    })});
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1C2B41"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8}}><span style={{fontWeight:700,color:"#1C2B41"}}>Innovative</span> <span style={{color:"#D4A03C"}}>Power Solutions</span></div>
  ...
  return (<>
    <style>{`
      ...
      :root{--primary:#1C2B41;--accent:#D4A03C;--cream:#FAF9F6;--white:#FFFFFF;--text:#2A2A3C;--text-mid:#5A5A6E;--text-light:#8A8A9A;--border:rgba(0,0,0,0.06)}
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(28,43,65,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}><span>Innovative</span> <span style={{color:"var(--accent)"}}>Power</span></div>
      ...
      <div className="hero-grid" style={{maxWidth:1200,width:"100%",display:"grid",gridTemplateColumns:"1.8fr 1fr",gap:48,alignItems:"center"}}>
        <div>
          <h1 style={{fontSize:"clamp(48px, 6vw, 84px)",lineHeight:1.0,color:"var(--primary)",letterSpacing:-2,marginBottom:24}}>
            North Little Rock's<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Commercial</span><br/>Electrical Experts
          </h1>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:32}}>
            <Stars />
            <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"Great professional work and friendly people." — Jack Stillblud</span>
          </div>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(212,160,60,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Our Team — {PHONE}</a>
        </div>
        <div>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",color:"var(--primary)",letterSpacing:-0.5}}>Commercial Electrical Services in North Little Rock</h2>
        </div>
        {/* Mobile trust */}
        <div className="trust-mobile" style={{display:"none",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:48,textAlign:"center"}}>
          {[{num:"5.0",label:"Google Rating",desc:"Perfect score"},{num:"2003",label:"Est. Founded",desc:"Over two decades"},{num:"200+",label:"Team Members",desc:"Full-service crew"},{num:"Sonoco",label:"Fortune 500 Client",desc:"Trusted at scale"}].map((s,i)=>(
            <div key={i}>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",position:"relative",overflow:"hidden"}}>
      <div className="serif" style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",fontSize:"clamp(120px,20vw,280px)",color:"var(--accent)",opacity:0.08,lineHeight:1,whiteSpace:"nowrap",pointerEvents:"none"}}>2003</div>
      <div style={{maxWidth:800,margin:"0 auto",position:"relative",zIndex:1}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,textAlign:"center",opacity:0.7}}>About Innovative Power Solutions</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff",textAlign:"center"}}>Why Industry Leaders Trust<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Joe Rice</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:24,textAlign:"center"}}>
          When Joe Rice founded Innovative Power Solutions in 2003, he set out to build more than an electrical company. He built an organization that Fortune 500 companies and industrial clients could rely on for mission-critical power infrastructure. From a small operation in North Little Rock, IPS has grown to a team of over 200 professionals serving facilities across Central Arkansas and the Southeast United States.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:36,textAlign:"center"}}>
          That commitment to excellence earned the trust of Sonoco and other Fortune 500 clients who demand precision, reliability, and zero downtime. Under Joe's leadership alongside co-owner William Wey, IPS has become the region's go-to firm for commercial electrical contracting, energy management, and emergency power systems. Every project is approached with the same standard: deliver on time, on budget, and built to last.
        </p>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:20}}>
          {["Founded 2003 — Over 20 Years","200+ Team Members","Fortune 500 Client Partnerships","Full-Service Electrical Contractor","Licensed, Bonded & Insured","Commercial & Industrial Specialists"].map((c,i)=>(
    ...
    <section id="reviews" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--white)"}}>
      <div style={{maxWidth:800,margin:"0 auto"}}>
        ...
        <p style={{textAlign:"center",marginTop:8,fontSize:13,color:"var(--text-light)"}}> Perfect 5.0 rating across 8 reviews on Google</p>
    ...
    <section id="areas" style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        ...
        <h2 style={{fontSize:"clamp(28px,3vw,38px)",color:"var(--primary)",marginBottom:48,letterSpacing:-0.5}}>Serving Central Arkansas & the Southeast</h2>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:12}}>
          {areas.map((a,i)=>(
            <span key={i} title={`Commercial Electrician in ${a}${a.includes("US")||a.includes("Arkansas")?"":`, AR`}`} style={{display:"inline-flex",alignItems:"center",gap:6,background:"#fff",border:"1px solid var(--border)",padding:"12px 24px",borderRadius:30,fontSize:15,fontWeight:500,color:"var(--text)",cursor:"default",transition:"all 0.25s"}}
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>Need a commercial electrician?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call our team.</span></h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>Serving Fortune 500 clients since 2003. 5.0 stars on Google. Get it done right.</p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16}}><span>Innovative</span> <span style={{color:"var(--accent)"}}>Power</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>Full-service commercial and industrial electrical contractor serving Central Arkansas and the Southeast since 2003.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>4101 Richards Rd<br/>North Little Rock, AR 72113<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
          <p style={{fontSize:13,opacity:0.5,marginTop:10}}>Mon–Fri: 7 AM – 5 PM</p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} Innovative Power Solutions. All rights reserved.</span>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Innovative Power Solutions</a>
    </div>
