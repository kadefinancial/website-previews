...
// --- Data ---
const PHONE = "(501) 612-8080";
const PHONE_TEL = "tel:+15016128080";

const services = [
  { name: "Air Conditioning Repair & Service", desc: "When your AC fails in an Arkansas summer, you need it fixed fast. Our team diagnoses problems accurately and restores your comfort with dependable repairs and routine maintenance that keeps your system running strong.", icon: Icons.wind },
  { name: "Heating Repair & Service", desc: "From furnace malfunctions to heat pump issues, we handle every heating problem with skill and urgency. Stay warm through Central Arkansas winters with reliable repairs and seasonal tune-ups.", icon: Icons.flame },
  { name: "HVAC Installation", desc: "New construction or upgrading an outdated system, we size and install heating and cooling equipment the right way. Proper installation means better efficiency, longer system life, and lower energy bills.", icon: Icons.thermometer },
  { name: "HVAC System Replacement", desc: "When repair costs outweigh the value of your aging system, we help you choose the right replacement. Honest recommendations, professional installation, and systems built to last.", icon: Icons.wrench },
  { name: "24/7 Emergency Service", desc: "Heating and cooling emergencies don't wait for business hours. Around the Clock means exactly that -- call any time, day or night, and our team responds fast when you need us most.", icon: Icons.zap },
  { name: "Multi-Property HVAC Service", desc: "Managing rentals or commercial properties? We provide consistent, reliable HVAC service across multiple locations so every property stays comfortable and every tenant stays happy.", icon: Icons.building },
];

const allServices = [
  "Air Conditioning Repair & Service", "Heating Repair & Service", "HVAC Installation",
  "HVAC System Replacement", "24/7 Emergency Service", "Multi-Property HVAC Service"
];

const areas = ["Little Rock", "Central Arkansas"];

// --- Main Component ---
export default function AroundTheClockHeatingAndAir() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("atc-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("atc-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "HVAC Service Little Rock AR | Around the Clock Heating & Air";
    const els = [];
    ...
    add("meta", { name: "description", content: "Around the Clock Heating & Air — 24/7 HVAC service in Little Rock, AR. 45+ reviews. Licensed & insured. Call (501) 612-8080." });
    ...
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      ...
      name: "Around the Clock Heating & Air", telephone: "+15016128080",
      email: "Aroundtheclockservice1@gmail.com",
      address: { "@type": "PostalAddress", streetAddress: "4106 Grant Ln", addressLocality: "Little Rock", addressRegion: "AR", postalCode: "72206", addressCountry: "US" },
      ...
      description: "Family-owned HVAC company serving Little Rock and Central Arkansas. 24/7 emergency heating and cooling service. Licensed, bonded, and insured.",
        ...
        <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#1C2B41" }}>
          <div style={{ background:"#fff", borderRadius:16, padding:"56px 44px", textAlign:"center", maxWidth:420, width:"92%", boxShadow:"0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:28, fontWeight:600, letterSpacing:-0.5, marginBottom:4 }}>
              <span style={{ color:"#1C2B41" }}>Around the Clock</span>{" "}<span style={{ color:"#C8934F" }}>Heating & Air</span>
            </div>
    ...
    <>
      ...
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display:"flex", alignItems:"center", justifyContent:"space-between", background: scrolled ? "rgba(28,43,65,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition:"all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        <div className="serif" style={{ fontSize:22, fontWeight:600, letterSpacing:-0.5, color:"#fff" }}>
          <span>Around the Clock</span>{" "}<span style={{ color:"var(--accent)" }}>Heating & Air</span>
        </div>
        ...
        <div className="hero-grid" style={{ maxWidth:1200, width:"100%", display:"grid", gridTemplateColumns:"1fr 1fr", gap:64, alignItems:"center" }}>
          ...
          <div style={{ maxWidth:560 }}>
            <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(255,255,255,0.1)", padding:"8px 18px", borderRadius:4, marginBottom:28 }}>
              ...
              <span style={{ fontSize:12, fontWeight:700, letterSpacing:3, textTransform:"uppercase", color:"var(--accent)" }}>24/7 HVAC Service</span>
            </div>
            <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(40px,5vw,68px)", lineHeight:1.05, letterSpacing:-1.5, color:"#fff", marginBottom:24 }}>
              Little Rock's{" "}<br/>
              <em style={{ color:"var(--accent)", fontStyle:"italic" }}>Around the Clock</em><br/>
              HVAC Experts
            </h1>
            <p style={{ fontSize:18, lineHeight:1.7, color:"rgba(255,255,255,0.75)", marginBottom:32, maxWidth:480 }}>
              When your heating or cooling breaks down, time matters. Our family-owned team answers the call 24 hours a day, 7 days a week -- delivering honest, dependable HVAC service across Central Arkansas.
            </p>
            {/* Star row with hero quote -- showing 5 stars for this specific 5-star review */}
            <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:32, flexWrap:"wrap" }}>
              <Stars />
              <span style={{ fontSize:14, color:"rgba(255,255,255,0.7)", fontStyle:"italic" }}>"No better HVAC repair company in central AR." -- Jonathon Cross</span>
            </div>
      ...
      <section style={{ background:"var(--white)", padding:"48px clamp(24px,5vw,64px)" }}>
        <div className="trust-grid" style={{ maxWidth:1200, margin:"0 auto", display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:32, textAlign:"center" }}>
          {[
            { num: "45+", label: "Customer Reviews", desc: "Trusted by homeowners" },
            { num: "24/7", label: "Always Available", desc: "Day or night, every day" },
            { num: "Licensed", label: "Bonded & Insured", desc: "Full protection" },
            { num: "Family", label: "Owned & Operated", desc: "Personal service" },
          ].map((s, i) => (
      ...
      <section id="about" className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          <div style={{ textAlign:"center", marginBottom:64 }}>
            ...
            <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(30px,3.5vw,44px)", lineHeight:1.15, letterSpacing:-0.5, color:"var(--primary)" }}>Family-Owned HVAC in Little Rock</h2>
          </div>
          <div className="why-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:64, alignItems:"start" }}>
            ...
            <div>
              <p style={{ fontSize:17, lineHeight:1.8, color:"var(--text-mid)", marginBottom:20 }}>
                Around the Clock Heating & Air was built on a simple promise: when your heating or cooling fails, someone should answer the phone. Based out of Little Rock, this family-owned company has earned the trust of homeowners and property managers across Central Arkansas by showing up when it matters most -- 24 hours a day, 7 days a week, no exceptions.
              </p>
              <p style={{ fontSize:17, lineHeight:1.8, color:"var(--text-mid)", marginBottom:32 }}>
                What sets our team apart is the combination of genuine care and professional expertise. From single-home repairs to multi-property maintenance, every job receives the same level of attention and honesty. We believe in doing the work right, explaining what we find, and charging a fair price. That philosophy has earned us 45+ reviews from customers who keep calling back year after year.
              </p>
              ...
              <div style={{ position:"relative", borderLeft:"3px solid var(--accent)", paddingLeft:24, marginBottom:36 }}>
                ...
                <p style={{ fontSize:18, fontStyle:"italic", lineHeight:1.7, color:"var(--primary)", fontFamily:"'Playfair Display',serif" }}>
                  "We have been using Around the Clock Heating and Air for years. They installed two heat & air units for us -- we were so satisfied that we call them for our rentals too."
                </p>
                <p style={{ fontSize:14, color:"var(--text-light)", marginTop:8, fontFamily:"'DM Sans',sans-serif" }}>-- Gale Rosby</p>
              </div>
              {/* Credentials */}
              <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                {[
                  ...
                  "Multi-Property HVAC Specialists",
            ...
            <div className="badge-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
              {[
                { icon: Icons.clock, title: "24/7 Service", desc: "Heating and cooling emergencies don't keep business hours. We answer every call, day or night, 365 days a year." },
                { icon: Icons.users, title: "Family-Owned", desc: "Our team treats every customer like a neighbor. Local roots, personal attention, and a name we stand behind." },
                { icon: Icons.shield, title: "Licensed & Insured", desc: "Fully licensed, bonded, and insured for your complete peace of mind on every job we perform." },
                { icon: Icons.building, title: "Multi-Property", desc: "From single homes to rental portfolios, we provide consistent HVAC service across every property you manage." },
              ].map((b, i) => (
        ...
        <div style={{ maxWidth:800, margin:"0 auto", textAlign:"center" }}>
          ...
          <div style={{ position:"relative", maxWidth:700, margin:"0 auto" }}>
            ...
            <p style={{ fontSize:"clamp(18px,2.5vw,22px)", lineHeight:1.7, color:"var(--text)", fontStyle:"italic", fontFamily:"'Playfair Display',serif", marginBottom:28, maxWidth:600, margin:"0 auto 28px" }}>
              "I would give them 10 stars if I could. There is no better HVAC repair company in central AR. I was referred to them several years ago and they are always my first and only call if I ever need help with heating or AC issues."
            </p>
            <p style={{ fontSize:17, fontWeight:700, color:"var(--primary)", marginBottom:4 }}>Jonathon Cross</p>
            <p style={{ fontSize:14, color:"var(--text-light)" }}>Google Review</p>
          </div>
          <p style={{ fontSize:14, color:"var(--text-light)", marginTop:40 }}>One of 45+ five-star reviews on Google</p>
          ...
          <div style={{ maxWidth:600, margin:"48px auto 0", padding:"36px 30px", background:"var(--cream)", borderRadius:12, border:"1px solid var(--border)" }}>
            ...
            <p style={{ fontSize:16, lineHeight:1.7, color:"var(--text)", fontStyle:"italic", marginBottom:16 }}>
              "I recently had my air conditioner replaced and I couldn't be more satisfied. From the moment I called, their customer service was friendly, responsive, and professional. The technician arrived right on time, explained everything clearly, and worked efficiently."
            </p>
            <p style={{ fontSize:15, fontWeight:700, color:"var(--primary)", marginBottom:2 }}>Lesley Stewart</p>
      ...
      <section className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--white)", borderTop:"1px solid var(--border)" }}>
        <div style={{ maxWidth:700, margin:"0 auto", textAlign:"center" }}>
          <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(30px,3.5vw,44px)", lineHeight:1.15, letterSpacing:-0.5, color:"var(--primary)", marginBottom:20 }}>
            ...
            <em style={{ color:"var(--accent)", fontStyle:"italic" }}>Call our team.</em>
          </h2>
          <p style={{ fontSize:17, lineHeight:1.7, color:"var(--text-mid)", marginBottom:36, maxWidth:520, margin:"0 auto 36px" }}>
            Family-owned, licensed, bonded, and insured. Open 24 hours a day, 7 days a week. Honest pricing and dependable HVAC service for Little Rock and all of Central Arkansas.
          </p>
      ...
      <footer style={{ background:"var(--primary)", padding:"80px clamp(24px,5vw,64px) 40px", color:"rgba(255,255,255,0.75)" }}>
        <div className="footer-grid" style={{ maxWidth:1200, margin:"0 auto", display:"grid", gridTemplateColumns:"1.4fr 1fr 1fr", gap:48, marginBottom:56 }}>
          ...
          <div>
            <div className="serif" style={{ fontSize:22, fontWeight:600, letterSpacing:-0.5, marginBottom:16 }}>
              <span style={{ color:"#fff" }}>Around the Clock</span>{" "}<span style={{ color:"var(--accent)" }}>Heating & Air</span>
            </div>
            <p style={{ fontSize:15, lineHeight:1.7, marginBottom:20, maxWidth:300 }}>
              Family-owned HVAC service for Little Rock and Central Arkansas. Open 24 hours a day, 7 days a week.
            </p>
            <p style={{ fontSize:14, lineHeight:1.8, marginBottom:8 }}>4106 Grant Ln, Little Rock, AR 72206</p>
        ...
        <div style={{ borderTop:"1px solid rgba(255,255,255,0.1)", paddingTop:28 }}>
          <div className="footer-btm-inner" style={{ maxWidth:1200, margin:"0 auto", display:"flex", justifyContent:"space-between", alignItems:"center", fontSize:13, color:"rgba(255,255,255,0.45)" }}>
            <span>Around the Clock Heating & Air. All rights reserved.</span>
