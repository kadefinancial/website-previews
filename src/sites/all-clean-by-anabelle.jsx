// Layout: D | Industry: Cleaning | City: Little Rock
import { useState, useEffect, useRef } from "react";

const PHONE = "(501) 403-0543";
const TEL = "tel:+15014030543";
...
const Icons = {
  ...
  // Spray bottle icon
  spray: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 6V2"/><path d="M8 6h8"/><rect x="8" y="6" width="8" height="14" rx="2"/><path d="M10 20v2"/><path d="M14 20v2"/><path d="M4 10h2"/><path d="M4 14h2"/><path d="M18 10h2"/><path d="M18 14h2"/><path d="M12 10v6"/><path d="M10 13h4"/></svg>,
  // Mop icon
  mop: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v10"/><path d="M6 12h12"/><path d="M6 12c-1 4-1 6 0 10"/><path d="M10 12c-.5 4-.5 6 0 10"/><path d="M14 12c.5 4 .5 6 0 10"/><path d="M18 12c1 4 1 6 0 10"/></svg>,
  // Broom icon
  broom: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2l2 8"/><path d="M8 14h8"/><path d="M7 14c-1 3-1 5 0 8"/><path d="M10 14c-.5 3-.5 5 0 8"/><path d="M14 14c.5 3 .5 5 0 8"/><path d="M17 14c1 3 1 5 0 8"/><path d="M10 10l4 0"/></svg>,
  // Home icon
  home: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>,
  // Calendar icon
...
const services = [
  { name: "Residential House Cleaning", desc: "Thorough, top-to-bottom cleaning for your home. Cheryl and her team handle every room with care — leaving your house spotless and smelling fresh.", icon: Icons.home },
  { name: "Deep Cleaning", desc: "An intensive clean that reaches every corner, baseboard, and hidden surface. Perfect for seasonal refreshes or when your home needs extra attention.", icon: Icons.sparkle },
  { name: "Move-In / Move-Out Cleaning", desc: "Make your transition seamless with a comprehensive clean before you move in or after you move out. Cheryl's team ensures 100% satisfaction before leaving.", icon: Icons.mop },
  { name: "Recurring Cleaning Services", desc: "Weekly, bi-weekly, or monthly cleaning schedules tailored to your household. Consistent quality from a team that gets to know your home.", icon: Icons.calendar },
  { name: "Office Cleaning", desc: "Professional cleaning for your workspace. A clean office means happier employees and a better impression on clients. Reliable and discreet service.", icon: Icons.broom },
  { name: "Special Occasion Cleaning", desc: "Hosting a holiday gathering, party, or event? Cheryl's team will make sure your space is immaculate before your guests arrive.", icon: Icons.spray },
];

const allServiceNames = ["Residential House Cleaning","Deep Cleaning","Move-In / Move-Out Cleaning","Recurring Cleaning Services","Office Cleaning","Special Occasion Cleaning"];

const areas = ["Little Rock","Central Arkansas"];

const reviews = [
  { text: "Cheryl and her team did an amazing job on our home. They got it cleaner than I expected, and it smelled so good. I chatted with them as they were finishing up and they were all very sweet. They are a very hard working team.", name: "Deidra Snyder", src: "Google" },
  { text: "Cheryl and her girls showed up and hit a home run. They made sure it was 100% to my satisfaction before leaving. They were so kind and sweet. Great job Ladies!", name: "Bruce Woosley", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function AllCleanByAnabelle() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("all-clean-by-anabelle-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="roof"){sessionStorage.setItem("all-clean-by-anabelle-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "House Cleaning Little Rock AR | All Clean by Anabelle";
    const els = [];
    ...
    add("meta",{name:"description",content:"Professional house cleaning in Little Rock, AR. 5.0 stars on Google. Cheryl's team delivers spotless results. Call (501) 403-0543."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"All Clean by Anabelle",telephone:"+15014030543",
      email:"athens@allcleanbyanabelle.com",
      address:{"@type":"PostalAddress",streetAddress:"4 Shackleford Plaza #210",addressLocality:"Little Rock",addressRegion:"AR",postalCode:"72211",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:34.7465,longitude:-92.3311},
      ...
      founder:{"@type":"Person",name:"Anabelle"},
      description:"All Clean by Anabelle provides professional house cleaning, deep cleaning, move-in/move-out cleaning, recurring cleaning, office cleaning, and special occasion cleaning in Little Rock, AR and Central Arkansas. Led by team leader Cheryl.",
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1E3A2F"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8}}><span style={{fontWeight:700,color:"#1E3A2F"}}>All Clean</span> <span style={{color:"#B8694A"}}>by Anabelle</span></div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(30,58,47,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}><span>All Clean</span> <span style={{color:"var(--accent)"}}>by Anabelle</span></div>
      ...
      <div className="hero-grid" style={{maxWidth:1200,width:"100%",display:"grid",gridTemplateColumns:"1.8fr 1fr",gap:48,alignItems:"center"}}>
        <div>
          <h1 style={{fontSize:"clamp(48px, 6vw, 84px)",lineHeight:1.0,color:"var(--primary)",letterSpacing:-2,marginBottom:24}}>
            Little Rock's<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Spotless</span><br/>Cleaning Team
          </h1>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:32}}>
            <Stars />
            <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"They got it cleaner than I expected." — Deidra Snyder</span>
          </div>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(184,105,74,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Us — {PHONE}</a>
        </div>
        <div>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        ...
        <div className="trust-mobile" style={{display:"none",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:48,textAlign:"center"}}>
          {[{num:"5.0",label:"Google Rating",desc:"Perfect score"},{num:"8+",label:"5-Star Reviews",desc:"All five stars"},{num:"Cheryl",label:"Team Leader",desc:"Praised by name"},{num:"Sat",label:"Open Saturdays",desc:"9 AM – 4 PM"}].map((s,i)=>(
            <div key={i}>
    ...
    {/* WHY US — Layout D: Giant "5.0" background, single column centered */}
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{maxWidth:800,margin:"0 auto",position:"relative",zIndex:1}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,textAlign:"center",opacity:0.7}}>About All Clean by Anabelle</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff",textAlign:"center"}}>Cheryl Leads<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>the Whole Team</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:24,textAlign:"center"}}>
          At All Clean by Anabelle, there's one name that comes up in every single review: Cheryl. She's the team leader who manages the cleaning crews — and the person customers can't stop praising. Cheryl doesn't just clean your home. She makes sure it's 100% to your satisfaction before she and her team walk out the door. That dedication is why All Clean by Anabelle holds a perfect 5.0 Google rating.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:36,textAlign:"center"}}>
          Every detail matters to Cheryl. Customers describe her team as "very hard working," "kind and sweet," and consistently exceeding expectations. Whether it's a routine house cleaning, a deep clean before the holidays, or a move-out clean that needs to pass inspection — Cheryl's crew shows up, works with care, and leaves your home smelling fresh and looking immaculate. That's the All Clean standard.
        </p>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:20}}>
          {["Perfect 5.0 Google Rating","Cheryl — Team Leader on Every Job","Attention to Detail Guaranteed","Open Saturdays for Your Convenience","Residential & Commercial Cleaning","Kind, Sweet & Professional Team"].map((c,i)=>(
    ...
    <section id="reviews" style={{padding:"110px 0",background:"var(--white)"}}>
      ...
      <div style={{position:"relative"}}>
        <div ref={revRef} className="scroll-strip" style={{display:"flex",gap:20,paddingLeft:"clamp(24px,5vw,64px)",paddingRight:"clamp(24px,5vw,64px)",paddingBottom:8}}>
          {reviews.map((r,i)=>(
            <div key={i} style={{flex:"0 0 380px",scrollSnapAlign:"start",background:"var(--cream)",borderRadius:12,padding:"36px 30px",border:"1px solid var(--border)",display:"flex",flexDirection:"column"}}>
    ...
    <section id="areas" style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:12}}>
          {areas.map((a,i)=>(
            <span key={i} title={`House Cleaning in ${a}, AR`} style={{display:"inline-flex",alignItems:"center",gap:6,background:"#fff",border:"1px solid var(--border)",padding:"12px 24px",borderRadius:30,fontSize:15,fontWeight:500,color:"var(--text)",cursor:"default",transition:"all 0.25s"}}
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>Need a clean home?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Cheryl's team.</span></h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>Perfect 5.0 rating. Attention to every detail. Satisfaction guaranteed before we leave.</p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16}}><span>All Clean</span> <span style={{color:"var(--accent)"}}>by Anabelle</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>Professional house cleaning for homes and offices across Little Rock and Central Arkansas. Led by team leader Cheryl — praised by name in every review.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>4 Shackleford Plaza #210<br/>Little Rock, AR 72211<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a><br/><a href="mailto:athens@allcleanbyanabelle.com" style={{color:"var(--accent)",fontWeight:600,fontSize:13}}>athens@allcleanbyanabelle.com</a></p>
          <p style={{fontSize:13,opacity:0.5,marginTop:10}}>Mon–Fri 8 AM – 4 PM | Sat 9 AM – 4 PM</p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} All Clean by Anabelle. All rights reserved.</span>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call All Clean by Anabelle</a>
    </div>
