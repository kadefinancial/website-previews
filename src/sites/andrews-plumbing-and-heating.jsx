// Layout: A | Industry: Plumber | City: Fort Smith
...
const Icons = {
  ...
  pipe: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 6h4v12H4z"/><path d="M8 10h8"/><path d="M8 14h8"/><path d="M16 6h4v12h-4z"/></svg>
  ),
  zap: (
  ...
  dollar: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="1" x2="12" y2="23"/><path d="M17 5H9.5a3.5 3.5 0 000 7h5a3.5 3.5 0 010 7H6"/></svg>
  ),
};
...
// --- Data ---
const PHONE = "(479) 783-8504";
const PHONE_TEL = "tel:+14797838504";

const services = [
  { name: "Plumbing Repair & Installation", desc: "Complete residential and commercial plumbing repair and new installation services backed by nearly nine decades of experience.", icon: Icons.wrench },
  { name: "Heating System Service", desc: "Expert heating system maintenance, repair, and installation to keep Fort Smith homes warm and comfortable all winter long.", icon: Icons.flame },
  { name: "Leak Detection & Repair", desc: "Professional leak detection using proven methods to locate and repair hidden leaks before they cause costly damage.", icon: Icons.droplets },
  { name: "Toilet & Fixture Installation", desc: "Precision installation and replacement of toilets, faucets, sinks, and bathroom fixtures for reliable performance.", icon: Icons.faucet },
  { name: "Pipe Repair", desc: "Dependable pipe repair and replacement for aging, corroded, or burst pipes in homes and businesses throughout the River Valley.", icon: Icons.pipe },
  { name: "24-Hour Emergency Service", desc: "Around-the-clock emergency plumbing response for burst pipes, major leaks, and urgent plumbing issues any time of day.", icon: Icons.zap },
];

const reviews = [
  { text: "Called in to request leak under my residence. The team of guys quickly arrived, located the area, fixed the broken pipe, and went on their merry way. Thank you!", name: "Chris Joannides", src: "Google" },
  { text: "Great work at a fair price. I definitely recommend them.", name: "Andy Ross", src: "Google" },
  { text: "Fast and friendly.", name: "Joe Dugas", src: "Google" },
];

const areas = ["Fort Smith", "Van Buren", "Alma", "Greenwood", "Barling", "River Valley", "Western Arkansas"];

// --- Main Component ---
export default function AndrewsPlumbingAndHeating() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("aph-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("aph-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Plumber Fort Smith AR | Andrews Plumbing & Heating";
    const els = [];
    ...
    add("meta", { name: "description", content: "Fort Smith plumber since 1937. 88 years of trusted service. BBB A+ Rated. Call (479) 783-8504 for a free quote." });
    ...
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      ...
      name: "Andrews Plumbing & Heating", telephone: "+14797838504",
      address: { "@type": "PostalAddress", streetAddress: "820 S 11th St", addressLocality: "Fort Smith", addressRegion: "AR", postalCode: "72901", addressCountry: "US" },
      geo: { "@type": "GeoCoordinates", latitude: 35.3859, longitude: -94.3985 },
      openingHoursSpecification: [{ "@type": "OpeningHoursSpecification", dayOfWeek: ["Monday","Tuesday","Wednesday","Thursday","Friday"], opens: "08:00", closes: "17:00" }],
      aggregateRating: { "@type": "AggregateRating", ratingValue: "4.2", reviewCount: "10", bestRating: "5" },
      founder: { "@type": "Person", name: "Delman Burris" },
      foundingDate: "1937",
      description: "Andrews Plumbing & Heating has served Fort Smith and the River Valley since 1937. BBB A+ rated plumbing and heating services.",
      areaServed: areas.map((a) => ({ "@type": "City", name: a })),
      hasOfferCatalog: { "@type": "OfferCatalog", name: "Plumbing & Heating Services", itemListElement: services.map((s) => ({ "@type": "Offer", itemOffered: { "@type": "Service", name: s.name } })) },
        ...
        <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#1C2B41" }}>
          <div style={{ background:"#fff", borderRadius:16, padding:"56px 44px", textAlign:"center", maxWidth:420, width:"92%", boxShadow:"0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:28, color:"#1C2B41", marginBottom:32 }}>
              <span style={{ fontWeight:700 }}>Andrews</span> <span style={{ color:"#8B3A3A" }}>Plumbing & Heating</span>
            </div>
    ...
    <>
      ...
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:1000, padding:scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display:"flex", alignItems:"center", justifyContent:"space-between", background: scrolled ? "rgba(28,43,65,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition:"all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        <div className="serif" style={{ fontSize:22, color: scrolled ? "#fff" : "var(--primary)", fontWeight:600, letterSpacing:-0.5 }}>
          <span>Andrews</span><span style={{ color:"var(--accent)", marginLeft:6 }}>Plumbing & Heating</span>
        </div>
      ...
      <section style={{ minHeight:"100vh", padding:"160px clamp(24px,5vw,64px) 100px", background:"var(--cream)", display:"flex", alignItems:"center", justifyContent:"center" }}>
        <div className="hero-grid" style={{ maxWidth:1200, width:"100%", display:"grid", gridTemplateColumns:"1.2fr 0.8fr", gap:64, alignItems:"center" }}>
          <div style={{ maxWidth:560 }}>
            <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(139,58,58,0.1)", padding:"8px 18px", borderRadius:4, marginBottom:28 }}>
              <div style={{ width:6, height:6, borderRadius:"50%", background:"var(--accent)" }} />
              <span style={{ fontSize:12, fontWeight:700, color:"var(--accent)", letterSpacing:2, textTransform:"uppercase" }}>Serving Fort Smith Since 1937</span>
            </div>
            ...
            <p style={{ fontSize:18, lineHeight:1.7, color:"var(--text-mid)", marginBottom:32, maxWidth:440 }}>
              For 88 years, Andrews Plumbing & Heating has delivered honest, reliable plumbing and heating service to Fort Smith and the River Valley. One of the oldest plumbing companies in Arkansas.
            </p>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:36 }}>
              <Stars />
              <span style={{ fontSize:14, color:"var(--text-light)", fontStyle:"italic", marginLeft:4 }}>"Fast and friendly." — Joe Dugas</span>
            </div>
            <div style={{ display:"flex", gap:14, flexWrap:"wrap" }}>
              <a href={PHONE_TEL} style={{ display:"inline-flex", alignItems:"center", gap:10, background:"var(--accent)", color:"#fff", padding:"18px 36px", borderRadius:6, fontSize:16, fontWeight:700, boxShadow:"0 4px 20px rgba(139,58,58,0.3)", fontFamily:"'DM Sans',sans-serif" }}>
            ...
            <div style={{ background:"#fff", borderRadius:12, padding:"44px 36px", boxShadow:"0 4px 40px rgba(28,43,65,0.08)", border:"1px solid var(--border)" }}>
              {!formSubmitted ? (
                ...
                <div style={{ textAlign:"center", padding:"24px 0" }}>
                  <div style={{ width:56, height:56, borderRadius:"50%", background:"rgba(139,58,58,0.12)", display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 16px", color:"var(--accent)" }}>{Icons.check}</div>
      ...
      <section style={{ padding:"72px clamp(24px,5vw,64px)", background:"#fff", borderTop:"1px solid var(--border)", borderBottom:"1px solid var(--border)" }}>
        <div className="trust-grid" style={{ maxWidth:1000, margin:"0 auto", display:"grid", gridTemplateColumns:"repeat(4, 1fr)", gap:32, textAlign:"center" }}>
          {[
            { num: "1937", label: "Est. Founded", sub: "88 years of service" },
            { num: "BBB", label: "A+ Rated", sub: "Trusted reputation" },
            { num: "24hr", label: "Emergency Service", sub: "Always available" },
            { num: "Fair", label: "Pricing Always", sub: "Honest estimates" },
          ].map((s, i) => (
            <div key={i}>
      ...
      <section id="services" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          <div style={{ textAlign:"center", marginBottom:64 }}>
            ...
            <h2 style={{ fontSize:"clamp(30px, 3.5vw, 44px)", color:"var(--primary)", letterSpacing:-0.5 }}>Plumbing & Heating Services in Fort Smith</h2>
          </div>
          ...
          <p style={{ textAlign:"center", marginTop:40, fontSize:15, color:"var(--text-light)" }}>
            Questions? Call Delman directly at <a href={PHONE_TEL} style={{ color:"var(--accent)", fontWeight:600 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--primary)", color:"#fff", position:"relative", overflow:"hidden" }}>
        <div style={{ position:"absolute", top:0, right:0, width:"40%", height:"100%", background:"linear-gradient(135deg, rgba(139,58,58,0.04) 0%, transparent 60%)", pointerEvents:"none" }} />
        <div className="why-grid" style={{ maxWidth:1100, margin:"0 auto", display:"grid", gridTemplateColumns:"1.1fr 0.9fr", gap:80, alignItems:"start", position:"relative" }}>
          <div>
            ...
            <div style={{ fontSize:12, fontWeight:700, textTransform:"uppercase", letterSpacing:3, color:"var(--accent)", marginBottom:14, opacity:0.7 }}>About Andrews Plumbing</div>
            <h2 style={{ fontSize:"clamp(30px, 3.5vw, 42px)", lineHeight:1.15, marginBottom:28, color:"#fff" }}>88 Years Serving<br/><span style={{ fontStyle:"italic", color:"var(--accent)" }}>Fort Smith</span></h2>
            <p style={{ fontSize:16, lineHeight:1.8, color:"rgba(255,255,255,0.65)", marginBottom:24 }}>
              Founded in 1937, Andrews Plumbing & Heating is one of the oldest plumbing companies in Arkansas. For nearly nine decades, this family-run business has been the name Fort Smith trusts for honest, dependable plumbing and heating work. Under the leadership of Vice President Delman Burris, the company continues the tradition of quality craftsmanship that has defined it since the very beginning.
            </p>
            <p style={{ fontSize:16, lineHeight:1.8, color:"rgba(255,255,255,0.65)", marginBottom:36 }}>
              BBB A+ rated and committed to fair pricing, Andrews Plumbing & Heating treats every job — from a simple fixture replacement to a full emergency repair — with the same care and professionalism that has kept families coming back for generations.
            </p>
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              {["Founded in 1937 — 88 Years Strong", "BBB A+ Rated Business", "24-Hour Emergency Service", "Licensed & Insured", "Fair, Upfront Pricing"].map((c, i) => (
          ...
          <div className="badge-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, paddingTop:20 }}>
            {[
              { icon: Icons.shield, title: "BBB A+ Rated", desc: "Trusted reputation" },
              { icon: Icons.clock, title: "24/7 Emergency", desc: "Always available" },
              { icon: Icons.home, title: "Since 1937", desc: "88 years of service" },
              { icon: Icons.dollar, title: "Fair Pricing", desc: "Honest estimates" },
            ].map((b, i) => (
      ...
      <section id="reviews" style={{ padding:"110px clamp(24px,5vw,64px)", background:"#fff" }}>
        <div style={{ maxWidth:800, margin:"0 auto", textAlign:"center" }}>
          ...
          <div style={{ display:"flex", justifyContent:"center", gap:10, marginTop:28 }}>
            {reviews.map((_, i) => (
              <button key={i} onClick={() => setReviewIdx(i)} aria-label={`View review ${i + 1}`} style={{ width: reviewIdx === i ? 28 : 10, height:10, borderRadius:5, background: reviewIdx === i ? "var(--accent)" : "var(--border)", border:"none", cursor:"pointer", transition:"all 0.3s ease" }} />
            ))}
          </div>
      ...
      <section style={{ padding:"100px clamp(24px,5vw,64px)", background:"#fff", textAlign:"center", borderTop:"1px solid var(--border)" }}>
        <div style={{ maxWidth:600, margin:"0 auto" }}>
          <h2 className="serif" style={{ fontSize:"clamp(32px, 4vw, 50px)", color:"var(--primary)", lineHeight:1.1, marginBottom:20, letterSpacing:-1 }}>
            Got a plumbing problem?<br/><span style={{ fontStyle:"italic", color:"var(--accent)" }}>Call Delman.</span>
          </h2>
          <p style={{ fontSize:17, color:"var(--text-mid)", marginBottom:36, lineHeight:1.6 }}>
            Fair pricing. 88 years of expertise. BBB A+ rated since day one.
          </p>
          <a href={PHONE_TEL} style={{ display:"inline-flex", alignItems:"center", gap:10, background:"var(--accent)", color:"#fff", padding:"22px 52px", borderRadius:8, fontSize:20, fontWeight:700, boxShadow:"0 6px 28px rgba(139,58,58,0.35)", fontFamily:"'DM Sans',sans-serif" }}>
            {Icons.phone} {PHONE}
          </a>
        </div>
      </section>
      ...
      <footer style={{ background:"var(--primary)", color:"rgba(255,255,255,0.7)", padding:"64px clamp(24px,5vw,64px) 0" }}>
        <div className="footer-grid" style={{ maxWidth:1100, margin:"0 auto", display:"grid", gridTemplateColumns:"1.5fr 1fr 1fr", gap:48, paddingBottom:40 }}>
          <div>
            <div className="serif" style={{ fontSize:22, color:"#fff", marginBottom:16 }}>Andrews <span style={{ color:"var(--accent)" }}>Plumbing & Heating</span></div>
            <p style={{ fontSize:14, lineHeight:1.7, opacity:0.7, marginBottom:16 }}>Fort Smith's trusted plumbing and heating company since 1937. 88 years of honest, reliable service to the River Valley.</p>
            <p style={{ fontSize:14, lineHeight:1.8, opacity:0.6 }}>
              820 S 11th St<br/>Fort Smith, AR 72901<br/>
        ...
        <div style={{ borderTop:"1px solid rgba(255,255,255,0.06)", padding:"20px 0", maxWidth:1100, margin:"0 auto" }}>
          <div className="footer-btm-inner" style={{ display:"flex", justifyContent:"space-between", fontSize:13, opacity:0.4 }}>
            <span>&copy; {new Date().getFullYear()} Andrews Plumbing & Heating. All rights reserved.</span>
            <span>BBB A+ Rated &middot; Mon–Fri 8 AM–5 PM</span>
          </div>
        </div>
      </footer>
      ...
      <div className="mobile-only" style={{ position:"fixed", bottom:0, left:0, right:0, zIndex:998, display:"flex" }}>
        <a href={PHONE_TEL} style={{ display:"flex", width:"100%", alignItems:"center", justifyContent:"center", gap:10, background:"var(--accent)", color:"#fff", padding:"18px", fontSize:17, fontWeight:700, boxShadow:"0 -4px 20px rgba(0,0,0,0.15)", fontFamily:"'DM Sans',sans-serif" }}>
          {Icons.phone} Call Andrews Plumbing
        </a>
      </div>
