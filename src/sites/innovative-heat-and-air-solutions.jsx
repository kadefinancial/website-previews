// Layout: D | Industry: HVAC | City: Mabelvale
import { useState, useEffect, useRef } from "react";

const PHONE = "(501) 773-9695";
const TEL = "tel:+15017739695";
...
const Icons = {
  ...
  flame: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22c4.97 0 8-3.58 8-8 0-3.07-2.31-6.64-4-8-1.18 2.5-3 4-5 3-1-.5-1.5-2-1-4-3.5 2-6 6.33-6 9 0 4.42 3.03 8 8 8z"/></svg>,
  ...
  star: <svg width="18" height="18" viewBox="0 0 24 24" fill="#8B3A3A" stroke="none"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>,
...
const services = [
  { name: "Air Conditioner Repair", desc: "Fast, accurate AC diagnostics and repair. Dallas identifies the real cause of failure -- not just the symptom -- so your system stays fixed.", icon: Icons.wind },
  { name: "Furnace Repair", desc: "Heating system not keeping up? 31+ years of hands-on experience means Dallas can troubleshoot what others miss and restore comfort quickly.", icon: Icons.flame },
  { name: "HVAC Installation", desc: "New construction or first-time install. Properly sized, properly designed systems that account for your home's unique layout and building characteristics.", icon: Icons.home },
  { name: "HVAC System Replacement", desc: "When repair no longer makes sense, Dallas provides honest assessments and fair pricing on full system replacement -- often completed next day.", icon: Icons.wrench },
  { name: "Home Comfort Analysis", desc: "A scientific approach to comfort. Dallas evaluates your home's design, ductwork, insulation, and airflow to find what's really causing hot and cold spots.", icon: Icons.thermometer },
  { name: "Energy Efficiency Assessments", desc: "Reduce utility bills without sacrificing comfort. A thorough evaluation of your system's performance, identifying waste and recommending practical upgrades.", icon: Icons.clipboard },
];

const allServiceNames = ["Air Conditioner Repair","Furnace Repair","HVAC Installation","HVAC System Replacement","Home Comfort Analysis","Energy Efficiency Assessments"];

const areas = ["Mabelvale","Little Rock","Benton","Bryant","Shannon Hills","Sweet Home","Alexander","Bauxite","Haskell"];

const reviews = [
  { text: "Called Dallas on a Thursday afternoon with a dead AC unit. Definitely needed to be replaced and not just repaired. Dallas had a new unit installed by Friday afternoon. Great service, great price, all around excellent experience.", name: "Thomas Blackmon", src: "Google" },
  { text: "Innovative heating and air is the absolute best!! They were so easy to do business with and did it quickly and efficiently. Highly recommend!", name: "Tracy Crockett", src: "Google" },
  { text: "I have used Dallas/Innovative Heat and Air Solutions for years. He kept my old heat and air unit up and running long past its expected life. When it was time to replace, he gave me reasonable prices and solved a 10 year problem.", name: "Karla Carmody", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function InnovativeHeatAndAirSolutions() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("innovative-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="roof"){sessionStorage.setItem("innovative-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "HVAC Service Mabelvale AR | Innovative Heat and Air";
    const els = [];
    ...
    add("meta",{name:"description",content:"HVAC service in Mabelvale AR. Perfect 5.0 stars, 24 reviews. 31+ years experience. Licensed HVACR. Call (501) 773-9695."});
    add("meta",{name:"geo.region",content:"US-AR"});
    add("meta",{name:"geo.placename",content:"Mabelvale"});
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      "@context":"https://schema.org","@type":"HVACBusiness",
      name:"Innovative Heat and Air Solutions",telephone:"+15017739695",
      email:"innovativeheatandairsolutions@gmail.com",
      address:{"@type":"PostalAddress",streetAddress:"6600 Green Rd",addressLocality:"Mabelvale",addressRegion:"AR",postalCode:"72103",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:34.6337,longitude:-92.3862},
      openingHoursSpecification:[{"@type":"OpeningHoursSpecification",dayOfWeek:["Monday","Tuesday","Wednesday","Thursday","Friday"],opens:"09:00",closes:"17:00"}],
      aggregateRating:{"@type":"AggregateRating",ratingValue:"5.0",reviewCount:"24",bestRating:"5"},
      founder:{"@type":"Person",name:"Dallas Henry"},
      foundingDate:"2011",
      description:"Innovative Heat and Air Solutions provides HVAC services in Mabelvale and Central Arkansas. 31+ years experience. Perfect 5.0 Google rating. Licensed HVACR 0858090.",
  ...
  if (!authed) {
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#2E1E3D"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8,fontWeight:600,letterSpacing:-0.5}}><span style={{color:"#2E1E3D"}}>Innovative</span> <span style={{color:"#8B3A3A"}}>Heat & Air</span></div>
          <p style={{fontSize:13,color:"#8A8A9A",letterSpacing:1,marginBottom:28,textTransform:"uppercase"}}>Website Preview</p>
          <form onSubmit={submitPw}>
            <input type="password" value={pw} onChange={e=>{setPw(e.target.value);setPwBad(false)}} placeholder="••••" autoFocus style={{width:"100%",padding:16,fontSize:20,border:`2px solid ${pwBad?"#ef4444":"#e2e8f0"}`,borderRadius:8,textAlign:"center",letterSpacing:6,outline:"none",fontFamily:"'DM Sans',sans-serif",color:"#2E1E3D"}} />
            {pwBad && <p style={{color:"#ef4444",fontSize:12,marginTop:8,fontWeight:500}}>Incorrect password</p>}
            <button type="submit" style={{width:"100%",padding:16,marginTop:20,fontSize:15,fontWeight:700,background:"#2E1E3D",color:"#fff",border:"none",borderRadius:8,cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>View Site</button>
          </form>
        </div>
      </div>
    </>);
  ...
  return (<>
    <style>{`
      ...
      :root{--primary:#2E1E3D;--accent:#8B3A3A;--cream:#FAF9F6;--white:#FFFFFF;--text:#2A2A3C;--text-mid:#5A5A6E;--text-light:#8A8A9A;--border:rgba(0,0,0,0.06)}
    ...
    {/* NAV */}
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(46,30,61,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}><span>Innovative</span> <span style={{color:"var(--accent)"}}>Heat & Air</span></div>
    ...
    {/* HERO -- Layout D: Asymmetric 65/35, extra-large H1, no badge, no subtitle, no secondary button */}
      ...
      <div className="hero-grid" style={{maxWidth:1200,width:"100%",display:"grid",gridTemplateColumns:"1.8fr 1fr",gap:48,alignItems:"center"}}>
        <div>
          <h1 style={{fontSize:"clamp(48px, 6vw, 84px)",lineHeight:1.0,color:"var(--primary)",letterSpacing:-2,marginBottom:24}}>
            Mabelvale's<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>HVAC Expert</span><br/>Since 2011
          </h1>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:32}}>
            <Stars />
            <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"Great service, great price, all around excellent experience." -- Thomas Blackmon</span>
          </div>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(139,58,58,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Dallas -- {PHONE}</a>
        </div>
        <div>
          <div style={{background:"#fff",borderRadius:12,padding:"32px 28px",boxShadow:"0 4px 40px rgba(0,0,0,0.08)",border:"1px solid var(--border)"}}>
            {!formDone ? (
              ...
              <div style={{textAlign:"center",padding:"20px 0"}}>
                <div style={{width:48,height:48,borderRadius:"50%",background:"rgba(139,58,58,0.12)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 14px",color:"var(--accent)"}}>{Icons.check}</div>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",color:"var(--primary)",letterSpacing:-0.5}}>HVAC Services in Mabelvale & Central Arkansas</h2>
        </div>
        {/* Mobile trust */}
        <div className="trust-mobile" style={{display:"none",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:48,textAlign:"center"}}>
          {[{num:"5.0",label:"Google Rating",desc:"Perfect score"},{num:"24+",label:"Reviews",desc:"On Google"},{num:"31+",label:"Years Experience",desc:"HVAC technician"},{num:"2011",label:"Established",desc:"Mabelvale, AR"}].map((s,i)=>(
            <div key={i}>
        ...
        <p style={{textAlign:"center",marginTop:48,fontSize:15,color:"var(--text-light)"}}>Questions? Call Dallas directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",position:"relative",overflow:"hidden"}}>
      <div className="serif" style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",fontSize:"clamp(120px,20vw,280px)",color:"var(--accent)",opacity:0.08,lineHeight:1,whiteSpace:"nowrap",pointerEvents:"none"}}>24+</div>
      <div style={{maxWidth:800,margin:"0 auto",position:"relative",zIndex:1}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,textAlign:"center",opacity:0.7}}>About Dallas</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff",textAlign:"center"}}>We Solve Problems<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Where Others Have Failed</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:24,textAlign:"center"}}>
          Dallas Henry doesn't just fix equipment -- he solves problems. With 31+ years as an HVAC technician and 15 years managing a service department, Dallas has seen every scenario imaginable. When he founded Innovative Heat and Air Solutions in 2011, he brought a scientific method approach to residential HVAC that most companies simply don't offer. He looks at the whole picture: your home's building design, ductwork layout, insulation, and airflow patterns -- not just the box on your roof.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:36,textAlign:"center"}}>
          As an owner-operator, Dallas works directly with every customer. There's no call center, no middleman, no junior tech guessing at your problem. When you call Innovative Heat and Air, you get Dallas -- Licensed HVACR 0858090, a perfect 5.0 Google rating with 24 reviews, and someone who takes personal pride in every job. That's why Mabelvale and Central Arkansas keep calling back.
        </p>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:20}}>
          {["31+ Years HVAC Experience","Licensed HVACR 0858090","Perfect 5.0 Google Rating","Owner-Operator","15 Years Service Dept. Manager","Scientific Method Approach"].map((c,i)=>(
    ...
    <section id="areas" style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        ...
        <h2 style={{fontSize:"clamp(28px,3vw,38px)",color:"var(--primary)",marginBottom:48,letterSpacing:-0.5}}>Serving Mabelvale & Central Arkansas</h2>
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>AC trouble or heating issue?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Dallas.</span></h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>31+ years of experience. Perfect 5.0 rating. Licensed, honest, and owner-operated since 2011.</p>
        <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"22px 52px",borderRadius:8,fontSize:20,fontWeight:700,boxShadow:"0 6px 28px rgba(139,58,58,0.35)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} {PHONE}</a>
      </div>
    </section>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16,fontWeight:600,letterSpacing:-0.5}}><span>Innovative</span> <span style={{color:"var(--accent)"}}>Heat & Air</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>HVAC services for Mabelvale, Little Rock, and surrounding Central Arkansas. 31+ years experience. Perfect 5.0 Google rating. Licensed HVACR 0858090.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>6600 Green Rd<br/>Mabelvale, AR 72103<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a><br/><a href="mailto:innovativeheatandairsolutions@gmail.com" style={{color:"rgba(255,255,255,0.5)"}}>innovativeheatandairsolutions@gmail.com</a></p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} Innovative Heat and Air Solutions. All rights reserved.</span>
          <span>Licensed HVACR 0858090 | Mon-Fri 9 AM-5 PM</span>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Dallas -- {PHONE}</a>
    </div>
