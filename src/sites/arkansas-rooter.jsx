...
const PHONE = "(501) 562-5553";
const TEL = "tel:+15015625553";
...
const Icons = {
  ...
  pipe: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M2 22l1-1h3l9-9"/><path d="M3 21v-3l9-9"/><path d="M14.5 7.5l2.914-2.914a2.062 2.062 0 112.914 2.914L17.414 10.414"/><path d="M14.5 7.5l3 3"/></svg>,
...
const services = [
  { name: "Sewer & Drain Cleaning", desc: "Floor drains, storm sewers, sewer mains, and stubborn sink clogs cleared with professional-grade equipment.", icon: Icons.droplets },
  { name: "High Pressure Water Jetting", desc: "Industrial-strength water jetting to blast through grease, roots, and buildup that standard snaking can't touch.", icon: Icons.pipe },
  { name: "Video Camera Inspection", desc: "See exactly what's happening inside your pipes with advanced camera technology before any digging begins.", icon: Icons.camera },
  { name: "Line Work & Repairs", desc: "Complete sewer and drain line repair, replacement, and restoration by licensed, experienced technicians.", icon: Icons.wrench },
  { name: "Maintenance & Service Contracts", desc: "Preventive sewer line maintenance programs that save you money and prevent emergency backups.", icon: Icons.clipboard },
  { name: "24-Hour Emergency Service", desc: "Sewer emergencies don't wait for business hours. Neither do we. Call any time, day or night.", icon: Icons.zap },
];

const allServiceNames = ["Sewer & Drain Cleaning","High Pressure Water Jetting","Video Camera Line Inspection","Line Work & Repairs","Sewer Line Maintenance","24-Hour Emergency Service","Free Estimates"];

const areas = ["Little Rock","North Little Rock","Sherwood","Jacksonville","Maumelle","Benton","Bryant","Cabot","Conway"];

const reviews = [
  { text: "Me and my family loved the service that Shane provided, last year and a couple of months ago he was able to unplug our sewage pipe and again this year he was able to do it again, he was quick and on time and we appreciated his service so much. Great company and great prices.", name: "Kimberly Toral", src: "Google" },
  { text: "I had a clogged up sewer line at my house. I called on a Sunday evening. Within one hour, Shane came out and got me taken care of for a very reasonable price. I highly recommend Arkansas Rooter.", name: "David Messersmith", src: "Google" },
  { text: "I called Arkansas Rooter and they sent someone next day, at my request. Troy arrived right on time and quickly analyzed the issue. He ran his line and ran several test flushes. Troy was so polite and didn't seem to mind my 20 questions! This is the second time I've used Arkansas Rooter and both times I've had excellent service.", name: "Deidre Burress", src: "Google" },
  { text: "Arkansas Rooter is the best! They are wonderful people, smart, so hard working! I'd single out Troy, the sweet uncomplaining genius who solved a problem three other companies couldn't. And worked nonstop! And Shane was also terrific, so very nice and I can't even express my gratitude enough.", name: "Rebecca Marshall", src: "Google" },
  { text: "After calling several companies, Arkansas Rooter was the only one that gave us a precise estimate over the phone without any sort of hassle. Troy returned our call promptly, was polite and professional, was on time, fixed the issue, and only charged us what was originally quoted. Highly recommend!", name: "Jonathan Edwards", src: "Google" },
];
...
export default function ArkansasRooter() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("ar-rooter-auth") === "1");
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Sewer & Drain Little Rock AR | Arkansas Rooter";
    const els = [];
    ...
    add("meta",{name:"description",content:"Sewer & drain cleaning in Little Rock, AR since 1981. 4.9 stars, 24hr emergency. Call (501) 562-5553."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"Arkansas Rooter Inc.",telephone:"+15015625553",
      address:{"@type":"PostalAddress",streetAddress:"11310 Arch St",addressLocality:"Little Rock",addressRegion:"AR",postalCode:"72206",addressCountry:"US"},
      ...
      founder:{"@type":"Person",name:"Shane Stoops"},foundingDate:"1981",
      description:"Family-owned sewer and drain cleaning company serving Little Rock, AR since 1981. 24-hour emergency service, video inspection, and high pressure water jetting.",
      areaServed:areas.map(a=>({"@type":"City",name:a})),
      hasOfferCatalog:{"@type":"OfferCatalog",name:"Rooter Services",itemListElement:allServiceNames.map(s=>({"@type":"Offer",itemOffered:{"@type":"Service",name:s}}))},
  ...
  return (<>
    ...
    <section style={{padding:"180px clamp(24px,5vw,64px) 100px",background:"var(--cream)",textAlign:"center"}}>
      <div className="hero-text" style={{maxWidth:780,margin:"0 auto"}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(193,127,94,0.1)",padding:"8px 18px",borderRadius:4,marginBottom:28}}>
          ...
          <span style={{fontSize:12,fontWeight:700,color:"var(--accent)",letterSpacing:2,textTransform:"uppercase"}}>Family Owned Since 1981</span>
        </div>
        <h1 style={{fontSize:"clamp(44px, 6vw, 80px)",lineHeight:1.05,color:"var(--primary)",marginBottom:24,letterSpacing:-2}}>
          Professional Sewer & Drain<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Little Rock Trusts</span>
        </h1>
        <p style={{fontSize:18,lineHeight:1.7,color:"var(--text-mid)",marginBottom:32,maxWidth:560,margin:"0 auto 32px"}}>
          Shane Stoops and his team have been keeping Little Rock's drains flowing for over 40 years. 4.9 stars, 24-hour emergency service, and free estimates over the phone.
        </p>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:36}}>
          <Stars />
          <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"The best! A problem three other companies couldn't solve." — Rebecca Marshall</span>
        </div>
        <div style={{display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap"}}>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(193,127,94,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Shane Now</a>
      ...
      <div className="form-trust-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1fr 1fr",gap:64,alignItems:"start"}}>
        ...
        <div className="trust-row" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:32,alignContent:"center",height:"100%"}}>
          {[
            ...
            {num:"60+",label:"Reviews",desc:"Verified customers"},
            {num:"44",label:"Years",desc:"Serving since 1981"},
            {num:"24hr",label:"Emergency",desc:"Day or night"},
          ].map((s,i)=>(
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",color:"var(--primary)",letterSpacing:-0.5}}>Sewer & Drain Services in Little Rock, AR</h2>
        </div>
        ...
        <p style={{textAlign:"center",marginTop:40,fontSize:15,color:"var(--text-light)"}}>
          Questions? Call Shane directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a>
        </p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",color:"#fff"}}>
      <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
        ...
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff"}}>Why Little Rock Has Trusted<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Shane Stoops</span> Since 1981</h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.65)",marginBottom:24,textAlign:"left"}}>
          When Shane and Tina Stoops took over Arkansas Rooter, the company already had decades of trust built with Little Rock homeowners. They've honored that legacy by keeping the same principles that built it: show up when you say you will, give an honest price over the phone, and don't leave until the job is done right.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.65)",marginBottom:36,textAlign:"left"}}>
          With 4.9 stars across 60 reviews, customers consistently praise their fair pricing, fast response times, and the kind of old-school professionalism that's hard to find. Shane still personally handles service calls, and their team — including Troy — has earned a reputation for solving problems that other companies can't.
        </p>
        <div className="why-creds-row" style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:24,marginTop:8}}>
          {["Family Owned Since 1981","BBB Accredited (A+ Rating)","Licensed Contractor","24-Hour Emergency Service","Free Estimates"].map((c,i)=>(
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0",borderTop:"1px solid rgba(255,255,255,0.06)"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          ...
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>Family-owned sewer and drain specialists serving Central Arkansas since 1981. BBB Accredited with an A+ rating.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>
            11310 Arch St<br/>Little Rock, AR 72206<br/>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto",display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
        <span>&copy; {yr} Arkansas Rooter Inc. All rights reserved.</span>
        <span>BBB Accredited (A+) &middot; Licensed Contractor</span>
      </div>
    </footer>

    {/* FLOATING MOBILE CTA */}
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Arkansas Rooter</a>
    </div>
