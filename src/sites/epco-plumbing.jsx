// Layout: C | Industry: Plumber | City: Fayetteville
...
const Icons = {
  ...
  pipe: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M4 14h6v6H4z"/><path d="M14 4h6v6h-6z"/><path d="M10 17h4a3 3 0 003-3V10"/><path d="M14 7H10a3 3 0 00-3 3v4"/></svg>
  ),
  search: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/><path d="M11 8v6"/><path d="M8 11h6"/></svg>
  ),
  zap: (
...
// --- Data ---
const PHONE = "(479) 249-6292";
const PHONE_TEL = "tel:+14792496292";
const BIZ = "EPCO Plumbing";

const services = [
  { name: "Residential Plumbing Repair", desc: "From leaky faucets to burst pipes, EPCO handles every residential plumbing repair across Northwest Arkansas. Nathan and the team respond fast, diagnose honestly, and get your home back to normal.", icon: Icons.wrench },
  { name: "Commercial Plumbing", desc: "Dental offices, restaurants, retail spaces — EPCO keeps Northwest Arkansas businesses running. Tracy's 25+ years of experience means commercial jobs are handled with precision and minimal disruption.", icon: Icons.building },
  { name: "Water Heater Installation & Repair", desc: "Whether your water heater needs a repair or a full replacement, EPCO's master plumber ensures proper installation and lasting performance. Same-day service available.", icon: Icons.flame },
  { name: "Drain Cleaning", desc: "Slow drains, stubborn clogs, and backed-up lines cleared quickly with professional equipment. EPCO gets your drains flowing again without cutting corners.", icon: Icons.pipe },
  { name: "Leak Detection & Repair", desc: "Hidden leaks can cause thousands in damage. EPCO locates and repairs leaks fast — in walls, under slabs, and throughout your plumbing system — before small problems become big ones.", icon: Icons.search },
  { name: "24/7 Emergency Plumbing", desc: "Plumbing emergencies don't wait for business hours. EPCO is available around the clock, seven days a week. Call anytime and Nathan or Tracy will be there.", icon: Icons.zap },
];

const allServices = [
  "Residential Plumbing Repair", "Commercial Plumbing", "Water Heater Installation & Repair",
  "Drain Cleaning", "Leak Detection & Repair", "24/7 Emergency Plumbing",
];

const reviews = [
  { text: "Nathan and his team came out to our new property to fix a corroded pipe, leaking fridge connection, and leaking washer/dryer connection. They did a fantastic job and we were so happy with how prompt they were.", name: "Megan Martinez", src: "Google" },
  { text: "PTSD Veteran and good ol' boy network seals of approval! Nate and his coworker came out and helped me with a main line repair.", name: "Mark Haag", src: "Google" },
  { text: "Nathan from EPCO Plumbing is the real MVP! I run a dental office and when something goes wrong, I need someone dependable.", name: "Adam Socia", src: "Google" },
];

const areas = ["Fayetteville", "Springdale", "Rogers", "Bentonville", "West Fork", "Farmington", "Northwest Arkansas"];

// --- Main Component ---
export default function EpcoPlumbing() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("epco-plumbing-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("epco-plumbing-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Plumber Fayetteville AR | EPCO Plumbing";
    const els = [];
    const add = (tag, attrs) => {
    ...
    add("meta", { name: "description", content: "EPCO Plumbing — 4.8 stars, Master Plumber #7131. 24/7 emergency plumbing in Fayetteville, AR. Call (479) 249-6292." });
    ...
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      ...
      "@type": "Plumber",
      name: "EPCO Plumbing",
      telephone: "+14792496292",
      address: { "@type": "PostalAddress", streetAddress: "375 N Main Ave", addressLocality: "Fayetteville", addressRegion: "AR", postalCode: "72701", addressCountry: "US" },
      geo: { "@type": "GeoCoordinates", latitude: 36.0822, longitude: -94.1719 },
      openingHoursSpecification: [{ "@type": "OpeningHoursSpecification", dayOfWeek: ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"], opens: "00:00", closes: "23:59" }],
      aggregateRating: { "@type": "AggregateRating", ratingValue: "4.8", reviewCount: "48", bestRating: "5" },
      founder: { "@type": "Person", name: "Tracy Poquette" },
      description: "EPCO Plumbing is a family-owned plumbing company serving Fayetteville and Northwest Arkansas. Master Plumber #7131, 25+ years experience, available 24/7.",
      ...
      memberOf: [{ "@type": "Organization", name: "BNI of the Ozarks" }],
      sameAs: [],
    })});
        ...
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#1E3A2F" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, fontWeight: 600, letterSpacing: -0.5, marginBottom: 4 }}>
              <span style={{ color: "#1E3A2F" }}>EPCO</span> <span style={{ color: "#B8694A" }}>Plumbing</span>
            </div>
    ...
    <>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled ? "rgba(30,58,47,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition: "all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        <div className="serif" style={{ fontSize: 22, fontWeight: 600, letterSpacing: -0.5, color: "#fff" }}>
          <span>EPCO</span><span style={{ color: "var(--accent)", marginLeft: 6 }}>Plumbing</span>
        </div>
        ...
        <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
          ...
          <div style={{ maxWidth: 560 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.1)", padding: "8px 18px", borderRadius: 4, marginBottom: 28 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: "var(--accent)" }}>Master Plumber #7131</span>
            </div>
            ...
            <p style={{ fontSize: 18, lineHeight: 1.7, color: "rgba(255,255,255,0.75)", marginBottom: 32, maxWidth: 480 }}>
              Tracy and Nathan Poquette run a family plumbing business with 25+ years of master plumber experience. From residential repairs to commercial emergencies across Northwest Arkansas — EPCO is always available.
            </p>
            {/* Star row with hero quote */}
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 32, flexWrap: "wrap" }}>
              <Stars />
              <span style={{ fontSize: 14, color: "rgba(255,255,255,0.7)", fontStyle: "italic" }}>"Nathan from EPCO is the real MVP!"</span>
              <span style={{ fontSize: 13, color: "rgba(255,255,255,0.5)" }}>- Adam Socia</span>
            </div>
      ...
      <section style={{ background: "var(--white)", padding: "48px clamp(24px,5vw,64px)" }}>
        <div className="trust-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 32, textAlign: "center" }}>
          {[
            { num: "4.8", label: "Google Rating", desc: "Verified reviews" },
            { num: "48+", label: "Customer Reviews", desc: "And counting" },
            { num: "24/7", label: "Always Available", desc: "Day or night" },
            { num: "Master", label: "Plumber #7131", desc: "Licensed & certified" },
          ].map((s, i) => (
      ...
      <section id="services" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)" }}>Plumbing Services in Fayetteville</h2>
          </div>
          {services.map((svc, i) => {
            const isEven = i % 2 === 1;
            return (
          ...
          <p style={{ textAlign: "center", marginTop: 48, fontSize: 15, color: "var(--text-mid)" }}>
            Questions? Call Tracy directly at <a href={PHONE_TEL} style={{ color: "var(--accent)", fontWeight: 600 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <div className="gold-line" />
            <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: "var(--text-light)", marginBottom: 14, fontFamily: "'DM Sans',sans-serif" }}>Why EPCO</p>
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)" }}>A Family Business Built on Trust</h2>
          </div>
          <div className="why-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "start" }}>
            ...
            <div>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "var(--text-mid)", marginBottom: 20 }}>
                EPCO Plumbing is a family operation. Tracy Poquette holds Master Plumber License #7131 and brings over 25 years of hands-on experience to every job. After relocating from Southern California to Northwest Arkansas, the Poquette family planted roots in Fayetteville and built EPCO into the go-to plumbing company for homes and businesses across the region.
              </p>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "var(--text-mid)", marginBottom: 32 }}>
                Nathan — the lead technician praised by name in nearly every customer review — is the one who shows up at your door. Whether it's a corroded pipe in a new home or a plumbing emergency at a dental office, Nathan and the EPCO team treat every job like it's their own home. Eric Poquette rounds out the family crew. As active members of BNI of the Ozarks, EPCO is woven into the Northwest Arkansas business community.
              </p>
              {/* Pull quote */}
              <div style={{ borderLeft: "3px solid var(--accent)", paddingLeft: 24, marginBottom: 36 }}>
                <p style={{ fontSize: 18, fontStyle: "italic", lineHeight: 1.7, color: "var(--primary)", fontFamily: "'Playfair Display',serif" }}>
                  "Nathan from EPCO Plumbing is the real MVP! I run a dental office and when something goes wrong, I need someone dependable."
                </p>
                <p style={{ fontSize: 14, color: "var(--text-light)", marginTop: 8, fontFamily: "'DM Sans',sans-serif" }}>- Adam Socia, Google Review</p>
              </div>
              {/* Credentials */}
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {[
                  "Master Plumber License #7131",
                  "25+ Years of Plumbing Experience",
                  "Family Owned & Operated",
                  "Available 24/7, 365 Days a Year",
                  "Residential & Commercial Service",
                  "BNI of the Ozarks Member",
                ].map((cred, i) => (
            ...
            <div className="badge-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {[
                { icon: Icons.heart, title: "Family Business", desc: "Tracy, Nathan, and Eric Poquette — a family team that relocated from California to serve Northwest Arkansas with honest, dependable plumbing." },
                { icon: Icons.award, title: "Master Plumber", desc: "Tracy holds Master Plumber License #7131 with 25+ years of experience. Every job is backed by master-level expertise and craftsmanship." },
                { icon: Icons.clock, title: "24/7 Availability", desc: "Plumbing emergencies don't wait. EPCO is available around the clock, seven days a week — including holidays. One call, fast response." },
                { icon: Icons.shield, title: "Trusted by Businesses", desc: "Dental offices, commercial properties, and homeowners across NWA trust EPCO for dependable service and honest pricing." },
              ].map((b, i) => (
        ...
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
          ...
          <div style={{ position: "relative", padding: "48px 40px", background: "var(--white)", borderRadius: 12 }}>
            ...
            <p style={{ fontSize: 20, lineHeight: 1.8, color: "var(--text)", fontStyle: "italic", fontFamily: "'Playfair Display',serif", marginBottom: 28 }}>
              "Nathan and his team came out to our new property to fix a corroded pipe, leaking fridge connection, and leaking washer/dryer connection. They did a fantastic job and we were so happy with how prompt they were."
            </p>
            <p style={{ fontSize: 17, fontWeight: 700, color: "var(--primary)" }}>Megan Martinez</p>
            <p style={{ fontSize: 13, color: "var(--text-light)", marginTop: 4 }}>Google Review</p>
          </div>
          <p style={{ fontSize: 14, color: "var(--text-light)", marginTop: 32, marginBottom: 40 }}>One of 48+ five-star reviews on Google</p>
      ...
      <section id="areas" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", textAlign: "center" }}>
          ...
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)", marginBottom: 56 }}>Serving Fayetteville & Northwest Arkansas</h2>
      ...
      <section className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)", borderTop: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)", marginBottom: 20 }}>
            Got a plumbing emergency?<br/><em style={{ color: "var(--accent)", fontStyle: "italic" }}>Call Tracy and Nathan.</em>
          </h2>
          <p style={{ fontSize: 17, lineHeight: 1.7, color: "var(--text-mid)", marginBottom: 40, maxWidth: 520, margin: "0 auto 40px" }}>
            Master plumber expertise, 24/7 availability, and a family that treats your home like their own. EPCO Plumbing — Fayetteville's trusted name in plumbing.
          </p>
      ...
      <footer style={{ background: "var(--primary)", padding: "80px clamp(24px,5vw,64px) 40px", color: "rgba(255,255,255,0.75)" }}>
        <div className="footer-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: 48, marginBottom: 48 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 15, lineHeight: 1.7, marginBottom: 20, maxWidth: 300 }}>
              Family-owned plumbing serving Fayetteville and Northwest Arkansas. Master Plumber #7131, available 24/7.
            </p>
            <p style={{ fontSize: 14, marginBottom: 8 }}>375 N Main Ave, Fayetteville, AR 72701</p>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 24 }}>
          <div className="footer-btm-inner" style={{ maxWidth: 1200, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>{new Date().getFullYear()} EPCO Plumbing. All rights reserved.</p>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>Master Plumber #7131 | BNI of the Ozarks</p>
          </div>
        </div>
      </footer>
