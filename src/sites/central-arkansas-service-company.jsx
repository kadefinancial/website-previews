...
const PHONE = "(501) 307-1310";
const TEL = "tel:+15013071310";
...
const Icons = {
  ...
  eye: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>,
};

const Stars = () => <span style={{display:"inline-flex",gap:2}}>{[...Array(5)].map((_,i)=><span key={i}>{Icons.star}</span>)}</span>;

const services = [
  { name: "Heating Service", desc: "Furnace repair, heat pump installation, and full heating system replacement to keep your home warm all winter long.", icon: Icons.flame },
  { name: "AC & Cooling Service", desc: "Air conditioning repair, replacement, and new system installation. Fast diagnostics and honest recommendations every time.", icon: Icons.wind },
  { name: "Plumbing", desc: "Drain cleaning, leak detection, sump pumps, backflow prevention, and general plumbing repairs for homes and businesses.", icon: Icons.droplet },
  { name: "Same-Day Service", desc: "When your system breaks down, we show up the same day. No waiting around, no runaround -- just fast, reliable service.", icon: Icons.zap },
  { name: "Maintenance Agreements", desc: "Scheduled tune-ups and priority service to extend system life, prevent breakdowns, and keep your equipment running efficiently.", icon: Icons.clipboard },
  { name: "Free Second Opinions", desc: "Not sure about a diagnosis or quote you received? We'll take a look at no charge and give you an honest assessment.", icon: Icons.eye },
];

const allServiceNames = ["Heating Repair & Installation","Cooling Repair & Replacement","Drain Cleaning","Sump Pump Service","Backflow Prevention","General Plumbing","Leak Detection","Maintenance Plans","Financing"];

const areas = ["North Little Rock","Little Rock","Sherwood","Jacksonville","Cabot","Maumelle","Conway","Benton","Bryant"];

const reviews = [
  { text: "Had our entire HVAC system replaced. The crew was professional, on time, and walked us through every step. Couldn't be happier with the result.", name: "Beau Turner", src: "Google" },
  { text: "Stacy in the office was incredibly helpful and kind. She made scheduling easy and followed up to make sure everything went smoothly. Great team.", name: "Michele Kulesa", src: "Google" },
  { text: "They were fast, professional, and honest. Gave us a fair price and did excellent work. Highly recommend Central Arkansas Service Company.", name: "Jyll L", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function CentralArkansasServiceCompany() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("centralar-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="2026"){sessionStorage.setItem("centralar-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "HVAC & Plumbing North Little Rock AR | Central Arkansas Service Company";
    const els = [];
    ...
    add("meta",{name:"description",content:"HVAC & plumbing in North Little Rock AR. 4.9 stars, 48+ reviews. 70+ years combined. Open 7 days. Call (501) 307-1310."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"Central Arkansas Service Company",telephone:"+15013071310",
      email:"brandon@centralarservice.com",
      address:{"@type":"PostalAddress",streetAddress:"8010 Counts Massie Rd B",addressLocality:"North Little Rock",addressRegion:"AR",postalCode:"72113",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:34.7695,longitude:-92.2668},
      openingHoursSpecification:[{"@type":"OpeningHoursSpecification",dayOfWeek:["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"],opens:"07:00",closes:"18:00"}],
      aggregateRating:{"@type":"AggregateRating",ratingValue:"4.9",reviewCount:"48",bestRating:"5"},
      founder:{"@type":"Person",name:"Brandon"},
      description:"Central Arkansas Service Company provides HVAC and plumbing services in North Little Rock and Central Arkansas. 70+ years combined experience. Open 7 days a week.",
      areaServed:areas.map(a=>({"@type":"City",name:a})),
      hasOfferCatalog:{"@type":"OfferCatalog",name:"HVAC & Plumbing Services",itemListElement:allServiceNames.map(s=>({"@type":"Offer",itemOffered:{"@type":"Service",name:s}}))},
      memberOf:[],sameAs:[],
    })});
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1C2B41"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8,fontWeight:600,letterSpacing:-0.5}}><span style={{color:"#1C2B41"}}>Central AR</span> <span style={{color:"#C8934F"}}>Service</span></div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(28,43,65,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}><span>Central AR</span> <span style={{color:"var(--accent)"}}>Service</span></div>
      ...
      <div className="hero-grid" style={{maxWidth:1200,width:"100%",display:"grid",gridTemplateColumns:"1.8fr 1fr",gap:48,alignItems:"center"}}>
        <div>
          <h1 style={{fontSize:"clamp(48px, 6vw, 84px)",lineHeight:1.0,color:"var(--primary)",letterSpacing:-2,marginBottom:24}}>
            North Little Rock's<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>HVAC & Plumbing</span><br/>Experts
          </h1>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:32}}>
            <Stars />
            <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"Fast, professional, and honest. Highly recommend." -- Jyll L</span>
          </div>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(200,147,79,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Brandon -- {PHONE}</a>
        </div>
        <div>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",color:"var(--primary)",letterSpacing:-0.5}}>HVAC & Plumbing Services in North Little Rock</h2>
        </div>
        {/* Mobile trust */}
        <div className="trust-mobile" style={{display:"none",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:48,textAlign:"center"}}>
          {[{num:"4.9",label:"Google Rating",desc:"48+ reviews"},{num:"48+",label:"Reviews",desc:"On Google"},{num:"70+",label:"Years Combined",desc:"Team experience"},{num:"7",label:"Days a Week",desc:"We're always open"}].map((s,i)=>(
            <div key={i}>
        ...
        <p style={{textAlign:"center",marginTop:48,fontSize:15,color:"var(--text-light)"}}>Questions? Call Brandon directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",position:"relative",overflow:"hidden"}}>
      <div className="serif" style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",fontSize:"clamp(120px,20vw,280px)",color:"var(--accent)",opacity:0.08,lineHeight:1,whiteSpace:"nowrap",pointerEvents:"none"}}>70</div>
      <div style={{maxWidth:800,margin:"0 auto",position:"relative",zIndex:1}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,textAlign:"center",opacity:0.7}}>About Us</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff",textAlign:"center"}}>Why North Little Rock Trusts<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Central AR Service</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:24,textAlign:"center"}}>
          Central Arkansas Service Company is the team North Little Rock calls when they need HVAC or plumbing done right. Led by Brandon, the crew brings over 70 years of combined experience to every job -- from heating and cooling repairs to drain cleaning and leak detection. "We know a guy" is the tagline, and it's earned. When your neighbors need help, your name comes up.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:36,textAlign:"center"}}>
          Open seven days a week from 7 AM to 6 PM, Central AR Service is a growing company built on same-day service, free second opinions, and a 100% satisfaction guarantee. Whether it's a busted AC in July or a plumbing emergency on a Sunday, Brandon and his team show up. Financing available. Maintenance agreements to keep your systems running year-round.
        </p>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:20}}>
          {["70+ Years Combined Experience","Same-Day Service","100% Satisfaction Guarantee","Free Second Opinions","Open 7 Days a Week","Financing Available"].map((c,i)=>(
    ...
    <section id="areas" style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:12}}>
          {areas.map((a,i)=>(
            <span key={i} title={`HVAC & Plumbing in ${a}, AR`} style={{display:"inline-flex",alignItems:"center",gap:6,background:"#fff",border:"1px solid var(--border)",padding:"12px 24px",borderRadius:30,fontSize:15,fontWeight:500,color:"var(--text)",cursor:"default",transition:"all 0.25s"}}
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>Need HVAC or plumbing?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Central AR Service.</span></h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>Same-day service. Free second opinions. 100% satisfaction guarantee. Open 7 days a week.</p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16,fontWeight:600,letterSpacing:-0.5}}><span>Central AR</span> <span style={{color:"var(--accent)"}}>Service</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>HVAC and plumbing services for North Little Rock and Central Arkansas. 70+ years combined experience. Open 7 days a week. "We know a guy."</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>8010 Counts Massie Rd B<br/>North Little Rock, AR 72113<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a><br/><a href="mailto:brandon@centralarservice.com" style={{color:"rgba(255,255,255,0.5)"}}>brandon@centralarservice.com</a></p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} Central Arkansas Service Company. All rights reserved.</span>
          <span>HVAC & Plumbing | Open 7 Days</span>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Central AR Service</a>
    </div>
