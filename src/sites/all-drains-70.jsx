// Layout: A | Industry: Plumber | City: Bryant
import { useState, useEffect, useRef } from "react";

const PHONE = "(501) 909-9172";
const TEL = "tel:+15019099172";
...
const services = [
  { name: "Water Heater Service", desc: "Tank and tankless water heater installation, repair, and replacement. Fast turnaround from a family team that's done it for 14 years.", icon: Icons.flame },
  { name: "Drain Cleaning", desc: "Stubborn clogs, slow drains, and recurring backups cleared with professional-grade equipment. Kitchens, bathrooms, and main lines.", icon: Icons.droplets },
  { name: "Gas Line Service", desc: "Licensed gas line repair and installation for appliances, water heaters, and new construction. Safety-first approach.", icon: Icons.zap },
  { name: "Sewer & Water Lines", desc: "Complete sewer and water line repair, replacement, and installation. Thorough diagnostics before any digging.", icon: Icons.wrench },
  { name: "Leak Detection & Pipe Repair", desc: "Find and fix hidden leaks before they cause costly damage. Full pipe repair and replacement services.", icon: Icons.search },
  { name: "Fixture Installation", desc: "Toilets, faucets, garbage disposals, and bathtubs installed and repaired with precision and care.", icon: Icons.home },
];

const allServiceNames = ["Water Heaters","Tankless Water Heaters","Faucets","Garbage Disposals","Toilets","Gas Lines","Sewer Lines","Water Lines","Leak Detection","Pipe Repair/Replacement","Drain Cleaning","Bathtub Repair","Faucet Installation","Drain Installation"];

const areas = ["Bryant","Benton","Alexander","Austin","Little Rock","North Little Rock","Sherwood","Jacksonville","Cabot","Maumelle","Conway"];

const reviews = [
  { text: "My husband and I were having problems with the tub and the dishwasher not draining properly. We had tried everything for the tub and nothing worked. We called and they came right out.", name: "Stephanye Smirnoff", src: "Google" },
  { text: "Ben came here to fix a gas leak. He was thorough, located the leak and fixed the leaking valve. Great family owned firm that I highly recommend.", name: "Ronald Cohen", src: "Google" },
  { text: "Very professional, reasonably priced, honest and hardworking! Would recommend!", name: "Sabrina Main", src: "Google" },
  { text: "I am officially updating my previous review to 5 stars. While my initial experience was beyond frustrating, the owner stepped up and made everything right.", name: "Dustin Singer", src: "Google" },
  { text: "We had an issue with a service technician they contract out to, but once we contacted the owner, the situation was resolved and taken care of professionally.", name: "Alyssa Singer", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function AllDrains70() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("ad70-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="2026"){sessionStorage.setItem("ad70-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => { if(!authed) return; revTimer.current=setInterval(()=>setRevIdx(i=>(i+1)%reviews.length),6000); return()=>clearInterval(revTimer.current); }, [authed]);

  useEffect(() => {
    if(!authed) return;
    document.title = "Plumber Bryant AR | All Drains 70 Plumbing";
    const els = [];
    ...
    add("meta",{name:"description",content:"Family-owned plumber in Bryant, AR. 4.8 stars, BBB A+ rated. Call (501) 909-9172 for a free quote."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"All Drains 70 Plumbing LLC",telephone:"+15019099172",
      address:{"@type":"PostalAddress",streetAddress:"3207 Meadowlake Dr",addressLocality:"Bryant",addressRegion:"AR",postalCode:"72022",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:34.5960,longitude:-92.4890},
      openingHoursSpecification:[{"@type":"OpeningHoursSpecification",dayOfWeek:["Monday","Tuesday","Wednesday","Thursday","Friday"],opens:"08:00",closes:"18:00"}],
      aggregateRating:{"@type":"AggregateRating",ratingValue:"4.8",reviewCount:"51",bestRating:"5"},
      founder:{"@type":"Person",name:"Gregory Connor"},foundingDate:"2011",
      description:"All Drains 70 Plumbing LLC is a family-owned plumbing company in Bryant, AR serving the greater Little Rock metro since 2011.",
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1C2B41"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8}}><span style={{fontWeight:700,color:"#1C2B41"}}>All Drains 70</span> <span style={{color:"#C8934F"}}>Plumbing</span></div>
  ...
  return (<>
    <style>{`
      ...
      *{margin:0;padding:0;box-sizing:border-box}html{scroll-behavior:smooth}
      ...
      @media(max-width:900px){.desktop-only{display:none!important}.mobile-only{display:flex!important}.hero-grid{grid-template-columns:1fr!important}.trust-grid{grid-template-columns:1fr 1fr!important}.svc-grid{grid-template-columns:1fr!important}.why-grid{grid-template-columns:1fr!important}.badge-grid{grid-template-columns:1fr 1fr!important}.footer-grid{grid-template-columns:1fr!important}.footer-btm-inner{flex-direction:column;gap:8px;text-align:center}}
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(28,43,65,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}><span>All Drains 70</span> <span style={{color:"var(--accent)"}}>Plumbing</span></div>
      ...
      <div className="hero-grid" style={{maxWidth:1200,width:"100%",display:"grid",gridTemplateColumns:"1.2fr 1fr",gap:48,alignItems:"center"}}>
        <div style={{maxWidth:560}}>
          <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(200,147,79,0.1)",padding:"8px 18px",borderRadius:4,marginBottom:28}}>
            ...
            <span style={{fontSize:12,fontWeight:700,color:"var(--accent)",letterSpacing:2,textTransform:"uppercase"}}>Family Owned Since 2011</span>
          </div>
          <h1 style={{fontSize:"clamp(40px, 5vw, 68px)",lineHeight:1.05,color:"var(--primary)",marginBottom:24,letterSpacing:-1.5}}>
            Bryant's<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Most Trusted</span><br/>Plumber
          </h1>
          <p style={{fontSize:18,lineHeight:1.7,color:"var(--text-mid)",marginBottom:28,maxWidth:460}}>
            Ben and Gregory Connor have been keeping drains flowing across Central Arkansas for over 14 years. 4.8 stars, BBB A+ rated, and built on family values.
          </p>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:32}}>
            <Stars />
            <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"Great family owned firm. Highly recommend." — Ronald Cohen</span>
          </div>
          <div style={{display:"flex",gap:14,flexWrap:"wrap"}}>
            <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(200,147,79,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Ben Now</a>
    ...
    <section style={{padding:"72px clamp(24px,5vw,64px)",background:"var(--white)",borderTop:"1px solid var(--border)",borderBottom:"1px solid var(--border)"}}>
      <div className="trust-grid" style={{maxWidth:1000,margin:"0 auto",display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:32,textAlign:"center"}}>
        {[{num:"4.8",label:"Google Rating",desc:"51+ reviews"},{num:"14+",label:"Years",desc:"Serving since 2011"},{num:"A+",label:"BBB Rating",desc:"Accredited"},{num:"Family",label:"Owned",desc:"Connor family"}].map((s,i)=>(
          <div key={i}>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",color:"var(--primary)",letterSpacing:-0.5}}>Plumbing Services in Bryant, AR</h2>
        </div>
        ...
        <p style={{textAlign:"center",marginTop:48,fontSize:15,color:"var(--text-light)"}}>Questions? Call Ben directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",color:"#fff"}}>
      <div className="why-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.2fr 0.8fr",gap:80,alignItems:"start"}}>
        <div>
          ...
          <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,opacity:0.7}}>About All Drains 70</div>
          <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff"}}>Why Bryant Trusts<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>the Connors</span></h2>
          <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.65)",marginBottom:24}}>
            All Drains 70 Plumbing is a family operation through and through. Gregory Connor handles the business side while his son Ben is the lead plumber in the field. Together, they've been serving Bryant and the greater Little Rock metro since 2011 with a simple approach: fair pricing, honest work, and showing up when they say they will.
          </p>
          <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.65)",marginBottom:36}}>
            With a 4.8-star Google rating and BBB A+ accreditation, customers consistently praise their professionalism, reasonable prices, and willingness to make things right. Ben personally handles service calls — when you call All Drains 70, you're getting the owner, not a subcontractor.
          </p>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            {["Family Owned & Operated Since 2011","BBB Accredited (A+ Rating)","Licensed & Insured","Serving Pulaski & Saline Counties","Owner-Operated Service Calls"].map((c,i)=>(
        ...
        <div className="badge-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,paddingTop:20}}>
          {[
            ...
            {icon:Icons.shield,title:"BBB A+",desc:"Accredited since 2022"},
            {icon:Icons.wrench,title:"Full Service",desc:"Install, repair, maintain"},
            {icon:Icons.zap,title:"Same-Day",desc:"Fast response times"},
          ].map((b,i)=>(
    ...
    <section id="reviews" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--white)"}}>
      <div style={{maxWidth:800,margin:"0 auto",textAlign:"center"}}>
        ...
        <div style={{background:"var(--cream)",borderRadius:12,padding:"52px 48px",border:"1px solid var(--border)",position:"relative",minHeight:180}}>
          ...
          <div style={{position:"relative",zIndex:1}}>
            ...
            <p style={{fontSize:18,fontStyle:"italic",color:"var(--text)",lineHeight:1.75,marginBottom:24}}>"{reviews[revIdx].text}"</p>
        ...
        <p style={{marginTop:36,fontSize:15,color:"var(--text-light)"}}>Ready to experience the difference? Call <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
      </div>
    </section>
    ...
    <section id="areas" style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        ...
        <h2 style={{fontSize:"clamp(28px,3vw,38px)",color:"var(--primary)",marginBottom:48,letterSpacing:-0.5}}>Serving Bryant & Central Arkansas</h2>
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>Got a plumbing problem?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Ben.</span></h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>Family owned, BBB A+ rated, and serving Bryant since 2011.</p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16}}><span>All Drains 70</span> <span style={{color:"var(--accent)"}}>Plumbing</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>Family-owned plumbing company serving Bryant and Central Arkansas since 2011. BBB A+ Accredited.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>3207 Meadowlake Dr<br/>Bryant, AR 72022<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} All Drains 70 Plumbing LLC. All rights reserved.</span>
          <span>BBB Accredited (A+) &middot; Licensed & Insured</span>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call All Drains 70</a>
    </div>
