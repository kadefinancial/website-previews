...
const PHONE = "(501) 753-2662";
const TEL = "tel:+15017532662";
...
const Icons = {
  ...
  leaf: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M11 20A7 7 0 019.8 6.9C15.5 4.9 17 3.5 17 3.5s-.3 3.5-1.5 6C14 12.5 11 14 11 20z"/><path d="M6.7 17.3c3-3 6-4 10.3-5.3"/></svg>,
...
const services = [
  { name: "Air Conditioning Repair & Service", desc: "Fast, dependable AC repair for all makes and models. Joe diagnoses the issue and fixes it right the first time so your home stays cool.", icon: Icons.wind },
  { name: "Furnace Repair & Service", desc: "Expert furnace diagnostics and repair to keep your home warm all winter. Honest assessments and straightforward pricing on every call.", icon: Icons.flame },
  { name: "Heat Pump Repair & Service", desc: "Year-round comfort from one efficient system. Professional heat pump repair, maintenance, and seasonal tune-ups you can count on.", icon: Icons.thermometer },
  { name: "AC & Furnace Installation", desc: "Complete system installation sized and designed for your home. Carrier equipment professionally installed for maximum efficiency and comfort.", icon: Icons.wrench },
  { name: "HVAC Maintenance & Tune-Ups", desc: "Preventative maintenance that extends the life of your system, lowers energy bills, and catches small issues before they become expensive repairs.", icon: Icons.clipboard },
  { name: "Indoor Air Quality Products", desc: "Breathe easier with filtration, purification, and humidity control solutions. Carrier-quality products installed by a team you trust.", icon: Icons.leaf },
];
...
const areas = ["North Little Rock", "Central Arkansas"];

const reviews = [
  { text: "Joe is great. Been with Northside for many many years and he always comes right away and fixes anything I am having issues with. There is no one better in my opinion.", name: "Julie Bevans", src: "Google" },
  { text: "We have used Northside for years. Shirley is always quick to respond and Joe is straightforward re: what needs to be done and how much it will cost.", name: "Jason Pederson", src: "Google" },
  { text: "Always Fair Pricing And Prifessional! Been using them for years", name: "Gail Nelms", src: "Google" },
];
...
export default function NorthsideHeatAndAir() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("northside-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("northside-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "HVAC Service North Little Rock AR | Northside Heat and Air";
    const els = [];
    const add = (tag, attrs) => {
    ...
    add("meta", { name: "description", content: "HVAC repair, installation & maintenance in North Little Rock, AR. Carrier dealer. Call Joe at (501) 753-2662." });
    ...
    add("script", {
      ...
      textContent: JSON.stringify({
        ...
        "@type": "HVACBusiness",
        name: "Northside Heat and Air",
        telephone: "+15017532662",
        address: { "@type": "PostalAddress", streetAddress: "1616 Splawn Dr", addressLocality: "North Little Rock", addressRegion: "AR", postalCode: "72118", addressCountry: "US" },
        geo: { "@type": "GeoCoordinates", latitude: 34.7834, longitude: -92.2671 },
        openingHoursSpecification: [{ "@type": "OpeningHoursSpecification", dayOfWeek: ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"], opens: "08:00", closes: "17:00" }],
        aggregateRating: { "@type": "AggregateRating", ratingValue: "4.3", reviewCount: "6", bestRating: "5" },
        founder: { "@type": "Person", name: "Joe" },
        description: "Carrier-authorized HVAC repair, installation, and maintenance in North Little Rock, AR and Central Arkansas. Owner Joe on every call.",
        ...
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#4A1A2E" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, marginBottom: 8, fontWeight: 600, letterSpacing: -0.5 }}>
              <span style={{ color: "#4A1A2E" }}>Northside</span>{" "}
              <span style={{ color: "#2C5282" }}>Heat and Air</span>
            </div>
  ...
  return (
    <>
      ...
      {/* NAV */}
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled ? "rgba(44,82,130,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition: "all 0.4s ease" }}>
        <div className="serif" style={{ fontSize: 22, color: scrolled ? "#fff" : "var(--primary)", fontWeight: 600, letterSpacing: -0.5 }}>
          <span>Northside</span>{" "}
          <span style={{ color: "var(--accent)" }}>Heat and Air</span>
        </div>
      ...
      <section style={{ minHeight: "100vh", padding: "140px clamp(24px,5vw,64px) 100px", background: "var(--cream)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 48, alignItems: "center" }}>
          <div>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(44,82,130,0.1)", padding: "8px 16px", borderRadius: 40, marginBottom: 24 }}>
              <span style={{ fontSize: 13, fontWeight: 600, color: "var(--accent)" }}>Carrier Authorized Dealer</span>
            </div>
            <h1 style={{ fontSize: "clamp(40px,5vw,68px)", lineHeight: 1.05, color: "var(--primary)", letterSpacing: -1.5, marginBottom: 20 }}>
              North Little Rock's<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Most Loyal</span><br />HVAC Team
            </h1>
            <p style={{ fontSize: 18, lineHeight: 1.7, color: "var(--text-mid)", maxWidth: 480, marginBottom: 28 }}>
              Joe and Shirley have been keeping Central Arkansas comfortable for decades. Carrier equipment, fair pricing, and the kind of service that keeps customers coming back for years.
            </p>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 32 }}>
              <Stars />
              <span style={{ fontSize: 14, color: "var(--text-light)", fontStyle: "italic", marginLeft: 4 }}>"There is no one better in my opinion." — Julie Bevans</span>
            </div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 14 }}>
              <a href={TEL} style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 36px", borderRadius: 6, fontSize: 16, fontWeight: 700, boxShadow: "0 4px 20px rgba(44,82,130,0.3)", fontFamily: "'DM Sans',sans-serif", transition: "transform 0.3s ease" }}>{Icons.phone} Call Joe — {PHONE}</a>
      ...
      <section style={{ background: "var(--white)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", padding: "48px clamp(24px,5vw,64px)" }}>
        <div className="trust-row" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 32, textAlign: "center" }}>
          {[
            { num: "Decades", label: "Of Customer Loyalty", desc: "Customers for years" },
            { num: "Carrier", label: "Authorized Dealer", desc: "Premium equipment" },
            { num: "Fair", label: "Pricing Always", desc: "Straightforward costs" },
            { num: "Joe", label: "On Every Call", desc: "Owner-operated service" },
          ].map((s, i) => (
            <div key={i}>
      ...
      <section id="services" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "var(--primary)", letterSpacing: -0.5 }}>HVAC Services in North Little Rock</h2>
          </div>
          ...
          <p style={{ textAlign: "center", marginTop: 40, fontSize: 15, color: "var(--text-mid)" }}>
            Questions? Call Joe directly at <a href={TEL} style={{ color: "var(--accent)", fontWeight: 700 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--primary)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <div className="gold-line" style={{ margin: "0 auto 20px" }} />
            <div style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: 3, color: "var(--accent)", marginBottom: 14 }}>Why Northside</div>
            <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "#fff", letterSpacing: -0.5 }}>The Team North Little Rock Trusts</h2>
          </div>
          <div className="why-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "start" }}>
            <div>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "rgba(255,255,255,0.82)", marginBottom: 20 }}>
                Northside Heat and Air is built on one simple idea: when you call for help, Joe shows up. Not a random technician, not a subcontractor — the owner himself. That personal touch is why customers have been coming back for decades, not years.
              </p>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "rgba(255,255,255,0.82)", marginBottom: 32 }}>
                Call the office and Shirley picks up — quick, friendly, and ready to get Joe to your door. Joe is straightforward about what needs to be done and what it costs. No surprises, no upsells. As a Carrier Authorized Dealer, Northside installs and services premium equipment with the expertise to back it up. Every customer who reviews them says the same thing: they come back for years.
              </p>
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {[
                  "Owner Joe on every service call",
                  "Carrier Authorized Dealer",
                  "Fair, straightforward pricing — no surprises",
                  "Shirley responds quickly to every call",
                  "Decades of loyal, returning customers",
                  "Mon-Fri 8AM-5PM service hours",
                ].map((c, i) => (
            ...
            <div className="badge-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {[
                { icon: Icons.truck, title: "Joe On Every Call", desc: "The owner handles your service personally, every time" },
                { icon: Icons.award, title: "Carrier Dealer", desc: "Authorized to install and service Carrier equipment" },
                { icon: Icons.dollar, title: "Fair Pricing", desc: "Straightforward costs with no surprises or upsells" },
                { icon: Icons.heart, title: "Decades of Loyalty", desc: "Customers keep coming back year after year" },
              ].map((b, i) => (
      ...
      <section id="reviews" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "var(--primary)", letterSpacing: -0.5 }}>What Our Customers Say</h2>
          </div>
      ...
      <section style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)", borderTop: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "var(--primary)", letterSpacing: -0.5, lineHeight: 1.15, marginBottom: 20 }}>
            HVAC acting up?<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Call Joe.</span>
          </h2>
          <p style={{ fontSize: 17, lineHeight: 1.7, color: "var(--text-mid)", marginBottom: 36, maxWidth: 520, margin: "0 auto 36px" }}>
            Fair pricing. Carrier-authorized service. The owner on every call. That's Northside.
          </p>
          <a href={TEL} style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "20px 44px", borderRadius: 6, fontSize: 18, fontWeight: 700, boxShadow: "0 4px 20px rgba(44,82,130,0.3)", fontFamily: "'DM Sans',sans-serif" }}>{Icons.phone} {PHONE}</a>
        </div>
      </section>
      ...
      <footer style={{ background: "var(--primary)", padding: "80px clamp(24px,5vw,64px) 40px" }}>
        <div className="footer-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: 48, marginBottom: 48 }}>
          <div>
            <div className="serif" style={{ fontSize: 22, fontWeight: 600, letterSpacing: -0.5, marginBottom: 16 }}>
              <span style={{ color: "#fff" }}>Northside</span>{" "}
              <span style={{ color: "var(--accent)" }}>Heat and Air</span>
            </div>
            <p style={{ fontSize: 14, lineHeight: 1.7, color: "rgba(255,255,255,0.6)", marginBottom: 16, maxWidth: 320 }}>
              Carrier-authorized HVAC repair, installation, and maintenance in North Little Rock, AR and Central Arkansas. Owner Joe on every call.
            </p>
            <p style={{ fontSize: 14, color: "rgba(255,255,255,0.6)", marginBottom: 6 }}>1616 Splawn Dr, North Little Rock, AR 72118</p>
            <a href={TEL} style={{ fontSize: 14, color: "var(--accent)", fontWeight: 700 }}>{PHONE}</a>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.45)", marginTop: 10 }}>Mon-Fri 8AM-5PM</p>
          </div>
          <div>
        ...
        <div className="footer-btm" style={{ maxWidth: 1200, margin: "0 auto", borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 24, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>&copy; {yr} Northside Heat and Air. All rights reserved.</p>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>Carrier Authorized Dealer</p>
        </div>
      </footer>
      ...
      <div className="mobile-only" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 998, display: "none" }}>
        <a href={TEL} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "16px 20px", fontSize: 16, fontWeight: 700, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 -4px 20px rgba(0,0,0,0.15)", textDecoration: "none" }}>
          {Icons.phone} Call Northside — {PHONE}
        </a>
      </div>
