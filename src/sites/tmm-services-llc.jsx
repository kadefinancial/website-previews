...
const Icons = {
  ...
  wind: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M9.59 4.59A2 2 0 1111 8H2"/><path d="M12.59 19.41A2 2 0 1014 16H2"/><path d="M17.73 7.73A2.5 2.5 0 1119.5 12H2"/></svg>
  ),
  droplet: (
  ...
  thermometer: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M14 14.76V3.5a2.5 2.5 0 00-5 0v11.26a4.5 4.5 0 105 0z"/></svg>
  ),
  flame: (
...
// --- Data ---
const PHONE = "(479) 251-9403";
const PHONE_TEL = "tel:+14792519403";
const BIZ = "TMM Services";

const services = [
  { name: "HVAC Replacement & Repair", desc: "Complete heating and cooling system replacement, repair, and maintenance for homes and businesses across Northwest Arkansas. Ronnie's team diagnoses problems other companies miss — and gets it right the first time.", icon: Icons.wind },
  { name: "Plumbing Repair & Installation", desc: "From leaking faucets to full pipe replacements, TMM handles every plumbing job with the same attention to detail. Honest diagnosis, quality work, and no surprise charges.", icon: Icons.droplet },
  { name: "Electrical Servicing", desc: "Licensed electrical work for residential and commercial properties. Panel upgrades, wiring repairs, outlet installations, and troubleshooting from a team you can trust.", icon: Icons.zap },
  { name: "AC Installation & Repair", desc: "Fast, reliable air conditioning installation and repair when Fayetteville summers demand it. TMM keeps your home cool with professional-grade equipment and honest pricing.", icon: Icons.thermometer },
  { name: "Furnace Repair", desc: "Expert furnace diagnostics and repair to keep your home warm through Arkansas winters. TMM identifies the root cause — not just the symptoms — so the fix lasts.", icon: Icons.flame },
  { name: "Residential & Commercial", desc: "Whether it's a family home or a commercial property, TMM Services brings the same multi-trade expertise to every job. One call covers HVAC, plumbing, and electrical.", icon: Icons.home },
];

const allServices = [
  "HVAC Replacement & Repair", "Plumbing Repair & Installation", "Electrical Servicing",
  "AC Installation & Repair", "Furnace Repair", "Residential & Commercial",
];

const reviews = [
  { text: "We had another HVAC company out to fix our upstairs air conditioner. They were about ready to open up a wall, because they couldn't find the leak and thought it was there. We decided to get a second opinion and I'm so glad we did! TMM came out and found the issue.", name: "Chris Washington", src: "Google" },
  { text: "Professional, fast servicing when my AC went out. Very friendly and honest. Would recommend!", name: "Dillon Gash", src: "Google" },
];

const areas = ["Fayetteville", "Springdale", "Rogers", "Bentonville", "Northwest Arkansas"];

// --- Main Component ---
export default function TmmServicesLlc() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("tmm-services-auth") === "1");
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "HVAC Plumbing Electrical Fayetteville AR | TMM Services";
    const els = [];
    const add = (tag, attrs) => {
    ...
    add("meta", { name: "description", content: "TMM Services — 5.0 stars, multi-trade HVAC, plumbing & electrical in Fayetteville, AR. Call (479) 251-9403 for a free quote." });
    ...
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      ...
      "@type": "Plumber",
      name: "TMM Services LLC",
      telephone: "+14792519403",
      address: { "@type": "PostalAddress", streetAddress: "135 W Sunbridge Dr", addressLocality: "Fayetteville", addressRegion: "AR", postalCode: "72703", addressCountry: "US" },
      ...
      founder: { "@type": "Person", name: "Ronnie Davidson" },
      description: "TMM Services is a multi-trade contractor serving Fayetteville and Northwest Arkansas with HVAC, plumbing, and electrical services. 5.0 stars on Google.",
        ...
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#1C2B41" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, fontWeight: 600, letterSpacing: -0.5, marginBottom: 4 }}>
              <span style={{ color: "#1C2B41" }}>TMM</span> <span style={{ color: "#C8934F" }}>Services</span>
            </div>
    ...
    <>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled ? "rgba(28,43,65,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition: "all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        <div className="serif" style={{ fontSize: 22, fontWeight: 600, letterSpacing: -0.5, color: "#fff" }}>
          <span>TMM</span><span style={{ color: "var(--accent)", marginLeft: 6 }}>Services</span>
        </div>
        ...
        <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
          ...
          <div style={{ maxWidth: 560 }}>
            ...
            <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(40px,5vw,68px)", lineHeight: 1.05, letterSpacing: -1.5, color: "#fff", marginBottom: 24 }}>
              Fayetteville's{" "}<br/>
              <em style={{ color: "var(--accent)", fontStyle: "italic" }}>Multi-Trade Pro</em><br/>
              For Every Job
            </h1>
            <p style={{ fontSize: 18, lineHeight: 1.7, color: "rgba(255,255,255,0.75)", marginBottom: 32, maxWidth: 480 }}>
              HVAC, plumbing, and electrical — all from one trusted team. Ronnie Davidson and the TMM Services crew handle what other companies can't, finding problems others miss across Northwest Arkansas.
            </p>
            {/* Star row with hero quote */}
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 32, flexWrap: "wrap" }}>
              <Stars />
              <span style={{ fontSize: 14, color: "rgba(255,255,255,0.7)", fontStyle: "italic" }}>"Professional, fast servicing. Very friendly and honest."</span>
              <span style={{ fontSize: 13, color: "rgba(255,255,255,0.5)" }}>- Dillon Gash</span>
            </div>
      ...
      <section style={{ background: "var(--white)", padding: "48px clamp(24px,5vw,64px)" }}>
        <div className="trust-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 32, textAlign: "center" }}>
          {[
            ...
            { num: "3-Trade", label: "HVAC + Plumbing + Electrical", desc: "One team, every trade" },
            { num: "Top 53%", label: "AR Contractor Score", desc: "BuildZoom verified" },
            { num: "Ronnie", label: "On Every Job", desc: "Owner-operated" },
          ].map((s, i) => (
            <div key={i} style={{ padding: "16px 0" }}>
              <div style={{ fontFamily: "'Playfair Display',serif", fontSize: i === 1 || i === 2 ? "clamp(22px,2.5vw,32px)" : "clamp(28px,3vw,40px)", fontWeight: 600, color: "var(--primary)", letterSpacing: -0.5, marginBottom: 6 }}>{s.num}</div>
      ...
      <section id="services" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)" }}>HVAC, Plumbing & Electrical in Fayetteville</h2>
          </div>
          {services.map((svc, i) => {
            const isEven = i % 2 === 1;
            return (
          ...
          <p style={{ textAlign: "center", marginTop: 48, fontSize: 15, color: "var(--text-mid)" }}>
            Questions? Call Ronnie directly at <a href={PHONE_TEL} style={{ color: "var(--accent)", fontWeight: 600 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <div className="gold-line" />
            <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: "var(--text-light)", marginBottom: 14, fontFamily: "'DM Sans',sans-serif" }}>Why TMM Services</p>
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)" }}>One Company for Every Trade in Fayetteville</h2>
          </div>
          <div className="why-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "start" }}>
            ...
            <div>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "var(--text-mid)", marginBottom: 20 }}>
                Most homeowners juggle separate contractors for heating, plumbing, and electrical work. TMM Services changes that. Owner Ronnie Davidson built a multi-trade team that handles all three disciplines under one roof — so you make one call, deal with one crew, and get one honest invoice. No finger-pointing between contractors. No scheduling headaches.
              </p>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "var(--text-mid)", marginBottom: 32 }}>
                That multi-trade expertise is exactly why customers call TMM for second opinions. When another HVAC company was about to open up a wall to find a leak, TMM came out and found the actual issue without the demolition. With a contractor score of 90 on BuildZoom — top 53% in Arkansas — and a perfect 5.0 Google rating, Ronnie's team earns trust by finding what others miss.
              </p>
              {/* Pull quote */}
              <div style={{ borderLeft: "3px solid var(--accent)", paddingLeft: 24, marginBottom: 36 }}>
                <p style={{ fontSize: 18, fontStyle: "italic", lineHeight: 1.7, color: "var(--primary)", fontFamily: "'Playfair Display',serif" }}>
                  "We decided to get a second opinion and I'm so glad we did! TMM came out and found the issue."
                </p>
                <p style={{ fontSize: 14, color: "var(--text-light)", marginTop: 8, fontFamily: "'DM Sans',sans-serif" }}>- Chris Washington, Google Review</p>
              </div>
              {/* Credentials */}
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {[
                  "Perfect 5.0-Star Google Rating",
                  "3-Trade: HVAC + Plumbing + Electrical",
                  "Contractor Score 90 — Top 53% in Arkansas",
                  "Owner Ronnie Davidson on Every Job",
                  "Donny Edwards — Verified Contractor on Staff",
                  "Mon-Fri 8 AM - 5 PM Service Hours",
                ].map((cred, i) => (
            ...
            <div className="badge-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {[
                { icon: Icons.award, title: "Perfect 5.0 Rating", desc: "Every single Google review is five stars. TMM Services doesn't just meet expectations — they exceed them on every call across every trade." },
                { icon: Icons.wrench, title: "3 Trades, 1 Team", desc: "HVAC, plumbing, and electrical expertise under one roof. One call to TMM replaces three separate contractor appointments." },
                { icon: Icons.shield, title: "Top 53% in Arkansas", desc: "BuildZoom contractor score of 90 puts TMM in the top half of all Arkansas contractors. Verified quality you can count on." },
                { icon: Icons.heart, title: "Ronnie on Every Job", desc: "Owner Ronnie Davidson is personally involved on every project. You're not getting a random technician — you're getting the owner's attention." },
              ].map((b, i) => (
      ...
      <section id="reviews" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
          ...
          {/* Large featured review — Chris Washington */}
          <div style={{ position: "relative", padding: "48px 40px", background: "var(--white)", borderRadius: 12 }}>
            ...
            <p style={{ fontSize: 20, lineHeight: 1.8, color: "var(--text)", fontStyle: "italic", fontFamily: "'Playfair Display',serif", marginBottom: 28 }}>
              "We had another HVAC company out to fix our upstairs air conditioner. They were about ready to open up a wall, because they couldn't find the leak and thought it was there. We decided to get a second opinion and I'm so glad we did! TMM came out and found the issue."
            </p>
            <p style={{ fontSize: 17, fontWeight: 700, color: "var(--primary)" }}>Chris Washington</p>
      ...
      <section id="areas" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", textAlign: "center" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 12, justifyContent: "center" }}>
            {areas.map((area, i) => (
              <span key={i} title={`HVAC Plumbing Electrical in ${area}, AR`} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "12px 22px", background: "var(--white)", border: "1px solid var(--border)", borderRadius: 100, fontSize: 14, fontWeight: 500, color: "var(--text)", cursor: "default", transition: "all 0.2s ease", fontFamily: "'DM Sans',sans-serif" }}
      ...
      <section className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)", borderTop: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)", marginBottom: 20 }}>
            Got an HVAC, plumbing, or electrical problem?<br/><em style={{ color: "var(--accent)", fontStyle: "italic" }}>Call Ronnie.</em>
          </h2>
          <p style={{ fontSize: 17, lineHeight: 1.7, color: "var(--text-mid)", marginBottom: 40, maxWidth: 520, margin: "0 auto 40px" }}>
            Three trades, one honest team, and a perfect Google rating. TMM Services — the Fayetteville contractor who finds what others miss.
          </p>
      ...
      <footer style={{ background: "var(--primary)", padding: "80px clamp(24px,5vw,64px) 40px", color: "rgba(255,255,255,0.75)" }}>
        <div className="footer-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: 48, marginBottom: 48 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 15, lineHeight: 1.7, marginBottom: 20, maxWidth: 300 }}>
              Multi-trade HVAC, plumbing, and electrical services for Fayetteville and Northwest Arkansas. 5.0 stars on Google.
            </p>
            <p style={{ fontSize: 14, marginBottom: 8 }}>135 W Sunbridge Dr, Fayetteville, AR 72703</p>
            <p style={{ fontSize: 14, marginBottom: 8 }}>Mon-Fri 8 AM - 5 PM</p>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 24 }}>
          <div className="footer-btm-inner" style={{ maxWidth: 1200, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>{new Date().getFullYear()} TMM Services LLC. All rights reserved.</p>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>HVAC + Plumbing + Electrical</p>
          </div>
        </div>
      </footer>
      ...
      <div className="mobile-only" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 998, display: "none" }}>
        <a href={PHONE_TEL} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 24px", fontSize: 16, fontWeight: 700, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 -4px 20px rgba(0,0,0,0.15)", textAlign: "center" }}>
          {Icons.phone} Call TMM Services - {PHONE}
        </a>
      </div>
