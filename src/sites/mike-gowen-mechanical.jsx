// Layout: D | Industry: HVAC | City: North Little Rock
import { useState, useEffect, useRef } from "react";

const PHONE = "(501) 537-3444";
const TEL = "tel:+15015373444";
...
const Icons = {
  ...
  hammer: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M15 12l-8.5 8.5c-.83.83-2.17.83-3 0 0 0 0 0 0 0a2.12 2.12 0 010-3L12 9"/><path d="M17.64 15L22 10.64"/><path d="M20.91 11.7l-1.25-1.25c-.6-.6-.93-1.4-.93-2.25V6.5a.5.5 0 01.5-.5h1.76c.85 0 1.65.33 2.25.93l1.25 1.25"/><path d="M11.71 7.04l-2.03 2.03a2 2 0 010 2.83l1.41 1.41a2 2 0 002.83 0l2.03-2.03"/></svg>,
  building: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="2" width="16" height="20" rx="2" ry="2"/><path d="M9 22V12h6v10"/><path d="M8 6h.01"/><path d="M16 6h.01"/><path d="M12 6h.01"/><path d="M12 10h.01"/><path d="M16 10h.01"/><path d="M8 10h.01"/></svg>,
...
const services = [
  { name: "Commercial HVAC Installation & Service", desc: "Complete heating, ventilation, and air conditioning systems for commercial and industrial facilities. From design through commissioning, we deliver reliable climate control built for scale.", icon: Icons.wind },
  { name: "Commercial Plumbing", desc: "Full-service commercial plumbing for new construction, renovations, and ongoing maintenance. Engineered plumbing systems that meet code and perform under demand.", icon: Icons.droplets },
  { name: "Mechanical Insulation Contracting", desc: "Professional insulation for piping, ductwork, and mechanical equipment. Maximizing energy efficiency and system performance across commercial and industrial applications.", icon: Icons.layers },
  { name: "Demolition Services", desc: "Controlled demolition and selective removal for commercial renovation and redevelopment projects. Safe, efficient teardown with proper disposal and site preparation.", icon: Icons.hammer },
  { name: "New Construction Mechanical", desc: "Ground-up mechanical systems for commercial new construction. Integrated HVAC, plumbing, and piping designed to meet the demands of modern facilities.", icon: Icons.building },
  { name: "Federal Government Contracting", desc: "Qualified federal contractor delivering mechanical services to government facilities. Meeting the rigorous compliance, security, and performance standards required by federal agencies.", icon: Icons.shield },
];
...
const areas = ["North Little Rock", "Conway", "Central Arkansas"];

const reviews = [
  { text: "Rated 5 stars for Punctuality, Quality, and Professionalism.", name: "Perry Steele", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function MikeGowenMechanical() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("mgm-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="roof"){sessionStorage.setItem("mgm-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "Commercial Mechanical Contractor North Little Rock AR | Mike Gowen Mechanical";
    const els = [];
    ...
    add("meta",{name:"description",content:"Commercial mechanical contractor in North Little Rock, AR. HVAC, plumbing & federal contracting since 1980. Call (501) 537-3444."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"Mike Gowen Mechanical",telephone:"+15015373444",
      address:{"@type":"PostalAddress",streetAddress:"220 Grant Park Cir",addressLocality:"North Little Rock",addressRegion:"AR",postalCode:"72117",addressCountry:"US"},
      ...
      founder:{"@type":"Person",name:"Mike Gowen",jobTitle:"Owner/Founder"},
      foundingDate:"1980",
      description:"Mike Gowen Mechanical is a commercial mechanical contractor in North Little Rock, AR. Founded in 1980, specializing in commercial HVAC, plumbing, insulation, demolition, and federal government contracting. Over $64M in building permits.",
      areaServed:areas.map(a=>({"@type":"City",name:a})),
      hasOfferCatalog:{"@type":"OfferCatalog",name:"Commercial Mechanical Services",itemListElement:allServiceNames.map(s=>({"@type":"Offer",itemOffered:{"@type":"Service",name:s}}))},
      memberOf:[{"@type":"Organization",name:"Conway Area Chamber of Commerce"}],sameAs:[],
    })});
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1C2B41"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8}}><span style={{fontWeight:700,color:"#1C2B41"}}>Mike Gowen</span> <span style={{color:"#C8934F"}}>Mechanical</span></div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(28,43,65,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}><span>Mike Gowen</span> <span style={{color:"var(--accent)"}}>Mechanical</span></div>
      ...
      <div className="hero-grid" style={{maxWidth:1200,width:"100%",display:"grid",gridTemplateColumns:"1.8fr 1fr",gap:48,alignItems:"center"}}>
        <div>
          <h1 style={{fontSize:"clamp(48px, 6vw, 84px)",lineHeight:1.0,color:"var(--primary)",letterSpacing:-2,marginBottom:24}}>
            North Little Rock's<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Commercial</span><br/>Mechanical Contractor
          </h1>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:32}}>
            <Stars />
            <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"Rated 5 stars for Punctuality, Quality, and Professionalism." — Perry Steele</span>
          </div>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(200,147,79,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Our Team — {PHONE}</a>
        </div>
        <div>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",color:"var(--primary)",letterSpacing:-0.5}}>Commercial Mechanical Services in North Little Rock</h2>
        </div>
        {/* Mobile trust */}
        <div className="trust-mobile" style={{display:"none",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:48,textAlign:"center"}}>
          {[{num:"5.0",label:"Google Rating",desc:"Perfect score"},{num:"1980",label:"Est. Founded",desc:"45+ years strong"},{num:"$64M+",label:"In Building Permits",desc:"Proven track record"},{num:"Federal",label:"Gov't Contractor",desc:"Cleared & qualified"}].map((s,i)=>(
            <div key={i}>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{maxWidth:800,margin:"0 auto",position:"relative",zIndex:1}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,textAlign:"center",opacity:0.7}}>About Mike Gowen Mechanical</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff",textAlign:"center"}}>45+ Years of Building<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Central Arkansas</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:24,textAlign:"center"}}>
          When Mike Gowen founded his mechanical contracting firm in 1980, commercial construction in Central Arkansas was entering a period of rapid growth. Over the next four decades, Mike Gowen Mechanical grew alongside the region — earning a reputation as the contractor that delivers on the largest, most demanding projects. With over $64.6 million in building permits, the company's portfolio speaks for itself: hospitals, schools, government buildings, and industrial facilities across North Little Rock, Conway, and beyond.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:36,textAlign:"center"}}>
          As a qualified federal government contractor, Mike Gowen Mechanical meets the rigorous compliance and performance standards required by federal agencies. A proud member of the Conway Area Chamber of Commerce, the firm remains rooted in the community it has served for over 45 years. From commercial HVAC and plumbing to mechanical insulation and demolition, every project is approached with the same principle Mike built the company on: do the job right, on time, and stand behind it.
        </p>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:20}}>
          {["Founded 1980 — 45+ Years","$64.6M in Building Permits","Federal Government Contractor","Conway Chamber Member","Commercial HVAC & Plumbing","Licensed, Bonded & Insured"].map((c,i)=>(
    ...
    <section id="reviews" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--white)"}}>
      <div style={{maxWidth:800,margin:"0 auto"}}>
        ...
        <div style={{background:"var(--cream)",borderRadius:12,padding:"48px 40px",border:"1px solid var(--border)",textAlign:"center",maxWidth:640,margin:"0 auto"}}>
          <div style={{marginBottom:20}}><Stars /></div>
          <p style={{fontSize:20,fontStyle:"italic",color:"var(--text)",lineHeight:1.75,marginBottom:24}}>"{reviews[0].text}"</p>
          <div style={{fontWeight:700,fontSize:17,color:"var(--primary)"}}>{reviews[0].name}</div>
          <div style={{fontSize:13,color:"var(--text-light)",marginTop:4}}>{reviews[0].src} Review</div>
        </div>
        <p style={{textAlign:"center",marginTop:8,fontSize:13,color:"var(--text-light)"}}>Perfect 5.0 rating across 5 reviews on Google</p>
        <p style={{textAlign:"center",marginTop:32,fontSize:15,color:"var(--text-light)"}}>Ready to experience the difference? Call <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
      </div>
    </section>
    ...
    <section id="areas" style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        ...
        <h2 style={{fontSize:"clamp(28px,3vw,38px)",color:"var(--primary)",marginBottom:48,letterSpacing:-0.5}}>Serving North Little Rock, Conway & Central Arkansas</h2>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:12}}>
          {areas.map((a,i)=>(
            <span key={i} title={`Commercial Mechanical Contractor in ${a}${a.includes("Arkansas")?"":`, AR`}`} style={{display:"inline-flex",alignItems:"center",gap:6,background:"#fff",border:"1px solid var(--border)",padding:"12px 24px",borderRadius:30,fontSize:15,fontWeight:500,color:"var(--text)",cursor:"default",transition:"all 0.25s"}}
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>Need a commercial mechanical contractor?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call our team.</span></h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>Serving Central Arkansas since 1980. $64M+ in building permits. 5.0 stars on Google.</p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16}}><span>Mike Gowen</span> <span style={{color:"var(--accent)"}}>Mechanical</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>Commercial mechanical contractor serving Central Arkansas since 1980. HVAC, plumbing, insulation, demolition, and federal government contracting.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>220 Grant Park Cir<br/>North Little Rock, AR 72117<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
          <p style={{fontSize:13,opacity:0.5,marginTop:10}}>Contact for Availability</p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} Mike Gowen Mechanical. All rights reserved.</span>
          <span>Conway Area Chamber of Commerce Member</span>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Mike Gowen Mechanical</a>
    </div>
