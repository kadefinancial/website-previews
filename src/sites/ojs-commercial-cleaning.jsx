// Layout: C | Industry: Cleaning | City: North Little Rock
...
const Icons = {
  ...
  sparkle: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M12 3v18M3 12h18M5.636 5.636l12.728 12.728M18.364 5.636L5.636 18.364"/><circle cx="12" cy="12" r="2"/></svg>
  ),
  spray: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="8" y="10" width="8" height="12" rx="1"/><path d="M10 10V6a2 2 0 014 0v4"/><circle cx="6" cy="4" r="1"/><circle cx="4" cy="7" r="1"/><circle cx="7" cy="2" r="1"/></svg>
  ),
  floor: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M2 22L12 2l10 20H2z"/><path d="M7 22l5-10 5 10"/><path d="M4 18h16"/></svg>
  ),
  window: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="12" y1="3" x2="12" y2="21"/><line x1="3" y1="12" x2="21" y2="12"/></svg>
  ),
  pressureWasher: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M12 2v6"/><path d="M8 8h8l2 4v6a2 2 0 01-2 2H8a2 2 0 01-2-2v-6l2-4z"/><path d="M12 14v4"/><path d="M9 14h6"/></svg>
  ),
  hardHat: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M2 18h20"/><path d="M4 18v-3a8 8 0 0116 0v3"/><path d="M12 5V2"/><path d="M8 14h8"/></svg>
  ),
  check: (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
  ),
  star: (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="#8B3A3A" stroke="none"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
  ),
  mapPin: (
...
// --- Data ---
const PHONE = "(501) 361-3308";
const PHONE_TEL = "tel:+15013613308";
const BIZ = "OJ's Commercial Cleaning";

const services = [
  { name: "Full Service Commercial Cleaning", desc: "Comprehensive cleaning for offices, warehouses, medical facilities, and commercial spaces across Arkansas. OJ's tailors every cleaning plan to your facility's specific needs — because no two buildings are the same.", icon: Icons.sparkle },
  { name: "Electrostatic Disinfectant Spraying", desc: "Advanced electrostatic technology delivers 360-degree disinfection coverage, wrapping around surfaces to eliminate pathogens in high-traffic commercial environments. Hospital-grade protection for your workplace.", icon: Icons.spray },
  { name: "Carpet & Floor Cleaning", desc: "Professional carpet extraction, tile restoration, and hard floor maintenance that transforms worn surfaces. Marq Rainwater's crew worked tirelessly on his tile floor — and the results spoke for themselves.", icon: Icons.floor },
  { name: "Window Cleaning", desc: "Interior and exterior window cleaning for commercial buildings of all sizes. Crystal-clear results that let natural light flood your space and make the right first impression on every visitor.", icon: Icons.window },
  { name: "Power Washing", desc: "High-pressure exterior cleaning for building facades, parking structures, sidewalks, and loading docks. Removes years of grime and restores your property's curb appeal across Arkansas.", icon: Icons.pressureWasher },
  { name: "Construction Cleaning", desc: "Post-construction cleanup for new builds and renovations. OJ's removes debris, dust, and residue so your finished project is move-in ready — on time and on budget.", icon: Icons.hardHat },
];

const allServices = [
  "Full Service Commercial Cleaning", "Electrostatic Disinfectant Spraying", "Carpet & Floor Cleaning",
  "Window Cleaning", "Power Washing", "Construction Cleaning",
];

const reviews = [
  { text: "This business offers a plethora of products that are customizable to fit your personal needs. Tiffany is extremely knowledgeable. She will guide you through the processes of selecting products that best work you and your family.", name: "Samantha Robertson", src: "Google" },
  { text: "We had OJ's come out recently to clean and buff our tile floor. We were extremely satisfied with the quality of work as well as their courteous staff. Our floor was in pretty bad shape but their crew worked tirelessly to complete the job.", name: "Marq Rainwater", src: "Google" },
  { text: "The OJ's team came out not one, but twice to address my emergency needs. The ultimate professionals, the team continued to follow up to make sure I was happy!", name: "Teri Todd", src: "Google" },
];

const areas = ["All of Arkansas"];

// --- Main Component ---
export default function OjsCommercialCleaning() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("ojs-cleaning-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("ojs-cleaning-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Commercial Cleaning North Little Rock AR | OJ's Commercial Cleaning";
    const els = [];
    const add = (tag, attrs) => {
    ...
    add("meta", { name: "description", content: "OJ's Commercial Cleaning — 33+ years serving all of Arkansas. 24/7 commercial cleaning, disinfection & floor care. Call (501) 361-3308." });
    ...
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      ...
      name: "O.J.'s Service Two, Inc.",
      telephone: "+15013613308",
      email: "service@ojscleaning.com",
      address: { "@type": "PostalAddress", streetAddress: "301 N Broadway St", addressLocality: "North Little Rock", addressRegion: "AR", postalCode: "72114", addressCountry: "US" },
      ...
      founder: { "@type": "Person", name: "Tiffany" },
      foundingDate: "1993",
      description: "OJ's Commercial Cleaning is a family-owned commercial cleaning company serving all of Arkansas since 1993. 24/7 availability, statewide coverage.",
      areaServed: [{ "@type": "State", name: "Arkansas" }],
      hasOfferCatalog: { "@type": "OfferCatalog", name: "Services", itemListElement: allServices.map((s) => ({ "@type": "Offer", itemOffered: { "@type": "Service", name: s } })) },
      memberOf: [],
      sameAs: [],
    })});
        ...
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#1C2B41" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, fontWeight: 600, letterSpacing: -0.5, marginBottom: 4 }}>
              <span style={{ color: "#1C2B41" }}>OJ's</span> <span style={{ color: "#8B3A3A" }}>Commercial Cleaning</span>
            </div>
    ...
    <>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled ? "rgba(28,43,65,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition: "all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        <div className="serif" style={{ fontSize: 22, fontWeight: 600, letterSpacing: -0.5, color: "#fff" }}>
          <span>OJ's</span><span style={{ color: "var(--accent)", marginLeft: 6 }}>Commercial Cleaning</span>
        </div>
        ...
        <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
          ...
          <div style={{ maxWidth: 560 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.1)", padding: "8px 18px", borderRadius: 4, marginBottom: 28 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: "var(--accent)" }}>Family-Owned Since 1993</span>
            </div>
            <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(40px,5vw,68px)", lineHeight: 1.05, letterSpacing: -1.5, color: "#fff", marginBottom: 24 }}>
              ...
              <em style={{ color: "var(--accent)", fontStyle: "italic" }}>Trusted Clean</em><br/>
              Since 1993
            </h1>
            <p style={{ fontSize: 18, lineHeight: 1.7, color: "rgba(255,255,255,0.75)", marginBottom: 32, maxWidth: 480 }}>
              33 years of commercial cleaning excellence across all of Arkansas. Tiffany and the OJ's team deliver customized cleaning solutions for every facility — available 24/7, including emergency response.
            </p>
            {/* Star row with hero quote */}
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 32, flexWrap: "wrap" }}>
              <Stars />
              <span style={{ fontSize: 14, color: "rgba(255,255,255,0.7)", fontStyle: "italic" }}>"The ultimate professionals — continued to follow up to make sure I was happy!"</span>
              <span style={{ fontSize: 13, color: "rgba(255,255,255,0.5)" }}>- Teri Todd</span>
            </div>
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              <a href={PHONE_TEL} style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 36px", borderRadius: 6, fontSize: 16, fontWeight: 700, letterSpacing: 0.3, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 4px 20px rgba(139,58,58,0.3)", transition: "transform 0.2s" }}>
      ...
      <section style={{ background: "var(--white)", padding: "48px clamp(24px,5vw,64px)" }}>
        <div className="trust-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 32, textAlign: "center" }}>
          {[
            { num: "1993", label: "Est. Founded", desc: "33+ years of service" },
            { num: "34+", label: "Customer Reviews", desc: "Trusted by Arkansas" },
            { num: "24/7", label: "Always Available", desc: "Day or night, we're here" },
            { num: "Statewide", label: "All of Arkansas", desc: "Complete coverage" },
          ].map((s, i) => (
            <div key={i} style={{ padding: "16px 0" }}>
              <div style={{ fontFamily: "'Playfair Display',serif", fontSize: i === 3 ? "clamp(22px,2.5vw,32px)" : "clamp(28px,3vw,40px)", fontWeight: 600, color: "var(--primary)", letterSpacing: -0.5, marginBottom: 6 }}>{s.num}</div>
      ...
      <section id="services" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)" }}>Commercial Cleaning Across Arkansas</h2>
          </div>
          {services.map((svc, i) => {
            const isEven = i % 2 === 1;
            return (
          ...
          <p style={{ textAlign: "center", marginTop: 48, fontSize: 15, color: "var(--text-mid)" }}>
            Questions? Call Tiffany directly at <a href={PHONE_TEL} style={{ color: "var(--accent)", fontWeight: 600 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <div className="gold-line" />
            <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: "var(--text-light)", marginBottom: 14, fontFamily: "'DM Sans',sans-serif" }}>Why OJ's</p>
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)" }}>33 Years of Cleaning Excellence in Arkansas</h2>
          </div>
          <div className="why-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "start" }}>
            ...
            <div>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "var(--text-mid)", marginBottom: 20 }}>
                When OJ's Commercial Cleaning opened its doors in 1993, Tiffany and her family made a simple promise: treat every client's facility like their own. Three decades later, that promise hasn't changed. As an Arkansas Business of the Year nominee, OJ's has grown from a North Little Rock cleaning company into a statewide operation — but the family values that built the business still drive every job.
              </p>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "var(--text-mid)", marginBottom: 32 }}>
                Tiffany personally guides every client through product selection and cleaning plans, customizing solutions for each facility's unique needs. Whether it's a midnight emergency call (ask Teri Todd — they came out twice) or routine commercial maintenance, OJ's team is available 24 hours a day, 7 days a week. That's the kind of commitment that keeps Arkansas businesses calling back for 33 years.
              </p>
              {/* Pull quote */}
              <div style={{ borderLeft: "3px solid var(--accent)", paddingLeft: 24, marginBottom: 36 }}>
                <p style={{ fontSize: 18, fontStyle: "italic", lineHeight: 1.7, color: "var(--primary)", fontFamily: "'Playfair Display',serif" }}>
                  "Tiffany is extremely knowledgeable. She will guide you through the processes of selecting products that best work you and your family."
                </p>
                <p style={{ fontSize: 14, color: "var(--text-light)", marginTop: 8, fontFamily: "'DM Sans',sans-serif" }}>- Samantha Robertson, Google Review</p>
              </div>
              {/* Credentials */}
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {[
                  "Family-Owned & Operated Since 1993",
                  "Arkansas Business of the Year Nominee",
                  "Available 24/7 — Including Emergency Response",
                  "Statewide Coverage Across All of Arkansas",
            ...
            <div className="badge-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {[
                { icon: Icons.award, title: "33+ Years in Business", desc: "Founded in 1993, OJ's Commercial Cleaning has served Arkansas for over three decades. Family-owned values with professional-grade results on every job." },
                { icon: Icons.clock, title: "24/7 Availability", desc: "Emergencies don't wait for business hours and neither does OJ's. Round-the-clock service means your facility is never left unattended when problems arise." },
                { icon: Icons.shield, title: "AR Business of the Year", desc: "Nominated for Arkansas Business of the Year, OJ's has earned recognition for the same commitment to excellence that clients experience on every single visit." },
                { icon: Icons.users, title: "Tiffany Leads the Team", desc: "Owner Tiffany personally consults on every cleaning plan. Her expertise ensures every product and process is customized to your facility's specific needs." },
              ].map((b, i) => (
      ...
      <section id="reviews" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
          ...
          {/* Large featured review — Teri Todd (emergency response story) */}
          <div style={{ position: "relative", padding: "48px 40px", background: "var(--white)", borderRadius: 12 }}>
            ...
            <p style={{ fontSize: 20, lineHeight: 1.8, color: "var(--text)", fontStyle: "italic", fontFamily: "'Playfair Display',serif", marginBottom: 28 }}>
              "The OJ's team came out not one, but twice to address my emergency needs. The ultimate professionals, the team continued to follow up to make sure I was happy!"
            </p>
            <p style={{ fontSize: 17, fontWeight: 700, color: "var(--primary)" }}>Teri Todd</p>
            <p style={{ fontSize: 13, color: "var(--text-light)", marginTop: 4 }}>Google Review</p>
          </div>
          <p style={{ fontSize: 14, color: "var(--text-light)", marginTop: 32, marginBottom: 40 }}>One of 34+ five-star reviews on Google</p>

          {/* Secondary reviews */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20, maxWidth: 800, margin: "0 auto" }}>
            {reviews.slice(0, 2).map((rev, i) => (
              <div key={i} style={{ padding: "32px 36px", background: "var(--cream)", borderRadius: 12, border: "1px solid var(--border)", textAlign: "left" }}>
                <div style={{ marginBottom: 12 }}>
                  <Stars />
                </div>
                <p style={{ fontSize: 15, lineHeight: 1.7, color: "var(--text)", fontStyle: "italic", marginBottom: 16 }}>
                  "{rev.text}"
                </p>
                <p style={{ fontSize: 15, fontWeight: 700, color: "var(--primary)" }}>{rev.name}</p>
                <p style={{ fontSize: 13, color: "var(--text-light)", marginTop: 2 }}>{rev.src} Review</p>
              </div>
            ))}
          </div>
      ...
      <section id="areas" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", textAlign: "center" }}>
          ...
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)", marginBottom: 56 }}>Serving All of Arkansas — Statewide Coverage</h2>
          <div style={{ display: "flex", flexWrap: "wrap", gap: 12, justifyContent: "center" }}>
            {["North Little Rock", "Little Rock", "Conway", "Jacksonville", "Sherwood", "Cabot", "Benton", "Bryant", "Pine Bluff", "Hot Springs", "Fayetteville", "Fort Smith", "Jonesboro", "Springdale", "Rogers", "All of Arkansas"].map((area, i) => (
              <span key={i} title={`Commercial Cleaning in ${area}, AR`} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "12px 22px", background: "var(--white)", border: "1px solid var(--border)", borderRadius: 100, fontSize: 14, fontWeight: 500, color: "var(--text)", cursor: "default", transition: "all 0.2s ease", fontFamily: "'DM Sans',sans-serif" }}
      ...
      <section className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)", borderTop: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)", marginBottom: 20 }}>
            Need your facility cleaned?<br/><em style={{ color: "var(--accent)", fontStyle: "italic" }}>Call Tiffany.</em>
          </h2>
          <p style={{ fontSize: 17, lineHeight: 1.7, color: "var(--text-mid)", marginBottom: 40, maxWidth: 520, margin: "0 auto 40px" }}>
            33 years of family-owned excellence. 24/7 availability. Statewide coverage. OJ's Commercial Cleaning — the Arkansas team that shows up twice if that's what it takes.
          </p>
          <a href={PHONE_TEL} style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 48px", borderRadius: 6, fontSize: 18, fontWeight: 700, letterSpacing: 0.3, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 4px 20px rgba(139,58,58,0.3)", transition: "transform 0.2s" }}>
            {Icons.phone} {PHONE}
          </a>
        </div>
      </section>
      ...
      <footer style={{ background: "var(--primary)", padding: "80px clamp(24px,5vw,64px) 40px", color: "rgba(255,255,255,0.75)" }}>
        <div className="footer-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: 48, marginBottom: 48 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 15, lineHeight: 1.7, marginBottom: 20, maxWidth: 300 }}>
              Family-owned commercial cleaning serving all of Arkansas since 1993. Available 24/7 for your facility's needs.
            </p>
            <p style={{ fontSize: 14, marginBottom: 8 }}>301 N Broadway St, North Little Rock, AR 72114</p>
            <p style={{ fontSize: 14, marginBottom: 8 }}>Open 24 Hours, 7 Days a Week</p>
            <a href={PHONE_TEL} style={{ fontSize: 14, color: "var(--accent)", fontWeight: 600 }}>{PHONE}</a>
            <br/>
            <a href="mailto:service@ojscleaning.com" style={{ fontSize: 14, color: "rgba(255,255,255,0.6)", marginTop: 4, display: "inline-block" }}>service@ojscleaning.com</a>
          </div>
          {/* Col 2: Services */}
          <div>
          ...
          {/* Col 3: Areas */}
          <div>
            <h4 style={{ fontSize: 14, fontWeight: 700, letterSpacing: 1, textTransform: "uppercase", color: "#fff", marginBottom: 20, fontFamily: "'DM Sans',sans-serif" }}>Service Coverage</h4>
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {["North Little Rock", "Little Rock", "Conway", "Jacksonville", "Hot Springs", "Fayetteville", "Fort Smith", "All of Arkansas"].map((a, i) => (
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 24 }}>
          <div className="footer-btm-inner" style={{ maxWidth: 1200, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>{new Date().getFullYear()} O.J.'s Service Two, Inc. All rights reserved.</p>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>Arkansas Business of the Year Nominee</p>
          </div>
        </div>
      </footer>
