...
const Icons = {
  ...
  gasLeak: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="M12 8v4"/><path d="M12 16h.01"/></svg>
  ),
  drain: (
...
// --- Data ---
const PHONE = "(479) 646-9494";
const PHONE_TEL = "tel:+14796469494";
const BIZ = "All American Plumbing";

const services = [
  { name: "Plumbing Repair", desc: "From leaking faucets to burst pipes, the All American team handles every residential plumbing repair with over 30 years of hands-on expertise. Honest diagnosis, quality parts, and work done right the first time.", icon: Icons.wrench },
  { name: "Water Leak Detection & Repair", desc: "Fast, accurate detection of hidden water leaks before they cause costly damage to your home. Same-day response and lasting repairs from a team Fort Smith trusts.", icon: Icons.droplet },
  { name: "Gas Leak Detection & Repair", desc: "Gas leaks are dangerous and demand immediate attention. All American provides expert gas line detection and repair to keep your family safe and your home protected.", icon: Icons.gasLeak },
  { name: "Water Heater Installation", desc: "Professional water heater installation and replacement for Fort Smith homeowners. Whether tank or tankless, Les and Jackson ensure proper setup and reliable hot water for years.", icon: Icons.flame },
  { name: "Drain Line Cleaning", desc: "Stubborn clogs and slow drains cleared fast with professional-grade equipment. All American restores full flow to your plumbing system without damaging your pipes.", icon: Icons.drain },
  { name: "Underground Pipe Repair", desc: "Specialized underground pipe repair and replacement for sewer lines, water mains, and drainage systems. Experienced crews minimize disruption to your property.", icon: Icons.pipe },
];

const allServices = [
  "Plumbing Repair", "Water Leak Detection & Repair", "Gas Leak Detection & Repair",
  "Water Heater Installation", "Drain Line Cleaning", "Underground Pipe Repair",
];

const reviews = [
  { text: "We needed a plumber and couldn't get a plumber to call us back much less show up at our home. We called All American just by chance at 4:45 PM Thursday evening and they told me they would be in at our front door at 7:30 AM the following day.", name: "Don Miller", src: "Google" },
  { text: "Jackson from All American Plumbing was great to work with! He was incredibly professional, knowledgeable, and hard working. He was thorough with putting a new water tank in and explaining this set up to me.", name: "Victoria Ballenger", src: "Google" },
  { text: "Wonderful service. Kind and knowledgable. I cannot recommend them enough. They have my business for years to come.", name: "Dillon Wilbourn", src: "Google" },
];

const areas = ["Fort Smith", "Van Buren", "Greenwood", "Alma", "Barling", "Lavaca", "Mountainburg", "Roland", "Muldrow", "Pocola"];

// --- Main Component ---
export default function AllAmericnPlumbing() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("all-americn-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("all-americn-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Plumber Fort Smith AR | All American Plumbing";
    const els = [];
    const add = (tag, attrs) => {
    ...
    add("meta", { name: "description", content: "All American Plumbing — 5.0 stars, 30+ years in Fort Smith, AR. Call (479) 646-9494 for plumbing repair, leak detection, and water heaters." });
    ...
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      ...
      telephone: "+14796469494",
      address: { "@type": "PostalAddress", addressLocality: "Fort Smith", addressRegion: "AR", postalCode: "72908", addressCountry: "US" },
      ...
      founder: { "@type": "Person", name: "Les Lester" },
      description: "All American Plumbing is a family-owned plumbing company with 30+ years of experience serving Fort Smith and the Arkansas-Oklahoma border region. 5.0 stars on Google.",
        ...
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#1C2B41" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, fontWeight: 600, letterSpacing: -0.5, marginBottom: 4 }}>
              <span style={{ color: "#1C2B41" }}>All American</span> <span style={{ color: "#C8934F" }}>Plumbing</span>
            </div>
    ...
    <>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled ? "rgba(28,43,65,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition: "all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        <div className="serif" style={{ fontSize: 22, fontWeight: 600, letterSpacing: -0.5, color: "#fff" }}>
          <span>All American</span><span style={{ color: "var(--accent)", marginLeft: 6 }}>Plumbing</span>
        </div>
        ...
        <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
          ...
          <div style={{ maxWidth: 560 }}>
            ...
            <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(40px,5vw,68px)", lineHeight: 1.05, letterSpacing: -1.5, color: "#fff", marginBottom: 24 }}>
              Fort Smith's{" "}<br/>
              <em style={{ color: "var(--accent)", fontStyle: "italic" }}>Go-To Plumber</em><br/>
              For 30+ Years
            </h1>
            <p style={{ fontSize: 18, lineHeight: 1.7, color: "rgba(255,255,255,0.75)", marginBottom: 32, maxWidth: 480 }}>
              Les Lester and the All American Plumbing team have served Fort Smith and the Arkansas-Oklahoma border for over three decades. From leak detection to underground pipe repair — honest work, fair pricing, and a perfect Google rating.
            </p>
            {/* Star row with hero quote */}
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 32, flexWrap: "wrap" }}>
              <Stars />
              <span style={{ fontSize: 14, color: "rgba(255,255,255,0.7)", fontStyle: "italic" }}>"They have my business for years to come."</span>
              <span style={{ fontSize: 13, color: "rgba(255,255,255,0.5)" }}>- Dillon Wilbourn</span>
            </div>
      ...
      <section style={{ background: "var(--white)", padding: "48px clamp(24px,5vw,64px)" }}>
        <div className="trust-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 32, textAlign: "center" }}>
          {[
            ...
            { num: "30+", label: "Years in Business", desc: "Since the early '90s" },
            { num: "AR+OK", label: "Bi-State Coverage", desc: "Fort Smith & beyond" },
            { num: "Jackson", label: "Expert on Staff", desc: "Professional service" },
          ].map((s, i) => (
            <div key={i} style={{ padding: "16px 0" }}>
              <div style={{ fontFamily: "'Playfair Display',serif", fontSize: i === 2 || i === 3 ? "clamp(22px,2.5vw,32px)" : "clamp(28px,3vw,40px)", fontWeight: 600, color: "var(--primary)", letterSpacing: -0.5, marginBottom: 6 }}>{s.num}</div>
      ...
      <section id="services" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)" }}>Plumbing Services in Fort Smith</h2>
          </div>
          {services.map((svc, i) => {
            const isEven = i % 2 === 1;
            return (
          ...
          <p style={{ textAlign: "center", marginTop: 48, fontSize: 15, color: "var(--text-mid)" }}>
            Questions? Call Les directly at <a href={PHONE_TEL} style={{ color: "var(--accent)", fontWeight: 600 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <div className="gold-line" />
            <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: "var(--text-light)", marginBottom: 14, fontFamily: "'DM Sans',sans-serif" }}>Why All American</p>
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)" }}>Fort Smith's Trusted Plumber for 30+ Years</h2>
          </div>
          <div className="why-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "start" }}>
            ...
            <div>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "var(--text-mid)", marginBottom: 20 }}>
                All American Plumbing is built on three decades of doing things right. Owner Les Lester started the company with a simple philosophy: show up when you say you will, diagnose honestly, and stand behind your work. That approach has earned a perfect 5.0-star Google rating and a loyal customer base across both sides of the Arkansas-Oklahoma border.
              </p>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "var(--text-mid)", marginBottom: 32 }}>
                Today, Jackson carries that same standard of professionalism forward — praised by name in customer reviews for his thoroughness, knowledge, and work ethic. When you call All American, you're not getting a faceless dispatch center. You're getting a team that treats every home like their own.
              </p>
              {/* Pull quote */}
              <div style={{ borderLeft: "3px solid var(--accent)", paddingLeft: 24, marginBottom: 36 }}>
                <p style={{ fontSize: 18, fontStyle: "italic", lineHeight: 1.7, color: "var(--primary)", fontFamily: "'Playfair Display',serif" }}>
                  "Jackson from All American Plumbing was great to work with! He was incredibly professional, knowledgeable, and hard working."
                </p>
                <p style={{ fontSize: 14, color: "var(--text-light)", marginTop: 8, fontFamily: "'DM Sans',sans-serif" }}>- Victoria Ballenger, Google Review</p>
              </div>
              {/* Credentials */}
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {[
                  "Perfect 5.0-Star Google Rating",
                  "30+ Years Serving Fort Smith",
                  "Bi-State Coverage — Arkansas & Oklahoma",
                  "Owner Les Lester on Every Job",
                  "Jackson — Expert Plumber on Staff",
            ...
            <div className="badge-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {[
                { icon: Icons.award, title: "Perfect 5.0 Rating", desc: "Every single Google review is five stars. All American doesn't just meet expectations — they exceed them on every call." },
                { icon: Icons.clock, title: "30+ Years Experience", desc: "Three decades of plumbing expertise in Fort Smith means Les has seen and solved every problem your home can throw at him." },
                { icon: Icons.shield, title: "Bi-State Coverage", desc: "Serving both Arkansas and Oklahoma across the Fort Smith metro. From Van Buren to Muldrow, All American has you covered." },
                { icon: Icons.heart, title: "Jackson on Staff", desc: "Customers call Jackson out by name for his professionalism and thorough work. That's the kind of team All American builds." },
              ].map((b, i) => (
      ...
      <section id="reviews" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
          ...
          {/* Large featured review — Don Miller */}
          <div style={{ position: "relative", padding: "48px 40px", background: "var(--white)", borderRadius: 12 }}>
            ...
            <p style={{ fontSize: 20, lineHeight: 1.8, color: "var(--text)", fontStyle: "italic", fontFamily: "'Playfair Display',serif", marginBottom: 28 }}>
              "We needed a plumber and couldn't get a plumber to call us back much less show up at our home. We called All American just by chance at 4:45 PM Thursday evening and they told me they would be in at our front door at 7:30 AM the following day."
            </p>
            <p style={{ fontSize: 17, fontWeight: 700, color: "var(--primary)" }}>Don Miller</p>
            <p style={{ fontSize: 13, color: "var(--text-light)", marginTop: 4 }}>Google Review</p>
          </div>
          <p style={{ fontSize: 14, color: "var(--text-light)", marginTop: 32, marginBottom: 40 }}>One of 6 five-star reviews on Google</p>
      ...
      <section id="areas" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", textAlign: "center" }}>
          ...
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)", marginBottom: 56 }}>Serving Fort Smith & the Arkansas-Oklahoma Border</h2>
      ...
      <section className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)", borderTop: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)", marginBottom: 20 }}>
            Got a plumbing problem?<br/><em style={{ color: "var(--accent)", fontStyle: "italic" }}>Call Les.</em>
          </h2>
          <p style={{ fontSize: 17, lineHeight: 1.7, color: "var(--text-mid)", marginBottom: 40, maxWidth: 520, margin: "0 auto 40px" }}>
            Honest diagnosis, fair pricing, and a team with 30+ years of experience. All American Plumbing — Fort Smith's perfect-rated plumber who actually shows up.
          </p>
      ...
      <footer style={{ background: "var(--primary)", padding: "80px clamp(24px,5vw,64px) 40px", color: "rgba(255,255,255,0.75)" }}>
        <div className="footer-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: 48, marginBottom: 48 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 15, lineHeight: 1.7, marginBottom: 20, maxWidth: 300 }}>
              Family-owned plumbing with 30+ years of experience serving Fort Smith and the Arkansas-Oklahoma border. 5.0 stars on Google.
            </p>
            <p style={{ fontSize: 14, marginBottom: 8 }}>Fort Smith, AR 72908</p>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 24 }}>
          <div className="footer-btm-inner" style={{ maxWidth: 1200, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>{new Date().getFullYear()} All American Plumbing. All rights reserved.</p>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>30+ Years Serving Fort Smith</p>
          </div>
        </div>
      </footer>
      ...
      <div className="mobile-only" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 998, display: "none" }}>
        <a href={PHONE_TEL} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 24px", fontSize: 16, fontWeight: 700, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 -4px 20px rgba(0,0,0,0.15)", textAlign: "center" }}>
          {Icons.phone} Call All American - {PHONE}
        </a>
      </div>
