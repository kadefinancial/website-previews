...
// --- Data ---
const PHONE = "(501) 707-5140";
const PHONE_TEL = "tel:+15017075140";

const services = [
  { name: "Water Heater Service", desc: "Installation, repair, and replacement of tank and tankless water heaters.", icon: Icons.flame },
  { name: "Drain Cleaning", desc: "Professional rooter service and hydro-jetting for stubborn blockages.", icon: Icons.droplets },
  { name: "Sewer Line Repair", desc: "Trenchless repair and full replacement of damaged sewer lines.", icon: Icons.pipette },
  { name: "Emergency Plumbing", desc: "Fast response for burst pipes, major leaks, and plumbing emergencies.", icon: Icons.zap },
  { name: "Fixture Installation", desc: "Expert installation of faucets, toilets, garbage disposals, and more.", icon: Icons.wrench },
  { name: "Gas Line Service", desc: "Licensed gas line repair, installation, and safety inspections.", icon: Icons.shield },
];
...
export default function AMGPlumbing() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("amg-auth") === "1");
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Plumbing Little Rock AR | AMG Plumbing";
    const els = [];
    ...
    add("meta", { name: "description", content: "Family-owned plumbing in Little Rock, AR. 5.0 stars, 195+ reviews. Call (501) 707-5140 for a free quote." });
    ...
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      ...
      name: "AMG Plumbing", telephone: "+15017075140",
        ...
        <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#1C2B41" }}>
          <div style={{ background:"#fff", borderRadius:16, padding:"56px 44px", textAlign:"center", maxWidth:420, width:"92%", boxShadow:"0 32px 80px rgba(0,0,0,.35)" }}>
            ...
            <form onSubmit={submitPw}>
              ...
              <button type="submit" style={{ width:"100%", padding:16, marginTop:20, fontSize:15, fontWeight:600, letterSpacing:.3, background:"#1C2B41", color:"#fff", border:"none", borderRadius:10, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>View Website</button>
            </form>
          </div>
        </div>
      </>
    ...
    <>
      ...
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:1000, padding:scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display:"flex", alignItems:"center", justifyContent:"space-between", background: scrolled ? "rgba(28,43,65,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition:"all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        ...
        <div className="desktop-only" style={{ display:"flex", alignItems:"center", gap:36 }}>
          ...
          <a href={PHONE_TEL} style={{ display:"inline-flex", alignItems:"center", gap:8, background:"var(--gold)", color:"#fff", padding:"10px 24px", borderRadius:6, fontSize:14, fontWeight:700, letterSpacing:0.3, fontFamily:"'DM Sans',sans-serif" }}>
            {Icons.phone} {PHONE}
          </a>
        </div>
        <button className="mobile-only" onClick={() => setMobileMenu(true)} style={{ background:"none", border:"none", cursor:"pointer", color: scrolled ? "#fff" : "var(--navy)", display:"flex" }}>{Icons.menu}</button>
      </nav>
      ...
      {mobileMenu && (
        <div style={{ position:"fixed", inset:0, background:"var(--primary)", zIndex:9999, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:28 }}>
          ...
          <a href={PHONE_TEL} style={{ background:"var(--gold)", color:"#fff", padding:"18px 48px", borderRadius:8, fontSize:18, fontWeight:700, fontFamily:"'DM Sans',sans-serif" }}>Call {PHONE}</a>
        </div>
      )}
        ...
        <div className="hero-grid" style={{ maxWidth:1200, width:"100%", display:"grid", gridTemplateColumns:"1fr 1fr", gap:64, alignItems:"center" }}>
          <div style={{ maxWidth:560 }}>
            <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(200,147,79,0.1)", padding:"8px 18px", borderRadius:4, marginBottom:28 }}>
              ...
              <span style={{ fontSize:12, fontWeight:700, color:"var(--gold)", letterSpacing:2, textTransform:"uppercase" }}>Family Owned & Operated</span>
            </div>
            <h1 style={{ fontSize:"clamp(40px, 5vw, 68px)", lineHeight:1.05, color:"var(--navy)", marginBottom:24, letterSpacing:-1.5 }}>
              Little Rock's<br /><span style={{ fontStyle:"italic", color:"var(--gold)" }}>Most Trusted</span><br />Plumber
            </h1>
            <p style={{ fontSize:18, lineHeight:1.7, color:"var(--text-mid)", marginBottom:32, maxWidth:440 }}>
              Led by Seth Grandbois, AMG Plumbing delivers honest, expert plumbing service to Central Arkansas. 5.0 stars across 195+ reviews.
            </p>
            ...
            <div style={{ display:"flex", gap:14, flexWrap:"wrap" }}>
              <a href={PHONE_TEL} style={{ display:"inline-flex", alignItems:"center", gap:10, background:"var(--gold)", color:"#fff", padding:"18px 36px", borderRadius:6, fontSize:16, fontWeight:700, boxShadow:"0 4px 20px rgba(200,147,79,0.3)", fontFamily:"'DM Sans',sans-serif" }}>
                {Icons.phone} Call Seth Now
              </a>
              <button onClick={() => scrollTo("services")} style={{ display:"inline-flex", alignItems:"center", gap:8, background:"transparent", color:"var(--navy)", padding:"18px 32px", borderRadius:6, fontSize:16, fontWeight:600, border:"1.5px solid var(--border)", cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>
            ...
            <div style={{ background:"#fff", borderRadius:12, padding:"44px 36px", boxShadow:"0 4px 40px rgba(28,43,65,0.08)", border:"1px solid var(--border)" }}>
              {!formSubmitted ? (
                <form onSubmit={handleForm}>
                  <h3 style={{ fontSize:26, color:"var(--navy)", marginBottom:6, fontFamily:"'Playfair Display',serif" }}>Get a Free Quote</h3>
                  ...
                  <button type="submit" style={{ width:"100%", padding:18, background:"var(--navy)", color:"#fff", border:"none", borderRadius:6, fontSize:16, fontWeight:700, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", letterSpacing:0.3 }}>Get My Free Quote</button>
                </form>
              ) : (
                <div style={{ textAlign:"center", padding:"24px 0" }}>
                  <div style={{ width:56, height:56, borderRadius:"50%", background:"rgba(200,147,79,0.12)", display:"flex", alignItems:"center", justifyContent:"center", margin:"0 auto 16px", color:"var(--gold)" }}>{Icons.check}</div>
                  <h3 style={{ fontSize:22, color:"var(--navy)", marginBottom:8, fontFamily:"'Playfair Display',serif" }}>We Got It!</h3>
                  <p style={{ fontSize:14, color:"var(--text-mid)" }}>We'll call you back shortly. For emergencies,<br/>call <a href={PHONE_TEL} style={{ color:"var(--gold)", fontWeight:700 }}>{PHONE}</a></p>
                </div>
              )}
            </div>
          </div>
      ...
      <section style={{ padding:"72px clamp(24px,5vw,64px)", background:"#fff", borderTop:"1px solid var(--border)", borderBottom:"1px solid var(--border)" }}>
        <div className="trust-grid" style={{ maxWidth:1000, margin:"0 auto", display:"grid", gridTemplateColumns:"repeat(4, 1fr)", gap:32, textAlign:"center" }}>
          {[
            ...
            { num: "195+", label: "Reviews", sub: "And counting" },
            { num: "1 Yr", label: "Guarantee", sub: "Parts & labor" },
            { num: "24hr", label: "Response", sub: "Fast turnaround" },
          ].map((s, i) => (
            <div key={i}>
              <div className="serif" style={{ fontSize:"clamp(36px, 4vw, 52px)", color:"var(--navy)", lineHeight:1, marginBottom:6, letterSpacing:-1 }}>{s.num}</div>
      ...
      <section id="services" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          <div style={{ textAlign:"center", marginBottom:64 }}>
            ...
            <div style={{ fontSize:12, fontWeight:700, textTransform:"uppercase", letterSpacing:3, color:"var(--gold)", marginBottom:14 }}>What We Do</div>
            <h2 style={{ fontSize:"clamp(30px, 3.5vw, 44px)", color:"var(--navy)", letterSpacing:-0.5 }}>Plumbing Services in Little Rock, AR</h2>
          </div>
          <div className="svc-grid" style={{ display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:20 }}>
            {services.map((s, i) => (
              <div key={i} style={{ background:"#fff", borderRadius:10, padding:"36px 30px", border:"1px solid var(--border)", transition:"all 0.3s ease", cursor:"default", position:"relative", overflow:"hidden" }}
                ...
                <div className="card-bar" style={{ position:"absolute", top:0, left:0, width:0, height:2, background:"var(--gold)", transition:"width 0.35s ease" }} />
                <div style={{ color:"var(--gold)", marginBottom:18 }}>{s.icon}</div>
                <h3 style={{ fontSize:19, color:"var(--navy)", marginBottom:10, fontWeight:600, fontFamily:"'DM Sans',sans-serif" }}>{s.name}</h3>
          ...
          <p style={{ textAlign:"center", marginTop:40, fontSize:15, color:"var(--text-light)" }}>
            Questions? Call Seth directly at <a href={PHONE_TEL} style={{ color:"var(--gold)", fontWeight:600 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--primary)", color:"#fff", position:"relative", overflow:"hidden" }}>
        ...
        <div className="why-grid" style={{ maxWidth:1100, margin:"0 auto", display:"grid", gridTemplateColumns:"1.1fr 0.9fr", gap:80, alignItems:"start", position:"relative" }}>
          <div>
            ...
            <div style={{ fontSize:12, fontWeight:700, textTransform:"uppercase", letterSpacing:3, color:"var(--gold)", marginBottom:14, opacity:0.7 }}>About AMG</div>
            <h2 style={{ fontSize:"clamp(30px, 3.5vw, 42px)", lineHeight:1.15, marginBottom:28, color:"#fff" }}>Why Little Rock Trusts<br/><span style={{ fontStyle:"italic", color:"var(--gold)" }}>Seth Grandbois</span></h2>
            <p style={{ fontSize:16, lineHeight:1.8, color:"rgba(255,255,255,0.65)", marginBottom:24 }}>
              AMG Plumbing is a family-owned company built on a simple principle: treat every home like it's your own. Seth and his team — including Angel, Jonathan, Lewis, and Grayson — don't just fix problems. They take the time to explain what's happening, show you your options, and make sure the job is done right the first time.
            </p>
            <p style={{ fontSize:16, lineHeight:1.8, color:"rgba(255,255,255,0.65)", marginBottom:36 }}>
              With a perfect 5.0 rating across 195+ reviews, AMG has earned the trust of homeowners across Central Arkansas through honest pricing, reliable craftsmanship, and a one-year guarantee on every job.
            </p>
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              {["Licensed & Insured", "BBB Accredited", "Little Rock Chamber Member", "1-Year Parts & Labor Guarantee", "Family Owned & Operated"].map((c, i) => (
                <div key={i} style={{ display:"flex", alignItems:"center", gap:14 }}>
                  <div style={{ color:"var(--gold)", flexShrink:0 }}>{Icons.check}</div>
          ...
          <div className="badge-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, paddingTop:20 }}>
            {[
              ...
              { icon: Icons.shield, title: "Guaranteed", desc: "1 year on all work" },
              { icon: Icons.zap, title: "Fast Response", desc: "Same-day available" },
              { icon: Icons.wrench, title: "Expert Team", desc: "Trained professionals" },
            ].map((b, i) => (
              <div key={i} style={{ background:"rgba(255,255,255,0.04)", border:"1px solid rgba(255,255,255,0.06)", borderRadius:10, padding:"28px 22px", textAlign:"center" }}>
                <div style={{ color:"var(--gold)", marginBottom:12, display:"flex", justifyContent:"center" }}>{b.icon}</div>
      ...
      <section id="reviews" style={{ padding:"110px clamp(24px,5vw,64px)", background:"#fff" }}>
        <div style={{ maxWidth:800, margin:"0 auto", textAlign:"center" }}>
          <div className="gold-line" style={{ margin:"0 auto 20px" }} />
          <div style={{ fontSize:12, fontWeight:700, textTransform:"uppercase", letterSpacing:3, color:"var(--gold)", marginBottom:14 }}>Reviews</div>
          <h2 style={{ fontSize:"clamp(30px, 3.5vw, 42px)", color:"var(--navy)", marginBottom:56, letterSpacing:-0.5 }}>5.0 Stars on Google</h2>
          <div style={{ background:"var(--cream)", borderRadius:12, padding:"52px 48px", border:"1px solid var(--border)", position:"relative", minHeight:240 }}>
            <div className="serif" style={{ position:"absolute", top:20, left:32, fontSize:72, color:"var(--gold)", opacity:0.15, lineHeight:1 }}>"</div>
            <div style={{ position:"relative", zIndex:1 }}>
              ...
              <div style={{ fontWeight:700, fontSize:15, color:"var(--navy)" }}>{reviews[reviewIdx].name}</div>
              <div style={{ fontSize:13, color:"var(--text-light)", marginTop:2 }}>{reviews[reviewIdx].src} Review</div>
            </div>
          </div>
          <div style={{ display:"flex", justifyContent:"center", gap:10, marginTop:28 }}>
            {reviews.map((_, i) => (
              <button key={i} onClick={() => setReviewIdx(i)} style={{ width:10, height:10, borderRadius:"50%", background: i === reviewIdx ? "var(--gold)" : "#ddd", border:"none", cursor:"pointer", padding:0, transition:"background 0.2s" }} />
            ))}
          </div>
          <p style={{ marginTop:36, fontSize:15, color:"var(--text-light)" }}>
            Ready to experience the difference? Call <a href={PHONE_TEL} style={{ color:"var(--gold)", fontWeight:600 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="areas" style={{ padding:"100px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:900, margin:"0 auto", textAlign:"center" }}>
          <div className="gold-line" style={{ margin:"0 auto 20px" }} />
          <div style={{ fontSize:12, fontWeight:700, textTransform:"uppercase", letterSpacing:3, color:"var(--gold)", marginBottom:14 }}>Service Areas</div>
          <h2 style={{ fontSize:"clamp(28px, 3vw, 38px)", color:"var(--navy)", marginBottom:48, letterSpacing:-0.5 }}>Serving Little Rock & Surrounding Areas</h2>
          <div style={{ display:"flex", flexWrap:"wrap", justifyContent:"center", gap:12 }}>
            {areas.map((a, i) => (
              <span key={i} title={`Plumbing in ${a}, AR`} style={{ display:"inline-flex", alignItems:"center", gap:6, background:"#fff", border:"1px solid var(--border)", padding:"12px 24px", borderRadius:30, fontSize:15, fontWeight:500, color:"var(--text)", cursor:"default", transition:"all 0.25s" }}
                ...
                onMouseLeave={e => { e.currentTarget.style.background="#fff"; e.currentTarget.style.color="var(--text)"; e.currentTarget.style.borderColor="rgba(28,43,65,0.08)"; }}>
      ...
      <section style={{ padding:"100px clamp(24px,5vw,64px)", background:"#fff", textAlign:"center", borderTop:"1px solid var(--border)" }}>
        <div style={{ maxWidth:600, margin:"0 auto" }}>
          <h2 className="serif" style={{ fontSize:"clamp(32px, 4vw, 50px)", color:"var(--navy)", lineHeight:1.1, marginBottom:20, letterSpacing:-1 }}>
            Got a plumbing problem?<br/><span style={{ fontStyle:"italic", color:"var(--gold)" }}>Call Seth.</span>
          </h2>
          <p style={{ fontSize:17, color:"var(--text-mid)", marginBottom:36, lineHeight:1.6 }}>
            Honest pricing. Expert work. One-year guarantee on every job.
          </p>
          <a href={PHONE_TEL} style={{ display:"inline-flex", alignItems:"center", gap:10, background:"var(--gold)", color:"#fff", padding:"22px 52px", borderRadius:8, fontSize:20, fontWeight:700, boxShadow:"0 6px 28px rgba(200,147,79,0.35)", fontFamily:"'DM Sans',sans-serif" }}>
            {Icons.phone} {PHONE}
          </a>
        </div>
      </section>
      ...
      <footer style={{ background:"var(--primary)", color:"rgba(255,255,255,0.7)", padding:"64px clamp(24px,5vw,64px) 0" }}>
        <div className="footer-grid" style={{ maxWidth:1100, margin:"0 auto", display:"grid", gridTemplateColumns:"1.5fr 1fr 1fr", gap:48, paddingBottom:40 }}>
          <div>
            ...
            <p style={{ fontSize:14, lineHeight:1.7, opacity:0.7, marginBottom:16 }}>Family-owned plumbing serving Central Arkansas with honest pricing and expert craftsmanship.</p>
            <p style={{ fontSize:14, lineHeight:1.8, opacity:0.6 }}>
              11415 Huron Ln #25711<br/>Little Rock, AR 72221<br/>
              <a href={PHONE_TEL} style={{ color:"var(--gold)", fontWeight:600 }}>{PHONE}</a>
            </p>
          </div>
          <div>
        ...
        <div style={{ borderTop:"1px solid rgba(255,255,255,0.06)", padding:"20px 0", maxWidth:1100, margin:"0 auto" }}>
          <div className="footer-btm-inner" style={{ display:"flex", justifyContent:"space-between", fontSize:13, opacity:0.4 }}>
            <span>&copy; {new Date().getFullYear()} AMG Plumbing. All rights reserved.</span>
            <span>BBB Accredited &middot; LR Chamber Member</span>
          </div>
        </div>
      </footer>
      ...
      <div className="mobile-only" style={{ position:"fixed", bottom:0, left:0, right:0, zIndex:998, display:"flex" }}>
        <a href={PHONE_TEL} style={{ display:"flex", width:"100%", alignItems:"center", justifyContent:"center", gap:10, background:"var(--gold)", color:"#fff", padding:"18px", fontSize:17, fontWeight:700, boxShadow:"0 -4px 20px rgba(0,0,0,0.15)", fontFamily:"'DM Sans',sans-serif" }}>
          {Icons.phone} Call AMG Plumbing
        </a>
      </div>
