// Layout: C | Industry: Plumber | City: Alma
...
const Icons = {
  ...
  zap: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>
  ),
  flame: (
  ...
  flag: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M4 15s1-1 4-1 5 2 8 2 4-1 4-1V3s-1 1-4 1-5-2-8-2-4 1-4 1z"/><line x1="4" y1="22" x2="4" y2="15"/></svg>
  ),
};
...
// --- Data ---
const PHONE = "(479) 632-2694";
const PHONE_TEL = "tel:+14796322694";
const BIZ = "Parker Plumbing & Electrical";

const services = [
  { name: "Plumbing Repair & Installation", desc: "From leaky faucets to full pipe replacements, Parker Plumbing handles every residential and commercial plumbing repair with over 70 years of expertise. Honest diagnosis, quality parts, and work that lasts.", icon: Icons.wrench },
  { name: "Electrical Services", desc: "Full electrical repair, installation, and troubleshooting from a licensed dual-trade team. Panel upgrades, wiring, outlets, and more — all from one trusted company.", icon: Icons.zap },
  { name: "Water Heater Sales & Installation", desc: "Professional water heater installation and replacement. Whether it's a standard tank or tankless unit, Parker Plumbing gets your hot water flowing fast with expert setup.", icon: Icons.flame },
  { name: "Drain Cleaning", desc: "Slow drains, stubborn clogs, and backed-up lines cleared quickly with professional equipment. Parker's team diagnoses the root cause so problems don't come back.", icon: Icons.droplet },
  { name: "Kitchen & Bathroom Plumbing", desc: "Complete plumbing services for kitchen and bathroom remodels, fixture upgrades, faucet installation, garbage disposals, and more. Quality craftsmanship every time.", icon: Icons.faucet },
  { name: "Sewer Line Service", desc: "Sewer line inspection, repair, and replacement for homes and businesses across the River Valley. Decades of experience handling even the toughest underground jobs.", icon: Icons.pipe },
];

const allServices = [
  "Plumbing Repair & Installation", "Electrical Services", "Water Heater Sales & Installation",
  "Drain Cleaning", "Kitchen & Bathroom Plumbing", "Sewer Line Service",
];

const reviews = [
  { text: "Very kind, professional gentleman who quickly solved the problem. Responded to my call within an hour. Gave me some good information to help in the future. Very reasonably priced.", name: "Billie Robbins", src: "Google" },
  { text: "I highly recommend Parker Plumbing & Electrical! The plumbers who installed my new kitchen sink, faucet, and garbage disposal were professional and polite. They didn't waste anytime, but went straight to work and had the old sink out and the new one in quickly.", name: "Zelda Lewallen", src: "Google" },
  { text: "Very prompt and very professional. The young man was very knowledgeable, and my dog liked him. They say that dogs are very good judge of character.", name: "Shirley Friddle", src: "Google" },
];

const areas = ["Alma", "Van Buren", "Fort Smith", "Mulberry", "Ozark", "Clarksville", "River Valley", "Western Arkansas"];

// --- Main Component ---
export default function ParkerPlumbingAndElectrical() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("parker-plumbing-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("parker-plumbing-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Plumber Alma AR | Parker Plumbing & Electrical";
    const els = [];
    const add = (tag, attrs) => {
    ...
    add("meta", { name: "description", content: "Parker Plumbing & Electrical — 4.6 stars, 105+ reviews. Veteran-owned since 1954. Call (479) 632-2694 for plumbing & electrical in Alma, AR." });
    add("meta", { name: "geo.region", content: "US-AR" });
    add("meta", { name: "geo.placename", content: "Alma" });
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      "@context": "https://schema.org",
      "@type": "Plumber",
      name: "Parker Plumbing & Electrical",
      telephone: "+14796322694",
      email: "parkerplumber@gmail.com",
      address: { "@type": "PostalAddress", streetAddress: "201 S Mountain Grove Rd", addressLocality: "Alma", addressRegion: "AR", postalCode: "72921", addressCountry: "US" },
      geo: { "@type": "GeoCoordinates", latitude: 35.4778, longitude: -94.2219 },
      openingHoursSpecification: [{ "@type": "OpeningHoursSpecification", dayOfWeek: ["Monday","Tuesday","Wednesday","Thursday","Friday"], opens: "08:00", closes: "17:00" }],
      aggregateRating: { "@type": "AggregateRating", ratingValue: "4.6", reviewCount: "105", bestRating: "5" },
      founder: { "@type": "Person", name: "Shane Parker" },
      foundingDate: "1954",
      description: "Parker Plumbing & Electrical is a veteran-owned plumbing and electrical company serving Alma and the River Valley since 1954. 4.6 stars with 105+ reviews.",
      areaServed: areas.map((a) => ({ "@type": "City", name: a })),
      hasOfferCatalog: { "@type": "OfferCatalog", name: "Plumbing & Electrical Services", itemListElement: allServices.map((s) => ({ "@type": "Offer", itemOffered: { "@type": "Service", name: s } })) },
        ...
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#1C2B41" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, fontWeight: 600, letterSpacing: -0.5, marginBottom: 4 }}>
              <span style={{ color: "#1C2B41" }}>Parker</span> <span style={{ color: "#C8934F" }}>Plumbing</span>
            </div>
    ...
    <>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled ? "rgba(28,43,65,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition: "all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        <div className="serif" style={{ fontSize: 22, fontWeight: 600, letterSpacing: -0.5, color: "#fff" }}>
          <span>Parker</span><span style={{ color: "var(--accent)", marginLeft: 6 }}>Plumbing</span>
        </div>
        ...
        <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
          ...
          <div style={{ maxWidth: 560 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.1)", padding: "8px 18px", borderRadius: 4, marginBottom: 28 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: "var(--accent)" }}>Veteran-Owned Since 1954</span>
            </div>
            ...
            <p style={{ fontSize: 18, lineHeight: 1.7, color: "rgba(255,255,255,0.75)", marginBottom: 32, maxWidth: 480 }}>
              Over 70 years of plumbing and electrical expertise serving the River Valley. Parker Plumbing & Electrical — where quality work and honest pricing have been the standard for three generations.
            </p>
            {/* Star row with hero quote */}
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 32, flexWrap: "wrap" }}>
              <Stars />
              <span style={{ fontSize: 14, color: "rgba(255,255,255,0.7)", fontStyle: "italic" }}>"Responded to my call within an hour."</span>
              <span style={{ fontSize: 13, color: "rgba(255,255,255,0.5)" }}>- Billie Robbins</span>
            </div>
      ...
      <section style={{ background: "var(--white)", padding: "48px clamp(24px,5vw,64px)" }}>
        <div className="trust-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 32, textAlign: "center" }}>
          {[
            ...
            { num: "105+", label: "Customer Reviews", desc: "And counting" },
            { num: "1954", label: "Est. Founded", desc: "70+ years strong" },
            { num: "Veteran", label: "Owned Business", desc: "Proudly serving" },
          ].map((s, i) => (
      ...
      <section id="services" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)" }}>Plumbing & Electrical Services in Alma</h2>
          </div>
          {services.map((svc, i) => {
            const isEven = i % 2 === 1;
            return (
          ...
          <p style={{ textAlign: "center", marginTop: 48, fontSize: 15, color: "var(--text-mid)" }}>
            Questions? Call Shane directly at <a href={PHONE_TEL} style={{ color: "var(--accent)", fontWeight: 600 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <div className="gold-line" />
            <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: "var(--text-light)", marginBottom: 14, fontFamily: "'DM Sans',sans-serif" }}>Why Parker</p>
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)" }}>70+ Years Serving the River Valley</h2>
          </div>
          <div className="why-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "start" }}>
            ...
            <div>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "var(--text-mid)", marginBottom: 20 }}>
                Parker Plumbing & Electrical has been a cornerstone of the Alma community since 1954 — that's over 70 years of honest, reliable service. Founded as a veteran-owned business, the company has built its reputation one job at a time, earning the trust of families and businesses across the entire River Valley.
              </p>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "var(--text-mid)", marginBottom: 32 }}>
                Today, owner Shane Parker carries on that tradition. He personally responds to every Google review, whether it's a five-star thank you or a question about service. That level of care — combined with dual plumbing and electrical expertise under one roof — is why Parker has earned a 4.6-star rating with over 105 reviews and a BBB A+ rating.
              </p>
              {/* Pull quote */}
              <div style={{ borderLeft: "3px solid var(--accent)", paddingLeft: 24, marginBottom: 36 }}>
                <p style={{ fontSize: 18, fontStyle: "italic", lineHeight: 1.7, color: "var(--primary)", fontFamily: "'Playfair Display',serif" }}>
                  "Very prompt and very professional. The young man was very knowledgeable, and my dog liked him. They say that dogs are very good judge of character."
                </p>
                <p style={{ fontSize: 14, color: "var(--text-light)", marginTop: 8, fontFamily: "'DM Sans',sans-serif" }}>- Shirley Friddle, Google Review</p>
              </div>
              {/* Credentials */}
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {[
                  "Veteran-Owned Business — Since 1954",
                  "4.6-Star Google Rating — 105+ Reviews",
                  "BBB A+ Rated",
                  "Dual Plumbing & Electrical Expertise",
                  "LGBTQ+ Friendly",
                  "Owner Shane Parker Responds to Every Review",
                ].map((cred, i) => (
            ...
            <div className="badge-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {[
                { icon: Icons.flag, title: "Veteran-Owned", desc: "Founded by veterans in 1954, Parker Plumbing carries forward a tradition of discipline, integrity, and service to the River Valley community." },
                { icon: Icons.shield, title: "BBB A+ Rated", desc: "Accredited with the Better Business Bureau at the highest rating. A track record of trust, transparency, and customer satisfaction." },
                { icon: Icons.zap, title: "Dual-Trade Experts", desc: "One of the few companies in the region offering both licensed plumbing and electrical services — saving you time and hassle." },
                { icon: Icons.heart, title: "Community First", desc: "LGBTQ+ friendly and proud to serve every family in the River Valley. Shane Parker personally stands behind every job." },
              ].map((b, i) => (
      ...
      {/* REVIEWS — Layout C: white bg, single LARGE testimonial + secondary cards */}
        ...
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
          ...
          <div style={{ position: "relative", padding: "48px 40px", background: "var(--white)", borderRadius: 12 }}>
            ...
            <p style={{ fontSize: 20, lineHeight: 1.8, color: "var(--text)", fontStyle: "italic", fontFamily: "'Playfair Display',serif", marginBottom: 28 }}>
              "Very kind, professional gentleman who quickly solved the problem. Responded to my call within an hour. Gave me some good information to help in the future. Very reasonably priced."
            </p>
            <p style={{ fontSize: 17, fontWeight: 700, color: "var(--primary)" }}>Billie Robbins</p>
            <p style={{ fontSize: 13, color: "var(--text-light)", marginTop: 4 }}>Google Review</p>
          </div>
          <p style={{ fontSize: 14, color: "var(--text-light)", marginTop: 32, marginBottom: 40 }}>One of 105+ five-star reviews on Google</p>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            {reviews.slice(1).map((r, i) => (
              <div key={i} style={{ padding: "32px 36px", background: "var(--cream)", borderRadius: 12, border: "1px solid var(--border)", textAlign: "left" }}>
                ...
                <p style={{ fontSize: 15, fontWeight: 700, color: "var(--primary)" }}>{r.name}</p>
                <p style={{ fontSize: 13, color: "var(--text-light)", marginTop: 2 }}>{r.src} Review</p>
              </div>
            ))}
          </div>
      ...
      <section id="areas" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", textAlign: "center" }}>
          ...
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)", marginBottom: 56 }}>Serving Alma & the River Valley</h2>
      ...
      <section className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)", borderTop: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)", marginBottom: 20 }}>
            Got a plumbing or electrical problem?<br/><em style={{ color: "var(--accent)", fontStyle: "italic" }}>Call Shane.</em>
          </h2>
          <p style={{ fontSize: 17, lineHeight: 1.7, color: "var(--text-mid)", marginBottom: 40, maxWidth: 520, margin: "0 auto 40px" }}>
            Veteran-owned, BBB A+ rated, and trusted for over 70 years. Parker Plumbing & Electrical — honest pricing and quality work that speaks for itself.
          </p>
      ...
      <footer style={{ background: "var(--primary)", padding: "80px clamp(24px,5vw,64px) 40px", color: "rgba(255,255,255,0.75)" }}>
        <div className="footer-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: 48, marginBottom: 48 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 15, lineHeight: 1.7, marginBottom: 20, maxWidth: 300 }}>
              Veteran-owned plumbing and electrical company serving Alma and the River Valley since 1954. BBB A+ rated.
            </p>
            <p style={{ fontSize: 14, marginBottom: 8 }}>201 S Mountain Grove Rd, Alma, AR 72921</p>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 24 }}>
          <div className="footer-btm-inner" style={{ maxWidth: 1200, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>{new Date().getFullYear()} Parker Plumbing & Electrical. All rights reserved.</p>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>BBB A+ Rated | Veteran-Owned Since 1954</p>
          </div>
        </div>
      </footer>
