...
const PHONE = "(479) 409-5733";
const TEL = "tel:+14794095733";
...
const Icons = {
  ...
  shower: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4v5a9 9 0 0018 0V4"/><path d="M8 17l-2 4"/><path d="M16 17l2 4"/><path d="M12 17l0 4"/><circle cx="12" cy="4" r="1"/></svg>,
...
const services = [
  { name: "General Plumbing Repair", desc: "From leaky faucets to running toilets, Rocky handles every plumbing repair with 24 years of hands-on expertise. Honest diagnosis, fair pricing, and work that lasts.", icon: Icons.wrench },
  { name: "Residential Plumbing", desc: "Complete plumbing solutions for your home. Rocky treats every house like his own — careful, thorough work with zero shortcuts and no surprises on the bill.", icon: Icons.home },
  { name: "Pipe Repair", desc: "Cracked, corroded, or burst pipes repaired right the first time. Rocky's decades of experience mean faster diagnosis and permanent fixes, not temporary patches.", icon: Icons.pipe },
  { name: "Bathroom Plumbing", desc: "Toilet replacements, shower repairs, faucet installs, and full bathroom plumbing service. Rocky gets it done efficiently so your bathroom is back in action fast.", icon: Icons.shower },
  { name: "Drain Cleaning", desc: "Stubborn clogs and slow drains cleared quickly and completely. Rocky finds the real source of the problem and eliminates it — no guesswork, no unnecessary upsells.", icon: Icons.droplets },
  { name: "Emergency Plumbing", desc: "Plumbing emergencies don't wait, and neither does Rocky. Available 7 days a week to handle urgent leaks, backups, and pipe failures when you need help most.", icon: Icons.zap },
];

const allServiceNames = ["General Plumbing Repair","Residential Plumbing","Pipe Repair","Bathroom Plumbing","Drain Cleaning","Emergency Plumbing"];

const areas = ["Springdale","Fayetteville","Rogers","Bentonville","Northwest Arkansas"];
...
const reviews = [
  { name:"Leanne Sanchez", text:"Explained to me how to fix the issue on my own so I was not charged. Definitely will be calling him back when I need something that I cannot do on my own." },
  { name:"Steve Bryant", text:"I would recommend Rocky, he is a good plumber, been at it a while." },
];

export default function RockyThePlumber() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("rtp-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("rtp-auth","1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Plumber Springdale AR | Rocky the Plumber";
    const els = [];
    ...
    add("meta",{name:"description",content:"Trusted plumber in Springdale, AR. 4.7-star Google rating. 24+ years experience. Charges by the job, not the hour. Call (479) 409-5733."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"Rocky the Plumber",telephone:"+14794095733",
      address:{"@type":"PostalAddress",streetAddress:"301 Wayland Ave",addressLocality:"Springdale",addressRegion:"AR",postalCode:"72764",addressCountry:"US"},
      ...
      aggregateRating:{"@type":"AggregateRating",ratingValue:"4.7",reviewCount:"7",bestRating:"5"},
      founder:{"@type":"Person",name:"Rocky Thayer"},
      description:"Professional plumbing services in Springdale and Northwest Arkansas. 24+ years experience. General plumbing repair, residential plumbing, pipe repair, drain cleaning, and emergency plumbing. Charges by the job, not the hour.",
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1C2B41"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8}}>
            <span style={{fontWeight:700,color:"#1C2B41"}}>Rocky</span> <span style={{color:"#C8934F"}}>the Plumber</span>
          </div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(28,43,65,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease",boxShadow:scrolled?"0 1px 0 rgba(255,255,255,0.06)":"none"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}>
        <span>Rocky</span> <span style={{color:"var(--accent)"}}>the Plumber</span>
      </div>
    ...
    <section style={{padding:"180px clamp(24px,5vw,64px) 100px",background:"var(--cream)",textAlign:"center"}}>
      <div className="hero-text" style={{maxWidth:780,margin:"0 auto"}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(200,147,79,0.1)",padding:"8px 18px",borderRadius:4,marginBottom:28}}>
          ...
          <span style={{fontSize:12,fontWeight:700,color:"var(--accent)",letterSpacing:2,textTransform:"uppercase"}}>4.7 Stars on Google</span>
        </div>
        <h1 style={{fontSize:"clamp(44px, 6vw, 80px)",lineHeight:1.05,color:"var(--primary)",marginBottom:24,letterSpacing:-2}}>
          Springdale's<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Most Honest</span><br/>Plumber
        </h1>
        <p style={{fontSize:18,lineHeight:1.7,color:"var(--text-mid)",marginBottom:32,maxWidth:580,margin:"0 auto 32px"}}>
          Rocky Thayer has been solving plumbing problems across Northwest Arkansas for over 24 years. He charges by the job, not the hour — and he's honest enough to teach you the fix when you don't need to pay for one.
        </p>
        ...
        <p style={{fontSize:15,color:"var(--text-mid)",fontStyle:"italic",maxWidth:520,margin:"0 auto 36px",lineHeight:1.6}}>
          "Explained to me how to fix the issue on my own so I was not charged. Definitely will be calling him back when I need something that I cannot do on my own." — <strong style={{color:"var(--primary)",fontStyle:"normal"}}>Leanne Sanchez</strong>
        </p>
        <div style={{display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap"}}>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(200,147,79,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Rocky Now</a>
    ...
    <section style={{padding:"80px clamp(24px,5vw,64px)",background:"var(--white)",borderTop:"1px solid var(--border)",borderBottom:"1px solid var(--border)"}}>
      ...
      <div className="trust-row" style={{display:"grid",gridTemplateColumns:"repeat(4, 1fr)",gap:32,maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        {[
          {num:"4.7",label:"Google Rating",desc:"Trusted reviews"},
          {num:"24+",label:"Years Experience",desc:"Since day one"},
          {num:"7 Days",label:"Open Every Day",desc:"Mon through Sun"},
          {num:"By Job",label:"Not By Hour",desc:"Fair, flat pricing"},
        ].map((s,i)=>(
          <div key={i}>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",color:"var(--primary)",letterSpacing:-0.5}}>Plumbing Services in Springdale</h2>
        </div>
        ...
        <p style={{textAlign:"center",marginTop:40,fontSize:15,color:"var(--text-light)"}}>
          Questions? Call Rocky directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a>
        </p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",color:"#fff"}}>
      <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,opacity:0.7}}>About Rocky the Plumber</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff"}}>24 Years of Doing<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Plumbing the Right Way.</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.65)",marginBottom:24,textAlign:"left"}}>
          Rocky Thayer doesn't just fix pipes — he's built a reputation across Northwest Arkansas for something rare in the trades: genuine honesty. When a customer called with a plumbing issue, Rocky didn't just show up and hand them a bill. He explained exactly how to fix it themselves, so they wouldn't be charged. That customer? Now a customer for life. That's the kind of plumber Rocky is.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.65)",marginBottom:36,textAlign:"left"}}>
          With over 24 years of experience serving Springdale, Fayetteville, Rogers, and Bentonville, Rocky charges by the job — not by the hour. No clock running while he works, no inflated bills, no surprises. Just straightforward pricing for straightforward plumbing work. From drain cleaning and pipe repair to full bathroom plumbing and emergency calls, Rocky handles it all seven days a week. "Rocky the Plumber" isn't just a catchy name — it's a promise that the guy who shows up is the same guy who's been doing this right for over two decades.
        </p>
        <div className="why-creds-row" style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:24,marginTop:8}}>
          {["4.7 Google Rating","24+ Years Experience","Charges by the Job","Open 7 Days a Week","Owner on Every Job"].map((c,i)=>(
    ...
    <section id="areas" style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        ...
        <h2 style={{fontSize:"clamp(28px,3vw,38px)",color:"var(--primary)",marginBottom:48,letterSpacing:-0.5}}>Serving Springdale &amp; Northwest Arkansas</h2>
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"#FFFFFF",textAlign:"center",borderTop:"1px solid rgba(0,0,0,0.06)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"#2A2A3C",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>
          Got a plumbing problem?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Rocky.</span>
        </h2>
        <p style={{fontSize:17,color:"#5A5A6E",marginBottom:36,lineHeight:1.6}}>
          Honest pricing, 24 years of experience, and charges by the job — not the hour. The plumber Springdale and Northwest Arkansas trust.
        </p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0",borderTop:"1px solid rgba(255,255,255,0.06)"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16}}><span>Rocky</span> <span style={{color:"var(--accent)"}}>the Plumber</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>Professional plumbing services in Springdale and Northwest Arkansas. Owner-operated by Rocky Thayer with over 24 years of experience. Charges by the job, not the hour.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>
            301 Wayland Ave<br/>Springdale, AR 72764<br/>
        ...
        <div>
          ...
          <p style={{fontSize:14,opacity:0.6,lineHeight:1.7}}>Mon – Sun: 8 AM – 5 PM<br/>Open 7 Days a Week</p>
        </div>
      </div>
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto",display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
        <span>&copy; {yr} Rocky the Plumber. All rights reserved.</span>
        <span>Owner-Operated by Rocky Thayer | 24+ Years Experience</span>
      </div>
    </footer>

    {/* FLOATING MOBILE CTA */}
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Rocky Now</a>
    </div>
