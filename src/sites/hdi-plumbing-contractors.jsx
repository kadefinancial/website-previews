// Layout: D | Industry: Plumber | City: Fayetteville
import { useState, useEffect, useRef } from "react";

const PHONE = "(479) 466-6920";
const TEL = "tel:+14794666920";

const Icons = {
  phone: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>,
  commercial: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M9 3v18"/><path d="M3 9h6"/><path d="M3 15h6"/><path d="M15 9h6"/><path d="M15 15h6"/><path d="M15 3v18"/></svg>,
  ...
  blueprint: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M2 6h4v12H2z"/><path d="M6 6h12v2H6z"/><path d="M6 16h12v2H6z"/><path d="M18 6h4v12h-4z"/><path d="M10 8v8"/><path d="M14 8v8"/><path d="M6 12h12"/></svg>,
  industrial: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M2 20h20"/><path d="M5 20V8l5 4V8l5 4V4h5v16"/><path d="M17 8h1"/><path d="M17 12h1"/><path d="M17 16h1"/></svg>,
  trenchless: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 19h16"/><path d="M4 19c0-4 2-6 4-8"/><path d="M20 19c0-4-2-6-4-8"/><circle cx="12" cy="8" r="3"/><path d="M12 11v4"/><path d="M9 22h6"/></svg>,
  construction: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="6" width="20" height="8" rx="1"/><path d="M17 14v7"/><path d="M7 14v7"/><path d="M17 3v3"/><path d="M7 3v3"/><path d="M12 6v8"/></svg>,
...
const services = [
  { name: "Commercial Plumbing", desc: "Full-service commercial plumbing for offices, retail, restaurants, and multi-story buildings across Northwest Arkansas. Code-compliant systems built to handle high-demand environments.", icon: Icons.commercial },
  { name: "Residential Plumbing", desc: "Expert plumbing service for homeowners throughout Fayetteville and the surrounding communities. Repairs, replacements, and installations done right the first time.", icon: Icons.home },
  { name: "Plumbing Systems Design & Installation", desc: "Engineered plumbing solutions from blueprint to final inspection. Custom system design for commercial and industrial projects of any scale.", icon: Icons.blueprint },
  { name: "Industrial Plumbing", desc: "Heavy-duty plumbing for manufacturing facilities, warehouses, and industrial operations. Built to meet rigorous demands and strict compliance standards.", icon: Icons.industrial },
  { name: "Trenchless Sewer Repair", desc: "Minimize disruption with advanced trenchless technology. Repair and replace sewer lines without excavating your property or tearing up landscaping.", icon: Icons.trenchless },
  { name: "New Construction Plumbing", desc: "Ground-up plumbing installations for commercial and residential new builds. Rough-in through finish work with precise scheduling and clean execution.", icon: Icons.construction },
];

const serviceNames = ["Commercial Plumbing","Residential Plumbing","Plumbing Systems Design & Installation","Industrial Plumbing","Trenchless Sewer Repair","New Construction Plumbing"];

const areas = ["Fayetteville","Springdale","Rogers","Bentonville","West Fork","Prairie Grove","Northwest Arkansas"];

const reviews = [
  { text: "We built a commercial building that was completed in 2018. The contractor used HDI for the plumbing in the building. I just wanted to take a minute to say thank you to you guys. Jacob, was out of the country last week.", name: "Josh Landreth", src: "Google" },
  { text: "Great service professional, and honest.", name: "Garry Martin", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function HDIPlumbingContractors() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("hdi-plumbing-contractors-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="roof"){sessionStorage.setItem("hdi-plumbing-contractors-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "Commercial Plumber Fayetteville AR | HDI Plumbing Contractors";
    const els = [];
    ...
    add("meta",{name:"description",content:"HDI Plumbing Contractors — 5-star commercial plumber in Fayetteville AR. Master Plumber, 13+ years experience. Call (479) 466-6920."});
    add("meta",{name:"geo.region",content:"US-AR"});
    add("meta",{name:"geo.placename",content:"Fayetteville"});
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      "@context":"https://schema.org","@type":"Plumber",
      name:"HDI Plumbing Contractors",telephone:"+14794666920",
      address:{"@type":"PostalAddress",streetAddress:"2169 Leeper Dr",addressLocality:"Fayetteville",addressRegion:"AR",postalCode:"72701",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:36.0822,longitude:-94.1719},
      ...
      founder:{"@type":"Person",name:"Michael Caudle",jobTitle:"Master Plumber"},
      foundingDate:"2012",
      description:"HDI Plumbing Contractors is a 5-star rated commercial plumbing contractor serving Fayetteville, AR and Northwest Arkansas. Specializing in commercial, industrial, and new construction plumbing.",
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1C2B41"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8}}>
            <span style={{fontWeight:700,color:"#1C2B41"}}>HDI</span> <span style={{color:"#C8934F"}}>Plumbing</span>
          </div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(28,43,65,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease",boxShadow:scrolled?"0 1px 0 rgba(255,255,255,0.06)":"none"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}>
        <span>HDI</span> <span style={{color:"var(--accent)"}}>Plumbing</span>
      </div>
      ...
      <div className="hero-grid" style={{maxWidth:1200,width:"100%",display:"grid",gridTemplateColumns:"1.8fr 1fr",gap:48,alignItems:"center"}}>
        <div>
          <h1 style={{fontSize:"clamp(48px, 6vw, 84px)",lineHeight:1.0,color:"var(--primary)",letterSpacing:-2,marginBottom:24}}>
            Fayetteville's<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Commercial</span><br/>Plumbing Experts
          </h1>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:32}}>
            <Stars />
            <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"Great service professional, and honest." — Garry Martin</span>
          </div>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(200,147,79,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Michael — {PHONE}</a>
        </div>
        <div>
          <div style={{background:"#fff",borderRadius:12,padding:"32px 28px",boxShadow:"0 4px 40px rgba(0,0,0,0.08)",border:"1px solid var(--border)"}}>
            {!formDone ? (
              <form onSubmit={e=>{e.preventDefault();setFormDone(true)}}>
                <h3 style={{fontSize:22,color:"var(--primary)",marginBottom:6,fontFamily:"'Playfair Display',serif"}}>Request a Consultation</h3>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",color:"var(--primary)",letterSpacing:-0.5}}>Commercial Plumbing Services in Fayetteville</h2>
        </div>
        ...
        <div className="trust-mobile" style={{display:"none",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:48,textAlign:"center"}}>
          {[
            ...
            {num:"13+",label:"Years in Business",desc:"Est. 2012"},
            {num:"$11M+",label:"Project Experience",desc:"Named subcontractor"},
            {num:"Master",label:"Licensed Plumber",desc:"Michael Caudle"},
          ].map((s,i)=>(
            <div key={i}>
    ...
    {/* WHY US — Layout D: Giant "2012" bg, single column centered */}
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",position:"relative",overflow:"hidden"}}>
      <div className="serif" style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",fontSize:"clamp(120px,20vw,280px)",color:"var(--accent)",opacity:0.08,lineHeight:1,whiteSpace:"nowrap",pointerEvents:"none"}}>2012</div>
      <div style={{maxWidth:800,margin:"0 auto",position:"relative",zIndex:1}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,textAlign:"center",opacity:0.7}}>About HDI Plumbing Contractors</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff",textAlign:"center"}}>Built for the Big Jobs,<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Trusted on Every One</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:24,textAlign:"center"}}>
          HDI Plumbing Contractors isn't your average service plumber. Founded by Michael Caudle — a Master Plumber with over 13 years in the trade — HDI is a commercial plumbing contractor built for serious work. From the ground up on new construction to complex system design for multi-story buildings, this is a team that operates at a scale most plumbing companies never reach. They were named as a subcontractor on an $11.37 million project — the kind of trust that's earned over years, not months.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:36,textAlign:"center"}}>
          Based in Fayetteville, Arkansas, HDI serves the entire Northwest Arkansas corridor — from Bentonville and Rogers to Springdale, West Fork, and Prairie Grove. With a crew of 10-19 skilled tradespeople, Michael has built a company with a perfect 5.0-star Google rating and the kind of reputation that keeps general contractors calling back. Commercial, industrial, residential, or new construction — HDI delivers professional plumbing with the precision and reliability that the region's biggest projects demand.
        </p>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:20}}>
          {["Perfect 5.0-Star Google Rating","Master Plumber — Michael Caudle","Named on $11.37M Project","13+ Years of Experience","10-19 Skilled Employees","Serving All of NW Arkansas"].map((c,i)=>(
    ...
    <section id="areas" style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        ...
        <h2 style={{fontSize:"clamp(28px,3vw,38px)",color:"var(--primary)",marginBottom:48,letterSpacing:-0.5}}>Serving Fayetteville & Northwest Arkansas</h2>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:12}}>
          {areas.map((a,i)=>(
            <span key={i} title={`Commercial Plumber in ${a}${a==="Northwest Arkansas"?"":", AR"}`} style={{display:"inline-flex",alignItems:"center",gap:6,background:"#fff",border:"1px solid var(--border)",padding:"12px 24px",borderRadius:30,fontSize:15,fontWeight:500,color:"var(--text)",cursor:"default",transition:"all 0.25s"}}
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>
          Got a commercial project?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Michael.</span>
        </h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>
          A perfect 5.0-star rating, a Master Plumber at the helm, and 13+ years of commercial plumbing experience across Northwest Arkansas. The team contractors trust.
        </p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16}}><span>HDI</span> <span style={{color:"var(--accent)"}}>Plumbing</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>Fayetteville's perfect-rated commercial plumbing contractor serving all of Northwest Arkansas. Master Plumber Michael Caudle and a skilled crew of 10-19 professionals.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>2169 Leeper Dr<br/>Fayetteville, AR 72701<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a><br/><span style={{fontSize:13}}>Mon–Fri: 7 AM – 5 PM</span></p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} HDI Plumbing Contractors. All rights reserved.</span>
          <span>Master Plumber — Michael Caudle</span>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call HDI Plumbing</a>
    </div>
