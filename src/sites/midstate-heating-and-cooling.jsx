...
const PHONE = "(501) 888-1118";
const TEL = "tel:+15018881118";

const Icons = {
  phone: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>,
  ac: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="3" width="20" height="10" rx="2"/><path d="M6 16v2"/><path d="M10 16v4"/><path d="M14 16v2"/><path d="M18 16v4"/><path d="M6 8h12"/></svg>,
  ...
  duct: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="6" width="20" height="4" rx="1"/><rect x="2" y="14" width="20" height="4" rx="1"/><path d="M6 10v4"/><path d="M12 10v4"/><path d="M18 10v4"/></svg>,
  sprayFoam: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><path d="M8 13h8"/><path d="M8 16h8"/><path d="M8 10h8"/></svg>,
  airPurify: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M9.59 4.59A2 2 0 1111 8H2m10.59 11.41A2 2 0 1014 16H2m15.73-8.27A2.5 2.5 0 1119.5 12H2"/></svg>,
...
const services = [
  { name: "A/C Installation & Repair", desc: "From new high-efficiency installations to emergency repairs, we keep your home cool through Arkansas summers. Fast diagnostics, honest pricing, and systems built to last.", icon: Icons.ac },
  { name: "Heating & Furnace Services", desc: "Expert heating installation and repair to keep your family warm all winter. We service all major brands and provide upfront pricing before any work begins.", icon: Icons.flame },
  { name: "Duct Work Installation", desc: "Properly designed ductwork is the backbone of your HVAC system. We install and replace duct systems for optimal airflow, efficiency, and comfort throughout your home.", icon: Icons.duct },
  { name: "Spray Foam Duct Insulation", desc: "Our specialized spray foam insulation seals and insulates your ductwork, eliminating energy loss and dramatically improving your system's efficiency and your comfort.", icon: Icons.sprayFoam },
  { name: "Air Purification Systems", desc: "Breathe cleaner, healthier air at home. We install whole-home air purification systems that remove allergens, bacteria, and pollutants from your indoor environment.", icon: Icons.airPurify },
  { name: "AC & Furnace Maintenance", desc: "Regular tune-ups extend equipment life, prevent costly breakdowns, and keep your system running at peak efficiency. Ask about our maintenance plans.", icon: Icons.maintenance },
];
...
const areas = ["Benton", "Little Rock", "Bryant", "Haskell", "Malvern", "Central Arkansas"];

const reviews = [
  { text: "I called and spoke with Bryan this morning. He was able to fit me into the schedule to check on my unit. Within 30 minutes of being at my house, he was able to see what the issue was and got it fixed. He was very professional and explained what was going on. Will use them in the future!", name: "Nicole Howard", src: "Google" },
  { text: "Honestly, I\u2019ve heard so many horror stories about long wait times and high repair bills\u2026I was prepared for the worst, and instead got the best experience possible. Bryan was professional, honest, and fair on pricing.", name: "Michael Ashley", src: "Google" },
  { text: "Excellent! Quick response, perfect professionalism, expert installers had my new heat pump air conditioning system up and running, with all the old stuff gone and everything cleaned up, in only two days.", name: "Cynthia C.", src: "Google" },
];
...
export default function MidstateHeatingAndCooling() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("midstate-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("midstate-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "HVAC Services Little Rock AR | Midstate Heating & Cooling";
    const els = [];
    const add = (tag, attrs) => {
    ...
    add("meta", { name: "description", content: "24/7 HVAC services in Little Rock, AR. 4.9 stars, 32 reviews. Family owned since 2009. Call (501) 888-1118 for a free quote." });
    ...
    add("script", {
      ...
      textContent: JSON.stringify({
        ...
        "@type": "HVACBusiness",
        name: "Midstate Heating & Cooling",
        telephone: "+15018881118",
        email: "bryanrclark@icloud.com",
        address: { "@type": "PostalAddress", streetAddress: "4901 Guyse Rd", addressLocality: "Little Rock", addressRegion: "AR", postalCode: "72206", addressCountry: "US" },
        ...
        founder: { "@type": "Person", name: "Bryan Clark" },
        foundingDate: "2009",
        description: "Midstate Heating & Cooling provides 24/7 HVAC installation, repair, and maintenance services in Little Rock, AR and Central Arkansas. Family owned since 2009. Entergy Solutions Partner.",
        ...
        memberOf: [{ "@type": "Organization", name: "Entergy Solutions" }],
        sameAs: [],
      }),
    });
        ...
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#1C2B41" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, marginBottom: 8, fontWeight: 600, letterSpacing: -0.5 }}>
              <span style={{ color: "#1C2B41" }}>Midstate</span>{" "}
              <span style={{ color: "#C8934F" }}>Heating & Cooling</span>
            </div>
    ...
    <>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled ? "rgba(28,43,65,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition: "all 0.4s ease" }}>
        <div className="serif" style={{ fontSize: 22, color: scrolled ? "#fff" : "var(--primary)", fontWeight: 600, letterSpacing: -0.5 }}>
          <span>Midstate</span>{" "}
          <span style={{ color: "var(--accent)" }}>Heating & Cooling</span>
        </div>
        ...
        <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 48, alignItems: "center" }}>
          <div>
            ...
            <h1 style={{ fontSize: "clamp(40px,5vw,68px)", lineHeight: 1.05, color: "var(--primary)", letterSpacing: -1.5, marginBottom: 20 }}>
              Little Rock's<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Most Trusted</span><br />HVAC Company
            </h1>
            <p style={{ fontSize: 18, lineHeight: 1.7, color: "var(--text-mid)", maxWidth: 480, marginBottom: 28 }}>
              Family owned since 2009. Open 24 hours, 7 days a week. Owner Bryan Clark works directly with every customer. Honest pricing, expert service, and no surprises.
            </p>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 32 }}>
              <Stars />
              <span style={{ fontSize: 14, color: "var(--text-light)", fontStyle: "italic", marginLeft: 4 }}>"Bryan was professional, honest, and fair on pricing." — Michael Ashley</span>
            </div>
      ...
      <section style={{ background: "var(--white)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", padding: "48px clamp(24px,5vw,64px)" }}>
        <div className="trust-row" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 32, textAlign: "center" }}>
          {[
            { num: "4.9", label: "Google Rating", desc: "32+ five-star reviews" },
            { num: "32+", label: "5-Star Reviews", desc: "Real customer feedback" },
            { num: "24/7", label: "Always Available", desc: "Open 24 hours, every day" },
            { num: "15+", label: "Years of Service", desc: "Family owned since 2009" },
          ].map((s, i) => (
            <div key={i}>
      ...
      <section id="services" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <p style={{ textAlign: "center", marginTop: 40, fontSize: 15, color: "var(--text-mid)" }}>
            Questions? Call Bryan directly at <a href={TEL} style={{ color: "var(--accent)", fontWeight: 700 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--primary)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <div className="gold-line" style={{ margin: "0 auto 20px" }} />
            <div style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: 3, color: "var(--accent)", marginBottom: 14 }}>Why Midstate</div>
            <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "#fff", letterSpacing: -0.5 }}>4.9 Stars on Google. 32 Reviews.</h2>
          </div>
          <div className="why-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "start" }}>
            <div>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "rgba(255,255,255,0.82)", marginBottom: 20 }}>
                Bryan Clark started Midstate Heating & Cooling in 2009 with a simple promise: treat every customer the way you'd want to be treated. Sixteen years later, that hasn't changed. Bryan still works directly with every customer — no runaround, no upselling, just honest diagnostics and fair pricing.
              </p>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "rgba(255,255,255,0.82)", marginBottom: 32 }}>
                As an Entergy Solutions Partner, Midstate brings expertise that goes beyond basic HVAC. From specialized spray foam duct insulation to whole-home air purification, we offer services most competitors don't. And with 24/7 availability, your family is never left waiting when comfort matters most. Veterans receive 10% off all services.
              </p>
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {[
                  "Open 24 hours, 7 days a week",
                  "Family owned and operated since 2009",
                  "Entergy Solutions Partner",
                  "Veterans receive 10% off all services",
                  "Financing available on major installations",
                  "Mobile home HVAC specialists",
                ].map((c, i) => (
            ...
            <div className="badge-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {[
                { icon: Icons.clock, title: "24/7 Service", desc: "Always available — day, night, weekends, and holidays" },
                { icon: Icons.shield, title: "Entergy Partner", desc: "Official Entergy Solutions Partner for energy efficiency" },
                { icon: Icons.award, title: "Since 2009", desc: "Over 15 years serving Little Rock and Central Arkansas" },
                { icon: Icons.dollar, title: "Financing", desc: "Flexible financing options available on major installations" },
              ].map((b, i) => (
      ...
      <section style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)", borderTop: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "var(--primary)", letterSpacing: -0.5, lineHeight: 1.15, marginBottom: 20 }}>
            Got an HVAC problem?<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Call Bryan.</span>
          </h2>
          <p style={{ fontSize: 17, lineHeight: 1.7, color: "var(--text-mid)", marginBottom: 36, maxWidth: 520, margin: "0 auto 36px" }}>
            Honest pricing. No surprise fees. Available 24/7. Family owned since 2009.
          </p>
      ...
      <footer style={{ background: "var(--primary)", padding: "80px clamp(24px,5vw,64px) 40px" }}>
        <div className="footer-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: 48, marginBottom: 48 }}>
          <div>
            <div className="serif" style={{ fontSize: 22, fontWeight: 600, letterSpacing: -0.5, marginBottom: 16 }}>
              <span style={{ color: "#fff" }}>Midstate</span>{" "}
              <span style={{ color: "var(--accent)" }}>Heating & Cooling</span>
            </div>
            <p style={{ fontSize: 14, lineHeight: 1.7, color: "rgba(255,255,255,0.6)", marginBottom: 16, maxWidth: 320 }}>
              Family owned HVAC services since 2009. Serving Little Rock and Central Arkansas 24 hours a day, 7 days a week.
            </p>
            <p style={{ fontSize: 14, color: "rgba(255,255,255,0.6)", marginBottom: 6 }}>4901 Guyse Rd, Little Rock, AR 72206</p>
            <a href={TEL} style={{ fontSize: 14, color: "var(--accent)", fontWeight: 700 }}>{PHONE}</a>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.45)", marginTop: 8 }}>Open 24/7</p>
          </div>
          <div>
        ...
        <div className="footer-btm" style={{ maxWidth: 1200, margin: "0 auto", borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 24, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>&copy; {yr} Midstate Heating & Cooling. All rights reserved.</p>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>Entergy Solutions Partner</p>
        </div>
      </footer>
      ...
      <div className="mobile-only" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 998, display: "none" }}>
        <a href={TEL} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "16px 20px", fontSize: 16, fontWeight: 700, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 -4px 20px rgba(0,0,0,0.15)", textDecoration: "none" }}>
          {Icons.phone} Call Midstate — {PHONE}
        </a>
      </div>
