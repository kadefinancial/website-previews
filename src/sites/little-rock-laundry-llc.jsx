// Layout: B | Industry: Cleaning | City: Little Rock
import { useState, useEffect } from "react";

const PHONE = "(501) 375-1585";
const TEL = "tel:+15013751585";

const Icons = {
  phone: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>,
  shirt: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M20.38 3.46L16 2a4 4 0 01-8 0L3.62 3.46a2 2 0 00-1.34 2.23l.58 3.47a1 1 0 00.99.84H6v10a2 2 0 002 2h8a2 2 0 002-2V10h2.15a1 1 0 00.99-.84l.58-3.47a2 2 0 00-1.34-2.23z"/></svg>,
  ...
  calendar: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2" ry="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/><path d="M8 14h.01"/><path d="M12 14h.01"/><path d="M16 14h.01"/><path d="M8 18h.01"/><path d="M12 18h.01"/></svg>,
  ...
  restaurant: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8h1a4 4 0 010 8h-1"/><path d="M2 8h16v9a4 4 0 01-4 4H6a4 4 0 01-4-4V8z"/><line x1="6" y1="1" x2="6" y2="4"/><line x1="10" y1="1" x2="10" y2="4"/><line x1="14" y1="1" x2="14" y2="4"/></svg>,
  linen: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 22h16a2 2 0 002-2V6a2 2 0 00-2-2H4a2 2 0 00-2 2v14a2 2 0 002 2z"/><path d="M2 10h20"/><path d="M6 4v6"/><path d="M10 4v6"/><path d="M14 4v6"/><path d="M18 4v6"/><path d="M8 14h8"/><path d="M8 18h8"/></svg>,
...
const services = [
  { name: "Commercial Laundry Services", desc: "High-volume commercial laundry for businesses across Central Arkansas. Three generations of expertise handling linens, uniforms, and textiles with the care and consistency your operation demands.", icon: Icons.shirt },
  { name: "Drop-Off Laundry Service", desc: "Bring it in, we handle the rest. Professional wash, dry, and fold service with fast turnaround and the personal attention Miss Kay and her crew are known for.", icon: Icons.building },
  { name: "Event-Day Laundry (Arenas/Venues)", desc: "Trusted by Simmons Bank Arena for event-day laundry needs. Towels, linens, and uniforms cleaned and returned on tight timelines so your venue is always show-ready.", icon: Icons.calendar },
  { name: "Disaster Restoration Laundry", desc: "Specialized laundry services for disaster restoration companies nationwide. From flood damage to fire recovery, we process high volumes fast so your crews can keep moving.", icon: Icons.storm },
  { name: "Restaurant & Hospitality Laundry", desc: "Tablecloths, napkins, chef coats, and kitchen linens cleaned to hospitality standards. Reliable pickup and delivery schedules that keep your restaurant running without interruption.", icon: Icons.restaurant },
  { name: "Linen Service", desc: "Complete linen management for hotels, medical offices, and commercial facilities. Fresh, professionally laundered linens delivered on your schedule, every time.", icon: Icons.linen },
];

const allServiceNames = ["Commercial Laundry Services","Drop-Off Laundry Service","Event-Day Laundry (Arenas/Venues)","Disaster Restoration Laundry","Restaurant & Hospitality Laundry","Linen Service"];
...
const reviews = [
  { name:"Tom Watkins", text:"Miss Kay and her crew are on point when it comes to drop of laundry service! I've done disaster restoration all over the country, Puerto Rico, and Canada. Miss Kay and her crew are the best." },
  { name:"Lily Hesen", text:"Simmons Bank Arena appreciates Kay Peoples and staff for taking such good care of us on our event days! They have excellent service, great communication and they are appreciated by everyone with the arena and tours." },
];

export default function LittleRockLaundryLlc() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("lrl-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("lrl-auth","1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Commercial Laundry Little Rock AR | Little Rock Laundry";
    const els = [];
    ...
    add("meta",{name:"description",content:"Commercial laundry in Little Rock, AR. 4.5-star Google rating. 3rd-generation family business. Arenas, restaurants, disaster restoration. Call (501) 375-1585."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      "@context":"https://schema.org","@type":"HousekeepingService",
      name:"Little Rock Laundry LLC",telephone:"+15013751585",
      address:{"@type":"PostalAddress",streetAddress:"1325 E 9th St",addressLocality:"Little Rock",addressRegion:"AR",postalCode:"72202",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:34.7465,longitude:-92.2615},
      openingHoursSpecification:[
        {"@type":"OpeningHoursSpecification",dayOfWeek:["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday"],opens:"08:00",closes:"16:30"},
      ],
      aggregateRating:{"@type":"AggregateRating",ratingValue:"4.5",reviewCount:"12",bestRating:"5"},
      founder:{"@type":"Person",name:"Verena K. Peoples"},
      description:"Third-generation commercial laundry service in Little Rock, AR. Serving arenas, restaurants, disaster restoration companies, and businesses across Central Arkansas since 2002.",
      areaServed:areas.map(a=>({"@type":"City",name:a})),
      hasOfferCatalog:{"@type":"OfferCatalog",name:"Commercial Laundry Services",itemListElement:allServiceNames.map(s=>({"@type":"Offer",itemOffered:{"@type":"Service",name:s}}))},
      memberOf:[],
      sameAs:[],
    })});
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1E3A2F"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8}}>
            <span style={{fontWeight:700,color:"#1E3A2F"}}>Little Rock</span> <span style={{color:"#B8694A"}}>Laundry</span>
          </div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(30,58,47,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease",boxShadow:scrolled?"0 1px 0 rgba(255,255,255,0.06)":"none"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}>
        <span>Little Rock</span> <span style={{color:"var(--accent)"}}>Laundry</span>
      </div>
    ...
    <section style={{padding:"180px clamp(24px,5vw,64px) 100px",background:"var(--cream)",textAlign:"center"}}>
      <div className="hero-text" style={{maxWidth:780,margin:"0 auto"}}>
        ...
        <h1 style={{fontSize:"clamp(44px, 6vw, 80px)",lineHeight:1.05,color:"var(--primary)",marginBottom:24,letterSpacing:-2}}>
          Commercial Laundry<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Little Rock Trusts</span>
        </h1>
        <p style={{fontSize:18,lineHeight:1.7,color:"var(--text-mid)",marginBottom:32,maxWidth:600,margin:"0 auto 32px"}}>
          Three generations of commercial laundry excellence. From Simmons Bank Arena event days to nationwide disaster restoration, Miss Kay and her crew deliver the quality and reliability your business depends on.
        </p>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:12,flexWrap:"wrap"}}>
          <Stars />
          <span style={{fontSize:14,color:"var(--text-light)",fontWeight:600,marginLeft:4}}>5-star review on Google</span>
        </div>
        <p style={{fontSize:15,color:"var(--text-mid)",fontStyle:"italic",maxWidth:540,margin:"0 auto 36px",lineHeight:1.6}}>
          "Simmons Bank Arena appreciates Kay Peoples and staff for taking such good care of us on our event days! They have excellent service, great communication and they are appreciated by everyone with the arena and tours." — <strong style={{color:"var(--primary)",fontStyle:"normal"}}>Lily Hesen</strong>
        </p>
        <div style={{display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap"}}>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(184,105,74,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Kay Now</a>
    ...
    <section style={{padding:"80px clamp(24px,5vw,64px)",background:"var(--white)",borderTop:"1px solid var(--border)",borderBottom:"1px solid var(--border)"}}>
        ...
        <div className="form-card-wrap" style={{background:"var(--white)",borderRadius:12,padding:"44px 36px",boxShadow:"0 4px 40px rgba(0,0,0,0.08)",border:"1px solid var(--border)",marginBottom:56}}>
          {!formDone ? (
            <form onSubmit={e=>{e.preventDefault();setFormDone(true)}}>
              <h3 style={{fontSize:26,color:"var(--primary)",marginBottom:6,fontFamily:"'Playfair Display',serif"}}>Request a Quote</h3>
            ...
            <div style={{textAlign:"center",padding:"24px 0"}}>
              ...
              <p style={{fontSize:14,color:"var(--text-mid)"}}>We'll call you back shortly. For urgent needs,<br/>call <a href={TEL} style={{color:"var(--accent)",fontWeight:700}}>{PHONE}</a></p>
            </div>
          )}
        </div>
      </div>
      <div className="trust-row" style={{display:"grid",gridTemplateColumns:"repeat(4, 1fr)",gap:32,maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        {[
          {num:"4.5",label:"Google Rating",desc:"Trusted reviews"},
          {num:"3rd Gen",label:"Family Business",desc:"Since 2002"},
          {num:"Simmons",label:"Arena Client",desc:"Event-day laundry"},
          {num:"National",label:"Disaster Restoration",desc:"Coast to coast"},
        ].map((s,i)=>(
          <div key={i}>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",color:"var(--primary)",letterSpacing:-0.5}}>Commercial Laundry Services in Little Rock</h2>
        </div>
        ...
        <p style={{textAlign:"center",marginTop:40,fontSize:15,color:"var(--text-light)"}}>
          Questions? Call Kay directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a>
        </p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",color:"#fff"}}>
      <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,opacity:0.7}}>About Little Rock Laundry</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff"}}>Three Generations of<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Commercial Laundry Excellence.</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.65)",marginBottom:24,textAlign:"left"}}>
          Little Rock Laundry is a family story. In 2002, a local doctor entrusted the shop to Kay Peoples' mother, and what started as a small neighborhood laundry has grown into one of Central Arkansas's most trusted commercial laundry operations. Now in its third generation, Miss Kay carries on the tradition with the same hands-on care and no-shortcuts work ethic that built the business from day one.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.65)",marginBottom:36,textAlign:"left"}}>
          Today, Little Rock Laundry serves clients most people never expect from a local shop. Simmons Bank Arena trusts Kay and her crew to handle event-day laundry for concerts and tours. Disaster restoration companies across the country — from Puerto Rico to Canada — call on them when they need fast, reliable textile processing. With Brittany Peoples driving the business forward into 2026, Little Rock Laundry is ready for its next chapter: serving new commercial clients across Central Arkansas with the same family values and professional results they've delivered for over two decades.
        </p>
        <div className="why-creds-row" style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:24,marginTop:8}}>
          {["4.5 Google Rating","3rd Generation Owned","Simmons Bank Arena Client","National Disaster Restoration","Mon-Sat 8 AM - 4:30 PM"].map((c,i)=>(
    ...
    <section id="areas" style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:12}}>
          {areas.map((a,i)=>(
            <span key={i} title={`Commercial Laundry in ${a}, AR`} style={{display:"inline-flex",alignItems:"center",gap:6,background:"#fff",border:"1px solid var(--border)",padding:"12px 24px",borderRadius:30,fontSize:15,fontWeight:500,color:"var(--text)",cursor:"default",transition:"all 0.25s"}}
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"#FFFFFF",textAlign:"center",borderTop:"1px solid rgba(0,0,0,0.06)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"#2A2A3C",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>
          Need reliable commercial laundry?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Kay.</span>
        </h2>
        <p style={{fontSize:17,color:"#5A5A6E",marginBottom:36,lineHeight:1.6}}>
          Third-generation quality, arena-tested reliability, and the personal service your business deserves. Little Rock's trusted commercial laundry partner.
        </p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0",borderTop:"1px solid rgba(255,255,255,0.06)"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16}}><span>Little Rock</span> <span style={{color:"var(--accent)"}}>Laundry</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>Third-generation commercial laundry service in Little Rock, Arkansas. Trusted by Simmons Bank Arena, disaster restoration companies nationwide, and businesses across Central Arkansas.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>
            1325 E 9th St<br/>Little Rock, AR 72202<br/>
            <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a><br/>
            <a href="mailto:lrlaundry@gmail.com" style={{color:"var(--accent)",fontWeight:600}}>lrlaundry@gmail.com</a>
          </p>
        </div>
        <div>
        ...
        <div>
          ...
          <p style={{fontSize:14,opacity:0.6,lineHeight:1.7}}>Mon – Sat: 8 AM – 4:30 PM<br/>Closed Sunday</p>
        </div>
      </div>
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto",display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
        <span>&copy; {yr} Little Rock Laundry LLC. All rights reserved.</span>
        <span>Third-Generation Family Business | Serving Central Arkansas</span>
      </div>
    </footer>

    {/* FLOATING MOBILE CTA */}
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Kay Now</a>
    </div>
