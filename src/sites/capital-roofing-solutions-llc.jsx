// Layout: C | Industry: Roofing | City: Alexander
...
const Icons = {
  ...
  metal: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M2 20h20"/><path d="M4 20V8l8-6 8 6v12"/><path d="M4 8h16"/><path d="M8 8v12"/><path d="M12 8v12"/><path d="M16 8v12"/></svg>
  ),
  building: (
  ...
  clipboard: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M16 4h2a2 2 0 012 2v14a2 2 0 01-2 2H6a2 2 0 01-2-2V6a2 2 0 012-2h2"/><rect x="8" y="2" width="8" height="4" rx="1"/><path d="M9 14l2 2 4-4"/></svg>
  ),
  search: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/><path d="M11 8v6"/><path d="M8 11h6"/></svg>
  ),
  check: (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
  ),
  star: (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="#2E7D52" stroke="none"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
  ),
  mapPin: (
  ...
  mapPinLarge: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>
  ),
};
...
// --- Data ---
const PHONE = "(501) 539-5810";
const PHONE_TEL = "tel:+15015395810";
const EMAIL = "balesmgmt@gmail.com";

const services = [
  { name: "Roof Replacement", desc: "Complete tear-off and replacement engineered for Arkansas weather. Capital Roofing manages every phase from initial assessment through final cleanup, using GAF and Owens Corning materials that protect your home for decades.", icon: Icons.roof },
  { name: "Storm Damage Restoration", desc: "Fast response after hail, wind, and severe storms. The Capital Roofing team assesses damage thoroughly, documents everything for your insurer, and handles the full restoration so you can focus on your family.", icon: Icons.storm },
  { name: "Insurance Claims Specialist", desc: "Navigating insurance claims is what Capital Roofing does best. Jonathan and Scott work directly with adjusters, handle all documentation, and fight to get you the coverage your policy promises.", icon: Icons.clipboard },
  { name: "Metal Roofing", desc: "Durable, energy-efficient metal roofing systems built to last 50+ years. Ideal for Arkansas homeowners looking for long-term protection with minimal maintenance and superior storm resistance.", icon: Icons.metal },
  { name: "Construction Management", desc: "Full-scope construction management for residential and commercial projects across central Arkansas. Capital Roofing coordinates every trade, timeline, and detail from start to finish.", icon: Icons.building },
  { name: "Free Inspections", desc: "No-cost, no-obligation roof inspections that give you an honest assessment of your roof's condition. Capital Roofing identifies issues early so you can make informed decisions before problems grow.", icon: Icons.search },
];

const allServices = [
  "Roof Replacement", "Roof Repair", "Storm Damage Restoration",
  "Insurance Claims Specialist", "Metal Roofing", "Construction Management",
  "Free Inspections & Estimates",
];

const reviews = [
  { text: "I have used capital roofing on my new house and my old house as well.", name: "Reggie Brown", src: "Google" },
  { text: "Quick response and clear communication! Very knowledgeable and easy to deal with.", name: "Harrison Dale", src: "Google" },
];

const areas = ["Alexander", "Benton", "Bryant", "Little Rock", "North Little Rock", "Sherwood", "Maumelle", "Saline County"];

// --- Main Component ---
export default function CapitalRoofingSolutionsLLC() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("capital-roofing-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("capital-roofing-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Roofing Alexander AR | Capital Roofing Solutions";
    const els = [];
    const add = (tag, attrs) => {
    ...
    add("meta", { name: "description", content: "Capital Roofing Solutions \u2014 4.5 stars, 35+ reviews. Insurance claims specialists. GAF certified. Call (501) 539-5810 for a free inspection in Alexander, AR." });
    add("meta", { name: "geo.region", content: "US-AR" });
    add("meta", { name: "geo.placename", content: "Alexander" });
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      "@context": "https://schema.org",
      "@type": "RoofingContractor",
      name: "Capital Roofing Solutions, LLC",
      telephone: "+15015395810",
      email: EMAIL,
      address: { "@type": "PostalAddress", streetAddress: "12401 Samples Rd", addressLocality: "Alexander", addressRegion: "AR", postalCode: "72002", addressCountry: "US" },
      geo: { "@type": "GeoCoordinates", latitude: 34.6298, longitude: -92.4413 },
      ...
      aggregateRating: { "@type": "AggregateRating", ratingValue: "4.5", reviewCount: "35", bestRating: "5" },
      founder: { "@type": "Person", name: "Jonathan Whitehead" },
      foundingDate: "2015",
      description: "Capital Roofing Solutions, LLC is a GAF certified roofing contractor and insurance claims specialist serving Alexander and central Arkansas since 2015. BBB A+ rated with 2 offices in Alexander and Fayetteville.",
        ...
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#1C2B41" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, fontWeight: 600, letterSpacing: -0.5, marginBottom: 4 }}>
              <span style={{ color: "#1C2B41" }}>Capital</span> <span style={{ color: "#2E7D52" }}>Roofing</span>
            </div>
    ...
    <>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled ? "rgba(28,43,65,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition: "all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        <div className="serif" style={{ fontSize: 22, fontWeight: 600, letterSpacing: -0.5, color: "#fff" }}>
          <span>Capital</span><span style={{ color: "var(--accent)", marginLeft: 6 }}>Roofing</span>
        </div>
        ...
        <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
          ...
          <div style={{ maxWidth: 560 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.1)", padding: "8px 18px", borderRadius: 4, marginBottom: 28 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: "var(--accent)" }}>Insurance Claims Specialists</span>
            </div>
            <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(40px,5vw,68px)", lineHeight: 1.05, letterSpacing: -1.5, color: "#fff", marginBottom: 24 }}>
              Alexander's{" "}<br/>
              <em style={{ color: "var(--accent)", fontStyle: "italic" }}>Trusted Roofer</em><br/>
              Since 2015
            </h1>
            <p style={{ fontSize: 18, lineHeight: 1.7, color: "rgba(255,255,255,0.75)", marginBottom: 32, maxWidth: 480 }}>
              Capital Roofing Solutions has protected central Arkansas homes and businesses since 2015. GAF certified, BBB A+ rated, and built on doing right by every customer.
            </p>
            {/* Star row with hero quote */}
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 32, flexWrap: "wrap" }}>
              <Stars />
              <span style={{ fontSize: 14, color: "rgba(255,255,255,0.7)", fontStyle: "italic" }}>"Quick response and clear communication!"</span>
              <span style={{ fontSize: 13, color: "rgba(255,255,255,0.5)" }}>- Harrison Dale</span>
            </div>
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              <a href={PHONE_TEL} style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 36px", borderRadius: 6, fontSize: 16, fontWeight: 700, letterSpacing: 0.3, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 4px 20px rgba(46,125,82,0.3)", transition: "transform 0.2s" }}>
                {Icons.phone} Call Jonathan Now
              </a>
      ...
      <section style={{ background: "var(--white)", padding: "48px clamp(24px,5vw,64px)" }}>
        <div className="trust-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 32, textAlign: "center" }}>
          {[
            { num: "4.5", label: "Google Rating", desc: "Verified reviews" },
            { num: "35+", label: "Reviews", desc: "Happy customers" },
            { num: "GAF", label: "Certified", desc: "Factory trained" },
            { num: "A+", label: "BBB Rated", desc: "Since 2015" },
          ].map((s, i) => (
      ...
      <section id="services" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)" }}>Roofing Services in Alexander</h2>
          </div>
          {services.map((svc, i) => {
            const isEven = i % 2 === 1;
            return (
          ...
          <p style={{ textAlign: "center", marginTop: 48, fontSize: 15, color: "var(--text-mid)" }}>
            Questions? Call Jonathan directly at <a href={PHONE_TEL} style={{ color: "var(--accent)", fontWeight: 600 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <div className="gold-line" />
            <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: "var(--text-light)", marginBottom: 14, fontFamily: "'DM Sans',sans-serif" }}>Why Capital Roofing</p>
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)" }}>Insurance Claims Specialists in Alexander</h2>
          </div>
          <div className="why-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "start" }}>
            ...
            <div>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "var(--text-mid)", marginBottom: 20 }}>
                Jonathan Whitehead founded Capital Roofing Solutions in 2015 with a simple belief: homeowners deserve a roofing company that treats them like family. That philosophy built a team that now operates out of two offices in Alexander and Fayetteville, with Shelly managing operations and Scott Bales leading sales across central Arkansas.
              </p>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "var(--text-mid)", marginBottom: 32 }}>
                What sets Capital Roofing apart is their insurance claims expertise. When storms hit and adjusters push back, Jonathan and Scott fight for homeowners to get the coverage their policies promise. GAF certified and Owens Corning listed, Capital Roofing pairs manufacturer-backed quality with the kind of personal attention that earns a BBB A+ rating and customers who come back for every property they own.
              </p>
              {/* Pull quote */}
              <div style={{ borderLeft: "3px solid var(--accent)", paddingLeft: 24, marginBottom: 36 }}>
                <p style={{ fontSize: 18, fontStyle: "italic", lineHeight: 1.7, color: "var(--primary)", fontFamily: "'Playfair Display',serif" }}>
                  "They take care of customers like family."
                </p>
                <p style={{ fontSize: 14, color: "var(--text-light)", marginTop: 8, fontFamily: "'DM Sans',sans-serif" }}>- Capital Roofing Customers</p>
              </div>
              {/* Credentials */}
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {[
                  "GAF Certified Roofing Contractor",
                  "Owens Corning Listed Installer",
                  "BBB A+ Rated Since 2015",
                  "Insurance Claims Specialists",
                  "2 Offices: Alexander + Fayetteville",
            ...
            <div className="badge-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {[
                { icon: Icons.award, title: "GAF Certified", desc: "Factory-trained and certified by GAF, North America's largest roofing manufacturer. Your roof is backed by industry-leading warranties and proven installation methods." },
                { icon: Icons.shield, title: "BBB A+ Rated", desc: "A+ rating with the Better Business Bureau reflects a decade of honest work, transparent pricing, and consistent customer satisfaction across central Arkansas." },
                { icon: Icons.clipboard, title: "Claims Experts", desc: "Capital Roofing specializes in insurance claims. The team handles documentation, works with adjusters, and fights to get homeowners the coverage they deserve." },
                { icon: Icons.mapPinLarge, title: "2 Offices", desc: "With locations in Alexander and Fayetteville, Capital Roofing serves homeowners across central and northwest Arkansas with local expertise and fast response times." },
              ].map((b, i) => (
      ...
      <section id="reviews" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
          ...
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)", marginBottom: 56 }}>4.5 Stars on Google</h2>
          {/* Large featured review — Reggie Brown (repeat customer) */}
          <div style={{ position: "relative", padding: "48px 40px", background: "var(--white)", borderRadius: 12 }}>
            ...
            <p style={{ fontSize: 20, lineHeight: 1.8, color: "var(--text)", fontStyle: "italic", fontFamily: "'Playfair Display',serif", marginBottom: 28 }}>
              "I have used Capital Roofing on my new house and my old house as well."
            </p>
            <p style={{ fontSize: 17, fontWeight: 700, color: "var(--primary)" }}>Reggie Brown</p>
            <p style={{ fontSize: 13, color: "var(--text-light)", marginTop: 4 }}>Repeat Customer, Google Review</p>
          </div>
          <p style={{ fontSize: 14, color: "var(--text-light)", marginTop: 32 }}>One of 35+ five-star reviews on Google</p>
      ...
      <section className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)", borderTop: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)", marginBottom: 20 }}>
            Need a new roof?<br/><em style={{ color: "var(--accent)", fontStyle: "italic" }}>Call Capital Roofing.</em>
          </h2>
          <p style={{ fontSize: 17, lineHeight: 1.7, color: "var(--text-mid)", marginBottom: 40, maxWidth: 520, margin: "0 auto 40px" }}>
            GAF certified, BBB A+ rated, and specializing in insurance claims since 2015. From free inspection to final installation, Capital Roofing Solutions has your back.
          </p>
          <a href={PHONE_TEL} style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 48px", borderRadius: 6, fontSize: 18, fontWeight: 700, letterSpacing: 0.3, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 4px 20px rgba(46,125,82,0.3)", transition: "transform 0.2s" }}>
            {Icons.phone} {PHONE}
          </a>
        </div>
      </section>
      ...
      <footer style={{ background: "var(--primary)", padding: "80px clamp(24px,5vw,64px) 40px", color: "rgba(255,255,255,0.75)" }}>
        <div className="footer-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: 48, marginBottom: 48 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 15, lineHeight: 1.7, marginBottom: 20, maxWidth: 300 }}>
              GAF certified roofing contractor and insurance claims specialist serving central Arkansas since 2015. BBB A+ rated with offices in Alexander and Fayetteville.
            </p>
            <p style={{ fontSize: 14, marginBottom: 8 }}>12401 Samples Rd, Alexander, AR 72002</p>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 24 }}>
          <div className="footer-btm-inner" style={{ maxWidth: 1200, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>{"\u00A9"} {new Date().getFullYear()} Capital Roofing Solutions, LLC. All rights reserved.</p>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>GAF Certified | BBB A+ Rated | Founded 2015</p>
          </div>
        </div>
      </footer>
      ...
      <div className="mobile-only" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 998, display: "none" }}>
        <a href={PHONE_TEL} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 24px", fontSize: 16, fontWeight: 700, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 -4px 20px rgba(0,0,0,0.15)", textAlign: "center" }}>
          {Icons.phone} Call Capital Roofing - {PHONE}
        </a>
      </div>
