// Layout: B | Industry: Roofing | City: Sherwood
import { useState, useEffect } from "react";

const PHONE = "(501) 882-4084";
const TEL = "tel:+15018824084";
...
const Icons = {
  ...
  commercial: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="2" width="16" height="20" rx="2"/><path d="M9 22V12h6v10"/><path d="M8 6h.01"/><path d="M16 6h.01"/><path d="M8 10h.01"/><path d="M16 10h.01"/><path d="M8 14h.01"/><path d="M16 14h.01"/></svg>,
  emergency: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M13 2L3 14h9l-1 8 10-12h-9l1-8z"/></svg>,
  maintenance: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M14.7 6.3a1 1 0 000 1.4l1.6 1.6a1 1 0 001.4 0l3.77-3.77a6 6 0 01-7.94 7.94l-6.91 6.91a2.12 2.12 0 01-3-3l6.91-6.91a6 6 0 017.94-7.94l-3.76 3.76z"/></svg>,
  ...
  siding: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18"/><path d="M3 15h18"/><path d="M9 3v18"/></svg>,
...
const services = [
  { name: "Residential Roofing", desc: "Complete roof installation and replacement for homes of all sizes. GAF, CertainTeed, and TAMKO certified materials backed by manufacturer warranties and expert craftsmanship.", icon: Icons.roof },
  { name: "Commercial Roofing", desc: "Flat roofs, metal systems, and full commercial builds. AQ Roofing handles projects of every scale with the same attention to detail and licensed professionalism.", icon: Icons.commercial },
  { name: "Emergency Repair", desc: "When your roof can't wait, neither can we. Same-day response for leaks, blow-offs, and urgent damage. Call any time and we'll be there.", icon: Icons.emergency },
  { name: "Roof Maintenance", desc: "Preventive maintenance plans that extend your roof's lifespan and catch small issues before they become big problems. Regular inspections and proactive care.", icon: Icons.maintenance },
  { name: "Storm Damage", desc: "Arkansas weather is unpredictable. AQ Roofing provides full storm damage restoration -- from emergency tarping through insurance documentation and final repair.", icon: Icons.storm },
  { name: "Siding Installation", desc: "Professional siding installation to protect and transform your home's exterior. Quality materials, clean installation, and a finished product you'll be proud of.", icon: Icons.siding },
];

const allServiceNames = ["Residential Roofing","Commercial Roofing","Roof Repair","Emergency Roof Repair","Roof Maintenance Plans","Storm Damage Restoration","Siding Installation","Free Inspections","Financing Available"];

const areas = ["Sherwood","Little Rock","North Little Rock","Jacksonville","Cabot","Maumelle","Conway","Benton"];

const reviews = [
  { text: "I used this company for my roof. I am very pleased with quality of the work they did. They are a real professional company.", name: "Rebecca Atherton", src: "Google" },
  { text: "I rarely write reviews like this; however, I was so impressed by the support and service provided.", name: "Vincent Escobar", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function AQRoofingLLC() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("aq-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="roof"){sessionStorage.setItem("aq-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "Roofing Contractor Sherwood AR | AQ Roofing LLC";
    const els = [];
    ...
    add("meta",{name:"description",content:"Trusted roofing contractor in Sherwood, AR. 4.5 stars, GAF/CertainTeed/TAMKO certified, family owned since 2013. Call (501) 882-4084."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"AQ Roofing LLC",telephone:"+15018824084",
      email:"info@aqroofs.com",
      address:{"@type":"PostalAddress",streetAddress:"117 Hillwood Dr",addressLocality:"Sherwood",addressRegion:"AR",postalCode:"72120",addressCountry:"US"},
      ...
      aggregateRating:{"@type":"AggregateRating",ratingValue:"4.5",reviewCount:"25",bestRating:"5"},
      founder:{"@type":"Person",name:"Marcos Quijano"},
      foundingDate:"2013",
      description:"AQ Roofing LLC is a family-owned roofing contractor in Sherwood, AR. Triple-certified (GAF, CertainTeed, TAMKO), serving Central Arkansas since 2013. License #327340321.",
      ...
      memberOf:[{name:"GAF Certified"},{name:"CertainTeed"},{name:"TAMKO"}],
      sameAs:[],
    })});
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#4A1A2E"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8,fontWeight:600,letterSpacing:-0.5}}><span style={{color:"#4A1A2E"}}>AQ</span> <span style={{color:"#2C5282"}}>Roofing</span></div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(74,26,46,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}><span>AQ</span> <span style={{color:"var(--accent)"}}>Roofing</span></div>
    ...
    <section className="hero-pad" style={{padding:"180px clamp(24px,5vw,64px) 100px",background:"var(--cream)",textAlign:"center"}}>
      <div style={{maxWidth:800,margin:"0 auto"}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(44,82,130,0.15)",padding:"8px 18px",borderRadius:4,marginBottom:28}}>
          <div style={{width:6,height:6,borderRadius:"50%",background:"var(--accent)"}} />
          <span style={{fontSize:12,fontWeight:700,color:"var(--accent)",letterSpacing:2,textTransform:"uppercase"}}>4.5 Stars -- Family Owned Since 2013</span>
        </div>
        <h1 style={{fontSize:"clamp(44px, 6vw, 80px)",lineHeight:1.05,color:"var(--primary)",marginBottom:24,letterSpacing:-2}}>
          Expert Roofing<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Sherwood Trusts</span>
        </h1>
        <p style={{fontSize:18,lineHeight:1.7,color:"var(--text-mid)",marginBottom:32,maxWidth:620,margin:"0 auto 32px"}}>
          AQ Roofing has served Central Arkansas since 2013. Triple-certified by GAF, CertainTeed, and TAMKO -- family owned by Marcos Quijano with four locations and Payzer financing available.
        </p>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:36,flexWrap:"wrap"}}>
          <Stars />
          <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"I rarely write reviews like this. I was so impressed." -- Vincent Escobar</span>
        </div>
        <div style={{display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap"}}>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(44,82,130,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call AQ Roofing</a>
      ...
      <div style={{maxWidth:480,margin:"0 auto"}}>
        <div style={{background:"var(--white)",borderRadius:12,padding:"44px 36px",boxShadow:"0 4px 40px rgba(0,0,0,0.08)",border:"1px solid var(--border)",marginBottom:56}}>
          {!formDone ? (
            ...
            <div style={{textAlign:"center",padding:"24px 0"}}>
              <div style={{width:56,height:56,borderRadius:"50%",background:"rgba(44,82,130,0.15)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px",color:"var(--accent)"}}>{Icons.check}</div>
        ...
        <div className="trust-row" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:24,textAlign:"center"}}>
          {[
            ...
            {num:"25+",label:"Reviews",desc:"Real customers"},
            {num:"GAF",label:"Certified",desc:"Triple-certified"},
            {num:"2013",label:"Since",desc:"Family owned"},
          ].map((s,i)=>(
            <div key={i}>
    ...
    <section id="services" className="section-pad" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:60}}>
          ...
          <h2 style={{fontSize:"clamp(30px, 3.5vw, 44px)",lineHeight:1.15,color:"var(--primary)",letterSpacing:-0.5}}>Roofing Services in Sherwood</h2>
        </div>
        ...
        <p style={{textAlign:"center",marginTop:40,fontSize:15,color:"var(--text-mid)"}}>Questions? Call Marcos directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:700}}>{PHONE}</a></p>
      </div>
    </section>
    ...
    <section id="about" className="section-pad" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)"}}>
      <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <p style={{fontSize:12,fontWeight:700,letterSpacing:3,textTransform:"uppercase",color:"rgba(255,255,255,0.5)",marginBottom:14}}>Why AQ Roofing</p>
        <h2 style={{fontSize:"clamp(30px, 3.5vw, 44px)",lineHeight:1.15,color:"#fff",letterSpacing:-0.5,marginBottom:40}}>A Family Business Built on <span style={{fontStyle:"italic",color:"var(--accent)"}}>Craftsmanship and Trust</span></h2>
        <p style={{fontSize:17,lineHeight:1.8,color:"rgba(255,255,255,0.75)",marginBottom:24}}>
          Marcos Quijano founded AQ Roofing in 2013 as a family business with one simple idea: do honest work at fair prices. What started as a local operation in Sherwood has grown into a company with four locations across Sherwood, Little Rock, Fayetteville, and Fort Worth -- all without losing the personal touch that built it.
        </p>
        <p style={{fontSize:17,lineHeight:1.8,color:"rgba(255,255,255,0.75)",marginBottom:48}}>
          AQ Roofing is triple-certified by GAF, CertainTeed, and TAMKO -- a rare distinction that means access to the best manufacturer warranties in the industry. Licensed (#327340321) and offering Payzer financing, Marcos and his team make quality roofing accessible to every homeowner in Central Arkansas.
        </p>
        <div className="why-creds-row" style={{display:"flex",justifyContent:"center",gap:28,flexWrap:"wrap"}}>
          {["GAF Certified","CertainTeed Certified","TAMKO Certified","License #327340321","Payzer Financing","4 Locations"].map((c,i)=>(
    ...
    {/* REVIEWS -- Layout B: 3 cards side by side (only 2 available + short excerpt for 3rd) */}
      ...
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        ...
        <div className="rev-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:20}}>
          {[
            reviews[0],
            reviews[1],
            { text: "Very pleased with the quality of work. A real professional company.", name: "Rebecca Atherton", src: "Google" },
          ].map((r,i)=>(
    ...
    <section className="section-pad" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--white)",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
        <h2 style={{fontSize:"clamp(30px, 3.5vw, 44px)",lineHeight:1.15,color:"var(--primary)",letterSpacing:-0.5,marginBottom:20,fontFamily:"'Playfair Display',serif"}}>
          Need a new roof?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call AQ Roofing.</span>
        </h2>
        <p style={{fontSize:17,lineHeight:1.7,color:"var(--text-mid)",marginBottom:36}}>Triple-certified by GAF, CertainTeed, and TAMKO. Family owned since 2013 with four locations across Arkansas and Texas. Payzer financing available.</p>
        <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(44,82,130,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} {PHONE}</a>
      </div>
    </section>
    ...
    <footer style={{padding:"80px clamp(24px,5vw,64px) 40px",background:"var(--primary)"}}>
      <div className="footer-grid" style={{maxWidth:1200,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,marginBottom:48}}>
        <div>
          <div className="serif" style={{fontSize:22,fontWeight:600,letterSpacing:-0.5,marginBottom:16}}><span style={{color:"#fff"}}>AQ</span> <span style={{color:"var(--accent)"}}>Roofing</span></div>
          <p style={{fontSize:14,lineHeight:1.7,color:"rgba(255,255,255,0.6)",marginBottom:16}}>Family-owned roofing contractor serving Sherwood and Central Arkansas since 2013. GAF, CertainTeed, and TAMKO certified.</p>
          <p style={{fontSize:14,color:"rgba(255,255,255,0.5)"}}>117 Hillwood Dr<br/>Sherwood, AR 72120</p>
          <a href={TEL} style={{display:"inline-block",marginTop:12,fontSize:15,fontWeight:700,color:"var(--accent)"}}>{PHONE}</a>
          <br/>
          <a href="mailto:info@aqroofs.com" style={{display:"inline-block",marginTop:4,fontSize:14,color:"rgba(255,255,255,0.5)"}}>info@aqroofs.com</a>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.1)",paddingTop:24}}>
        <div className="footer-btm-inner" style={{maxWidth:1200,margin:"0 auto",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <p style={{fontSize:13,color:"rgba(255,255,255,0.35)"}}>&copy; {yr} AQ Roofing LLC. All rights reserved. License #327340321</p>
          <p style={{fontSize:13,color:"rgba(255,255,255,0.35)"}}>GAF Certified | CertainTeed | TAMKO</p>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,width:"100%",padding:"18px 24px",background:"var(--accent)",color:"#fff",fontSize:16,fontWeight:700,fontFamily:"'DM Sans',sans-serif",boxShadow:"0 -4px 20px rgba(0,0,0,0.15)"}}>{Icons.phone} Call AQ Roofing Now</a>
    </div>
