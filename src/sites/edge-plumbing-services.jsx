// Layout: A | Industry: Plumber | City: Fayetteville
import { useState, useEffect } from "react";

const PHONE = "(479) 387-5552";
const TEL = "tel:+14793875552";
...
const services = [
  { name: "Plumbing Repair & Installation", desc: "From minor leaks to complete system installations, Edge Plumbing delivers precise, dependable work every time. Jeff Edge personally ensures every repair is done right the first time.", icon: Icons.wrench },
  { name: "Residential Plumbing", desc: "Complete home plumbing services for Fayetteville families. We treat your home with respect and keep everything flowing smoothly — backed by a perfect 5.0-star reputation.", icon: Icons.home },
  { name: "Commercial Plumbing", desc: "Professional plumbing solutions for businesses throughout Northwest Arkansas. Minimal disruption, maximum reliability — we understand that downtime costs you money.", icon: Icons.building },
  { name: "Pipe Repair", desc: "Burst pipes, corroded lines, and hidden leaks diagnosed and repaired with lasting results. We use quality materials and proven techniques to fix it right the first time.", icon: Icons.pipe },
  { name: "Fixture Installation", desc: "Upgrade your kitchen and bathroom with expert fixture installation. Faucets, sinks, toilets, and more — professionally installed so they work perfectly for years.", icon: Icons.fixture },
  { name: "General Plumbing Services", desc: "No job too small, no problem too complex. From routine maintenance to emergency calls, Edge Plumbing covers every plumbing need your home or business has.", icon: Icons.droplet },
];
...
const areas = ["Fayetteville", "West Fork", "Farmington", "Northwest Arkansas"];

const reviews = [
  { text: "Quick install service and professional.", name: "Reynolds Sharp", src: "Google" },
];
...
export default function EdgePlumbingServices() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("edge-plumbing-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("edge-plumbing-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Plumber Fayetteville AR | Edge Plumbing Services";
    const els = [];
    const add = (tag, attrs) => {
    ...
    add("meta", { name: "description", content: "Fayetteville plumber with a perfect 5.0-star Google rating. Residential & commercial plumbing, pipe repair, installations. Call (479) 387-5552." });
    ...
    add("script", {
      ...
      textContent: JSON.stringify({
        ...
        "@type": "Plumber",
        name: "Edge Plumbing Services",
        telephone: "+14793875552",
        address: { "@type": "PostalAddress", streetAddress: "645 N Riding Ridge Ln", addressLocality: "Fayetteville", addressRegion: "AR", postalCode: "72704", addressCountry: "US" },
        ...
        founder: { "@type": "Person", name: "Jeff Edge" },
        description: "Edge Plumbing Services provides expert residential and commercial plumbing in Fayetteville and Northwest Arkansas. Perfect 5.0-star Google rating. Owner Jeff Edge on every job.",
        ...
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#1C2B41" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, marginBottom: 8, fontWeight: 600, letterSpacing: -0.5 }}>
              <span style={{ color: "#1C2B41" }}>Edge</span>{" "}
    ...
    <>
        ...
        <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 48, alignItems: "center" }}>
          <div>
            ...
            <h1 style={{ fontSize: "clamp(40px,5vw,68px)", lineHeight: 1.05, color: "var(--primary)", letterSpacing: -1.5, marginBottom: 20 }}>
              Fayetteville's<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Most Trusted</span><br />Plumber
            </h1>
            <p style={{ fontSize: 18, lineHeight: 1.7, color: "var(--text-mid)", maxWidth: 480, marginBottom: 28 }}>
              Owner Jeff Edge is on every job. Residential and commercial plumbing services across Northwest Arkansas — backed by a perfect 5-star reputation.
            </p>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 32 }}>
              <Stars />
              <span style={{ fontSize: 14, color: "var(--text-light)", fontStyle: "italic", marginLeft: 4 }}>"Quick install service and professional." — Reynolds Sharp</span>
            </div>
      ...
      <section style={{ background: "var(--white)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", padding: "48px clamp(24px,5vw,64px)" }}>
        <div className="trust-row" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 32, textAlign: "center" }}>
          {[
            { num: "5.0", label: "Google Rating", desc: "Perfect score from customers" },
            { num: "Inc.", label: "Incorporated", desc: "Licensed & established business" },
            { num: "Jeff Edge", label: "Owner on Every Job", desc: "Hands-on quality guaranteed" },
      ...
      <section id="services" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "var(--primary)", letterSpacing: -0.5 }}>Plumbing Services in Fayetteville</h2>
          </div>
          ...
          <p style={{ textAlign: "center", marginTop: 40, fontSize: 15, color: "var(--text-mid)" }}>
            Questions? Call Jeff directly at <a href={TEL} style={{ color: "var(--accent)", fontWeight: 700 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--primary)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <div className="gold-line" style={{ margin: "0 auto 20px" }} />
            <div style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: 3, color: "var(--accent)", marginBottom: 14 }}>Why Edge Plumbing</div>
            <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "#fff", letterSpacing: -0.5 }}>The Owner Is on Every Job</h2>
          </div>
          <div className="why-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "start" }}>
            <div>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "rgba(255,255,255,0.82)", marginBottom: 20 }}>
                Edge Plumbing Services isn't a faceless corporation — it's Jeff Edge's name on the truck, and Jeff Edge on every job. When a business is named after its owner, accountability is built in. That's the difference between a company that cuts corners and one that stakes its reputation on every single call.
              </p>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "rgba(255,255,255,0.82)", marginBottom: 32 }}>
                Serving Fayetteville, West Fork, Farmington, and the wider Northwest Arkansas region, Edge Plumbing handles everything from residential repairs to commercial installations. The perfect 5.0-star Google rating speaks for itself — every customer leaves satisfied.
              </p>
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {[
                  "Perfect 5.0-star Google rating",
                  "Owner Jeff Edge personally on every job",
                  "Incorporated and BBB-listed business",
            ...
            <div className="badge-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {[
                { icon: Icons.award, title: "5.0 Stars", desc: "Perfect Google rating from every customer who's reviewed us" },
                { icon: Icons.shield, title: "BBB Listed", desc: "Better Business Bureau listed — trusted and verified" },
                { icon: Icons.users, title: "Owner-Operated", desc: "Jeff Edge personally handles every plumbing job" },
      ...
      {/* REVIEWS -- white, centered single-card (only 1 review) */}
      ...
      <section style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)", borderTop: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "var(--primary)", letterSpacing: -0.5, lineHeight: 1.15, marginBottom: 20 }}>
            Got a plumbing problem?<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Call Jeff Edge.</span>
          </h2>
          <p style={{ fontSize: 17, lineHeight: 1.7, color: "var(--text-mid)", marginBottom: 36, maxWidth: 520, margin: "0 auto 36px" }}>
            Honest pricing. Owner on every job. Serving Fayetteville and Northwest Arkansas with a perfect 5-star reputation.
          </p>
      ...
      <footer style={{ background: "var(--primary)", padding: "80px clamp(24px,5vw,64px) 40px" }}>
        <div className="footer-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: 48, marginBottom: 48 }}>
          <div>
            ...
            <p style={{ fontSize: 14, lineHeight: 1.7, color: "rgba(255,255,255,0.6)", marginBottom: 16, maxWidth: 320 }}>
              Fayetteville's trusted plumber with a perfect 5.0-star Google rating. Owner Jeff Edge on every job — residential and commercial plumbing across Northwest Arkansas.
            </p>
            <p style={{ fontSize: 14, color: "rgba(255,255,255,0.6)", marginBottom: 6 }}>645 N Riding Ridge Ln, Fayetteville, AR 72704</p>
        ...
        <div className="footer-btm" style={{ maxWidth: 1200, margin: "0 auto", borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 24, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>&copy; {yr} Edge Plumbing Services. All rights reserved.</p>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>BBB Listed Business</p>
        </div>
      </footer>
