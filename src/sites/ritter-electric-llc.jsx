// Layout: B | Industry: Electrician | City: Quitman
import { useState, useEffect } from "react";

const PHONE = "(501) 556-7137";
const TEL = "tel:+15015567137";
...
const Icons = {
  ...
  build: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="6" width="20" height="15" rx="2"/><path d="M2 10h20"/><path d="M12 6V2"/><path d="M7 2h10"/></svg>,
  ...
  garage: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 21V8l9-6 9 6v13"/><path d="M5 21V12h14v9"/><path d="M5 15h14"/><path d="M5 18h14"/></svg>,
...
const services = [
  { name: "Residential Electrical", desc: "Complete electrical service for your home. From outlets and switches to full system diagnostics, Tyler handles every residential job with care and precision.", icon: Icons.home },
  { name: "New Construction", desc: "Electrical rough-in and finish work for new builds. Clean, code-compliant wiring from the ground up -- done right the first time so you never worry.", icon: Icons.build },
  { name: "Electrical Remodels", desc: "Upgrading a kitchen, bathroom, or addition? Tyler rewires and reconfigures your electrical to match the new layout safely and to code.", icon: Icons.rewire },
  { name: "Generator Installation", desc: "Professional generator installation to keep your home powered through Arkansas storms. Reliable backup power for the Greers Ferry Lake area.", icon: Icons.generator },
  { name: "Garage Wiring", desc: "Dedicated circuits, lighting, and outlet installation for garages and workshops. Proper wiring for tools, EV chargers, and workspace lighting.", icon: Icons.garage },
  { name: "Service Calls & Repairs", desc: "Fast, honest electrical repairs when something goes wrong. Tyler shows up on time, diagnoses the issue, and gets it fixed -- no runaround.", icon: Icons.wrench },
];

const allServiceNames = ["Residential Electrical","New Construction Electrical","Electrical Remodels","Service Calls & Repairs","Generator Installation","Garage Wiring","Outlet & Switch Work","Electrical Troubleshooting"];

const areas = ["Quitman","Greers Ferry","Heber Springs","Searcy","Cabot","Cleburne County","Central Arkansas"];

const reviews = [
  { text: "Super professional, reliable, and really knows his stuff. He showed up on time, got everything done quickly, and made sure everything was perfect before leaving.", name: "Hailey Long", src: "Google" },
  { text: "Tyler was very professional and quick to take care of our electrical needs. He is also very reasonably priced!", name: "Monica Hardt", src: "Google" },
  { text: "Tyler has done several repairs for me and always does a great job.", name: "Eric Johnson", src: "Google" },
  { text: "Very fast and dependable. He did a great job.", name: "Matthew King", src: "Google" },
  { text: "Easy to contact, great prices, quick turn around, quality work!", name: "Carol Ann Johnson", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function RitterElectricLLC() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("re-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="2026"){sessionStorage.setItem("re-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "Electrician Quitman AR | Ritter Electric LLC";
    const els = [];
    ...
    add("meta",{name:"description",content:"Licensed electrician in Quitman, AR. Residential, new construction, generators & repairs. 5.0 stars, 17 reviews. Call (501) 556-7137."});
    add("meta",{name:"geo.region",content:"US-AR"});
    add("meta",{name:"geo.placename",content:"Quitman"});
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      "@context":"https://schema.org","@type":"Electrician",
      name:"Ritter Electric LLC",telephone:"+15015567137",
      address:{"@type":"PostalAddress",streetAddress:"715 Bettis Mountain Rd",addressLocality:"Quitman",addressRegion:"AR",postalCode:"72131",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:35.3887,longitude:-92.2210},
      ...
      founder:{"@type":"Person",name:"Tyler Ritter"},
      foundingDate:"2025",
      description:"Ritter Electric LLC is an owner-operated electrical contractor in Quitman, AR serving Cleburne County and the Greers Ferry Lake area. Residential electrical, new construction, remodels, generators, and repairs.",
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#2D2D2D"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8,fontWeight:600,letterSpacing:-0.5}}><span style={{color:"#2D2D2D"}}>Ritter</span> <span style={{color:"#D4A03C"}}>Electric</span></div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(45,45,45,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}><span>Ritter</span> <span style={{color:"var(--accent)"}}>Electric</span></div>
    ...
    <section style={{padding:"180px clamp(24px,5vw,64px) 100px",background:"var(--cream)",textAlign:"center"}}>
      <div style={{maxWidth:780,margin:"0 auto"}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(212,160,60,0.1)",padding:"8px 18px",borderRadius:4,marginBottom:28}}>
          ...
          <span style={{fontSize:12,fontWeight:700,color:"var(--accent)",letterSpacing:2,textTransform:"uppercase"}}>Perfect 5.0 Rating -- 17 Five-Star Reviews</span>
        </div>
        <h1 style={{fontSize:"clamp(44px, 6vw, 80px)",lineHeight:1.05,color:"var(--primary)",marginBottom:24,letterSpacing:-2}}>
          Quitman's Trusted<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Electrician</span>
        </h1>
        <p style={{fontSize:18,lineHeight:1.7,color:"var(--text-mid)",marginBottom:32,maxWidth:580,margin:"0 auto 32px"}}>
          Tyler Ritter is the owner and operator behind every job. From new construction wiring to service calls, he treats every home in the Greers Ferry Lake area like his own.
        </p>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:36}}>
          <Stars />
          <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"Super professional, reliable, and really knows his stuff." -- Hailey Long</span>
        </div>
        <div style={{display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap"}}>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(212,160,60,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Tyler Now</a>
      ...
      <div style={{maxWidth:480,margin:"0 auto"}}>
        ...
        <div className="trust-row" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:24,textAlign:"center"}}>
          {[
            ...
            {num:"17+",label:"Reviews",desc:"All five stars"},
            {num:"1",label:"Owner-Operated",desc:"Tyler on every job"},
            {num:"New",label:"Construction",desc:"Ground-up wiring"},
          ].map((s,i)=>(
            <div key={i}>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",color:"var(--primary)",letterSpacing:-0.5}}>Electrical Services in Quitman, AR</h2>
        </div>
        ...
        <p style={{textAlign:"center",marginTop:40,fontSize:15,color:"var(--text-light)"}}>Questions? Call Tyler directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",color:"#fff"}}>
      <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,opacity:0.7}}>About Ritter Electric</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff"}}>Why Quitman Trusts<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Tyler Ritter</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.65)",marginBottom:24,textAlign:"left"}}>
          Tyler Ritter started Ritter Electric in 2025 and in less than six months earned a perfect 5.0 Google rating with 17 reviews -- every single one five stars. That does not happen by accident. It happens because Tyler is a solo operator who treats every home like it matters, shows up when he says he will, and does not leave until the job is done right.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.65)",marginBottom:36,textAlign:"left"}}>
          Serving the rural Greers Ferry Lake and Cleburne County area, Tyler is the electrician locals recommend by name in community Facebook groups. Whether it is wiring a new build, installing a generator, or fixing a tripped breaker at 7 AM, every customer gets the owner -- not a crew, not a subcontractor. Just Tyler, doing the work himself.
        </p>
        <div className="why-creds-row" style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:24,marginTop:8}}>
          {["Perfect 5.0 Google Rating","17 Five-Star Reviews","Owner-Operated","New Construction Specialist","Community Recommended","Cleburne County Local"].map((c,i)=>(
    ...
    <section id="areas" style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        ...
        <h2 style={{fontSize:"clamp(28px,3vw,38px)",color:"var(--primary)",marginBottom:48,letterSpacing:-0.5}}>Serving Quitman & the Greers Ferry Lake Area</h2>
    ...
    {/* CTA -- Layout B: White with top border */}
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>Need an electrician?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Tyler.</span></h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>Perfect 5.0 rating. 17 five-star reviews. Owner-operated -- Tyler on every job.</p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16,fontWeight:600,letterSpacing:-0.5}}><span>Ritter</span> <span style={{color:"var(--accent)"}}>Electric</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>Owner-operated electrical contractor serving Quitman, Cleburne County, and the Greers Ferry Lake area. Residential electrical, new construction, remodels, generators, and repairs.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>715 Bettis Mountain Rd<br/>Quitman, AR 72131<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
          <p style={{fontSize:13,opacity:0.5,marginTop:10}}>Mon-Fri 7AM-4PM</p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} Ritter Electric LLC. All rights reserved.</span>
          <span>Owner-Operated &middot; Est. 2025</span>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Ritter Electric</a>
    </div>
