...
const Icons = {
  ...
  tank: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="6" width="18" height="12" rx="3"/><path d="M8 6V4"/><path d="M16 6V4"/><path d="M12 18v2"/><path d="M7 11h10"/><path d="M7 14h10"/></svg>
  ),
  wrench: (
  ...
  building: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="2" width="16" height="20" rx="1"/><path d="M9 22V12h6v10"/><path d="M8 6h.01"/><path d="M16 6h.01"/><path d="M8 10h.01"/><path d="M16 10h.01"/><path d="M8 14h.01"/><path d="M16 14h.01"/></svg>
  ),
  shield: (
...
// --- Data ---
const PHONE = "(479) 646-5200";
const PHONE_TEL = "tel:+14796465200";

const services = [
  { name: "Sewer Line Cleaning", desc: "Professional sewer line cleaning to restore flow and prevent costly backups. Our team uses proven techniques to clear even the toughest blockages.", icon: Icons.pipe },
  { name: "Drain Cleaning", desc: "Fast, effective drain cleaning for kitchens, bathrooms, and floor drains. We get your water moving again without damage to your pipes.", icon: Icons.droplets },
  { name: "Septic Tank Service", desc: "Complete septic tank pumping, inspection, and maintenance to keep your system running smoothly and protect your property.", icon: Icons.tank },
  { name: "General Plumbing Repair", desc: "Dependable repair for leaks, broken fixtures, and everything in between. No job is too small for our experienced crew.", icon: Icons.wrench },
  { name: "Residential Plumbing", desc: "Full-service residential plumbing for Fort Smith homeowners — from routine maintenance to urgent repairs, done right the first time.", icon: Icons.home },
  { name: "Commercial Plumbing", desc: "Reliable commercial plumbing solutions for businesses throughout the River Valley. We minimize downtime and keep your operations flowing.", icon: Icons.building },
];

const reviews = [
  { text: "Would absolutely use this company again. Tried to get someone out for 4 day's with no luck. Called Tuesday morning the day after memorial day and they were here within the hour. Very grateful for immediate response & knowledge on septic tank.", name: "Tallie Dale", src: "Google" },
  { text: "Came out on a Sunday and got the job done at a reasonable cost. I'd recommend them to anyone!", name: "Preston James", src: "Google" },
];

const areas = ["Fort Smith", "Van Buren", "Alma", "Greenwood", "Barling", "River Valley", "Western Arkansas"];

// --- Main Component ---
export default function ProRooterProfessionalSewer() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("prps-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("prps-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Sewer & Drain Cleaning Fort Smith AR | Pro-Rooter";
    const els = [];
    ...
    add("meta", { name: "description", content: "Fort Smith sewer & drain cleaning. 5.0 stars on Google. Same day service, weekends too. Call (479) 646-5200 for a free quote." });
    ...
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      ...
      name: "Pro-Rooter Professional Sewer", telephone: "+14796465200",
      address: { "@type": "PostalAddress", streetAddress: "1212 Armistead Rd", addressLocality: "Fort Smith", addressRegion: "AR", postalCode: "72916", addressCountry: "US" },
      geo: { "@type": "GeoCoordinates", latitude: 35.3386, longitude: -94.3936 },
      openingHoursSpecification: [{ "@type": "OpeningHoursSpecification", dayOfWeek: ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"], opens: "00:00", closes: "23:59" }],
      aggregateRating: { "@type": "AggregateRating", ratingValue: "5.0", reviewCount: "5", bestRating: "5" },
      description: "Pro-Rooter Professional Sewer provides expert sewer line cleaning, drain cleaning, and septic tank service in Fort Smith and the River Valley.",
      areaServed: areas.map((a) => ({ "@type": "City", name: a })),
      hasOfferCatalog: { "@type": "OfferCatalog", name: "Sewer & Plumbing Services", itemListElement: services.map((s) => ({ "@type": "Offer", itemOffered: { "@type": "Service", name: s.name } })) },
      memberOf: [],
      sameAs: [],
    })});
        ...
        <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#1C2B41" }}>
          <div style={{ background:"#fff", borderRadius:16, padding:"56px 44px", textAlign:"center", maxWidth:420, width:"92%", boxShadow:"0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:28, color:"#1C2B41", marginBottom:32 }}>
              <span style={{ fontWeight:700 }}>Pro-Rooter</span> <span style={{ color:"#C8934F" }}>Professional Sewer</span>
            </div>
    ...
    <>
      ...
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:1000, padding:scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display:"flex", alignItems:"center", justifyContent:"space-between", background: scrolled ? "rgba(28,43,65,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition:"all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        <div className="serif" style={{ fontSize:22, color: scrolled ? "#fff" : "var(--primary)", fontWeight:600, letterSpacing:-0.5 }}>
          <span>Pro-Rooter</span><span style={{ color:"var(--accent)", marginLeft:6 }}>Professional Sewer</span>
        </div>
        ...
        <div className="hero-grid" style={{ maxWidth:1200, width:"100%", display:"grid", gridTemplateColumns:"1.2fr 0.8fr", gap:64, alignItems:"center" }}>
          <div style={{ maxWidth:560 }}>
            <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(200,147,79,0.1)", padding:"8px 18px", borderRadius:4, marginBottom:28 }}>
              ...
              <span style={{ fontSize:12, fontWeight:700, color:"var(--accent)", letterSpacing:2, textTransform:"uppercase" }}>5.0 Stars on Google</span>
            </div>
            <h1 style={{ fontSize:"clamp(40px, 5vw, 68px)", lineHeight:1.05, color:"var(--primary)", marginBottom:24, letterSpacing:-1.5 }}>
              Fort Smith's<br /><span style={{ fontStyle:"italic", color:"var(--accent)" }}>Sewer & Drain</span><br />Experts
            </h1>
            <p style={{ fontSize:18, lineHeight:1.7, color:"var(--text-mid)", marginBottom:32, maxWidth:440 }}>
              When you need it done right and done fast, our team delivers professional sewer and drain service with same-day availability — even on weekends.
            </p>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:36 }}>
              <Stars />
              <span style={{ fontSize:14, color:"var(--text-light)", fontStyle:"italic", marginLeft:4 }}>"I'd recommend them to anyone!" — Preston James</span>
            </div>
      ...
      <section style={{ padding:"72px clamp(24px,5vw,64px)", background:"#fff", borderTop:"1px solid var(--border)", borderBottom:"1px solid var(--border)" }}>
        <div className="trust-grid" style={{ maxWidth:1000, margin:"0 auto", display:"grid", gridTemplateColumns:"repeat(4, 1fr)", gap:32, textAlign:"center" }}>
          {[
            ...
            { num: "Same Day", label: "Service Available", sub: "Fast response times" },
            { num: "Weekend", label: "Calls Welcome", sub: "We're here for you" },
      ...
      <section id="services" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          <div style={{ textAlign:"center", marginBottom:64 }}>
            ...
            <h2 style={{ fontSize:"clamp(30px, 3.5vw, 44px)", color:"var(--primary)", letterSpacing:-0.5 }}>Sewer & Plumbing Services in Fort Smith</h2>
          </div>
      ...
      <section id="about" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--primary)", color:"#fff", position:"relative", overflow:"hidden" }}>
        ...
        <div className="why-grid" style={{ maxWidth:1100, margin:"0 auto", display:"grid", gridTemplateColumns:"1.1fr 0.9fr", gap:80, alignItems:"start", position:"relative" }}>
          <div>
            ...
            <div style={{ fontSize:12, fontWeight:700, textTransform:"uppercase", letterSpacing:3, color:"var(--accent)", marginBottom:14, opacity:0.7 }}>About Pro-Rooter</div>
            <h2 style={{ fontSize:"clamp(30px, 3.5vw, 42px)", lineHeight:1.15, marginBottom:28, color:"#fff" }}>The Team<br/><span style={{ fontStyle:"italic", color:"var(--accent)" }}>Fort Smith Calls</span></h2>
            <p style={{ fontSize:16, lineHeight:1.8, color:"rgba(255,255,255,0.65)", marginBottom:24 }}>
              Pro-Rooter Professional Sewer is the name Fort Smith turns to when sewer and drain problems strike. Our team specializes in the work other plumbers avoid — sewer line blockages, stubborn drains, and septic systems that need expert attention. We show up fast, diagnose accurately, and fix it right the first time.
            </p>
            <p style={{ fontSize:16, lineHeight:1.8, color:"rgba(255,255,255,0.65)", marginBottom:36 }}>
              With a perfect 5.0-star rating on Google and a reputation for same-day service — including weekends and holidays — our team is committed to being there when you need us most. Fair pricing, honest communication, and professional results every time.
            </p>
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              {["Perfect 5.0-Star Google Rating", "Same-Day Service Available", "Weekend & Holiday Availability", "Licensed & Insured", "Fair, Upfront Pricing"].map((c, i) => (
          ...
          <div className="badge-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, paddingTop:20 }}>
            {[
              { icon: Icons.shield, title: "5.0 Stars", desc: "Perfect Google rating" },
              { icon: Icons.clock, title: "Same Day", desc: "Rapid response times" },
              { icon: Icons.calendar, title: "Weekends", desc: "Always available" },
      ...
      <section id="areas" style={{ padding:"100px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:900, margin:"0 auto", textAlign:"center" }}>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", justifyContent:"center", gap:12 }}>
            {areas.map((a, i) => (
              <span key={i} title={`Sewer & drain cleaning in ${a}, AR`} style={{ display:"inline-flex", alignItems:"center", gap:6, background:"#fff", border:"1px solid var(--border)", padding:"12px 24px", borderRadius:30, fontSize:15, fontWeight:500, color:"var(--text)", cursor:"default", transition:"all 0.25s" }}
      ...
      <section style={{ padding:"100px clamp(24px,5vw,64px)", background:"#fff", textAlign:"center", borderTop:"1px solid var(--border)" }}>
        <div style={{ maxWidth:600, margin:"0 auto" }}>
          <h2 className="serif" style={{ fontSize:"clamp(32px, 4vw, 50px)", color:"var(--primary)", lineHeight:1.1, marginBottom:20, letterSpacing:-1 }}>
            Got a sewer problem?<br/><span style={{ fontStyle:"italic", color:"var(--accent)" }}>Call our team.</span>
          </h2>
          <p style={{ fontSize:17, color:"var(--text-mid)", marginBottom:36, lineHeight:1.6 }}>
            Fair pricing. Same-day service. Perfect 5-star reputation.
          </p>
      ...
      <footer style={{ background:"var(--primary)", color:"rgba(255,255,255,0.7)", padding:"64px clamp(24px,5vw,64px) 0" }}>
        <div className="footer-grid" style={{ maxWidth:1100, margin:"0 auto", display:"grid", gridTemplateColumns:"1.5fr 1fr 1fr", gap:48, paddingBottom:40 }}>
          <div>
            <div className="serif" style={{ fontSize:22, color:"#fff", marginBottom:16 }}>Pro-Rooter <span style={{ color:"var(--accent)" }}>Professional Sewer</span></div>
            <p style={{ fontSize:14, lineHeight:1.7, opacity:0.7, marginBottom:16 }}>Fort Smith's trusted sewer and drain cleaning professionals. 5.0 stars on Google. Same-day service, weekends included.</p>
            <p style={{ fontSize:14, lineHeight:1.8, opacity:0.6 }}>
              1212 Armistead Rd<br/>Fort Smith, AR 72916<br/>
        ...
        <div style={{ borderTop:"1px solid rgba(255,255,255,0.06)", padding:"20px 0", maxWidth:1100, margin:"0 auto" }}>
          <div className="footer-btm-inner" style={{ display:"flex", justifyContent:"space-between", fontSize:13, opacity:0.4 }}>
            <span>&copy; {new Date().getFullYear()} Pro-Rooter Professional Sewer. All rights reserved.</span>
            <span>5.0 Stars &middot; Call Anytime</span>
          </div>
        </div>
      </footer>
      ...
      <div className="mobile-only" style={{ position:"fixed", bottom:0, left:0, right:0, zIndex:998, display:"flex" }}>
        <a href={PHONE_TEL} style={{ display:"flex", width:"100%", alignItems:"center", justifyContent:"center", gap:10, background:"var(--accent)", color:"#fff", padding:"18px", fontSize:17, fontWeight:700, boxShadow:"0 -4px 20px rgba(0,0,0,0.15)", fontFamily:"'DM Sans',sans-serif" }}>
          {Icons.phone} Call Pro-Rooter Now
        </a>
      </div>
