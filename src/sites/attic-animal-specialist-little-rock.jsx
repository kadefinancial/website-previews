// Layout: D | Industry: Pest Control | City: Little Rock
import { useState, useEffect, useRef } from "react";

const PHONE = "(501) 514-9874";
const TEL = "tel:+15015149874";

const Icons = {
  phone: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>,
  // Wildlife/animal icon
  animal: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M8 14v.5"/><path d="M16 14v.5"/><path d="M11.25 16.25h1.5L12 17l-.75-.75z"/><path d="M4.42 11.247A13.152 13.152 0 004 14.556C4 18.728 7.582 21 12 21s8-2.272 8-6.444a13.152 13.152 0 00-.42-3.31"/><path d="M8.5 2c1.727 1.091 2.5 3 2.5 5"/><path d="M15.5 2c-1.727 1.091-2.5 3-2.5 5"/><path d="M20 8c.5-2.5-.5-4-2-5"/><path d="M4 8c-.5-2.5.5-4 2-5"/></svg>,
  // Trap/cage icon
  trap: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="8" width="18" height="12" rx="2"/><path d="M7 8V6a5 5 0 0110 0v2"/><path d="M12 12v4"/><circle cx="12" cy="12" r="1"/></svg>,
  // Broom/cleanup icon
  broom: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v8"/><path d="M4.93 10.93l2.83 2.83"/><path d="M19.07 10.93l-2.83 2.83"/><path d="M7 14h10"/><path d="M5 18c0-2 2-4 7-4s7 2 7 4"/><path d="M8 22v-4"/><path d="M12 22v-4"/><path d="M16 22v-4"/></svg>,
  // Skull/dead animal icon
  skull: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="10" r="8"/><path d="M12 18v4"/><path d="M8 22h8"/><circle cx="9" cy="10" r="1.5"/><circle cx="15" cy="10" r="1.5"/><path d="M10 15h4"/><path d="M11 15v2"/><path d="M13 15v2"/></svg>,
  // Snake icon
  snake: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 20c0-4 4-4 4-8s-2-6 2-8c4 2 2 4 2 8s4 4 4 8"/><path d="M12 4l2-2"/><path d="M12 4l-2-2"/><circle cx="10" cy="5" r="0.5" fill="currentColor"/></svg>,
  // Hammer/repair icon
  hammer: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M15 12l-8.5 8.5a2.12 2.12 0 01-3-3L12 9"/><path d="M17.64 15L22 10.64"/><path d="M20.91 11.7l-1.25-1.25a.76.76 0 00-.54-.22h0a3 3 0 01-2.83-2l-.17-.51a3 3 0 00-2.83-2H13a2 2 0 00-1.41.59L9 9"/></svg>,
...
const services = [
  { name: "Wildlife Removal & Trapping", desc: "Professional removal of raccoons, squirrels, opossums, and other nuisance wildlife from your home. David uses humane, effective methods refined over 10+ years in Central Arkansas.", icon: Icons.animal },
  { name: "Humane Animal Exclusion", desc: "Seal every entry point so animals can't return. David identifies how wildlife gets in and installs professional-grade exclusion barriers that last.", icon: Icons.trap },
  { name: "Attic Cleanup & Sanitation", desc: "Complete attic restoration after animal infestation. Removal of contaminated insulation, droppings, and debris followed by thorough sanitization.", icon: Icons.broom },
  { name: "Dead Animal Removal", desc: "Fast response for dead animal removal from walls, crawlspaces, attics, and hard-to-reach areas. Eliminates odor and health hazards.", icon: Icons.skull },
  { name: "Snake Identification & Control", desc: "Expert snake ID from photos or on-site visits. David can identify any Arkansas species and determine if it's venomous — giving you peace of mind fast.", icon: Icons.snake },
  { name: "Wildlife Damage Repair", desc: "Repair chewed wiring, damaged insulation, torn ductwork, and compromised rooflines caused by wildlife. Restore your home to pre-infestation condition.", icon: Icons.hammer },
];

const allServiceNames = ["Wildlife Removal & Trapping","Humane Animal Exclusion","Attic Cleanup & Sanitation","Dead Animal Removal","Snake Identification & Control","Wildlife Damage Repair"];

const areas = ["Little Rock","North Little Rock","Jacksonville","Maumelle","Sherwood","Cammack Village","Wrightsville","Roland","Central Arkansas"];

const reviews = [
  { text: "David was very helpful in identifying a snake skin that we found in our shed. Thanks David for your awesome help.", name: "Betty Graham", src: "Google" },
  { text: "I sent David a photo of a snake skin (found in one of my cars) for ID. He responded quickly with an ID. Peace of mind that it was not venomous, just a good old rat snake. Thanks David!", name: "Richard Rew", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function AtticAnimalSpecialistLittleRock() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("attic-animal-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="roof"){sessionStorage.setItem("attic-animal-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "Wildlife Removal Little Rock AR | Attic Animal Specialist";
    const els = [];
    ...
    add("meta",{name:"description",content:"Wildlife removal & attic animal control in Little Rock, AR. 5.0 stars, 10+ years experience. Call (501) 514-9874."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"Attic Animal Specialist",telephone:"+15015149874",
      email:"info@Littlerockanimalattic.com",
      address:{"@type":"PostalAddress",streetAddress:"11701 I-30",addressLocality:"Little Rock",addressRegion:"AR",postalCode:"72209",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:34.7465,longitude:-92.2896},
      openingHoursSpecification:[
        {"@type":"OpeningHoursSpecification",dayOfWeek:["Tuesday","Wednesday","Thursday","Friday"],opens:"07:00",closes:"20:00"},
      ...
      description:"Attic Animal Specialist provides humane wildlife removal, attic cleanup, snake identification, and animal exclusion services in Little Rock, AR and surrounding Central Arkansas communities. 10+ years experience. Available 24/7/365.",
      areaServed:areas.map(a=>({"@type":"City",name:a})),
      hasOfferCatalog:{"@type":"OfferCatalog",name:"Wildlife Removal Services",itemListElement:allServiceNames.map(s=>({"@type":"Offer",itemOffered:{"@type":"Service",name:s}}))},
      memberOf:[],sameAs:[],
    })});
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1E3A2F"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8}}><span style={{fontWeight:700,color:"#1E3A2F"}}>Attic Animal</span> <span style={{color:"#B8694A"}}>Specialist</span></div>
  ...
  return (<>
    <style>{`
      ...
      :root{--primary:#1E3A2F;--accent:#B8694A;--cream:#FAF9F6;--white:#FFFFFF;--text:#2A2A3C;--text-mid:#5A5A6E;--text-light:#8A8A9A;--border:rgba(0,0,0,0.06)}
    ...
    {/* NAV */}
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(30,58,47,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}><span>Attic Animal</span> <span style={{color:"var(--accent)"}}>Specialist</span></div>
      ...
      <div className="hero-grid" style={{maxWidth:1200,width:"100%",display:"grid",gridTemplateColumns:"1.8fr 1fr",gap:48,alignItems:"center"}}>
        <div>
          <h1 style={{fontSize:"clamp(48px, 6vw, 84px)",lineHeight:1.0,color:"var(--primary)",letterSpacing:-2,marginBottom:24}}>
            Little Rock's<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Wildlife Removal</span><br/>Expert
          </h1>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:32}}>
            <Stars />
            <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"David was very helpful in identifying a snake skin." — Betty Graham</span>
          </div>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(184,105,74,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call David — {PHONE}</a>
        </div>
        <div>
          <div style={{background:"#fff",borderRadius:12,padding:"32px 28px",boxShadow:"0 4px 40px rgba(0,0,0,0.08)",border:"1px solid var(--border)"}}>
            {!formDone ? (
              ...
              <div style={{textAlign:"center",padding:"20px 0"}}>
                <div style={{width:48,height:48,borderRadius:"50%",background:"rgba(184,105,74,0.12)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 14px",color:"var(--accent)"}}>{Icons.check}</div>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",color:"var(--primary)",letterSpacing:-0.5}}>Wildlife Removal Services in Little Rock</h2>
        </div>
        {/* Mobile trust */}
        <div className="trust-mobile" style={{display:"none",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:48,textAlign:"center"}}>
          {[{num:"5.0",label:"Google Rating",desc:"Perfect score"},{num:"24+",label:"5-Star Reviews",desc:"Verified on Google"},{num:"10+",label:"Years Experience",desc:"Wildlife specialist"},{num:"24/7",label:"Always Available",desc:"365 days a year"}].map((s,i)=>(
            <div key={i}>
        ...
        <p style={{textAlign:"center",marginTop:48,fontSize:15,color:"var(--text-light)"}}>Questions? Call David directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",position:"relative",overflow:"hidden"}}>
      <div className="serif" style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",fontSize:"clamp(120px,20vw,280px)",color:"var(--accent)",opacity:0.08,lineHeight:1,whiteSpace:"nowrap",pointerEvents:"none"}}>5.0</div>
      <div style={{maxWidth:800,margin:"0 auto",position:"relative",zIndex:1}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,textAlign:"center",opacity:0.7}}>About Attic Animal Specialist</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff",textAlign:"center"}}>10+ Years Protecting<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Little Rock Homes</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:24,textAlign:"center"}}>
          When you hear something scratching in your attic at 2 AM, David is the person Little Rock calls. As the owner and operator of Attic Animal Specialist, David has spent over a decade handling every kind of wildlife situation Central Arkansas can throw at him — from raccoon families nesting in attics to snakes turning up in sheds and cars. Every one of his 24 Google reviews is five stars, and that's not an accident.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:36,textAlign:"center"}}>
          What sets David apart is his hands-on expertise and genuine willingness to help. Customers regularly send him photos of snake skins for identification, and he responds quickly with a definitive answer — often providing peace of mind that it's a harmless species. His approach is always humane, always thorough, and always available. Day, night, weekends, holidays — David picks up the phone because wildlife doesn't wait for business hours.
        </p>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:20}}>
          {["Perfect 5.0 Google Rating","Humane Removal Methods","10+ Years Experience","Available 24/7/365","Expert Snake Identification","Owner-Operated"].map((c,i)=>(
    ...
    <section id="reviews" style={{padding:"110px 0",background:"var(--white)"}}>
      ...
      <div style={{position:"relative"}}>
        <div ref={revRef} className="scroll-strip" style={{display:"flex",gap:20,paddingLeft:"clamp(24px,5vw,64px)",paddingRight:"clamp(24px,5vw,64px)",paddingBottom:8}}>
          {reviews.map((r,i)=>(
            <div key={i} style={{flex:"0 0 420px",scrollSnapAlign:"start",background:"var(--cream)",borderRadius:12,padding:"36px 30px",border:"1px solid var(--border)",display:"flex",flexDirection:"column"}}>
    ...
    <section id="areas" style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:12}}>
          {areas.map((a,i)=>(
            <span key={i} title={`Wildlife Removal in ${a}, AR`} style={{display:"inline-flex",alignItems:"center",gap:6,background:"#fff",border:"1px solid var(--border)",padding:"12px 24px",borderRadius:30,fontSize:15,fontWeight:500,color:"var(--text)",cursor:"default",transition:"all 0.25s"}}
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>Got critters?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call David.</span></h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>Humane wildlife removal. 10+ years experience. Available 24/7/365.</p>
        <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"22px 52px",borderRadius:8,fontSize:20,fontWeight:700,boxShadow:"0 6px 28px rgba(184,105,74,0.35)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} {PHONE}</a>
      </div>
    </section>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16}}><span>Attic Animal</span> <span style={{color:"var(--accent)"}}>Specialist</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>Professional wildlife removal, attic cleanup, and animal exclusion for homes across Central Arkansas. Humane methods. 10+ years experience. Available 24/7.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>11701 I-30<br/>Little Rock, AR 72209<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a><br/><a href="mailto:info@Littlerockanimalattic.com" style={{color:"var(--accent)",fontWeight:600}}>info@Littlerockanimalattic.com</a></p>
          <p style={{fontSize:13,opacity:0.5,marginTop:10}}>Tue-Fri 7 AM - 8 PM | Sat 8 AM - 7 PM | Sun 9 AM - 7 PM | Mon 24 hrs</p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} Attic Animal Specialist. All rights reserved.</span>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Attic Animal Specialist</a>
    </div>
