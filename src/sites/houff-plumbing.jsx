...
const Icons = {
  ...
  toilet: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M7 12h10a5 5 0 01-1.5 8H8.5A5 5 0 017 12z"/><path d="M7 12V4h3v3"/><path d="M17 12V8h-3"/><path d="M10 20v2h4v-2"/></svg>
  ),
  showerHead: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h4v4H4z"/><path d="M6 8v4"/><path d="M6 12a6 6 0 006 6"/><path d="M15 2v4"/><path d="M13 2v4"/><path d="M17 2v4"/><path d="M19 2v4"/><path d="M15 6h4a2 2 0 012 2v1a2 2 0 01-2 2h-4"/></svg>
  ),
  faucet: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v6"/><path d="M6 8h12"/><path d="M6 8a4 4 0 00-4 4v2h20v-2a4 4 0 00-4-4"/><path d="M10 16v4a2 2 0 004 0v-4"/></svg>
  ),
  settings: (
  ...
  store: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l1-4h16l1 4"/><path d="M3 9h18v12H3z"/><path d="M9 21V14h6v7"/><path d="M3 9a3 3 0 003 3 3 3 0 003-3"/><path d="M9 9a3 3 0 003 3 3 3 0 003-3"/><path d="M15 9a3 3 0 003 3 3 3 0 003-3"/></svg>
  ),
};
...
// --- Data ---
const PHONE = "(501) 225-2411";
const PHONE_TEL = "tel:+15012252411";

const services = [
  { name: "Plumbing Repair & Service", desc: "Complete repair and service for residential plumbing systems. From small fixes to major overhauls.", icon: Icons.wrench },
  { name: "Toilet Repair & Replacement", desc: "Expert diagnosis and repair of running, leaking, or clogged toilets. Full replacement when needed.", icon: Icons.toilet },
  { name: "Plumbing Fixtures & Supply", desc: "Visit our showroom for quality fixtures, fittings, and plumbing supplies for any project.", icon: Icons.store },
  { name: "Bathroom Plumbing", desc: "Full-service bathroom plumbing including showers, tubs, sinks, and complete remodels.", icon: Icons.showerHead },
  { name: "Outdoor Faucet Repair", desc: "Repair and replacement of outdoor spigots, hose bibs, and frost-proof faucets.", icon: Icons.faucet },
  { name: "General Maintenance", desc: "Routine plumbing inspections and preventive maintenance to keep your system running smoothly.", icon: Icons.settings },
];

const allServices = [
  "Plumbing Repair & Service",
  "Toilet/Commode Repair & Replacement",
...
const reviews = [
  { text: "The Plumbers got here right away in the morning and on time. They did a great job of repairing the commode with every minute counting. They were courteous, hard working and left everything in the master bathroom better than they found it.", name: "Abrunoi Brunson", src: "Google" },
  { text: "Breakneck fast! I called them during lunch, they were out in an hour and left before I got off work. Problem fixed and wasn't expensive.", name: "Matthew Littleton", src: "Google" },
  { text: "Great company. They have always done a fantastic job. We have used them for several years. Very dependable", name: "Jackie Perkins", src: "Google" },
  { text: "What's not to like? Great customer service, very knowledgeable, very patient with questions, very fast!", name: "Joe Vaughn", src: "Google" },
  { text: "They couldn't get to my job the same day, but instead of just saying that they couldn't and letting me run around, they instantly recommended someone that they trusted who could.", name: "Timothy Hose", src: "Google" },
];

const areas = ["Little Rock", "West Little Rock", "Maumelle", "North Little Rock", "Benton", "Bryant", "Sherwood", "Jacksonville"];

// --- Main Component ---
export default function HouffPlumbing() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("houff-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "2026") { sessionStorage.setItem("houff-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Plumber Little Rock AR | Houff Plumbing";
    const els = [];
    ...
    add("meta", { name: "description", content: "Plumbing repair, fixtures & supply in Little Rock, AR. Showroom on Kanis Rd. Call (501) 225-2411 for service." });
    ...
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      ...
      name: "Don M Houff Plumbing Co", telephone: "+15012252411",
      address: { "@type": "PostalAddress", streetAddress: "11802 Kanis Rd", addressLocality: "Little Rock", addressRegion: "AR", postalCode: "72211", addressCountry: "US" },
      geo: { "@type": "GeoCoordinates", latitude: 34.7165, longitude: -92.3896 },
      openingHoursSpecification: [{ "@type": "OpeningHoursSpecification", dayOfWeek: ["Monday","Tuesday","Wednesday","Thursday","Friday"], opens: "07:30", closes: "16:30" }],
      founder: { "@type": "Person", name: "Don Houff Jr." },
      description: "Don M Houff Plumbing Co is a family plumbing company with a physical showroom in Little Rock, AR, offering plumbing repair and fixtures supply.",
      ...
      <>
        ...
        <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#1C2B41" }}>
          <div style={{ background:"#fff", borderRadius:16, padding:"56px 44px", textAlign:"center", maxWidth:420, width:"92%", boxShadow:"0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:28, fontWeight:600, color:"#1C2B41", marginBottom:8, letterSpacing:-0.5 }}>
              <span>Houff</span> <span style={{ color:"#C8934F" }}>Plumbing</span>
            </div>
            ...
            <form onSubmit={submitPw}>
              ...
              <button type="submit" style={{ width:"100%", padding:16, marginTop:20, fontSize:15, fontWeight:700, letterSpacing:.3, background:"#1C2B41", color:"#fff", border:"none", borderRadius:10, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>View Site</button>
            </form>
          </div>
        </div>
      </>
    ...
    <>
      ...
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:1000, padding:scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display:"flex", alignItems:"center", justifyContent:"space-between", background: scrolled ? "rgba(28,43,65,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition:"all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        <div className="serif" style={{ fontSize:22, color: scrolled ? "#fff" : "var(--primary)", fontWeight:600, letterSpacing:-0.5 }}>
          <span>Houff</span><span style={{ color:"var(--accent)", marginLeft:6 }}>Plumbing</span>
        </div>
      ...
      <section style={{ minHeight:"100vh", padding:"160px clamp(24px,5vw,64px) 100px", background:"var(--cream)", display:"flex", alignItems:"center", justifyContent:"center" }}>
        <div className="hero-grid" style={{ maxWidth:1200, width:"100%", display:"grid", gridTemplateColumns:"55% 45%", gap:64, alignItems:"center" }}>
          <div style={{ maxWidth:560 }}>
            <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(200,147,79,0.1)", padding:"8px 18px", borderRadius:4, marginBottom:28 }}>
              ...
              <span style={{ fontSize:12, fontWeight:700, color:"var(--accent)", letterSpacing:2, textTransform:"uppercase" }}>Showroom on Kanis Rd</span>
            </div>
            <h1 style={{ fontSize:"clamp(40px, 5vw, 68px)", lineHeight:1.05, color:"var(--primary)", marginBottom:24, letterSpacing:-1.5 }}>
              Little Rock's<br /><span style={{ fontStyle:"italic", color:"var(--accent)" }}>Reliable</span><br />Plumber
            </h1>
            <p style={{ fontSize:18, lineHeight:1.7, color:"var(--text-mid)", marginBottom:32, maxWidth:440 }}>
              Don Houff Jr. and the Houff Plumbing team deliver dependable plumbing repair and supply to Central Arkansas. Visit our showroom or call for fast service.
            </p>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:36 }}>
              <Stars />
              <span style={{ fontSize:14, color:"var(--text-light)", fontStyle:"italic", marginLeft:4 }}>"Breakneck fast! Problem fixed and wasn't expensive." — Matthew Littleton</span>
            </div>
            <div style={{ display:"flex", gap:14, flexWrap:"wrap" }}>
              <a href={PHONE_TEL} style={{ display:"inline-flex", alignItems:"center", gap:10, background:"var(--accent)", color:"#fff", padding:"18px 36px", borderRadius:6, fontSize:16, fontWeight:700, boxShadow:"0 4px 20px rgba(200,147,79,0.3)", fontFamily:"'DM Sans',sans-serif" }}>
          ...
          <div>
            <div style={{ background:"#fff", borderRadius:12, padding:"44px 36px", boxShadow:"0 4px 40px rgba(28,43,65,0.08)", border:"1px solid var(--border)" }}>
              {!formSubmitted ? (
      ...
      <section style={{ padding:"72px clamp(24px,5vw,64px)", background:"#fff", borderTop:"1px solid var(--border)", borderBottom:"1px solid var(--border)" }}>
        <div className="trust-grid" style={{ maxWidth:1000, margin:"0 auto", display:"grid", gridTemplateColumns:"repeat(4, 1fr)", gap:32, textAlign:"center" }}>
          {[
            { num: "12+", label: "Reviews", sub: "Trusted by locals" },
            { num: "BBB", label: "Listed", sub: "Verified business" },
            { num: "Yes", label: "Showroom", sub: "On Kanis Rd" },
            { num: "Fast", label: "Response", sub: "Quick turnaround" },
          ].map((s, i) => (
            <div key={i}>
      ...
      <section id="services" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          <div style={{ textAlign:"center", marginBottom:64 }}>
            ...
            <h2 style={{ fontSize:"clamp(30px, 3.5vw, 44px)", color:"var(--primary)", letterSpacing:-0.5 }}>Plumbing Services in Little Rock</h2>
          </div>
          <div className="svc-grid" style={{ display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:20 }}>
            {services.map((s, i) => (
              <div key={i} style={{ background:"#fff", borderRadius:10, padding:"36px 30px", border:"1px solid var(--border)", transition:"all 0.3s ease", cursor:"default", position:"relative", overflow:"hidden" }}
                onMouseEnter={e => { e.currentTarget.style.transform="translateY(-4px)"; e.currentTarget.style.boxShadow="0 12px 40px rgba(28,43,65,0.1)"; e.currentTarget.querySelector('.card-bar').style.width="100%"; }}
          ...
          <p style={{ textAlign:"center", marginTop:40, fontSize:15, color:"var(--text-light)" }}>
            Questions? Call Don directly at <a href={PHONE_TEL} style={{ color:"var(--accent)", fontWeight:600 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--primary)", color:"#fff", position:"relative", overflow:"hidden" }}>
        ...
        <div className="why-grid" style={{ maxWidth:1100, margin:"0 auto", display:"grid", gridTemplateColumns:"1.1fr 0.9fr", gap:80, alignItems:"start", position:"relative" }}>
          <div>
            ...
            <div style={{ fontSize:12, fontWeight:700, textTransform:"uppercase", letterSpacing:3, color:"var(--accent)", marginBottom:14, opacity:0.7 }}>About Houff Plumbing</div>
            <h2 style={{ fontSize:"clamp(30px, 3.5vw, 42px)", lineHeight:1.15, marginBottom:28, color:"#fff" }}>Why Little Rock Trusts<br/><span style={{ fontStyle:"italic", color:"var(--accent)" }}>Houff Plumbing</span></h2>
            <p style={{ fontSize:16, lineHeight:1.8, color:"rgba(255,255,255,0.65)", marginBottom:24 }}>
              Houff Plumbing is a family plumbing company with deep roots on Kanis Road. Under the leadership of Don Houff Jr., the team handles both service plumbing and fixture supply — which means you get the parts and the labor from the same trusted source.
            </p>
            <p style={{ fontSize:16, lineHeight:1.8, color:"rgba(255,255,255,0.65)", marginBottom:36 }}>
              With a physical showroom where you can browse fixtures and supplies in person, Houff Plumbing is more than a service call. Customers consistently praise the fast response times and fair, upfront pricing that have defined the company for years.
            </p>
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              {["BBB Listed", "Physical Showroom on Kanis Rd", "Service Plumbing & Fixture Supply", "Fast Response Times", "Family Owned & Operated"].map((c, i) => (
          ...
          <div className="badge-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, paddingTop:20 }}>
            {[
              ...
              { icon: Icons.store, title: "Showroom", desc: "Browse fixtures in person" },
              { icon: Icons.zap, title: "Fast Response", desc: "Quick turnaround times" },
              { icon: Icons.shield, title: "BBB Listed", desc: "Verified & trusted" },
            ].map((b, i) => (
      ...
      <section id="areas" style={{ padding:"100px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:900, margin:"0 auto", textAlign:"center" }}>
          ...
          <h2 style={{ fontSize:"clamp(28px, 3vw, 38px)", color:"var(--primary)", marginBottom:48, letterSpacing:-0.5 }}>Serving Little Rock & Surrounding Areas</h2>
          <div style={{ display:"flex", flexWrap:"wrap", justifyContent:"center", gap:12 }}>
            {areas.map((a, i) => (
              <span key={i} title={`Plumbing in ${a}, AR`} style={{ display:"inline-flex", alignItems:"center", gap:6, background:"#fff", border:"1px solid var(--border)", padding:"12px 24px", borderRadius:30, fontSize:15, fontWeight:500, color:"var(--text)", cursor:"default", transition:"all 0.25s" }}
      ...
      <section style={{ padding:"100px clamp(24px,5vw,64px)", background:"#fff", textAlign:"center", borderTop:"1px solid var(--border)" }}>
        <div style={{ maxWidth:600, margin:"0 auto" }}>
          <h2 className="serif" style={{ fontSize:"clamp(32px, 4vw, 50px)", color:"var(--primary)", lineHeight:1.1, marginBottom:20, letterSpacing:-1 }}>
            Got a plumbing problem?<br/><span style={{ fontStyle:"italic", color:"var(--accent)" }}>Call Houff Plumbing.</span>
          </h2>
          <p style={{ fontSize:17, color:"var(--text-mid)", marginBottom:36, lineHeight:1.6 }}>
            Fair pricing. Fast response. A family company you can count on.
          </p>
          <a href={PHONE_TEL} style={{ display:"inline-flex", alignItems:"center", gap:10, background:"var(--accent)", color:"#fff", padding:"22px 52px", borderRadius:8, fontSize:20, fontWeight:700, boxShadow:"0 6px 28px rgba(200,147,79,0.35)", fontFamily:"'DM Sans',sans-serif" }}>
            {Icons.phone} {PHONE}
          </a>
        </div>
      </section>
      ...
      <footer style={{ background:"var(--primary)", color:"rgba(255,255,255,0.7)", padding:"64px clamp(24px,5vw,64px) 0" }}>
        <div className="footer-grid" style={{ maxWidth:1100, margin:"0 auto", display:"grid", gridTemplateColumns:"1.5fr 1fr 1fr", gap:48, paddingBottom:40 }}>
          <div>
            <div className="serif" style={{ fontSize:22, color:"#fff", fontWeight:600, letterSpacing:-0.5, marginBottom:16 }}>Houff <span style={{ color:"var(--accent)" }}>Plumbing</span></div>
            <p style={{ fontSize:14, lineHeight:1.7, opacity:0.7, marginBottom:16 }}>Family plumbing company serving Central Arkansas with reliable repair and a full showroom of fixtures and supplies.</p>
            <p style={{ fontSize:14, lineHeight:1.8, opacity:0.6 }}>
              11802 Kanis Rd<br/>Little Rock, AR 72211<br/>
              <a href={PHONE_TEL} style={{ color:"var(--accent)", fontWeight:600 }}>{PHONE}</a>
            </p>
            <p style={{ fontSize:13, opacity:0.5, marginTop:8 }}>Mon-Fri 7:30AM-4:30PM</p>
          </div>
          <div>
        ...
        <div style={{ borderTop:"1px solid rgba(255,255,255,0.06)", padding:"20px 0", maxWidth:1100, margin:"0 auto" }}>
          <div className="footer-btm-inner" style={{ display:"flex", justifyContent:"space-between", fontSize:13, opacity:0.4 }}>
            <span>&copy; {new Date().getFullYear()} Don M Houff Plumbing Co. All rights reserved.</span>
      ...
      <div className="mobile-only" style={{ position:"fixed", bottom:0, left:0, right:0, zIndex:998, display:"flex" }}>
        <a href={PHONE_TEL} style={{ display:"flex", width:"100%", alignItems:"center", justifyContent:"center", gap:10, background:"var(--accent)", color:"#fff", padding:"18px", fontSize:17, fontWeight:700, boxShadow:"0 -4px 20px rgba(0,0,0,0.15)", fontFamily:"'DM Sans',sans-serif" }}>
          {Icons.phone} Call Houff Plumbing
        </a>
      </div>
