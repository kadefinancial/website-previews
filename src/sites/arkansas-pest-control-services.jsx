...
const services = [
  { name: "General Pest Control", desc: "Year-round protection against ants, spiders, roaches, and other common household pests. Jeff and his team use proven treatments tailored to Arkansas conditions.", icon: Icons.bug },
  { name: "Termite Treatment", desc: "Comprehensive termite control and prevention. Protect your home or business from costly structural damage with professional-grade solutions.", icon: Icons.shield },
  { name: "Certified Inspections", desc: "Thorough property inspections for termites, wood-destroying organisms, and pest activity. Certified reports for real estate transactions and peace of mind.", icon: Icons.search },
  { name: "Residential Pest Control", desc: "Keep your family safe and comfortable. Custom treatment plans for homes of all sizes, with no long-term contracts required.", icon: Icons.home },
  { name: "Commercial Pest Control", desc: "Reliable pest management for offices, restaurants, warehouses, and retail spaces. Discreet service that keeps your business running smoothly.", icon: Icons.building },
  { name: "No-Contract Plans", desc: "Flexible pest protection without the commitment. Pay for what you need, when you need it. Cancel anytime with zero penalties.", icon: Icons.clipboard },
];

const allServiceNames = ["General Pest Control","Termite Control & Treatment","Certified Pest Inspections","Residential Pest Control","Commercial Pest Control","No-Contract Pest Plans"];

const areas = ["North Little Rock","Little Rock","Conway","Cabot","Jacksonville","Maumelle","Benton","Bryant","Pine Bluff","Searcy"];

const reviews = [
  { text: "Jeff is always courteous & takes care of keeping our home free of pests!", name: "Paige Martin", src: "Google" },
  { text: "They got right to me, did a GREAT job, and for a low price.", name: "Donna Shelton", src: "Google" },
  { text: "Great customer service and very reasonable on pricing versus all the other competitors.", name: "Dawna Monroe", src: "Google" },
  { text: "They have been great to work with and competitive pricing. I really like that I can send a picture of a bug to them so they don't have to come out unless they need to.", name: "Wes Buchman", src: "Google" },
  { text: "Awesome services, friendly staff, and great pricing.", name: "Scott Carrington", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function ArkansasPestControlServices() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("arkpest-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="2026"){sessionStorage.setItem("arkpest-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "Pest Control North Little Rock AR | Arkansas Pest Control";
    const els = [];
    ...
    add("meta",{name:"description",content:"Pest control in North Little Rock, AR since 1989. No contracts required. Call (501) 753-2727."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"Arkansas Pest Control Services",telephone:"+15017532727",
      email:"info@arkansaspest.com",
      address:{"@type":"PostalAddress",streetAddress:"1404 W Pershing Blvd",addressLocality:"North Little Rock",addressRegion:"AR",postalCode:"72114",addressCountry:"US"},
      ...
      aggregateRating:{"@type":"AggregateRating",ratingValue:"4.6",reviewCount:"31",bestRating:"5"},
      founder:{"@type":"Person",name:"Jeff Akins"},
      foundingDate:"1989",
      description:"Arkansas Pest Control Services provides residential and commercial pest control in North Little Rock, AR and 28+ surrounding cities. No contracts required. In business since 1989.",
      areaServed:areas.map(a=>({"@type":"City",name:a})),
      hasOfferCatalog:{"@type":"OfferCatalog",name:"Pest Control Services",itemListElement:allServiceNames.map(s=>({"@type":"Offer",itemOffered:{"@type":"Service",name:s}}))},
      memberOf:[{"@type":"Organization",name:"BBB"}],sameAs:[],
    })});
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1E3A2F"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8}}><span style={{fontWeight:700,color:"#1E3A2F"}}>Arkansas</span> <span style={{color:"#B8694A"}}>Pest Control</span></div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(30,58,47,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}><span>Arkansas</span> <span style={{color:"var(--accent)"}}>Pest Control</span></div>
      ...
      <div className="hero-grid" style={{maxWidth:1200,width:"100%",display:"grid",gridTemplateColumns:"1.8fr 1fr",gap:48,alignItems:"center"}}>
        <div>
          <h1 style={{fontSize:"clamp(48px, 6vw, 84px)",lineHeight:1.0,color:"var(--primary)",letterSpacing:-2,marginBottom:24}}>
            North Little Rock's<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>No-Contract</span><br/>Pest Control
          </h1>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:32}}>
            <Stars />
            <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"Great to work with and competitive pricing." — Wes Buchman</span>
          </div>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(184,105,74,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Jeff — {PHONE}</a>
        </div>
        <div>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",color:"var(--primary)",letterSpacing:-0.5}}>Pest Control Services in North Little Rock</h2>
        </div>
        {/* Mobile trust */}
        <div className="trust-mobile" style={{display:"none",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:48,textAlign:"center"}}>
          {[{num:"4.6",label:"Google Rating",desc:"31+ verified reviews"},{num:"31+",label:"Reviews",desc:"Real customer feedback"},{num:"37",label:"Years",desc:"In business since 1989"},{num:"A+",label:"BBB Rating",desc:"Accredited since 2001"}].map((s,i)=>(
            <div key={i}>
        ...
        <p style={{textAlign:"center",marginTop:48,fontSize:15,color:"var(--text-light)"}}>Questions? Call Jeff directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",position:"relative",overflow:"hidden"}}>
      <div className="serif" style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",fontSize:"clamp(120px,20vw,280px)",color:"var(--accent)",opacity:0.08,lineHeight:1,whiteSpace:"nowrap",pointerEvents:"none"}}>1989</div>
      <div style={{maxWidth:800,margin:"0 auto",position:"relative",zIndex:1}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,textAlign:"center",opacity:0.7}}>About Arkansas Pest Control</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff",textAlign:"center"}}>37 Years of Protecting<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Arkansas Homes</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:24,textAlign:"center"}}>
          Arkansas Pest Control Services has been protecting homes and businesses across Central Arkansas since 1989. When Jeff Akins took over ownership in 2014, he brought a hands-on approach that customers notice immediately. When you call, you're talking to real people who know pest control — not a call center reading from a script.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:36,textAlign:"center"}}>
          Jeff kept what worked — the BBB A+ rating held since 2001, the certified technicians, the fair pricing — and added what was missing: no contracts required. That's been a huge differentiator. You stay because the service is good, not because you're locked in. With 28+ cities served across the state, Arkansas Pest Control has the reach and experience to handle anything that crawls, flies, or burrows.
        </p>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:20}}>
          {["No Contract Required","BBB A+ Since 2001","In Business Since 1989","Certified Technicians","28+ Cities Served","Hands-On Owner"].map((c,i)=>(
    ...
    <section id="areas" style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        ...
        <h2 style={{fontSize:"clamp(28px,3vw,38px)",color:"var(--primary)",marginBottom:48,letterSpacing:-0.5}}>Serving North Little Rock & 28+ Cities</h2>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:12}}>
          {areas.map((a,i)=>(
            <span key={i} title={`Pest Control in ${a}, AR`} style={{display:"inline-flex",alignItems:"center",gap:6,background:"#fff",border:"1px solid var(--border)",padding:"12px 24px",borderRadius:30,fontSize:15,fontWeight:500,color:"var(--text)",cursor:"default",transition:"all 0.25s"}}
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>Got pests?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Jeff.</span></h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>No contracts. Fair pricing. Protecting Arkansas homes since 1989.</p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16}}><span>Arkansas</span> <span style={{color:"var(--accent)"}}>Pest Control</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>Professional pest control for residential and commercial properties across Central Arkansas. No contracts required. In business since 1989.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>1404 W Pershing Blvd<br/>North Little Rock, AR 72114<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a><br/><a href="mailto:info@arkansaspest.com" style={{color:"var(--accent)",fontWeight:600}}>info@arkansaspest.com</a></p>
          <p style={{fontSize:13,opacity:0.5,marginTop:10}}>Mon-Fri 8:30 AM - 5:00 PM</p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} Arkansas Pest Control Services. All rights reserved.</span>
          <span>BBB A+ Accredited Since 2001</span>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Arkansas Pest Control</a>
    </div>
