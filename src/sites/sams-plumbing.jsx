// Layout: B | Industry: Plumber | City: Hensley
import { useState, useEffect } from "react";

const PHONE = "(501) 412-1315";
const TEL = "tel:+15014121315";
...
const services = [
  { name: "General Plumbing Repair", desc: "Every residential plumbing need handled by Sam personally — faucets, toilets, pipes, and fixtures done right the first time.", icon: Icons.wrench },
  { name: "Water Heater Service", desc: "Installation, repair, and replacement of residential water heaters. Fast turnaround and fair pricing.", icon: Icons.flame },
  { name: "Gas Line Work", desc: "Gas main installation and repair for residential properties. Licensed, safe, and code-compliant.", icon: Icons.zap },
  { name: "Drain Cleaning", desc: "Stubborn clogs and slow drains cleared with professional equipment. Kitchens, bathrooms, and main lines.", icon: Icons.droplets },
  { name: "Leak Detection & Repair", desc: "Find and fix hidden leaks before they cause costly water damage. Thorough diagnosis, reliable repairs.", icon: Icons.search },
  { name: "Emergency Plumbing", desc: "Plumbing emergencies don't wait for business hours. Call Sam anytime — available 24 hours.", icon: Icons.shield },
];

const allServiceNames = ["General Plumbing Service & Repair","Gas Line Installation & Repair","Bathroom Remodeling","Emergency Plumbing","Drain Cleaning","Water Heater Installation & Repair","Leak Detection & Repair"];

const areas = ["Hensley","Little Rock","Benton","Bryant","Bauxite","Shannon Hills","Sweet Home","Alexander","Sheridan"];

const reviews = [
  { text: "Sam is the best plumber in Arkansas. Have used his services multiple times. Extremely professional and reasonable.", name: "Patrick Kountz", src: "Google" },
  { text: "Very pleased with the service received from Sam's Plumbing. Had to replace an outside faucet. Sam did a fantastic job!", name: "Billie Harriss", src: "Google" },
  { text: "Excellent plumbing service.", name: "Cathy Outlaw", src: "Google" },
  { text: "Great job.", name: "Dan Johnson", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function SamsPlumbing() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("sams-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="2026"){sessionStorage.setItem("sams-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "Plumber Hensley AR | Sam's Plumbing";
    const els = [];
    ...
    add("meta",{name:"description",content:"Local plumber in Hensley, AR. Serving Saline County since 2010. Call (501) 412-1315."});
    add("meta",{name:"geo.region",content:"US-AR"});
    add("meta",{name:"geo.placename",content:"Hensley"});
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      "@context":"https://schema.org","@type":"Plumber",
      name:"Sam's Plumbing",telephone:"+15014121315",
      address:{"@type":"PostalAddress",streetAddress:"1291 Royce Rd",addressLocality:"Hensley",addressRegion:"AR",postalCode:"72065",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:34.5850,longitude:-92.3700},
      openingHoursSpecification:[{"@type":"OpeningHoursSpecification",dayOfWeek:["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"],opens:"00:00",closes:"23:59"}],
      founder:{"@type":"Person",name:"Sam Crook"},foundingDate:"2010",
      description:"Sam's Plumbing provides residential plumbing services in Hensley, AR and surrounding Saline County communities since 2010.",
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1C2B41"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8}}><span style={{fontWeight:700,color:"#1C2B41"}}>Sam's</span> <span style={{color:"#C8934F"}}>Plumbing</span></div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(28,43,65,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}><span>Sam's</span> <span style={{color:"var(--accent)"}}>Plumbing</span></div>
    ...
    <section style={{padding:"180px clamp(24px,5vw,64px) 100px",background:"var(--cream)",textAlign:"center"}}>
      <div style={{maxWidth:780,margin:"0 auto"}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(200,147,79,0.1)",padding:"8px 18px",borderRadius:4,marginBottom:28}}>
          ...
          <span style={{fontSize:12,fontWeight:700,color:"var(--accent)",letterSpacing:2,textTransform:"uppercase"}}>Serving Saline County Since 2010</span>
        </div>
        <h1 style={{fontSize:"clamp(44px, 6vw, 80px)",lineHeight:1.05,color:"var(--primary)",marginBottom:24,letterSpacing:-2}}>
          Hensley's<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Trusted Plumber</span>
        </h1>
        <p style={{fontSize:18,lineHeight:1.7,color:"var(--text-mid)",marginBottom:32,maxWidth:560,margin:"0 auto 32px"}}>
          Sam Crook has been the plumber Saline County calls for over 15 years. Owner-operated, fair pricing, and available when you need him.
        </p>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:36}}>
          <Stars />
          <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"The best plumber in Arkansas." — Patrick Kountz</span>
        </div>
        <div style={{display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap"}}>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(200,147,79,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Sam Now</a>
    ...
    {/* FORM + TRUST — Layout B: Combined, rating hidden */}
        ...
        <div style={{background:"var(--white)",borderRadius:12,padding:"44px 36px",boxShadow:"0 4px 40px rgba(0,0,0,0.08)",border:"1px solid var(--border)",maxWidth:480}}>
          {!formDone ? (
            ...
            <div style={{textAlign:"center",padding:"24px 0"}}>
              ...
              <p style={{fontSize:14,color:"var(--text-mid)"}}>Sam will call you back shortly.<br/>Call <a href={TEL} style={{color:"var(--accent)",fontWeight:700}}>{PHONE}</a> for emergencies.</p>
            </div>
          )}
        </div>
        <div className="trust-row" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:32,alignContent:"center",height:"100%"}}>
          {[{num:"15+",label:"Years",desc:"Serving since 2010"},{num:"Owner",label:"Operated",desc:"Sam handles every job"},{num:"24hr",label:"Availability",desc:"Emergency calls"},{num:"Local",label:"Family Business",desc:"Hensley, AR"}].map((s,i)=>(
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",color:"var(--primary)",letterSpacing:-0.5}}>Plumbing Services in Hensley, AR</h2>
        </div>
        ...
        <p style={{textAlign:"center",marginTop:40,fontSize:15,color:"var(--text-light)"}}>Questions? Call Sam directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",color:"#fff"}}>
      <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,opacity:0.7}}>About Sam's Plumbing</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff"}}>Why Hensley Trusts<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Sam Crook</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.65)",marginBottom:24,textAlign:"left"}}>
          Sam Crook has been running Sam's Plumbing as a one-man operation since 2010. When you call Sam, you get Sam — not a dispatcher, not a subcontractor, not a call center. He answers the phone, he shows up, and he does the work himself. That's the kind of accountability that builds trust over 15 years.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.65)",marginBottom:36,textAlign:"left"}}>
          Based in Hensley and serving communities across Saline County and the southern Little Rock metro, Sam is known for being professional, reasonable, and dependable. His customers call him "the best plumber in Arkansas" — and they keep calling him back year after year.
        </p>
        <div className="why-creds-row" style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:24,marginTop:8}}>
          {["Owner-Operated Since 2010","BBB Listed","24-Hour Availability","Saline County & Little Rock Metro","Licensed Sole Proprietor"].map((c,i)=>(
    ...
    {/* REVIEWS — Layout B: 3 cards (we have 4, show top 3), no rating in heading */}
    ...
    <section id="areas" style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        ...
        <h2 style={{fontSize:"clamp(28px,3vw,38px)",color:"var(--primary)",marginBottom:48,letterSpacing:-0.5}}>Serving Hensley & Saline County</h2>
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>Got a plumbing problem?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Sam.</span></h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>Owner-operated since 2010. Fair pricing. Available 24 hours.</p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16}}><span>Sam's</span> <span style={{color:"var(--accent)"}}>Plumbing</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>Owner-operated plumbing serving Hensley, Saline County, and the greater Little Rock metro since 2010.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>1291 Royce Rd<br/>Hensley, AR 72065<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} Sam's Plumbing. All rights reserved.</span>
          <span>BBB Listed &middot; Hensley, AR</span>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Sam's Plumbing</a>
    </div>
