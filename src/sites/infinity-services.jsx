// Layout: A | Industry: Electrician | City: North Little Rock
...
const Icons = {
  ...
  building: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="2" width="16" height="20" rx="2"/><path d="M9 22v-4h6v4"/><path d="M8 6h.01"/><path d="M16 6h.01"/><path d="M8 10h.01"/><path d="M16 10h.01"/><path d="M8 14h.01"/><path d="M16 14h.01"/></svg>
  ),
  wind: (
  ...
  snowflake: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="2" x2="12" y2="22"/><path d="M20 12H4"/><path d="M17.27 3.27l-5.27 5.27-5.27-5.27"/><path d="M6.73 20.73l5.27-5.27 5.27 5.27"/><path d="M20.73 6.73l-5.27 5.27 5.27 5.27"/><path d="M3.27 17.27l5.27-5.27-5.27-5.27"/></svg>
  ),
  flame: (
  ...
  truck: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M1 3h15v13H1z"/><path d="M16 8h4l3 3v5h-7V8z"/><circle cx="5.5" cy="18.5" r="2.5"/><circle cx="18.5" cy="18.5" r="2.5"/></svg>
  ),
};
...
// --- Data ---
const PHONE = "(501) 772-4822";
const PHONE_TEL = "tel:+15017724822";

const services = [
  { name: "Residential Electrical", desc: "Complete home electrical service including panel upgrades, wiring, outlets, and lighting installations.", icon: Icons.home },
  { name: "Commercial Electrical", desc: "Reliable electrical solutions for businesses, offices, and commercial properties throughout Central Arkansas.", icon: Icons.building },
  { name: "HVAC Installation & Repair", desc: "Full-service heating and cooling installation, repair, and replacement for year-round comfort.", icon: Icons.wind },
  { name: "AC Maintenance", desc: "Routine seasonal maintenance to keep your air conditioning running efficiently and prevent costly breakdowns.", icon: Icons.snowflake },
  { name: "Heating Service", desc: "Expert furnace and heating system repair, maintenance, and installation to keep your home warm all winter.", icon: Icons.flame },
  { name: "Electrical Troubleshooting", desc: "Diagnostic services to identify and resolve electrical issues safely and efficiently.", icon: Icons.zap },
];

const allServices = [
  "Residential Electrical", "Commercial Electrical", "HVAC Installation & Repair",
  "AC Maintenance", "Heating System Service", "Electrical Troubleshooting", "Unit Installation"
];

const reviews = [
  { text: "I used Infinity Services for routine seasonal maintenance and the visit was great! James showed up early to our appointment which is such a plus for me. He was also quick and efficient!", name: "Kamrin Reeves", src: "Google" },
  { text: "We don't have words to describe how thankful and blessed we've been to have the owner of the company help us. He went above and beyond for our family, since the first call with Molly we knew this was a trustworthy and the right company.", name: "Lyen Plasencia", src: "Google" },
];

const areas = ["North Little Rock", "Little Rock", "Sherwood", "Jacksonville", "Maumelle", "Central Arkansas"];

// --- Main Component ---
export default function InfinityServices() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("infinity-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "2026") { sessionStorage.setItem("infinity-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Electrician North Little Rock AR | Infinity Services";
    const els = [];
    ...
    add("meta", { name: "description", content: "Electrical & HVAC service in North Little Rock, AR. Locally owned. Call (501) 772-4822 for a free quote." });
    ...
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      ...
      name: "Infinity Services", telephone: "+15017724822",
      address: { "@type": "PostalAddress", streetAddress: "1203 Pine St", addressLocality: "North Little Rock", addressRegion: "AR", postalCode: "72114", addressCountry: "US" },
      ...
      description: "Infinity Services is a locally owned electrical and HVAC company in North Little Rock, AR.",
      ...
      <>
        ...
        <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#2D2D2D" }}>
          <div style={{ background:"#fff", borderRadius:16, padding:"56px 44px", textAlign:"center", maxWidth:420, width:"92%", boxShadow:"0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:28, marginBottom:4, letterSpacing:-0.5 }}>
              <span style={{ fontWeight:600, color:"#2D2D2D" }}>Infinity</span> <span style={{ color:"#D4A03C", fontWeight:600 }}>Services</span>
            </div>
            ...
            <form onSubmit={submitPw}>
              ...
              <button type="submit" style={{ width:"100%", padding:16, marginTop:20, fontSize:15, fontWeight:700, letterSpacing:.3, background:"#2D2D2D", color:"#fff", border:"none", borderRadius:10, cursor:"pointer", fontFamily:"'DM Sans',sans-serif" }}>View Site</button>
            </form>
          </div>
        </div>
      </>
    ...
    <>
      ...
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:1000, padding:scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display:"flex", alignItems:"center", justifyContent:"space-between", background: scrolled ? "rgba(45,45,45,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition:"all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        <div className="serif" style={{ fontSize:22, color: scrolled ? "#fff" : "var(--primary)", fontWeight:600, letterSpacing:-0.5 }}>
          <span>Infinity</span><span style={{ color:"var(--accent)", marginLeft:6 }}>Services</span>
        </div>
        ...
        <div className="hero-grid" style={{ maxWidth:1200, width:"100%", display:"grid", gridTemplateColumns:"1.2fr 0.8fr", gap:64, alignItems:"center" }}>
          <div style={{ maxWidth:560 }}>
            <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(212,160,60,0.1)", padding:"8px 18px", borderRadius:4, marginBottom:28 }}>
              ...
              <span style={{ fontSize:12, fontWeight:700, color:"var(--accent)", letterSpacing:2, textTransform:"uppercase" }}>Locally Owned &amp; Operated</span>
            </div>
            <h1 style={{ fontSize:"clamp(40px, 5vw, 68px)", lineHeight:1.05, color:"var(--primary)", marginBottom:24, letterSpacing:-1.5 }}>
              North Little Rock's<br /><span style={{ fontStyle:"italic", color:"var(--accent)" }}>Trusted Experts</span><br />in Electrical &amp; HVAC
            </h1>
            <p style={{ fontSize:18, lineHeight:1.7, color:"var(--text-mid)", marginBottom:32, maxWidth:440 }}>
              From electrical troubleshooting to HVAC installation, Infinity Services delivers reliable, honest work across Central Arkansas.
            </p>
            <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:36 }}>
              <Stars />
              <span style={{ fontSize:14, color:"var(--text-light)", fontStyle:"italic", marginLeft:4 }}>"He went above and beyond for our family." — Lyen Plasencia</span>
            </div>
            <div style={{ display:"flex", gap:14, flexWrap:"wrap" }}>
              <a href={PHONE_TEL} style={{ display:"inline-flex", alignItems:"center", gap:10, background:"var(--accent)", color:"#fff", padding:"18px 36px", borderRadius:6, fontSize:16, fontWeight:700, boxShadow:"0 4px 20px rgba(212,160,60,0.3)", fontFamily:"'DM Sans',sans-serif" }}>
          ...
          <div>
            <div style={{ background:"#fff", borderRadius:12, padding:"44px 36px", boxShadow:"0 4px 40px rgba(45,45,45,0.08)", border:"1px solid var(--border)" }}>
              {!formSubmitted ? (
      ...
      <section style={{ padding:"72px clamp(24px,5vw,64px)", background:"#fff", borderTop:"1px solid var(--border)", borderBottom:"1px solid var(--border)" }}>
        <div className="trust-grid" style={{ maxWidth:1000, margin:"0 auto", display:"grid", gridTemplateColumns:"repeat(4, 1fr)", gap:32, textAlign:"center" }}>
          {[
            { num: "15+", label: "Reviews", sub: "And counting" },
            { num: "Local", label: "Locally Owned", sub: "North Little Rock" },
            { num: "2-in-1", label: "Electrical & HVAC", sub: "Dual-trade experts" },
      ...
      <section id="services" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          <div style={{ textAlign:"center", marginBottom:64 }}>
            ...
            <h2 style={{ fontSize:"clamp(30px, 3.5vw, 44px)", color:"var(--primary)", letterSpacing:-0.5 }}>Electrical &amp; HVAC Services in North Little Rock</h2>
          </div>
          <div className="svc-grid" style={{ display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:20 }}>
            {services.map((s, i) => (
              <div key={i} style={{ background:"#fff", borderRadius:10, padding:"36px 30px", border:"1px solid var(--border)", transition:"all 0.3s ease", cursor:"default", position:"relative", overflow:"hidden" }}
                onMouseEnter={e => { e.currentTarget.style.transform="translateY(-4px)"; e.currentTarget.style.boxShadow="0 12px 40px rgba(45,45,45,0.1)"; e.currentTarget.querySelector('.card-bar').style.width="100%"; }}
      ...
      <section id="about" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--primary)", color:"#fff", position:"relative", overflow:"hidden" }}>
        ...
        <div className="why-grid" style={{ maxWidth:1100, margin:"0 auto", display:"grid", gridTemplateColumns:"1.1fr 0.9fr", gap:80, alignItems:"start", position:"relative" }}>
          <div>
            ...
            <div style={{ fontSize:12, fontWeight:700, textTransform:"uppercase", letterSpacing:3, color:"var(--accent)", marginBottom:14, opacity:0.7 }}>About Us</div>
            <h2 style={{ fontSize:"clamp(30px, 3.5vw, 42px)", lineHeight:1.15, marginBottom:28, color:"#fff" }}>Why North Little Rock Trusts<br/><span style={{ fontStyle:"italic", color:"var(--accent)" }}>Infinity Services</span></h2>
            <p style={{ fontSize:16, lineHeight:1.8, color:"rgba(255,255,255,0.65)", marginBottom:24 }}>
              Infinity Services is a locally owned company that handles both electrical and HVAC work — a rare combination that means fewer contractors, fewer headaches, and one team you can trust for your home's most critical systems. From the first phone call with Molly to the technician at your door, every interaction is built on honesty and respect.
            </p>
            <p style={{ fontSize:16, lineHeight:1.8, color:"rgba(255,255,255,0.65)", marginBottom:36 }}>
              Whether it's James arriving early for a seasonal maintenance visit or the owner personally stepping in to help a family in need, this team goes above and beyond — every single time. That's not a slogan. It's how they operate.
            </p>
            <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
              {["Licensed & Insured", "Electrical & HVAC Under One Roof", "Locally Owned & Operated", "Fast, Reliable Response Times", "Honest, Upfront Pricing"].map((c, i) => (
          ...
          <div className="badge-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16, paddingTop:20 }}>
            {[
              { icon: Icons.zap, title: "Electrical", desc: "Full residential & commercial" },
              { icon: Icons.wind, title: "HVAC", desc: "Heating & cooling experts" },
              { icon: Icons.shield, title: "Trustworthy", desc: "Honest, transparent service" },
              { icon: Icons.truck, title: "Fast Response", desc: "On time, every time" },
            ].map((b, i) => (
      ...
      <section id="areas" style={{ padding:"100px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:900, margin:"0 auto", textAlign:"center" }}>
          ...
          <div style={{ display:"flex", flexWrap:"wrap", justifyContent:"center", gap:12 }}>
            {areas.map((a, i) => (
              <span key={i} title={`Electrician & HVAC in ${a}, AR`} style={{ display:"inline-flex", alignItems:"center", gap:6, background:"#fff", border:"1px solid var(--border)", padding:"12px 24px", borderRadius:30, fontSize:15, fontWeight:500, color:"var(--text)", cursor:"default", transition:"all 0.25s" }}
      ...
      <section style={{ padding:"100px clamp(24px,5vw,64px)", background:"#fff", textAlign:"center", borderTop:"1px solid var(--border)" }}>
        <div style={{ maxWidth:600, margin:"0 auto" }}>
          <h2 className="serif" style={{ fontSize:"clamp(32px, 4vw, 50px)", color:"var(--primary)", lineHeight:1.1, marginBottom:20, letterSpacing:-1 }}>
            Need electrical or HVAC help?<br/><span style={{ fontStyle:"italic", color:"var(--accent)" }}>Call Infinity Services.</span>
          </h2>
          <p style={{ fontSize:17, color:"var(--text-mid)", marginBottom:36, lineHeight:1.6 }}>
            Honest pricing. Reliable work. One team for electrical and HVAC.
          </p>
          <a href={PHONE_TEL} style={{ display:"inline-flex", alignItems:"center", gap:10, background:"var(--accent)", color:"#fff", padding:"22px 52px", borderRadius:8, fontSize:20, fontWeight:700, boxShadow:"0 6px 28px rgba(212,160,60,0.35)", fontFamily:"'DM Sans',sans-serif" }}>
            {Icons.phone} {PHONE}
          </a>
        </div>
      </section>
      ...
      <footer style={{ background:"var(--primary)", color:"rgba(255,255,255,0.7)", padding:"64px clamp(24px,5vw,64px) 0" }}>
        <div className="footer-grid" style={{ maxWidth:1100, margin:"0 auto", display:"grid", gridTemplateColumns:"1.5fr 1fr 1fr", gap:48, paddingBottom:40 }}>
          <div>
            <div className="serif" style={{ fontSize:22, color:"#fff", marginBottom:16, fontWeight:600 }}>Infinity <span style={{ color:"var(--accent)" }}>Services</span></div>
            <p style={{ fontSize:14, lineHeight:1.7, opacity:0.7, marginBottom:16 }}>Locally owned electrical and HVAC company serving North Little Rock and Central Arkansas with honest, reliable service.</p>
            <p style={{ fontSize:14, lineHeight:1.8, opacity:0.6 }}>
              1203 Pine St<br/>North Little Rock, AR 72114<br/>
              <a href={PHONE_TEL} style={{ color:"var(--accent)", fontWeight:600 }}>{PHONE}</a><br/>
              <a href="mailto:infinityservicesllc@outlook.com" style={{ color:"rgba(255,255,255,0.6)" }}>infinityservicesllc@outlook.com</a>
            </p>
            <p style={{ fontSize:13, opacity:0.5, marginTop:12 }}>Mon-Fri 7:30AM - 5:00PM</p>
          </div>
          <div>
        ...
        <div style={{ borderTop:"1px solid rgba(255,255,255,0.06)", padding:"20px 0", maxWidth:1100, margin:"0 auto" }}>
          <div className="footer-btm-inner" style={{ display:"flex", justifyContent:"space-between", fontSize:13, opacity:0.4 }}>
            <span>&copy; {new Date().getFullYear()} Infinity Services. All rights reserved.</span>
      ...
      <div className="mobile-only" style={{ position:"fixed", bottom:0, left:0, right:0, zIndex:998, display:"flex" }}>
        <a href={PHONE_TEL} style={{ display:"flex", width:"100%", alignItems:"center", justifyContent:"center", gap:10, background:"var(--accent)", color:"#fff", padding:"18px", fontSize:17, fontWeight:700, boxShadow:"0 -4px 20px rgba(0,0,0,0.15)", fontFamily:"'DM Sans',sans-serif" }}>
          {Icons.phone} Call Infinity Services
        </a>
      </div>
