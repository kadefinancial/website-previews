// Layout: B | Industry: Plumber | City: Fayetteville
import { useState, useEffect } from "react";

const PHONE = "(479) 381-4995";
const TEL = "tel:+14793814995";

const Icons = {
  phone: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>,
  sewer: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 14h6v6H4z"/><path d="M14 4h6v6h-6z"/><path d="M10 17h4a4 4 0 004-4V10"/></svg>,
  ...
  septic: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="10" width="18" height="10" rx="2"/><path d="M7 10V6a2 2 0 012-2h6a2 2 0 012 2v4"/><path d="M12 14v3"/><circle cx="12" cy="18" r="1"/></svg>,
  pipe: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M6 6h4v12H6z"/><path d="M14 6h4v12h-4z"/><path d="M10 10h4"/><path d="M10 14h4"/></svg>,
  ...
  emergency: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>,
...
const services = [
  { name: "Sewer Cleaning", desc: "Professional sewer line cleaning to restore flow and prevent backups. Tony uses proven techniques to clear even the toughest blockages and keep your system running.", icon: Icons.sewer },
  { name: "Drain Cleaning", desc: "Slow drains, clogged lines, and stubborn buildup cleared fast. From kitchen sinks to main lines, we diagnose the problem and get water moving again.", icon: Icons.droplets },
  { name: "Septic Pump Replacement & Service", desc: "Complete septic pump replacement, maintenance, and emergency service. When your system fails -- even on a holiday weekend -- Tony answers the call.", icon: Icons.septic },
  { name: "Pipe Repair & Replacement", desc: "From corroded pipes to burst lines, we locate the issue and repair or replace with precision. Minimal disruption, lasting results.", icon: Icons.pipe },
  { name: "Natural Gas Line Service", desc: "Licensed natural gas line installation, repair, and leak detection. Safety-first approach for residential and commercial properties throughout NWA.", icon: Icons.flame },
  { name: "24/7 Emergency Plumbing", desc: "Plumbing emergencies don't wait for business hours, and neither does Tony. Around-the-clock service when you need it most -- holidays and weekends included.", icon: Icons.emergency },
];

const allServiceNames = ["Sewer Cleaning","Drain Cleaning","Septic Pump Replacement & Service","Pipe Repair & Replacement","Natural Gas Line Service","24/7 Emergency Plumbing"];

const areas = ["Fayetteville","Springdale","Rogers","Bentonville","Northwest Arkansas"];

const reviews = [
  { text: "We needed a septic pump replacement over the labor day holiday weekend, Tony was the only person that could make it out that day to diagnose our issue. Got it diagnosed and had a temporary solution in place until he could get the pump.", name: "John Raines", src: "Google" },
  { text: "The only place we go for our plumbing needs! Tony is wonderful to work with, always professional and gets back to you quickly with reasonable pricing.", name: "Kelly Sokolosky", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function DrainKing() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("dk-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="roof"){sessionStorage.setItem("dk-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "Plumber Fayetteville AR | Drain King";
    const els = [];
    ...
    add("meta",{name:"description",content:"Drain cleaning, sewer service & emergency plumber in Fayetteville, AR. 4.8 stars, BBB A+ rated since 2011. Call (479) 381-4995."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"Drain King",telephone:"+14793814995",
      address:{"@type":"PostalAddress",streetAddress:"2200 N Hummingbird Ln",addressLocality:"Fayetteville",addressRegion:"AR",postalCode:"72703",addressCountry:"US"},
      ...
      founder:{"@type":"Person",name:"John Ciaravino"},
      description:"Drain King provides drain cleaning, sewer service, septic pump replacement, pipe repair, gas line service, and 24/7 emergency plumbing in Fayetteville and Northwest Arkansas. BBB A+ rated since 2011.",
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1C2B41"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8,fontWeight:600,letterSpacing:-0.5}}><span style={{color:"#1C2B41"}}>Drain</span> <span style={{color:"#C8934F"}}>King</span></div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(28,43,65,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5,cursor:"pointer"}} onClick={()=>window.scrollTo({top:0,behavior:"smooth"})}><span>Drain</span> <span style={{color:"var(--accent)"}}>King</span></div>
    ...
    <section style={{padding:"180px clamp(24px,5vw,64px) 100px",background:"var(--cream)",textAlign:"center"}}>
      <div style={{maxWidth:800,margin:"0 auto"}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(200,147,79,0.1)",padding:"8px 18px",borderRadius:4,marginBottom:28}}>
          ...
          <span style={{fontSize:12,fontWeight:700,color:"var(--accent)",letterSpacing:2,textTransform:"uppercase"}}>BBB A+ Rated Since 2011</span>
        </div>
        <h1 style={{fontSize:"clamp(44px, 6vw, 80px)",lineHeight:1.05,color:"var(--primary)",marginBottom:24,letterSpacing:-2}}>
          Fayetteville's Drain<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>& Sewer Experts</span>
        </h1>
        <p style={{fontSize:18,lineHeight:1.7,color:"var(--text-mid)",marginBottom:32,maxWidth:600,margin:"0 auto 32px"}}>
          When your drains back up at 2 AM on a holiday weekend, Tony answers the phone. 24/7 drain, sewer, and plumbing service Northwest Arkansas trusts.
        </p>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:36}}>
          <Stars />
          <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"Tony is wonderful to work with, always professional" -- Kelly Sokolosky</span>
        </div>
    ...
    <section id="form" style={{padding:"80px clamp(24px,5vw,64px)",background:"var(--white)",borderTop:"1px solid var(--border)",borderBottom:"1px solid var(--border)"}}>
      <div style={{maxWidth:1200,margin:"0 auto",display:"flex",flexDirection:"column",alignItems:"center"}}>
        ...
        {/* Trust Stats Row -- 4.8 rating: show prominently */}
        <div className="trust-row" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:32,marginTop:56,maxWidth:800,width:"100%",textAlign:"center"}}>
          {[
            {num:"4.8",label:"Google Rating",desc:"Stars on Google"},
            {num:"24+",label:"Customer Reviews",desc:"Verified reviews"},
            {num:"24/7",label:"Always Available",desc:"Holidays & weekends"},
            {num:"BBB",label:"A+ Since 2011",desc:"14+ years accredited"},
          ].map((s,i)=>(
            <div key={i}>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        ...
        <p style={{textAlign:"center",marginTop:48,fontSize:15,color:"var(--text-mid)"}}>
          Questions? Call Tony directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a>
        </p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)"}}>
      <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <p style={{fontSize:12,fontWeight:700,letterSpacing:3,textTransform:"uppercase",color:"var(--accent)",marginBottom:14}}>Why Drain King</p>
        <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",lineHeight:1.15,color:"#fff",letterSpacing:-0.5,marginBottom:40}}>The Guy Who Answers the Phone<br/>on a Holiday Weekend.</h2>
        <p style={{fontSize:17,lineHeight:1.8,color:"rgba(255,255,255,0.75)",marginBottom:24}}>
          Ask anyone in Northwest Arkansas who to call when a drain backs up, a sewer line fails, or a septic pump dies at the worst possible time -- and the answer is always the same: Tony. He is the face of Drain King and the reason customers call back year after year. When a family needed a septic pump replacement over Labor Day weekend, Tony was the only one who showed up. He diagnosed the problem, put a temporary fix in place that day, and came back with the permanent solution. That is who he is.
        </p>
        <p style={{fontSize:17,lineHeight:1.8,color:"rgba(255,255,255,0.75)",marginBottom:48}}>
          Drain King has been BBB A+ accredited since 2011 -- over 14 years of verified trust. From routine drain cleaning to emergency calls at 2 AM, Tony delivers honest pricing, fast response, and work that holds up. No upselling. No runaround. Just a plumber who treats your home like his own and answers the phone when nobody else will.
        </p>
        <div className="why-creds-row" style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:24}}>
          {["BBB A+ Since 2011","4.8 Stars on Google","24/7 Emergency Service","Licensed & Insured","Serving All of NWA","Holiday & Weekend Availability"].map((c,i)=>(
    ...
    {/* REVIEWS -- Layout B: 2 cards side by side (only 2 reviews) */}
    <section id="reviews" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--white)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",lineHeight:1.15,color:"var(--primary)",letterSpacing:-0.5}}>4.8 Stars on Google</h2>
        </div>
        <div className="rev-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,maxWidth:900,margin:"0 auto"}}>
          {reviews.map((r,i)=>(
            <div key={i} style={{background:"var(--cream)",border:"1px solid var(--border)",borderRadius:12,padding:"44px 36px",textAlign:"center"}}>
              ...
              <p style={{fontSize:17,lineHeight:1.7,color:"var(--text-mid)",fontStyle:"italic",marginBottom:24}}>"{r.text}"</p>
              <div>
    ...
    <section id="areas" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto",textAlign:"center"}}>
        ...
        <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",lineHeight:1.15,color:"var(--primary)",letterSpacing:-0.5,marginBottom:56}}>Serving Northwest Arkansas</h2>
    ...
    <section style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--white)",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
        <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",lineHeight:1.15,color:"var(--primary)",letterSpacing:-0.5,marginBottom:20}}>
          Got a drain emergency?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Tony.</span>
        </h2>
        <p style={{fontSize:17,lineHeight:1.7,color:"var(--text-mid)",marginBottom:36}}>
          Honest pricing, 24/7 availability, and 14+ years of BBB A+ service in Fayetteville and all of Northwest Arkansas.
        </p>
    ...
    <footer className="mob-pad" style={{padding:"80px clamp(24px,5vw,64px) 40px",background:"var(--primary)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div className="footer-grid" style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:48,marginBottom:48}}>
          <div>
            <div className="serif" style={{fontSize:22,fontWeight:600,letterSpacing:-0.5,marginBottom:16}}><span style={{color:"#fff"}}>Drain</span> <span style={{color:"var(--accent)"}}>King</span></div>
            <p style={{fontSize:14,lineHeight:1.7,color:"rgba(255,255,255,0.6)",marginBottom:16,maxWidth:320}}>Drain cleaning, sewer service, and 24/7 emergency plumbing in Fayetteville and Northwest Arkansas. BBB A+ accredited since 2011.</p>
            <p style={{fontSize:14,color:"rgba(255,255,255,0.6)",marginBottom:6}}>2200 N Hummingbird Ln, Fayetteville, AR 72703</p>
            <a href={TEL} style={{fontSize:14,color:"var(--accent)",fontWeight:600}}>{PHONE}</a>
            <p style={{fontSize:13,color:"rgba(255,255,255,0.4)",marginTop:8,display:"flex",alignItems:"center",gap:6}}><span style={{color:"var(--accent)",display:"inline-flex"}}>{Icons.clock}</span> Open 24 Hours, 7 Days a Week</p>
          </div>
          <div>
        ...
        <div style={{borderTop:"1px solid rgba(255,255,255,0.1)",paddingTop:24}}>
          <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <p style={{fontSize:13,color:"rgba(255,255,255,0.35)"}}>{"\u00A9"} {yr} Drain King. All rights reserved.</p>
            <p style={{fontSize:13,color:"rgba(255,255,255,0.35)"}}>BBB A+ Since 2011 | Open 24/7</p>
          </div>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,width:"100%",padding:"16px",background:"var(--accent)",color:"#fff",fontSize:16,fontWeight:700,fontFamily:"'DM Sans',sans-serif",boxShadow:"0 -4px 20px rgba(0,0,0,0.15)"}}>{Icons.phone} Call Drain King Now</a>
    </div>
  </>);
}
