// Layout: D | Industry: HVAC | City: Sherwood
import { useState, useEffect, useRef } from "react";

const PHONE = "(501) 920-2494";
const TEL = "tel:+15019202494";
...
const Icons = {
  ...
  sprayFoam: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 3v18"/></svg>,
  miniSplit: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="4" width="20" height="8" rx="2"/><path d="M12 12v4M8 20h8M6 8h12"/></svg>,
  airQuality: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v4m0 12v4M4.93 4.93l2.83 2.83m8.48 8.48l2.83 2.83M2 12h4m12 0h4M4.93 19.07l2.83-2.83m8.48-8.48l2.83-2.83"/><circle cx="12" cy="12" r="3"/></svg>,
...
const services = [
  { name: "HVAC Installation & Repair", desc: "Full-system heating and cooling installation, replacement, and repair for residential and commercial properties across Central Arkansas.", icon: Icons.wrench },
  { name: "Spray Foam Insulation", desc: "Professional spray foam insulation to seal your home, reduce energy bills, and keep conditioned air exactly where it belongs.", icon: Icons.sprayFoam },
  { name: "Mini Split Systems", desc: "Ductless mini split installation and service for room-by-room comfort control, perfect for additions, garages, and older homes.", icon: Icons.miniSplit },
  { name: "Indoor Air Quality", desc: "Air purification, filtration, and humidity control solutions to keep your family breathing clean, healthy air year-round.", icon: Icons.airQuality },
  { name: "New Construction HVAC", desc: "Complete HVAC system design and installation for new builds. From blueprints to final inspection, we handle every detail.", icon: Icons.home },
  { name: "Maintenance Plans", desc: "Scheduled tune-ups and priority service to extend system life, prevent breakdowns, and keep your warranty intact.", icon: Icons.shield },
];

const allServiceNames = ["HVAC Sales & Installation","HVAC Repairs","Maintenance Plans","Spray Foam Insulation","Mini Split Systems","Indoor Air Quality","Duct Work","New Construction HVAC"];

const areas = ["Sherwood","North Little Rock","Little Rock","Jacksonville","Cabot","Maumelle","Conway","Benton"];

const reviews = [
  { text: "Discovered our heater stopped working at 6:30 PM. Reached out to Justin right away because of amazing past service. He walked me through troubleshooting steps and had us taken care of.", name: "Tanya Winward", src: "Google" },
  { text: "Excellent workmanship. Professional in every way. Competitive price. Honest, personable, and accurate free estimate.", name: "Terry Haley", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function AllAmericanHeatAndAir() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("allamerican-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="roof"){sessionStorage.setItem("allamerican-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "HVAC Service Sherwood AR | All American Heat & Air";
    const els = [];
    ...
    add("meta",{name:"description",content:"Sherwood AR HVAC service. 4.9 stars, 176+ reviews. Tempstar Elite Dealer. 24/7 emergency. Call (501) 920-2494."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"All American Heat & Air, Inc.",telephone:"+15019202494",
      address:{"@type":"PostalAddress",streetAddress:"28 Jackson Oaks Dr",addressLocality:"North Little Rock",addressRegion:"AR",postalCode:"72120",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:34.8151,longitude:-92.2243},
      openingHoursSpecification:[{"@type":"OpeningHoursSpecification",dayOfWeek:["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"],opens:"00:00",closes:"23:59"}],
      aggregateRating:{"@type":"AggregateRating",ratingValue:"4.9",reviewCount:"176",bestRating:"5"},
      founder:{"@type":"Person",name:"Justin Fox"},
      description:"All American Heat & Air is a women-owned, Tempstar Elite Dealer providing HVAC installation, repair, spray foam insulation, and 24/7 emergency service in Sherwood, AR.",
      ...
      memberOf:[{"@type":"Organization",name:"Tempstar Elite Dealer"},{"@type":"Organization",name:"AY Magazine 2025 Best of HVAC"}],sameAs:[],
    })});
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1C2B41"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8,fontWeight:600,letterSpacing:-0.5}}><span style={{color:"#1C2B41"}}>All American</span> <span style={{color:"#8B3A3A"}}>Heat & Air</span></div>
  ...
  return (<>
    <style>{`
      ...
      :root{--primary:#1C2B41;--accent:#8B3A3A;--cream:#FAF9F6;--white:#FFFFFF;--text:#2A2A3C;--text-mid:#5A5A6E;--text-light:#8A8A9A;--border:rgba(0,0,0,0.06)}
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(28,43,65,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}><span>All American</span> <span style={{color:"var(--accent)"}}>Heat & Air</span></div>
      ...
      <div className="hero-grid" style={{maxWidth:1200,width:"100%",display:"grid",gridTemplateColumns:"1.8fr 1fr",gap:48,alignItems:"center"}}>
        <div>
          <h1 style={{fontSize:"clamp(48px, 6vw, 84px)",lineHeight:1.0,color:"var(--primary)",letterSpacing:-2,marginBottom:24}}>
            Sherwood's<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Most Trusted</span><br/>HVAC Team
          </h1>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:32}}>
            <Stars />
            <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"Excellent workmanship. Professional in every way." -- Terry Haley</span>
          </div>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(139,58,58,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Justin -- {PHONE}</a>
        </div>
        <div>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",color:"var(--primary)",letterSpacing:-0.5}}>HVAC Services in Sherwood, AR</h2>
        </div>
        {/* Mobile trust */}
        <div className="trust-mobile" style={{display:"none",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:48,textAlign:"center"}}>
          {[{num:"4.9",label:"Google Rating",desc:"176+ reviews"},{num:"176+",label:"Reviews",desc:"On Google"},{num:"2025",label:"AY Best of HVAC",desc:"Arkansas award"},{num:"24/7",label:"Women-Owned",desc:"Family business"}].map((s,i)=>(
            <div key={i}>
        ...
        <p style={{textAlign:"center",marginTop:48,fontSize:15,color:"var(--text-light)"}}>Questions? Call Justin directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{maxWidth:800,margin:"0 auto",position:"relative",zIndex:1}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,textAlign:"center",opacity:0.7}}>About All American</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff",textAlign:"center"}}>Why Sherwood Trusts<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>All American Heat & Air</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:24,textAlign:"center"}}>
          All American Heat & Air is a women-owned, family-operated HVAC company serving Sherwood and Central Arkansas. Justin Fox is the face of every job -- showing up personally, diagnosing honestly, and making sure every system runs right. As a Tempstar Elite Dealer, All American installs top-tier equipment backed by industry-leading warranties and factory-trained expertise.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:36,textAlign:"center"}}>
          Named AY Magazine's 2025 Best of HVAC in Arkansas, All American goes beyond traditional heating and cooling. Spray foam insulation, indoor air quality solutions, and mini split systems set this team apart. With 1.8K Facebook followers, 176 Google reviews at 4.9 stars, and HVAC License #0005769, this is the company Sherwood calls 24/7/365 -- licensed, bonded, and insured.
        </p>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:20}}>
          {["Women-Owned Business","AY 2025 Best of HVAC","Tempstar Elite Dealer","Licensed, Bonded & Insured","24/7/365 Emergency Service"].map((c,i)=>(
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>AC not working?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call All American.</span></h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>Tempstar Elite Dealer. Women-owned. Available 24/7/365. Honest pricing, every time.</p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16,fontWeight:600,letterSpacing:-0.5}}><span>All American</span> <span style={{color:"var(--accent)"}}>Heat & Air</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>Women-owned Tempstar Elite Dealer providing HVAC installation, repair, spray foam insulation, and 24/7 emergency service in Sherwood and Central Arkansas.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>28 Jackson Oaks Dr<br/>North Little Rock, AR 72120<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a><br/><a href="mailto:danielle@allamericanheatandair.net" style={{color:"rgba(255,255,255,0.5)"}}>danielle@allamericanheatandair.net</a></p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} All American Heat & Air, Inc. All rights reserved.</span>
          <span>Tempstar Elite Dealer | HVAC License #0005769</span>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call All American Heat & Air</a>
    </div>
