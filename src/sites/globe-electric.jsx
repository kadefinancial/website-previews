// Layout: C | Industry: Electrician | City: North Little Rock
...
const Icons = {
  ...
  wind: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M9.59 4.59A2 2 0 1111 8H2"/><path d="M12.59 19.41A2 2 0 1014 16H2"/><path d="M15.73 2.27A2.5 2.5 0 1118 6H2"/></svg>
  ),
  lightbulb: (
  ...
  search: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/><path d="M11 8v6"/><path d="M8 11h6"/></svg>
  ),
  shield: (
...
// --- Data ---
const PHONE = "(501) 351-3357";
const PHONE_TEL = "tel:+15013513357";
const EMAIL = "sethcweaver81@yahoo.com";

const services = [
  { name: "Residential Electrical", desc: "Full-service electrical work for your home -- from panel upgrades and outlet installation to whole-house rewiring. Safe, code-compliant, and built to last.", icon: Icons.home },
  { name: "HVAC Installation & Service", desc: "Authorized Goodman dealer providing expert heating and cooling installation, repair, and maintenance. Dual-trade expertise means one call handles both systems.", icon: Icons.wind },
  { name: "Light Fixture Installation", desc: "Upgrade your home with professionally installed lighting -- recessed cans, chandeliers, under-cabinet lights, and outdoor fixtures done right the first time.", icon: Icons.lightbulb },
  { name: "Circuit Diagnostics", desc: "Tripping breakers, flickering lights, or dead outlets? Our team pinpoints the problem fast with thorough circuit testing and delivers lasting repairs.", icon: Icons.search },
  { name: "Home Warranty Service", desc: "We work with home warranty companies to deliver prompt, professional electrical and HVAC service calls. No-hassle scheduling and reliable results every time.", icon: Icons.shield },
  { name: "Commercial Electrical", desc: "Dependable commercial electrical solutions for businesses across central Arkansas. Office buildouts, lighting retrofits, and code compliance handled with precision.", icon: Icons.building },
];
...
const allServices = [
  ...
  "Attic Wiring",
  "Circuit Diagnostics & Repair",
  "Home Warranty Service Calls",
];

const reviews = [
  { text: "Seth and his crew have worked on several projects and issues since she bought her home. They are always professional, reliable, and do quality work every single time.", name: "Michelle Langley", src: "Google" },
  { text: "Outstanding experience. Professionalism top-notch. The work was completed quickly and everything was explained clearly. Would absolutely recommend to anyone.", name: "Tia Tackett", src: "Google" },
  { text: "Seth Weaver and the Globe Electric team were great. Honest and did great work. Fair pricing and no surprises. Will definitely use them again.", name: "Josh Taylor", src: "Google" },
];

const areas = ["North Little Rock", "Sherwood", "Little Rock", "Jacksonville", "Cabot", "Maumelle", "Conway", "Benton"];

// --- Main Component ---
export default function GlobeElectric() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("globe-electric-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "2026") { sessionStorage.setItem("globe-electric-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Electrician North Little Rock AR | Globe Electric";
    const els = [];
    ...
    add("meta", { name: "description", content: "Licensed electrician & HVAC in North Little Rock, AR. 4.9 stars, 518+ reviews. Call (501) 351-3357 for residential and commercial electrical service." });
    ...
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      ...
      name: "Globe Electric", telephone: "+15013513357",
      email: EMAIL,
      address: { "@type": "PostalAddress", streetAddress: "10228 US-70", addressLocality: "North Little Rock", addressRegion: "AR", postalCode: "72117", addressCountry: "US" },
      geo: { "@type": "GeoCoordinates", latitude: 34.769, longitude: -92.243 },
      openingHoursSpecification: [
        { "@type": "OpeningHoursSpecification", dayOfWeek: ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"], opens: "00:00", closes: "23:59" },
      ],
      aggregateRating: { "@type": "AggregateRating", ratingValue: "4.9", reviewCount: "518", bestRating: "5" },
      founder: { "@type": "Person", name: "Seth Weaver" },
      description: "Globe Electric is a women-owned electrical and HVAC contractor in North Little Rock, AR. 4.9 stars from 518+ reviews. Authorized Goodman dealer. Residential, commercial, and 24/7 emergency service across central Arkansas.",
      areaServed: areas.map((a) => ({ "@type": "City", name: a })),
      hasOfferCatalog: { "@type": "OfferCatalog", name: "Electrical & HVAC Services", itemListElement: services.map((s) => ({ "@type": "Offer", itemOffered: { "@type": "Service", name: s.name } })) },
      memberOf: [],
      sameAs: [],
    })});
        ...
        <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#1C2B41" }}>
          <div style={{ background:"#fff", borderRadius:16, padding:"56px 44px", textAlign:"center", maxWidth:420, width:"92%", boxShadow:"0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:28, fontWeight:600, letterSpacing:-0.5, marginBottom:4 }}>
              <span style={{ color:"#1C2B41" }}>Globe</span> <span style={{ color:"#C8934F" }}>Electric</span>
            </div>
    ...
    <>
      ...
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display:"flex", alignItems:"center", justifyContent:"space-between", background: scrolled ? "rgba(28,43,65,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition:"all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        <div className="serif" style={{ fontSize:22, fontWeight:600, letterSpacing:-0.5, color:"#fff" }}>
          <span>Globe</span><span style={{ color:"var(--accent)", marginLeft:6 }}>Electric</span>
        </div>
        ...
        <div className="hero-grid" style={{ maxWidth:1200, width:"100%", display:"grid", gridTemplateColumns:"1fr 1fr", gap:64, alignItems:"center" }}>
          ...
          <div style={{ maxWidth:560 }}>
            <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(255,255,255,0.1)", padding:"8px 18px", borderRadius:4, marginBottom:28 }}>
              ...
              <span style={{ fontSize:12, fontWeight:700, letterSpacing:3, textTransform:"uppercase", color:"var(--accent)" }}>4.9 Stars on Google</span>
            </div>
            ...
            <p style={{ fontSize:18, lineHeight:1.7, color:"rgba(255,255,255,0.75)", marginBottom:32, maxWidth:480 }}>
              Women-owned, dual-trade electrical and HVAC service. Authorized Goodman dealer. 4.9 stars from 518+ reviews -- and available 24 hours a day, 7 days a week.
            </p>
            {/* Star row with hero quote */}
            <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:32, flexWrap:"wrap" }}>
              <Stars />
              <span style={{ fontSize:14, color:"rgba(255,255,255,0.7)", fontStyle:"italic" }}>"Seth Weaver and the Globe Electric team were great. Honest and did great work."</span>
              <span style={{ fontSize:13, color:"rgba(255,255,255,0.5)" }}>- Josh Taylor</span>
            </div>
      ...
      <section style={{ background:"var(--white)", padding:"48px clamp(24px,5vw,64px)" }}>
        <div className="trust-grid" style={{ maxWidth:1200, margin:"0 auto", display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:32, textAlign:"center" }}>
          {[
            { num: "4.9", label: "Google Rating", desc: "518+ reviews" },
            { num: "518+", label: "Reviews", desc: "502 are five-star" },
            { num: "Women", label: "Owned", desc: "Women-owned business" },
      ...
      <section id="services" className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          <div style={{ textAlign:"center", marginBottom:64 }}>
            ...
            <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(30px,3.5vw,44px)", lineHeight:1.15, letterSpacing:-0.5, color:"var(--primary)" }}>Electrical &amp; HVAC Services in North Little Rock</h2>
          </div>
          {services.map((svc, i) => {
            const isEven = i % 2 === 1;
            return (
          ...
          <p style={{ textAlign:"center", marginTop:48, fontSize:15, color:"var(--text-mid)" }}>
            Questions? Call Seth directly at <a href={PHONE_TEL} style={{ color:"var(--accent)", fontWeight:600 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          <div style={{ textAlign:"center", marginBottom:64 }}>
            <div className="gold-line" />
            <p style={{ fontSize:12, fontWeight:700, letterSpacing:3, textTransform:"uppercase", color:"var(--text-light)", marginBottom:14, fontFamily:"'DM Sans',sans-serif" }}>Why Globe Electric</p>
            <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(30px,3.5vw,44px)", lineHeight:1.15, letterSpacing:-0.5, color:"var(--primary)" }}>4.9 Stars Across 518 Reviews</h2>
          </div>
          <div className="why-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:64, alignItems:"start" }}>
            ...
            <div>
              <p style={{ fontSize:17, lineHeight:1.8, color:"var(--text-mid)", marginBottom:20 }}>
                Globe Electric is a women-owned, dual-trade electrical and HVAC company based in North Little Rock. Led by owner Seth Weaver, the team has built an exceptional reputation with a 4.9-star rating from over 518 Google reviews -- 502 of them five stars.
              </p>
              <p style={{ fontSize:17, lineHeight:1.8, color:"var(--text-mid)", marginBottom:32 }}>
                As an authorized Goodman dealer, Globe Electric handles both your electrical and heating-and-cooling needs in a single call. Open 24 hours a day, 7 days a week, they are the team North Little Rock counts on for honest work, fair pricing, and no surprise charges. Repeat customers say it best -- once you call Globe Electric, you don't call anyone else.
              </p>
              {/* Pull quote */}
              <div style={{ borderLeft:"3px solid var(--accent)", paddingLeft:24, marginBottom:36 }}>
                <p style={{ fontSize:18, fontStyle:"italic", lineHeight:1.7, color:"var(--primary)", fontFamily:"'Playfair Display',serif" }}>
                  "Globe Electric is the only one I trust. Pricing up front, no surprise charges."
                </p>
                <p style={{ fontSize:14, color:"var(--text-light)", marginTop:8, fontFamily:"'DM Sans',sans-serif" }}>- Community endorsement</p>
              </div>
              {/* Credentials */}
              <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                {[
                  "4.9-Star Google Rating (518+ Reviews)",
                  "Women-Owned Business",
                  "Authorized Goodman Dealer",
                  "Dual-Trade: Electrical + HVAC",
            ...
            <div className="badge-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
              {[
                { icon: Icons.award, title: "4.9-Star Rating", desc: "518 reviews and 502 of them are five stars. That kind of consistency speaks for itself." },
                { icon: Icons.heart, title: "Women-Owned", desc: "A women-owned business built on integrity, community trust, and the kind of service that earns repeat customers." },
                { icon: Icons.clock, title: "24/7 Availability", desc: "Electrical and HVAC emergencies don't follow business hours. Globe Electric is available around the clock." },
                { icon: Icons.wind, title: "Dual-Trade Experts", desc: "Electrical and HVAC under one roof. Authorized Goodman dealer with the expertise to handle both systems." },
              ].map((b, i) => (
      ...
      <section id="reviews" className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--white)" }}>
        <div style={{ maxWidth:800, margin:"0 auto", textAlign:"center" }}>
          ...
          {/* Large featured review -- Michelle Langley (repeat customer loyalty) */}
          <div style={{ position:"relative", padding:"48px 40px", background:"var(--white)", borderRadius:12 }}>
            ...
            <p style={{ fontSize:20, lineHeight:1.8, color:"var(--text)", fontStyle:"italic", fontFamily:"'Playfair Display',serif", marginBottom:28 }}>
              "Seth and his crew have worked on several projects and issues since she bought her home. They are always professional, reliable, and do quality work every single time."
            </p>
            <p style={{ fontSize:17, fontWeight:700, color:"var(--primary)" }}>Michelle Langley</p>
            <p style={{ fontSize:13, color:"var(--text-light)", marginTop:4 }}>Google Review</p>
          </div>
          <p style={{ fontSize:14, color:"var(--text-light)", marginTop:32 }}>One of 502+ five-star reviews on Google</p>
      ...
      <section className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--white)", borderTop:"1px solid var(--border)" }}>
        <div style={{ maxWidth:700, margin:"0 auto", textAlign:"center" }}>
          <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(30px,3.5vw,44px)", lineHeight:1.15, letterSpacing:-0.5, color:"var(--primary)", marginBottom:20 }}>
            Need an electrician?<br/><em style={{ color:"var(--accent)", fontStyle:"italic" }}>Call Globe Electric.</em>
          </h2>
          <p style={{ fontSize:17, lineHeight:1.7, color:"var(--text-mid)", marginBottom:36, maxWidth:520, margin:"0 auto 36px" }}>
            4.9 stars from 518+ customers. Women-owned. Dual-trade electrical and HVAC. Honest pricing, no surprises, and 24/7 availability across central Arkansas.
          </p>
      ...
      <footer style={{ background:"var(--primary)", padding:"80px clamp(24px,5vw,64px) 40px", color:"rgba(255,255,255,0.7)" }}>
        <div className="footer-grid" style={{ maxWidth:1200, margin:"0 auto", display:"grid", gridTemplateColumns:"1.5fr 1fr 1fr", gap:48, marginBottom:48 }}>
          ...
          <div>
            ...
            <p style={{ fontSize:14, lineHeight:1.7, marginBottom:20, maxWidth:300 }}>
              Women-owned electrical and HVAC contractor in North Little Rock, AR. 4.9-star rating from 518+ customers. Authorized Goodman dealer. 24/7 service across central Arkansas.
            </p>
          ...
          <div>
            ...
            <ul style={{ listStyle:"none", display:"flex", flexDirection:"column", gap:10 }}>
              {["Residential Electrical","HVAC Installation & Service","Light Fixture Installation","Circuit Diagnostics","Home Warranty Service","Commercial Electrical"].map((s, i) => (
        ...
        <div style={{ borderTop:"1px solid rgba(255,255,255,0.1)", paddingTop:24 }}>
          <div className="footer-btm-inner" style={{ maxWidth:1200, margin:"0 auto", display:"flex", justifyContent:"space-between", alignItems:"center", fontSize:13, color:"rgba(255,255,255,0.45)" }}>
            <span>&copy; {new Date().getFullYear()} Globe Electric. All rights reserved.</span>
            <span>Open 24 Hours | Women-Owned | Goodman Dealer</span>
          </div>
        </div>
      </footer>
      ...
      <div className="mobile-only" style={{ position:"fixed", bottom:0, left:0, right:0, zIndex:998, display:"none" }}>
        <a href={PHONE_TEL} style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, background:"var(--accent)", color:"#fff", padding:"18px 24px", fontSize:16, fontWeight:700, fontFamily:"'DM Sans',sans-serif", boxShadow:"0 -4px 20px rgba(0,0,0,0.15)", textDecoration:"none" }}>
          {Icons.phone} Call Globe Electric
        </a>
      </div>
