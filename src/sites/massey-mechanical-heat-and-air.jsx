// Layout: A | Industry: HVAC | City: Greenbrier
import { useState, useEffect, useRef } from "react";

const PHONE = "(501) 314-9757";
const TEL = "tel:+15013149757";
...
const Icons = {
  ...
  miniSplit: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="4" width="20" height="8" rx="2"/><path d="M6 12v4"/><path d="M18 12v4"/><path d="M12 12v6"/><path d="M9 18h6"/></svg>,
  ...
  mapPinLg: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0118 0z"/><circle cx="12" cy="10" r="3"/></svg>,
...
const services = [
  { name: "HVAC Repair & Installation", desc: "Complete system diagnostics, repair, and full installation for residential and commercial properties across Greenbrier and Central Arkansas.", icon: Icons.wrench },
  ...
  { name: "New Construction HVAC", desc: "Full HVAC system design and installation for new builds. Proper load calculations and ductwork for maximum efficiency from day one.", icon: Icons.home },
  { name: "Mini-Split Installation", desc: "Ductless mini-split systems for additions, garages, and spaces where traditional ductwork is not practical or cost-effective.", icon: Icons.miniSplit },
  { name: "Maintenance Programs", desc: "Massey Mechanical Club membership with bi-annual system cleaning, priority scheduling, and discounts on all repairs and parts.", icon: Icons.shield },
];
...
const allServiceNames = [
  ...
  "Bi-Annual System Cleaning",
  "Massey Mechanical Club",
  "Rental Property HVAC",
];

const areas = ["Greenbrier", "Conway", "Vilonia", "Cabot", "Jacksonville", "North Little Rock", "Little Rock", "Fairfield Bay"];

const reviews = [
  { text: "Very professional and clean. Tech communicated about everything going on. Thorough explanation of issues.", name: "Tenika Brewster", src: "Google" },
  { text: "Dustin and Neil handled bi-annual clean. Came back to install additional component.", name: "Tiffany Graves", src: "Google" },
  { text: "Grant was awesome \u2014 kind and professional. First time using Massey and now a repeat customer.", name: "Fallon Martin", src: "Google" },
];
...
export default function MasseyMechanicalHeatAndAir() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("massey-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "2026") { sessionStorage.setItem("massey-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "HVAC Service Greenbrier AR | Massey Mechanical";
    const els = [];
    const add = (tag, attrs) => {
    ...
    add("meta", { name: "description", content: "HVAC repair & installation in Greenbrier, AR. Perfect 5.0 stars, 282+ reviews. 24/7 service. Call (501) 314-9757." });
    ...
    add("script", {
      ...
      textContent: JSON.stringify({
        ...
        "@type": "HVACBusiness",
        name: "Massey Mechanical - Heat and Air",
        telephone: "+15013149757",
        email: "alex@masseymech.com",
        address: { "@type": "PostalAddress", streetAddress: "1189 US-65", addressLocality: "Greenbrier", addressRegion: "AR", postalCode: "72058", addressCountry: "US" },
        geo: { "@type": "GeoCoordinates", latitude: 35.2337, longitude: -92.3871 },
        openingHoursSpecification: [{ "@type": "OpeningHoursSpecification", dayOfWeek: ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday", "Sunday"], opens: "00:00", closes: "23:59" }],
        aggregateRating: { "@type": "AggregateRating", ratingValue: "5.0", reviewCount: "282", bestRating: "5" },
        founder: { "@type": "Person", name: "Alex" },
        description: "Massey Mechanical provides HVAC repair, installation, heating, cooling, and maintenance services in Greenbrier, AR and surrounding areas.",
        ...
        memberOf: [{ "@type": "Organization", name: "Massey Mechanical Club" }],
        sameAs: [],
      }),
    });
        ...
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#1C2B41" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, marginBottom: 8, fontWeight: 600, letterSpacing: -0.5 }}>
              <span style={{ color: "#1C2B41" }}>Massey</span>{" "}
              <span style={{ color: "#C8934F" }}>Mechanical</span>
            </div>
    ...
    <>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled ? "rgba(28,43,65,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition: "all 0.4s ease" }}>
        <div className="serif" style={{ fontSize: 22, color: scrolled ? "#fff" : "var(--primary)", fontWeight: 600, letterSpacing: -0.5 }}>
          <span>Massey</span>{" "}
          <span style={{ color: "var(--accent)" }}>Mechanical</span>
        </div>
        ...
        <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 48, alignItems: "center" }}>
          <div>
            ...
            <h1 style={{ fontSize: "clamp(40px,5vw,68px)", lineHeight: 1.05, color: "var(--primary)", letterSpacing: -1.5, marginBottom: 20 }}>
              Greenbrier's<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Most Trusted</span><br />HVAC Team
            </h1>
            <p style={{ fontSize: 18, lineHeight: 1.7, color: "var(--text-mid)", maxWidth: 480, marginBottom: 28 }}>
              150+ years of combined experience. 282 reviews and counting. Open 24/7 for your comfort.
            </p>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 32 }}>
              <Stars />
              <span style={{ fontSize: 14, color: "var(--text-light)", fontStyle: "italic", marginLeft: 4 }}>"Grant was awesome. First time using Massey and now a repeat customer." — Fallon Martin</span>
            </div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 14 }}>
              <a href={TEL} style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 36px", borderRadius: 6, fontSize: 16, fontWeight: 700, boxShadow: "0 4px 20px rgba(200,147,79,0.3)", fontFamily: "'DM Sans',sans-serif", transition: "transform 0.3s ease" }}>{Icons.phone} Call Alex — {PHONE}</a>
      ...
      <section style={{ background: "var(--white)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", padding: "48px clamp(24px,5vw,64px)" }}>
        <div className="trust-row" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 32, textAlign: "center" }}>
          {[
            { num: "5.0", label: "Google Rating", desc: "282+ reviews" },
            { num: "282+", label: "Reviews", desc: "99.3% five-star" },
            { num: "150+", label: "Years Combined", desc: "Team experience" },
      ...
      <section id="services" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "var(--primary)", letterSpacing: -0.5 }}>HVAC Services in Greenbrier</h2>
          </div>
          ...
          <p style={{ textAlign: "center", marginTop: 40, fontSize: 15, color: "var(--text-mid)" }}>
            Questions? Call Alex directly at <a href={TEL} style={{ color: "var(--accent)", fontWeight: 700 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--primary)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <div className="gold-line" style={{ margin: "0 auto 20px" }} />
            <div style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: 3, color: "var(--accent)", marginBottom: 14 }}>Why Massey Mechanical</div>
            <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "#fff", letterSpacing: -0.5 }}>5.0 Stars on Google. 282 Reviews.</h2>
          </div>
          <div className="why-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "start" }}>
            <div>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "rgba(255,255,255,0.82)", marginBottom: 20 }}>
                Alex runs Massey Mechanical with a simple standard: get it right, and treat people well. That approach has earned a perfect 5.0-star rating across 282 Google reviews — 280 of them five stars. That is a 99.3% five-star rate.
              </p>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "rgba(255,255,255,0.82)", marginBottom: 32 }}>
                The team — Grant, Dustin, Neil, Ben, and Justin Walker — brings over 150 years of combined HVAC experience to every job. Grant alone has been named by customers in 7 separate reviews. Gabi keeps scheduling running smoothly. From Greenbrier to Fairfield Bay, Massey Mechanical is the HVAC team Central Arkansas counts on.
              </p>
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {[
                  "Perfect 5.0 Google rating — 99.3% five-star reviews",
                  "150+ years combined team experience",
                  "Massey Mechanical Club membership program",
                  "Open 24 hours — always available",
                  "Expanding service to Fairfield Bay",
                  "HouseCall Pro booking for convenience",
                ].map((c, i) => (
            ...
            <div className="badge-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {[
                { icon: Icons.award, title: "Perfect 5.0", desc: "282 reviews, 99.3% five-star rate on Google" },
                { icon: Icons.users, title: "150+ Years", desc: "Combined experience across the entire team" },
                { icon: Icons.clock, title: "Open 24/7", desc: "Emergency HVAC service day and night" },
                { icon: Icons.mapPinLg, title: "Expanding", desc: "Growing from Greenbrier to Fairfield Bay" },
              ].map((b, i) => (
      ...
      <section style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)", borderTop: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "var(--primary)", letterSpacing: -0.5, lineHeight: 1.15, marginBottom: 20 }}>
            AC not working?<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Call Massey Mechanical.</span>
          </h2>
          <p style={{ fontSize: 17, lineHeight: 1.7, color: "var(--text-mid)", marginBottom: 36, maxWidth: 520, margin: "0 auto 36px" }}>
            Honest pricing. 24/7 availability. A perfect track record across 282 reviews.
          </p>
      ...
      <footer style={{ background: "var(--primary)", padding: "80px clamp(24px,5vw,64px) 40px" }}>
        <div className="footer-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: 48, marginBottom: 48 }}>
          <div>
            <div className="serif" style={{ fontSize: 22, fontWeight: 600, letterSpacing: -0.5, marginBottom: 16 }}>
              <span style={{ color: "#fff" }}>Massey</span>{" "}
              <span style={{ color: "var(--accent)" }}>Mechanical</span>
            </div>
            <p style={{ fontSize: 14, lineHeight: 1.7, color: "rgba(255,255,255,0.6)", marginBottom: 16, maxWidth: 320 }}>
              Professional HVAC repair, installation, heating, and cooling services in Greenbrier, AR and Central Arkansas. Open 24 hours.
            </p>
            <p style={{ fontSize: 14, color: "rgba(255,255,255,0.6)", marginBottom: 6 }}>1189 US-65, Greenbrier, AR 72058</p>
            <a href={TEL} style={{ fontSize: 14, color: "var(--accent)", fontWeight: 700 }}>{PHONE}</a>
            <br />
            <a href="mailto:alex@masseymech.com" style={{ fontSize: 14, color: "rgba(255,255,255,0.6)", marginTop: 4, display: "inline-block" }}>alex@masseymech.com</a>
          </div>
          <div>
        ...
        <div className="footer-btm" style={{ maxWidth: 1200, margin: "0 auto", borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 24, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>&copy; {yr} Massey Mechanical - Heat and Air. All rights reserved.</p>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>Massey Mechanical Club Member</p>
        </div>
      </footer>
      ...
      <div className="mobile-only" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 998, display: "none" }}>
        <a href={TEL} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "16px 20px", fontSize: 16, fontWeight: 700, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 -4px 20px rgba(0,0,0,0.15)", textDecoration: "none" }}>
          {Icons.phone} Call Massey Mechanical — {PHONE}
        </a>
      </div>
