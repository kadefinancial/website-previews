// Layout: D | Industry: Landscaping | City: Little Rock
import { useState, useEffect, useRef } from "react";

const PHONE = "(501) 868-7744";
const TEL = "tel:+15018687744";

const Icons = {
  phone: <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 01-2.18 2 19.79 19.79 0 01-8.63-3.07 19.5 19.5 0 01-6-6 19.79 19.79 0 01-3.07-8.67A2 2 0 014.11 2h3a2 2 0 012 1.72c.127.96.361 1.903.7 2.81a2 2 0 01-.45 2.11L8.09 9.91a16 16 0 006 6l1.27-1.27a2 2 0 012.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0122 16.92z"/></svg>,
  sprinkler: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22V8"/><path d="M12 8c-1-3-4-5-4-5"/><path d="M12 8c1-3 4-5 4-5"/><path d="M6 14c-2-1-4 0-4 0"/><path d="M18 14c2-1 4 0 4 0"/><path d="M9 11c-2-2-5-1-5-1"/><path d="M15 11c2-2 5-1 5-1"/><path d="M8 17c-3 0-5 1-5 1"/><path d="M16 17c3 0 5 1 5 1"/></svg>,
  ...
  lightbulb: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18h6"/><path d="M10 22h4"/><path d="M12 2a7 7 0 00-4 12.7V17h8v-2.3A7 7 0 0012 2z"/></svg>,
  valve: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="4"/><path d="M12 2v4"/><path d="M12 18v4"/><path d="M4.93 4.93l2.83 2.83"/><path d="M16.24 16.24l2.83 2.83"/><path d="M2 12h4"/><path d="M18 12h4"/></svg>,
...
const services = [
  { name: "Sprinkler System Repair", desc: "Complete diagnosis and repair of broken sprinkler systems. From controller issues to line breaks, we get your system running right the first time.", icon: Icons.sprinkler },
  { name: "Sprinkler Maintenance", desc: "Seasonal tune-ups, winterization, and spring startups to keep your irrigation system performing efficiently year-round.", icon: Icons.wrench },
  { name: "Landscape Lighting", desc: "Professional landscape lighting design and installation to highlight your home's best features and improve curb appeal after dark.", icon: Icons.lightbulb },
  { name: "Valve & Solenoid Repair", desc: "Expert repair and replacement of faulty valves and solenoids that control water flow to each zone of your sprinkler system.", icon: Icons.valve },
  { name: "Leak Detection", desc: "Precision leak detection to find hidden water loss in your irrigation lines. We pinpoint the problem and fix it fast to save water and money.", icon: Icons.droplet },
  { name: "Sprinkler Head Replacement", desc: "Broken, clogged, or misaligned sprinkler heads replaced with the right match for proper coverage across every zone of your yard.", icon: Icons.target },
];

const allServiceNames = ["Sprinkler System Repair", "Sprinkler System Maintenance", "Landscape Lighting Installation", "Valve & Solenoid Repair", "Leak Detection & Repair", "Sprinkler Head Replacement"];

const areas = ["Little Rock", "West Little Rock", "Benton", "Bryant", "North Little Rock", "Sherwood", "Maumelle", "Saline County"];

const reviews = [
  { text: "Matt was able to squeeze us into the afternoon but did not rush through the service or repair.", name: "Steven Petersen", src: "Google" },
  { text: "Matt and his assistant were just out. They really took good care of my 20 year-old sprinkler system.", name: "Gunnar Gibson", src: "Google" },
  { text: "Matt and his team were booked over a week out but he managed to add us at the end of the day.", name: "Saundra Sorenson", src: "Google" },
];
...
const trustStats = [
  ...
  { num: "84+", label: "Reviews", desc: "81 of 84 are 5-star" },
  { num: "BBB", label: "Listed", desc: "Better Business Bureau" },
  { num: "#1", label: "Fair Pricing", desc: "Top review keyword" },
];

export default function CollinsSprinklerRepairAndLandscapeLighting() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("collins-sprinkler-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if (pw === "2026") { sessionStorage.setItem("collins-sprinkler-auth", "1"); setAuthed(true); } else { setPwBad(true); setPw(""); } };
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Sprinkler Repair Little Rock AR | Collins Sprinkler";
    const els = [];
    ...
    add("meta", { name: "description", content: "Sprinkler repair & landscape lighting in Little Rock, AR. 4.9 stars, 84 reviews. BBB listed. Call (501) 868-7744." });
    ...
    add("script", {
      type: "application/ld+json", textContent: JSON.stringify({
        ...
        name: "Collins Sprinkler Repair & Landscape Lighting Inc.", telephone: "+15018687744",
        address: { "@type": "PostalAddress", streetAddress: "16506 Lamarche Dr", addressLocality: "Little Rock", addressRegion: "AR", postalCode: "72223", addressCountry: "US" },
        ...
        founder: { "@type": "Person", name: "James Collins" },
        description: "Collins Sprinkler Repair & Landscape Lighting Inc. provides sprinkler system repair, maintenance, landscape lighting, and irrigation services in Little Rock, AR. 4.9 stars with 84 Google reviews. BBB listed.",
        areaServed: areas.map(a => ({ "@type": "City", name: a })),
        hasOfferCatalog: { "@type": "OfferCatalog", name: "Sprinkler & Lighting Services", itemListElement: allServiceNames.map(s => ({ "@type": "Offer", itemOffered: { "@type": "Service", name: s } })) },
    ...
    return (<>
      ...
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#1E3A2F" }}>
        <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
          <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, marginBottom: 8, fontWeight: 600, letterSpacing: -0.5 }}><span style={{ color: "#1E3A2F" }}>Collins</span>{" "}<span style={{ color: "#B8694A" }}>Sprinkler</span></div>
  ...
  return (<>
    ...
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled ? "rgba(30,58,47,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition: "all 0.4s ease" }}>
      <div className="serif" style={{ fontSize: 22, color: scrolled ? "#fff" : "var(--primary)", fontWeight: 600, letterSpacing: -0.5 }}><span>Collins</span>{" "}<span style={{ color: "var(--accent)" }}>Sprinkler</span></div>
      ...
      <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1.8fr 1fr", gap: 48, alignItems: "center" }}>
        <div>
          <h1 style={{ fontSize: "clamp(48px, 6vw, 84px)", lineHeight: 1.0, color: "var(--primary)", letterSpacing: -2, marginBottom: 24 }}>
            Little Rock's<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Sprinkler Repair</span><br />Experts
          </h1>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 32 }}>
            <Stars />
            <span style={{ fontSize: 14, color: "var(--text-light)", fontStyle: "italic", marginLeft: 4 }}>"They really took good care of my 20 year-old sprinkler system." — Gunnar Gibson</span>
          </div>
          <a href={TEL} style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 36px", borderRadius: 6, fontSize: 16, fontWeight: 700, boxShadow: "0 4px 20px rgba(184,105,74,0.3)", fontFamily: "'DM Sans',sans-serif" }}>{Icons.phone} Call Collins — {PHONE}</a>
        </div>
        <div>
    ...
    <section id="services" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 64 }}>
          ...
          <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "var(--primary)", letterSpacing: -0.5 }}>Sprinkler & Lighting Services in Little Rock</h2>
        </div>
        {/* Mobile trust */}
        ...
        <p style={{ textAlign: "center", marginTop: 48, fontSize: 15, color: "var(--text-light)" }}>Questions? Call James directly at <a href={TEL} style={{ color: "var(--accent)", fontWeight: 600 }}>{PHONE}</a></p>
      </div>
    </section>
    ...
    <section id="about" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--primary)", position: "relative", overflow: "hidden" }}>
      <div className="serif" style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", fontSize: "clamp(120px,20vw,280px)", color: "var(--accent)", opacity: 0.08, lineHeight: 1, whiteSpace: "nowrap", pointerEvents: "none" }}>84</div>
      <div style={{ maxWidth: 800, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <div className="gold-line" style={{ margin: "0 auto 20px" }} />
        <div style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: 3, color: "var(--accent)", marginBottom: 14, textAlign: "center", opacity: 0.7 }}>About Collins Sprinkler</div>
        <h2 style={{ fontSize: "clamp(30px,3.5vw,42px)", lineHeight: 1.15, marginBottom: 28, color: "#fff", textAlign: "center" }}>84 Reviews. 4.9 Stars.<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Pure Word of Mouth.</span></h2>
        <p style={{ fontSize: 16, lineHeight: 1.8, color: "rgba(255,255,255,0.6)", marginBottom: 24, textAlign: "center" }}>
          Collins Sprinkler Repair & Landscape Lighting Inc. is owned by James Collins and has been serving the Little Rock area for years. With zero online presence beyond Google Maps, every one of their 84 reviews came from genuine word-of-mouth recommendations — homeowners telling neighbors, friends telling friends. That kind of reputation cannot be bought.
        </p>
        <p style={{ fontSize: 16, lineHeight: 1.8, color: "rgba(255,255,255,0.6)", marginBottom: 36, textAlign: "center" }}>
          Matt is the primary technician and the name praised in nearly every review. Customers consistently highlight his thoroughness, fairness, and willingness to work people into a packed schedule. With 81 of 84 reviews at five stars and a BBB listing backing them up, Collins Sprinkler has quietly built one of the strongest reputations in Central Arkansas irrigation — one yard at a time.
        </p>
        <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 20 }}>
          {["BBB Listed Business", "4.9 Google Rating", "84 Customer Reviews", "81 Five-Star Reviews", "Mon-Fri 8AM-5PM", "Serving Central Arkansas"].map((c, i) => (
    ...
    <section id="areas" style={{ padding: "100px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
      <div style={{ maxWidth: 900, margin: "0 auto", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 12 }}>
          {areas.map((a, i) => (
            <span key={i} title={`Sprinkler repair in ${a}, AR`} style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "#fff", border: "1px solid var(--border)", padding: "12px 24px", borderRadius: 30, fontSize: 15, fontWeight: 500, color: "var(--text)", cursor: "default", transition: "all 0.25s" }}
    ...
    <section style={{ padding: "100px clamp(24px,5vw,64px)", background: "var(--white)", textAlign: "center", borderTop: "1px solid var(--border)" }}>
      <div style={{ maxWidth: 600, margin: "0 auto" }}>
        <h2 className="serif" style={{ fontSize: "clamp(32px,4vw,50px)", color: "var(--primary)", lineHeight: 1.1, marginBottom: 20, letterSpacing: -1 }}>Need sprinkler repair?<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Call Collins.</span></h2>
        <p style={{ fontSize: 17, color: "var(--text-mid)", marginBottom: 36, lineHeight: 1.6 }}>4.9 stars. 84 reviews. BBB listed. Fair pricing every time.</p>
    ...
    <footer style={{ background: "var(--primary)", color: "rgba(255,255,255,0.7)", padding: "64px clamp(24px,5vw,64px) 0" }}>
      <div className="footer-grid" style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "1.5fr 1fr 1fr", gap: 48, paddingBottom: 40 }}>
        <div>
          <div className="serif" style={{ fontSize: 22, color: "#fff", marginBottom: 16, fontWeight: 600, letterSpacing: -0.5 }}><span>Collins</span>{" "}<span style={{ color: "var(--accent)" }}>Sprinkler</span></div>
          <p style={{ fontSize: 14, lineHeight: 1.7, opacity: 0.7, marginBottom: 16 }}>Sprinkler repair and landscape lighting for Little Rock and Central Arkansas. 4.9 Google stars with 84 reviews. BBB listed. Owner: James Collins.</p>
          <p style={{ fontSize: 14, lineHeight: 1.8, opacity: 0.6 }}>16506 Lamarche Dr<br />Little Rock, AR 72223<br /><a href={TEL} style={{ color: "var(--accent)", fontWeight: 600 }}>{PHONE}</a></p>
        </div>
        <div>
      ...
      <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "20px 0", maxWidth: 1100, margin: "0 auto" }}>
        <div className="footer-btm-inner" style={{ display: "flex", justifyContent: "space-between", fontSize: 13, opacity: 0.4, flexWrap: "wrap", gap: 8 }}>
          <span>&copy; {yr} Collins Sprinkler Repair & Landscape Lighting Inc. All rights reserved.</span>
    ...
    <div className="mobile-only" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 998, display: "flex" }}>
      <a href={TEL} style={{ display: "flex", width: "100%", alignItems: "center", justifyContent: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px", fontSize: 17, fontWeight: 700, boxShadow: "0 -4px 20px rgba(0,0,0,0.15)", fontFamily: "'DM Sans',sans-serif" }}>{Icons.phone} Call Collins Sprinkler</a>
    </div>
