// Layout: D | Industry: Plumber | City: Hartford
import { useState, useEffect, useRef } from "react";

const PHONE = "(479) 639-2007";
const TEL = "tel:+14796392007";
...
const Icons = {
  ...
  fixture: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v6"/><path d="M8 8h8"/><path d="M6 8c0 4-2 6-2 10a4 4 0 008 0"/><path d="M18 8c0 4 2 6 2 10a4 4 0 01-8 0"/></svg>,
...
const services = [
  { name: "Residential Plumbing Repair", desc: "From leaky faucets to broken water lines, Sean and Keith handle every residential repair with precision and care. Fast diagnosis, lasting fixes.", icon: Icons.wrench },
  { name: "General Plumbing Services", desc: "Complete plumbing solutions for homes and businesses across the River Valley. Professional service from a team that treats every job like it's their own.", icon: Icons.droplets },
  { name: "Emergency Plumbing", desc: "When a pipe bursts or a water heater fails at night, Circle W responds. Call anytime for fast, reliable emergency plumbing service.", icon: Icons.zap },
  { name: "New Construction Plumbing", desc: "Expert plumbing installation for new builds. Code-compliant rough-ins and finish work that builders and homeowners trust.", icon: Icons.construction },
  { name: "Pipe Repair", desc: "Professional pipe repair and replacement for damaged, corroded, or aging plumbing. Quality materials and workmanship that stands the test of time.", icon: Icons.pipe },
  { name: "Fixture Installation", desc: "Upgrading your kitchen or bathroom? We install faucets, toilets, sinks, water heaters, and more with clean, reliable craftsmanship.", icon: Icons.fixture },
];

const serviceNames = ["Residential Plumbing Repair","General Plumbing Services","Emergency Plumbing","New Construction Plumbing","Pipe Repair","Fixture Installation"];

const areas = ["Hartford","Fort Smith","Van Buren","Alma","Greenwood","Barling","River Valley","Western Arkansas"];

const reviews = [
  { text: "Keith to the Rescue! He came to my home in the evening to fix a hot water heater issue. He was very professional and knowledgeable. I would highly recommend Circle W Plumbing!", name: "Lorraine Caldwell", src: "Google" },
  { text: "Sean and Keith are both great guys. Very professional and knowledgeable. They do great work at a fair price. I highly recommend them.", name: "Patricia Weaver", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function CircleWPlumbing() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("circle-w-plumbing-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="roof"){sessionStorage.setItem("circle-w-plumbing-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "Plumber Hartford AR | Circle W Plumbing";
    const els = [];
    ...
    add("meta",{name:"description",content:"Circle W Plumbing — 5-star plumber serving Hartford AR & the River Valley. Sean & Keith deliver expert service. Call (479) 639-2007."});
    add("meta",{name:"geo.region",content:"US-AR"});
    add("meta",{name:"geo.placename",content:"Hartford"});
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      "@context":"https://schema.org","@type":"Plumber",
      name:"Circle W Plumbing",telephone:"+14796392007",
      address:{"@type":"PostalAddress",streetAddress:"10211 AR-96",addressLocality:"Hartford",addressRegion:"AR",postalCode:"72938",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:35.3934,longitude:-94.3651},
      openingHoursSpecification:[],
      aggregateRating:{"@type":"AggregateRating",ratingValue:"5.0",reviewCount:"7",bestRating:"5"},
      founder:{"@type":"Person",name:"Sean Wright",jobTitle:"Owner"},
      description:"Circle W Plumbing is a 5-star rated plumbing company serving Hartford, AR and the River Valley. Specializing in residential plumbing repair, emergency service, and new construction plumbing.",
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1C2B41"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8}}>
            <span style={{fontWeight:700,color:"#1C2B41"}}>Circle W</span> <span style={{color:"#C8934F"}}>Plumbing</span>
          </div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(28,43,65,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease",boxShadow:scrolled?"0 1px 0 rgba(255,255,255,0.06)":"none"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}>
        <span>Circle W</span> <span style={{color:"var(--accent)"}}>Plumbing</span>
      </div>
      ...
      <div className="hero-grid" style={{maxWidth:1200,width:"100%",display:"grid",gridTemplateColumns:"1.8fr 1fr",gap:48,alignItems:"center"}}>
        <div>
          <h1 style={{fontSize:"clamp(48px, 6vw, 84px)",lineHeight:1.0,color:"var(--primary)",letterSpacing:-2,marginBottom:24}}>
            Hartford's<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Perfect-Rated</span><br/>Plumber
          </h1>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:32}}>
            <Stars />
            <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"Keith to the Rescue!" — Lorraine Caldwell</span>
          </div>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(200,147,79,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Sean — {PHONE}</a>
        </div>
        <div>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",color:"var(--primary)",letterSpacing:-0.5}}>Plumbing Services in Hartford, AR</h2>
        </div>
        ...
        <div className="trust-mobile" style={{display:"none",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:48,textAlign:"center"}}>
          {[
            ...
            {num:"7+",label:"5-Star Reviews",desc:"100% satisfied"},
            {num:"Sean+Keith",label:"Expert Team",desc:"Professional crew"},
            {num:"BBB",label:"Listed Business",desc:"Verified & trusted"},
          ].map((s,i)=>(
            <div key={i}>
              <div className="serif" style={{fontSize:i===2?"clamp(24px,3vw,32px)":"clamp(32px,4vw,44px)",color:"var(--primary)",lineHeight:1,marginBottom:6,letterSpacing:-1}}>{s.num}</div>
          ...
          <div className="trust-sidebar" style={{display:"flex",flexDirection:"column",gap:32,paddingTop:8}}>
            {[
              ...
              <div key={i} style={{borderLeft:"4px solid var(--accent)",paddingLeft:20}}>
                <div className="serif" style={{fontSize:i===2?24:36,color:"var(--primary)",lineHeight:1,marginBottom:4,letterSpacing:-1}}>{s.num}</div>
                <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:2,color:"var(--text)",marginBottom:2}}>{s.label}</div>
                <div style={{fontSize:13,color:"var(--text-light)"}}>{s.desc}</div>
              </div>
            ))}
        ...
        <p style={{textAlign:"center",marginTop:48,fontSize:15,color:"var(--text-light)"}}>
          Questions? Call Sean directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a>
        </p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{maxWidth:800,margin:"0 auto",position:"relative",zIndex:1}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,textAlign:"center",opacity:0.7}}>About Circle W Plumbing</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff",textAlign:"center"}}>Small Town Roots,<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Perfect Reputation</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:24,textAlign:"center"}}>
          Circle W Plumbing is what happens when good people do honest work. Based in Hartford, Arkansas — a small town where your name is your reputation — Sean Wright and his right-hand man Keith have built something rare: a plumbing company with a perfect 5.0-star Google rating. Every single customer who has reviewed them gave five stars. That doesn't happen by accident.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:36,textAlign:"center"}}>
          Whether it's an evening emergency with a failing water heater or a new construction rough-in, Sean and Keith show up with the same professionalism and knowledge every time. They serve the entire River Valley — from Fort Smith and Van Buren to Alma and Greenwood — bringing small-town integrity to every job. Fair prices, expert work, and the kind of service that makes customers say "I highly recommend them" without being asked.
        </p>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:20}}>
          {["Perfect 5.0-Star Google Rating","BBB Listed Business","Evening & Emergency Availability","Serving the Entire River Valley","Professional & Knowledgeable Team","Fair & Honest Pricing"].map((c,i)=>(
    ...
    <section id="areas" style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        ...
        <h2 style={{fontSize:"clamp(28px,3vw,38px)",color:"var(--primary)",marginBottom:48,letterSpacing:-0.5}}>Serving Hartford & the River Valley</h2>
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>
          Got a plumbing problem?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Sean.</span>
        </h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>
          A perfect 5.0-star rating, a BBB-listed business, and a two-man team that treats every home like their own. Fair pricing, every time.
        </p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16}}><span>Circle W</span> <span style={{color:"var(--accent)"}}>Plumbing</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>Hartford's perfect-rated plumber serving the entire River Valley. Sean and Keith deliver professional, knowledgeable plumbing service at a fair price.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>10211 AR-96<br/>Hartford, AR 72938<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} Circle W Plumbing. All rights reserved.</span>
          <span>BBB Listed Business</span>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Circle W Plumbing</a>
    </div>
