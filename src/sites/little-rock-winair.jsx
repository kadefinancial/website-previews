...
const PHONE = "(501) 562-8254";
const TEL = "tel:+15015628254";
...
const Icons = {
  ...
  snowflake: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="2" x2="12" y2="22"/><path d="M20 12l-4-4M20 12l-4 4M4 12l4-4M4 12l4 4"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2l4 4M12 2l-4 4M12 22l4-4M12 22l-4-4"/></svg>,
...
const services = [
  { name: "HVAC Equipment Sales", desc: "Complete residential and commercial HVAC systems from leading manufacturers. Winair stocks the units contractors need and helps homeowners find the right equipment for any job.", icon: Icons.wind },
  { name: "Air Conditioning Parts & Supplies", desc: "Capacitors, contactors, fan motors, refrigerant, copper fittings, and everything else you need to keep AC systems running. In-stock and ready to go at the counter.", icon: Icons.snowflake },
  { name: "Heating Equipment & Parts", desc: "Furnace components, heat exchangers, ignitors, gas valves, and complete heating units. Knowledgeable staff help you find the exact part the first time.", icon: Icons.flame },
  { name: "Refrigeration Supplies", desc: "Commercial refrigeration parts, controls, and supplies for restaurants, convenience stores, and food service operations throughout Central Arkansas.", icon: Icons.thermometer },
  { name: "HVAC Tools & Accessories", desc: "Professional-grade tools, gauges, vacuum pumps, and installation accessories. Everything HVAC technicians need to get the job done right in the field.", icon: Icons.wrench },
  { name: "Material Delivery to Job Sites", desc: "Need equipment and parts delivered directly to the job site? Winair runs deliveries so contractors can stay on the clock instead of making supply runs.", icon: Icons.truck },
];

const productCategories = ["HVAC Equipment","AC Parts & Supplies","Heating Parts","Refrigeration Supplies","Tools & Accessories","Delivery Request","Other / Not Sure"];

const areas = ["Little Rock","Central Arkansas"];

const reviews = [
  { text: "Very helpful staff. Called about a part and they had it waiting for me when I got there. They offered tips on the install and were very polite. Will be going back tomorrow for another part.", name: "Brandon Matsubara", src: "Google" },
  { text: "Very good attention and you always find what you are looking for.", name: "Ezequias Cortez", src: "Google" },
  { text: "Very friendly. They knew what I needed and didn't try to sell me a part that I didn't need and the prices were reasonable. Also they were OPEN SATURDAYS!!", name: "Gary Johnson", src: "Google" },
];

const navLinks = [{label:"Products",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function LittleRockWinair() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("lrw-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="roof"){sessionStorage.setItem("lrw-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "HVAC Supply Little Rock AR | Little Rock Winair";
    const els = [];
    ...
    add("meta",{name:"description",content:"HVAC parts, equipment & supplies in Little Rock, AR. 4.5 stars, 40+ years. Open Saturdays. Call (501) 562-8254."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"Little Rock Winair",telephone:"+15015628254",
      address:{"@type":"PostalAddress",streetAddress:"6621 Geyer Springs Rd G",addressLocality:"Little Rock",addressRegion:"AR",postalCode:"72209",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:34.6870,longitude:-92.3340},
      openingHoursSpecification:[
        {"@type":"OpeningHoursSpecification",dayOfWeek:["Monday","Tuesday","Wednesday","Thursday","Friday"],opens:"07:00",closes:"17:00"},
        {"@type":"OpeningHoursSpecification",dayOfWeek:["Saturday"],opens:"08:00",closes:"12:00"}
      ],
      aggregateRating:{"@type":"AggregateRating",ratingValue:"4.5",reviewCount:"24",bestRating:"5"},
      foundingDate:"1981",
      description:"Little Rock Winair is a wholesale HVAC distributor and supply house serving contractors and homeowners in Little Rock and Central Arkansas for over 40 years. Equipment sales, parts, supplies, tools, and job-site delivery.",
      areaServed:areas.map(a=>({"@type":"City",name:a})),
      hasOfferCatalog:{"@type":"OfferCatalog",name:"HVAC Products & Services",itemListElement:productCategories.map(s=>({"@type":"Offer",itemOffered:{"@type":"Service",name:s}}))},
      memberOf:[],sameAs:[],
    })});
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1C2B41"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8,fontWeight:600,letterSpacing:-0.5}}><span style={{color:"#1C2B41"}}>Little Rock</span> <span style={{color:"#C8934F"}}>Winair</span></div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(28,43,65,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5,cursor:"pointer"}} onClick={()=>window.scrollTo({top:0,behavior:"smooth"})}><span>Little Rock</span> <span style={{color:"var(--accent)"}}>Winair</span></div>
    ...
    <section style={{padding:"180px clamp(24px,5vw,64px) 100px",background:"var(--cream)",textAlign:"center"}}>
      <div style={{maxWidth:800,margin:"0 auto"}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(200,147,79,0.1)",padding:"8px 18px",borderRadius:4,marginBottom:28}}>
          ...
          <span style={{fontSize:12,fontWeight:700,color:"var(--accent)",letterSpacing:2,textTransform:"uppercase"}}>4.5 Stars on Google</span>
        </div>
        <h1 style={{fontSize:"clamp(44px, 6vw, 80px)",lineHeight:1.05,color:"var(--primary)",marginBottom:24,letterSpacing:-2}}>
          Little Rock's HVAC<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Supply Headquarters</span>
        </h1>
        <p style={{fontSize:18,lineHeight:1.7,color:"var(--text-mid)",marginBottom:32,maxWidth:600,margin:"0 auto 32px"}}>
          Your wholesale HVAC distributor for over 40 years. Equipment, parts, tools, and supplies for contractors and homeowners -- with local expertise and national buying power.
        </p>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:36}}>
          <Stars />
          <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"Very helpful staff. They had my part waiting when I got there." -- Brandon Matsubara</span>
        </div>
        <div className="hero-btns" style={{display:"flex",gap:16,justifyContent:"center",flexWrap:"wrap"}}>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,fontFamily:"'DM Sans',sans-serif",boxShadow:"0 4px 20px rgba(200,147,79,0.3)",transition:"transform 0.2s"}}>{Icons.phone} Call for Parts</a>
          <button onClick={()=>go("form")} style={{display:"inline-flex",alignItems:"center",gap:10,background:"transparent",color:"var(--primary)",padding:"18px 32px",borderRadius:6,fontSize:16,fontWeight:700,fontFamily:"'DM Sans',sans-serif",border:"1.5px solid var(--border)",cursor:"pointer",transition:"all 0.2s"}}>Check Availability</button>
        </div>
      </div>
    </section>
    ...
    <section id="form" style={{padding:"80px clamp(24px,5vw,64px)",background:"var(--white)",borderTop:"1px solid var(--border)",borderBottom:"1px solid var(--border)"}}>
      <div style={{maxWidth:1200,margin:"0 auto",display:"flex",flexDirection:"column",alignItems:"center"}}>
        ...
        <div style={{background:"var(--white)",border:"1px solid var(--border)",borderRadius:12,padding:"44px 36px",maxWidth:480,width:"100%",boxShadow:"0 4px 40px rgba(0,0,0,0.08)"}}>
          {!formDone ? (<>
            <h2 style={{fontSize:"clamp(24px,3vw,32px)",color:"var(--primary)",textAlign:"center",marginBottom:6}}>Request a Quote</h2>
            <p style={{fontSize:14,color:"var(--text-light)",textAlign:"center",marginBottom:28}}>Tell us what you need -- we'll check stock and pricing.</p>
            <form onSubmit={e=>{e.preventDefault();setFormDone(true)}} style={{display:"flex",flexDirection:"column",gap:14}}>
              ...
              <select required value={formData.service} onChange={e=>setFormData({...formData,service:e.target.value})} style={{padding:"14px 16px",borderRadius:8,border:"1px solid var(--border)",background:"var(--cream)",fontSize:15,fontFamily:"'DM Sans',sans-serif",outline:"none",color:formData.service?"var(--text)":"var(--text-light)"}}>
                ...
                {productCategories.map(s=><option key={s} value={s}>{s}</option>)}
              </select>
              <button type="submit" style={{padding:"16px",borderRadius:8,background:"var(--primary)",color:"#fff",fontSize:16,fontWeight:700,fontFamily:"'DM Sans',sans-serif",border:"none",cursor:"pointer",marginTop:4}}>Check Availability</button>
            </form>
          </>) : (
            <div style={{textAlign:"center",padding:"20px 0"}}>
              ...
              <p style={{fontSize:14,color:"var(--text-mid)",lineHeight:1.6}}>We'll check stock and get back to you shortly. Need it now? Call <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
            </div>
          )}
        </div>
        ...
        <div className="trust-row" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:32,marginTop:56,maxWidth:800,width:"100%",textAlign:"center"}}>
          {[
            {num:"4.5",label:"Google Rating",desc:"Based on 24 reviews"},
            {num:"40+",label:"Years Serving Arkansas",desc:"Established ~1981"},
            {num:"24",label:"Customer Reviews",desc:"Verified on Google"},
            {num:"Sat",label:"Open Saturdays",desc:"8 AM - 12 PM"},
          ].map((s,i)=>(
            <div key={i}>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <p style={{fontSize:12,fontWeight:700,letterSpacing:3,textTransform:"uppercase",color:"var(--accent)",marginBottom:14}}>What We Carry</p>
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",lineHeight:1.15,color:"var(--primary)",letterSpacing:-0.5}}>Products & Services</h2>
        </div>
        ...
        <p style={{textAlign:"center",marginTop:48,fontSize:15,color:"var(--text-mid)"}}>
          Questions? Call the Winair counter directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a>
        </p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)"}}>
      <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <p style={{fontSize:12,fontWeight:700,letterSpacing:3,textTransform:"uppercase",color:"var(--accent)",marginBottom:14}}>Why Winair</p>
        <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",lineHeight:1.15,color:"#fff",letterSpacing:-0.5,marginBottom:40}}>Four Decades at the Counter</h2>
        <p style={{fontSize:17,lineHeight:1.8,color:"rgba(255,255,255,0.75)",marginBottom:24}}>
          Little Rock Winair has been Central Arkansas's go-to HVAC supply house since the early 1980s. For over 40 years, contractors and homeowners have walked through the doors on Geyer Springs Road knowing they will find the right part, the right advice, and a fair price. Under the longtime leadership of president Jerry Kennedy -- who retired in April 2024 after 43 years of service -- Winair built a reputation that every competitor in the state respects.
        </p>
        <p style={{fontSize:17,lineHeight:1.8,color:"rgba(255,255,255,0.75)",marginBottom:48}}>
          The philosophy is simple: local ownership, national buying power. That means competitive pricing on name-brand HVAC equipment without sacrificing the personal service a local distributor provides. The counter staff knows the product inside and out, will never oversell you a part you do not need, and will even have your order waiting when you walk in. Open Saturdays, too -- because HVAC does not take weekends off.
        </p>
        <div className="why-creds-row" style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:24}}>
          {["40+ Year Legacy Distributor","Local Ownership, National Buying Power","Open Saturdays 8 AM - 12 PM","Job-Site Material Delivery","Fair Pricing -- No Upselling","Knowledgeable Counter Staff"].map((c,i)=>(
    ...
    <section id="reviews" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--white)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        ...
        <p style={{textAlign:"center",marginTop:48,fontSize:15,color:"var(--text-mid)"}}>
          Ready to see why contractors trust Winair? Call <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a>
        </p>
      </div>
    </section>
    ...
    <section id="areas" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto",textAlign:"center"}}>
        ...
        <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",lineHeight:1.15,color:"var(--primary)",letterSpacing:-0.5,marginBottom:56}}>Where We Serve</h2>
        <div style={{display:"flex",flexWrap:"wrap",gap:12,justifyContent:"center"}}>
          {areas.map((a,i)=>(
            <span key={i} title={`HVAC supply in ${a}, AR`} style={{display:"inline-flex",alignItems:"center",gap:6,padding:"10px 20px",borderRadius:100,border:"1px solid var(--border)",background:"var(--white)",fontSize:14,fontWeight:500,color:"var(--text-mid)",cursor:"default",transition:"all 0.2s"}}
    ...
    <section style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--white)",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
        <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",lineHeight:1.15,color:"var(--primary)",letterSpacing:-0.5,marginBottom:20}}>
          Need a part today?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Winair.</span>
        </h2>
        <p style={{fontSize:17,lineHeight:1.7,color:"var(--text-mid)",marginBottom:36}}>
          Over 40 years of HVAC supply expertise. Fair prices, knowledgeable staff, and we are open Saturdays. Stop by the counter at 6621 Geyer Springs Rd or give us a call.
        </p>
    ...
    <footer className="mob-pad" style={{padding:"80px clamp(24px,5vw,64px) 40px",background:"var(--primary)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div className="footer-grid" style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:48,marginBottom:48}}>
          <div>
            <div className="serif" style={{fontSize:22,fontWeight:600,letterSpacing:-0.5,marginBottom:16}}><span style={{color:"#fff"}}>Little Rock</span> <span style={{color:"var(--accent)"}}>Winair</span></div>
            <p style={{fontSize:14,lineHeight:1.7,color:"rgba(255,255,255,0.6)",marginBottom:16,maxWidth:320}}>Wholesale HVAC distributor serving contractors and homeowners in Little Rock and Central Arkansas for over 40 years. Local ownership. National buying power.</p>
            <p style={{fontSize:14,color:"rgba(255,255,255,0.6)",marginBottom:6}}>6621 Geyer Springs Rd G, Little Rock, AR 72209</p>
            <a href={TEL} style={{fontSize:14,color:"var(--accent)",fontWeight:600}}>{PHONE}</a>
            <p style={{fontSize:13,color:"rgba(255,255,255,0.4)",marginTop:8,display:"flex",alignItems:"center",gap:6}}><span style={{color:"var(--accent)",display:"inline-flex"}}>{Icons.clock}</span> Mon-Fri 7AM-5PM | Sat 8AM-12PM</p>
          </div>
          <div>
            <h4 style={{fontSize:14,fontWeight:700,color:"#fff",marginBottom:16,textTransform:"uppercase",letterSpacing:1}}>Products</h4>
            {["HVAC Equipment Sales","AC Parts & Supplies","Heating Equipment & Parts","Refrigeration Supplies","HVAC Tools & Accessories","Job-Site Delivery"].map((s,i)=><p key={i} style={{fontSize:13,color:"rgba(255,255,255,0.55)",marginBottom:8,lineHeight:1.5}}>{s}</p>)}
          </div>
          <div>
        ...
        <div style={{borderTop:"1px solid rgba(255,255,255,0.1)",paddingTop:24}}>
          <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <p style={{fontSize:13,color:"rgba(255,255,255,0.35)"}}>{"\u00A9"} {yr} Little Rock Winair. All rights reserved.</p>
            <p style={{fontSize:13,color:"rgba(255,255,255,0.35)"}}>Serving Arkansas HVAC Contractors Since 1981</p>
          </div>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,width:"100%",padding:"16px",background:"var(--accent)",color:"#fff",fontSize:16,fontWeight:700,fontFamily:"'DM Sans',sans-serif",boxShadow:"0 -4px 20px rgba(0,0,0,0.15)"}}>{Icons.phone} Call Winair Now</a>
    </div>
  </>);
}