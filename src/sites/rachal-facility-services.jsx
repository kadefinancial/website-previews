...
const PHONE = "(501) 350-6366";
const TEL = "tel:+15013506366";
...
const Icons = {
  ...
  building: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="2" width="16" height="20" rx="2"/><path d="M9 22v-4h6v4"/><path d="M8 6h.01"/><path d="M16 6h.01"/><path d="M8 10h.01"/><path d="M16 10h.01"/><path d="M8 14h.01"/><path d="M16 14h.01"/></svg>,
  ...
  toilet: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M7 12h10a5 5 0 01-10 0z"/><path d="M6 12V4a2 2 0 012-2h1"/><path d="M8 17l-1.5 4h11L16 17"/><path d="M10 21h4"/></svg>,
...
const services = [
  { name: "Residential Plumbing", desc: "Complete plumbing solutions for your home — from faucet repairs and fixture replacements to whole-house repiping. Chris handles every job personally with care and precision.", icon: Icons.home },
  { name: "Commercial Plumbing", desc: "Reliable plumbing service for offices, retail spaces, and commercial properties throughout Central Arkansas. Professional, timely, and built for minimal disruption to your business.", icon: Icons.building },
  { name: "Leak Detection & Repair", desc: "Expert leak detection using proven methods to find hidden leaks behind walls, under slabs, and in supply lines. Fast, accurate diagnosis followed by lasting repairs.", icon: Icons.droplets },
  { name: "Toilet Repair & Installation", desc: "From running toilets and persistent clogs to full replacement installations, Chris delivers swift, respectful service and leaves your bathroom cleaner than he found it.", icon: Icons.toilet },
  { name: "Water Heater Service", desc: "Installation, repair, and maintenance for tank and tankless water heaters. Honest assessment of whether a repair or replacement is the right call for your situation.", icon: Icons.flame },
  { name: "Emergency Service Calls", desc: "When plumbing emergencies strike, Rachal Facility Services responds fast. Call Chris directly for urgent leaks, backups, and water damage situations.", icon: Icons.zap },
];

const serviceNames = ["Residential Plumbing","Commercial Plumbing","Leak Detection & Repair","Toilet Repair & Installation","Water Heater Service","Emergency Service Calls"];

const areas = ["Little Rock","Maumelle","North Little Rock","Sherwood","Jacksonville","Cabot","Conway","Bryant","Benton","Central Arkansas"];

const reviews = [
  { text: "Christopher provided excellent service and was super friendly. Took the time to talk about the issue, explain what he found, and went over the various methods to fix it. He was on time (ahead of time, actually), and a general pleasure to deal with. Highly recommended!", name: "Jay Beard", src: "Google" },
  { text: "Chris Rachal communicated with me and arrived to repair 2 toilets. He was respectful of our property, knowledgeable and swift with his work. Mr. Rachal cleaned up after his work and I recommend him highly.", name: "Mary Ann Campbell", src: "Google" },
  { text: "Christopher came out to our house, and he was very knowledgeable and friendly. Not only did he help us with our leak, but helped us with other issues. Most people that come out with our home warranty aren't very responsive or don't really care, but Christopher was the complete opposite.", name: "Kiki West", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function RachalFacilityServices() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("rachal-facility-services-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="roof"){sessionStorage.setItem("rachal-facility-services-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "Plumber Little Rock AR | Rachal Facility Services";
    const els = [];
    ...
    add("meta",{name:"description",content:"Rachal Facility Services — trusted plumber in Little Rock AR. 4.6 stars on Google. Call Chris at (501) 350-6366."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"Rachal Facility Services",telephone:"+15013506366",
      address:{"@type":"PostalAddress",streetAddress:"300 S Rodney Parham Rd Ste.1-130",addressLocality:"Little Rock",addressRegion:"AR",postalCode:"72205",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:34.7465,longitude:-92.2896},
      openingHoursSpecification:[{"@type":"OpeningHoursSpecification",dayOfWeek:["Monday","Tuesday","Wednesday","Thursday","Friday"],opens:"08:00",closes:"17:00"}],
      aggregateRating:{"@type":"AggregateRating",ratingValue:"4.6",reviewCount:"10",bestRating:"5"},
      founder:{"@type":"Person",name:"Chris Rachal"},
      description:"Rachal Facility Services is a trusted plumbing company serving Little Rock and Central Arkansas. Owner Chris Rachal is praised by name in every review for punctuality, knowledge, and friendly service.",
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1C2B41"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8}}>
            <span style={{fontWeight:700,color:"#1C2B41"}}>Rachal</span> <span style={{color:"#C8934F"}}>Facility Services</span>
          </div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(28,43,65,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease",boxShadow:scrolled?"0 1px 0 rgba(255,255,255,0.06)":"none"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}>
        <span>Rachal</span> <span style={{color:"var(--accent)"}}>Facility Services</span>
      </div>
      ...
      <div className="hero-grid" style={{maxWidth:1200,width:"100%",display:"grid",gridTemplateColumns:"1.8fr 1fr",gap:48,alignItems:"center"}}>
        <div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:32}}>
            <Stars />
            <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"Excellent service and super friendly" — Jay Beard</span>
          </div>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(200,147,79,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Chris — {PHONE}</a>
        </div>
        <div>
      ...
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        ...
        <div className="trust-mobile" style={{display:"none",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:48,textAlign:"center"}}>
          {[
            ...
            {num:"10+",label:"5-Star Reviews",desc:"Real customers"},
            {num:"Chris",label:"On Every Job",desc:"Owner-operated"},
            {num:"LLC",label:"Registered Business",desc:"Licensed & insured"},
          ].map((s,i)=>(
            <div key={i}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"200px 1fr",gap:48}}>
          ...
          <div className="trust-sidebar" style={{display:"flex",flexDirection:"column",gap:32,paddingTop:8}}>
            {[
              ...
              {num:"10+",label:"5-Star Reviews",desc:"Real customers"},
              {num:"Chris",label:"On Every Job",desc:"Owner-operated"},
              {num:"LLC",label:"Registered",desc:"Licensed business"},
            ].map((s,i)=>(
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",position:"relative",overflow:"hidden"}}>
      <div className="serif" style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",fontSize:"clamp(120px,20vw,280px)",color:"var(--accent)",opacity:0.08,lineHeight:1,whiteSpace:"nowrap",pointerEvents:"none"}}>10+</div>
      <div style={{maxWidth:800,margin:"0 auto",position:"relative",zIndex:1}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,textAlign:"center",opacity:0.7}}>About Chris Rachal</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff",textAlign:"center"}}>Praised by Name<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>In Every Review</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:24,textAlign:"center"}}>
          When customers talk about Rachal Facility Services, they don't mention a company — they mention Chris. "Christopher" and "Chris" appear in every single review because he's the one who shows up, explains the problem, and fixes it right. That's the difference between hiring a plumbing company and hiring a plumber who takes personal pride in every job.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:36,textAlign:"center"}}>
          Chris Rachal is known for arriving early, taking the time to walk customers through their options, and leaving the property cleaner than he found it. Whether it's a routine toilet repair or an urgent leak, he treats every home with respect. He's also one of the rare contractors that home warranty companies send out who actually cares — and his customers notice the difference immediately. Rachal Facility Services is a registered LLC serving Little Rock and all of Central Arkansas.
        </p>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:20}}>
          {["4.6 Stars on Google","Owner on Every Job","Punctual — Often Early","Explains Everything Clearly","Home Warranty Contractor","Registered LLC"].map((c,i)=>(
    ...
    <section id="areas" style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:12}}>
          {areas.map((a,i)=>(
            <span key={i} title={`Plumber in ${a}${a==="Central Arkansas"?"":", AR"}`} style={{display:"inline-flex",alignItems:"center",gap:6,background:"#fff",border:"1px solid var(--border)",padding:"12px 24px",borderRadius:30,fontSize:15,fontWeight:500,color:"var(--text)",cursor:"default",transition:"all 0.25s"}}
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>
          Got a plumbing problem?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Chris.</span>
        </h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>
          Honest pricing, clear explanations, and the kind of service that earns 5-star reviews by name. Monday through Friday, 8 AM to 5 PM.
        </p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16}}><span>Rachal</span> <span style={{color:"var(--accent)"}}>Facility Services</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>Trusted plumbing service for Little Rock and Central Arkansas. Owner Chris Rachal is on every job — praised by name in every review.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>300 S Rodney Parham Rd Ste.1-130<br/>Little Rock, AR 72205<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} Rachal Facility Services. All rights reserved.</span>
          <span>Mon–Fri 8 AM – 5 PM | Little Rock, AR</span>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Chris — {PHONE}</a>
    </div>
