// Layout: D | Industry: Roofing | City: Little Rock
import { useState, useEffect, useRef } from "react";

const PHONE = "(501) 404-4742";
const TEL = "tel:+15014044742";
...
const Icons = {
  ...
  siding: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 8h18"/><path d="M3 13h18"/><path d="M3 18h18"/></svg>,
...
const services = [
  { name: "Roof Replacement", desc: "Full residential and commercial roof replacement using Malarkey, Owens Corning, Atlas, CertainTeed, and GAF shingles. Premium materials installed by experienced crews.", icon: Icons.roof },
  { name: "Storm Damage", desc: "Rapid response after hail, wind, and severe weather across Central Arkansas. We document all damage thoroughly and restore your roof to pre-storm condition.", icon: Icons.storm },
  { name: "Insurance Claims", desc: "We walk you through the entire insurance claims process from start to finish. Professional documentation and direct communication with your adjuster.", icon: Icons.clipboard },
  { name: "Siding Installation", desc: "Quality siding installation and replacement to protect and beautify your home. We handle everything from removal to final trim work.", icon: Icons.siding },
  { name: "Gutter Installation", desc: "Seamless gutter installation to protect your foundation and landscaping. Proper drainage is essential for every Central Arkansas home.", icon: Icons.gutter },
  { name: "Free Inspections", desc: "Complimentary roof inspections to assess condition, identify damage, and provide honest recommendations. No pressure, no obligation.", icon: Icons.shield },
];

const allServiceNames = ["Residential Roof Replacement", "Commercial Roof Replacement", "Roof Repair", "Storm Damage Restoration", "Insurance Claims Assistance", "Siding Installation", "Gutter Installation", "Free Roof Inspections"];

const areas = ["Little Rock", "North Little Rock", "Sherwood", "Jacksonville", "Benton", "Bryant", "Maumelle", "Conway"];

const reviews = [
  { text: "Elevate Roofing installed a new Malarkey shingles roof. They had my shingles delivered within days.", name: "Karen Murphree", src: "Google" },
  { text: "Sales staff was professional and courteous and knowledgeable. Made the process of getting insurance to pay easy.", name: "TJ Frame", src: "Google" },
  { text: "Elevate replaced my roof and siding after major storm damage in 2023. They helped me in dealing with insurance.", name: "Kenneth Davis", src: "Google" },
];
...
const trustStats = [
  ...
  { num: "39+", label: "Reviews", desc: "All 39 are 5-star" },
  { num: "W", label: "Women-Owned", desc: "Brianna Moore, President" },
  { num: "BBB", label: "Accredited", desc: "Since June 2023" },
];

export default function ElevateRoofingAndConstruction() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("elevaterc-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if (pw === "2026") { sessionStorage.setItem("elevaterc-auth", "1"); setAuthed(true); } else { setPwBad(true); setPw(""); } };
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Roofing Little Rock AR | Elevate Roofing & Construction";
    const els = [];
    ...
    add("meta", { name: "description", content: "Top-rated roofing in Little Rock, AR. 5.0 stars, 39 perfect reviews. Women-owned. BBB accredited. Call (501) 404-4742." });
    ...
    add("script", {
      type: "application/ld+json", textContent: JSON.stringify({
        ...
        name: "Elevate Roofing & Construction", telephone: "+15014044742",
        email: "sales@elevateark.com",
        address: { "@type": "PostalAddress", streetAddress: "2306 North Cantrell Road", addressLocality: "Little Rock", addressRegion: "AR", postalCode: "72202", addressCountry: "US" },
        ...
        founder: { "@type": "Person", name: "Brianna Moore" },
        description: "Elevate Roofing & Construction provides residential and commercial roofing, storm damage restoration, and insurance claims assistance in Little Rock, AR. Women-owned. BBB accredited since 2023. 5.0 stars with 39 perfect reviews.",
    ...
    return (<>
      ...
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#1E1E1E" }}>
        <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
          <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, marginBottom: 8, fontWeight: 600, letterSpacing: -0.5 }}><span style={{ color: "#1E1E1E" }}>Elevate</span>{" "}<span style={{ color: "#D4C5A9" }}>Roofing</span></div>
  ...
  return (<>
    ...
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled ? "rgba(30,30,30,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition: "all 0.4s ease" }}>
      <div className="serif" style={{ fontSize: 22, color: scrolled ? "#fff" : "var(--primary)", fontWeight: 600, letterSpacing: -0.5 }}><span>Elevate</span>{" "}<span style={{ color: "var(--accent)" }}>Roofing</span></div>
      ...
      <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1.8fr 1fr", gap: 48, alignItems: "center" }}>
        <div>
          <h1 style={{ fontSize: "clamp(48px, 6vw, 84px)", lineHeight: 1.0, color: "var(--primary)", letterSpacing: -2, marginBottom: 24 }}>
            Little Rock's<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Premier</span><br />Roofer
          </h1>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 32 }}>
            <Stars />
            <span style={{ fontSize: 14, color: "var(--text-light)", fontStyle: "italic", marginLeft: 4 }}>"Professional, courteous, knowledgeable. Made insurance easy." — TJ Frame</span>
          </div>
          <a href={TEL} style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 36px", borderRadius: 6, fontSize: 16, fontWeight: 700, boxShadow: "0 4px 20px rgba(212,197,169,0.3)", fontFamily: "'DM Sans',sans-serif" }}>{Icons.phone} Call Brianna Moore — {PHONE}</a>
        </div>
        <div>
    ...
    <section id="services" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <p style={{ textAlign: "center", marginTop: 48, fontSize: 15, color: "var(--text-light)" }}>Questions? Call Brianna directly at <a href={TEL} style={{ color: "var(--accent)", fontWeight: 600 }}>{PHONE}</a></p>
      </div>
    </section>
    ...
    <section id="about" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--primary)", position: "relative", overflow: "hidden" }}>
      <div className="serif" style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", fontSize: "clamp(120px,20vw,280px)", color: "var(--accent)", opacity: 0.08, lineHeight: 1, whiteSpace: "nowrap", pointerEvents: "none" }}>39</div>
      <div style={{ maxWidth: 800, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <div className="gold-line" style={{ margin: "0 auto 20px" }} />
        <div style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: 3, color: "var(--accent)", marginBottom: 14, textAlign: "center", opacity: 0.7 }}>About Elevate Roofing & Construction</div>
        <h2 style={{ fontSize: "clamp(30px,3.5vw,42px)", lineHeight: 1.15, marginBottom: 28, color: "#fff", textAlign: "center" }}>39 Perfect Reviews.<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Women-Owned Excellence.</span></h2>
        <p style={{ fontSize: 16, lineHeight: 1.8, color: "rgba(255,255,255,0.6)", marginBottom: 24, textAlign: "center" }}>
          Elevate Roofing & Construction is led by President Brianna Moore, making it one of Little Rock's standout women-owned businesses. With COO Jake Michael rounding out the leadership team, Elevate operates from 2306 North Cantrell Road and is open 24/7 for emergencies. They work with five premium shingle brands — Malarkey, Owens Corning, Atlas, CertainTeed, and GAF — ensuring every homeowner gets the right product for their roof and budget.
        </p>
        <p style={{ fontSize: 16, lineHeight: 1.8, color: "rgba(255,255,255,0.6)", marginBottom: 36, textAlign: "center" }}>
          BBB accredited since June 2023, Elevate has earned a perfect 5.0 Google rating with all 39 reviews at five stars — a remarkable achievement in the roofing industry. Beyond roofing, the team also operates Elevate Windows & Doors, bringing the same quality and professionalism to every project. With 2.8K Facebook followers and active community engagement across Central Arkansas, Elevate has built a reputation that speaks for itself.
        </p>
        <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 20 }}>
          {["Women-Owned Business", "BBB Accredited Since 2023", "Perfect 5.0 Google Rating", "5 Premium Shingle Brands", "Open 24/7", "Also: Elevate Windows & Doors"].map((c, i) => (
    ...
    <section style={{ padding: "100px clamp(24px,5vw,64px)", background: "var(--white)", textAlign: "center", borderTop: "1px solid var(--border)" }}>
      <div style={{ maxWidth: 600, margin: "0 auto" }}>
        <h2 className="serif" style={{ fontSize: "clamp(32px,4vw,50px)", color: "var(--primary)", lineHeight: 1.1, marginBottom: 20, letterSpacing: -1 }}>Need a new roof?<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Call Elevate Roofing.</span></h2>
        <p style={{ fontSize: 17, color: "var(--text-mid)", marginBottom: 36, lineHeight: 1.6 }}>Women-owned. BBB accredited. 39 perfect reviews. Open 24/7.</p>
    ...
    <footer style={{ background: "var(--primary)", color: "rgba(255,255,255,0.7)", padding: "64px clamp(24px,5vw,64px) 0" }}>
      <div className="footer-grid" style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "1.5fr 1fr 1fr", gap: 48, paddingBottom: 40 }}>
        <div>
          <div className="serif" style={{ fontSize: 22, color: "#fff", marginBottom: 16, fontWeight: 600, letterSpacing: -0.5 }}><span>Elevate</span>{" "}<span style={{ color: "var(--accent)" }}>Roofing</span></div>
          <p style={{ fontSize: 14, lineHeight: 1.7, opacity: 0.7, marginBottom: 16 }}>Women-owned and led by President Brianna Moore. BBB accredited since 2023. Perfect 5.0 Google rating. Serving Little Rock and Central Arkansas 24/7.</p>
          <p style={{ fontSize: 14, lineHeight: 1.8, opacity: 0.6 }}>2306 North Cantrell Road<br />Little Rock, AR 72202<br /><a href={TEL} style={{ color: "var(--accent)", fontWeight: 600 }}>{PHONE}</a><br /><a href="mailto:sales@elevateark.com" style={{ color: "var(--accent)", fontWeight: 600 }}>sales@elevateark.com</a></p>
        </div>
        <div>
      ...
      <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "20px 0", maxWidth: 1100, margin: "0 auto" }}>
        <div className="footer-btm-inner" style={{ display: "flex", justifyContent: "space-between", fontSize: 13, opacity: 0.4, flexWrap: "wrap", gap: 8 }}>
          <span>&copy; {yr} Elevate Roofing & Construction. All rights reserved.</span>
          <span>Women-Owned | BBB Accredited</span>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 998, display: "flex" }}>
      <a href={TEL} style={{ display: "flex", width: "100%", alignItems: "center", justifyContent: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px", fontSize: 17, fontWeight: 700, boxShadow: "0 -4px 20px rgba(0,0,0,0.15)", fontFamily: "'DM Sans',sans-serif" }}>{Icons.phone} Call Elevate Roofing</a>
    </div>
