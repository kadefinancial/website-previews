...
const PHONE = "(501) 753-7300";
const TEL = "tel:+15017537300";
...
const services = [
  { name: "HVAC Service & Maintenance", desc: "Comprehensive tune-ups, diagnostics, and preventive maintenance to keep your heating and cooling running at peak efficiency year-round.", icon: Icons.wrench },
  { name: "HVAC Replacement & Installation", desc: "Expert system design and installation from a Trane dealer. Right-sized equipment, proper load calculations, and commissioning done right.", icon: Icons.wind },
  { name: "Energy Audits", desc: "Whole-home energy analysis using BPI-certified building science. Pinpoint exactly where your home is losing energy and what to fix first.", icon: Icons.search },
  { name: "Crawlspace Encapsulation", desc: "Seal and condition your crawlspace to eliminate moisture, improve indoor air quality, and dramatically reduce energy waste from below your home.", icon: Icons.home },
  { name: "Emergency 24/7 Service", desc: "When your system fails at 2 AM, Energy Master answers the phone. Around-the-clock emergency HVAC service for when you need it most.", icon: Icons.zap },
  { name: "Building Science Diagnostics", desc: "Advanced diagnostic testing that goes beyond HVAC. Blower door tests, duct leakage testing, and thermal imaging to solve problems no one else can.", icon: Icons.thermometer },
];

const allServiceNames = ["HVAC Service & Maintenance","HVAC Replacement & Installation","Energy Analysis","Crawlspace Encapsulation","Emergency Service","Building Science Diagnostics","TempStar Service"];

const areas = ["Little Rock","North Little Rock","Sherwood","Conway","Bryant","Benton","Mayflower","Alexander","Jacksonville"];

const reviews = [
  { text: "I've been using Energy Master for about 20 years now. Jeff Brun has been to my home for many inspections and has always been friendly.", name: "Bob Bullock", src: "Google" },
  { text: "As a Property Manager who has worked with Energy Masters for many years, I cannot recommend them highly enough.", name: "Thom Jesse", src: "Google" },
  { text: "We've been very happy with Energy Master over the last 10+ years. The original energy audit they performed resulted in significant savings on our utility bills.", name: "Pat Beaird", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function EnergyMasterHome() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("emh-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="roof"){sessionStorage.setItem("emh-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "HVAC Service North Little Rock AR | Energy Master Home";
    const els = [];
    ...
    add("meta",{name:"description",content:"HVAC, energy audits & crawlspace encapsulation in North Little Rock, AR. 4.6 stars, 128 reviews. Call (501) 753-7300."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"Energy Master Home Inc.",telephone:"+15017537300",email:"dallas@energymasterhomeinc.com",
      address:{"@type":"PostalAddress",streetAddress:"5 Collins Industrial Pl",addressLocality:"North Little Rock",addressRegion:"AR",postalCode:"72113",addressCountry:"US"},
      ...
      description:"Energy Master Home Inc. provides HVAC service, energy audits, crawlspace encapsulation, and building science diagnostics in North Little Rock and Central Arkansas. NATE Certified, BPI Certified, Trane dealer.",
      ...
      memberOf:[{"@type":"Organization",name:"NATE"},{"@type":"Organization",name:"BPI"}],sameAs:[],
    })});
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#2D2D2D"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8,fontWeight:600,letterSpacing:-0.5}}><span style={{color:"#2D2D2D"}}>Energy Master</span> <span style={{color:"#2E7D52"}}>Home</span></div>
  ...
  return (<>
    <style>{`
      ...
      :root{--primary:#2D2D2D;--accent:#2E7D52;--cream:#FAF9F6;--white:#FFFFFF;--text:#2A2A3C;--text-mid:#5A5A6E;--text-light:#8A8A9A;--border:rgba(0,0,0,0.06)}
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(45,45,45,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5,cursor:"pointer"}} onClick={()=>window.scrollTo({top:0,behavior:"smooth"})}><span>Energy Master</span> <span style={{color:"var(--accent)"}}>Home</span></div>
    ...
    <section style={{padding:"180px clamp(24px,5vw,64px) 100px",background:"var(--cream)",textAlign:"center"}}>
      <div style={{maxWidth:800,margin:"0 auto"}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(46,125,82,0.1)",padding:"8px 18px",borderRadius:4,marginBottom:28}}>
          ...
          <span style={{fontSize:12,fontWeight:700,color:"var(--accent)",letterSpacing:2,textTransform:"uppercase"}}>NATE Certified + BPI Certified</span>
        </div>
        <h1 style={{fontSize:"clamp(44px, 6vw, 80px)",lineHeight:1.05,color:"var(--primary)",marginBottom:24,letterSpacing:-2}}>
          HVAC Experts<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>North Little Rock Trusts</span>
        </h1>
        <p style={{fontSize:18,lineHeight:1.7,color:"var(--text-mid)",marginBottom:32,maxWidth:600,margin:"0 auto 32px"}}>
          Solving problems no one else can. Dallas Henry and the Energy Master team deliver whole-home energy performance -- not just HVAC. 20+ years of trusted service.
        </p>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:36}}>
          <Stars />
          <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"I cannot recommend them highly enough." -- Thom Jesse</span>
        </div>
        <div className="hero-btns" style={{display:"flex",gap:16,justifyContent:"center",flexWrap:"wrap"}}>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,fontFamily:"'DM Sans',sans-serif",boxShadow:"0 4px 20px rgba(46,125,82,0.3)",transition:"transform 0.2s"}}>{Icons.phone} Call Now</a>
    ...
    <section id="form" style={{padding:"80px clamp(24px,5vw,64px)",background:"var(--white)",borderTop:"1px solid var(--border)",borderBottom:"1px solid var(--border)"}}>
      <div style={{maxWidth:1200,margin:"0 auto",display:"flex",flexDirection:"column",alignItems:"center"}}>
        ...
        <div className="trust-row" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:32,marginTop:56,maxWidth:800,width:"100%",textAlign:"center"}}>
          {[
            {num:"4.6",label:"Google Rating",desc:"Based on 128 reviews"},
            {num:"128+",label:"Reviews",desc:"Verified on Google"},
            {num:"NATE & BPI",label:"Certified",desc:"Dual-certified technicians"},
            {num:"Trane",label:"Dealer",desc:"Factory-authorized equipment"},
          ].map((s,i)=>(
            <div key={i}>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        ...
        <p style={{textAlign:"center",marginTop:48,fontSize:15,color:"var(--text-mid)"}}>
          Questions? Call Dallas directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a>
        </p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)"}}>
      <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <p style={{fontSize:12,fontWeight:700,letterSpacing:3,textTransform:"uppercase",color:"var(--accent)",marginBottom:14}}>Why Energy Master</p>
        <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",lineHeight:1.15,color:"#fff",letterSpacing:-0.5,marginBottom:40}}>Solving Problems No One Else Can</h2>
        <p style={{fontSize:17,lineHeight:1.8,color:"rgba(255,255,255,0.75)",marginBottom:24}}>
          Dallas Henry founded Energy Master Home with a different approach to HVAC. Instead of just swapping equipment, his team looks at the whole house. NATE Certified technicians handle the mechanical side. BPI Certified Building Analysts diagnose what is actually causing comfort and efficiency problems -- a dual certification most HVAC companies simply do not carry.
        </p>
        <p style={{fontSize:17,lineHeight:1.8,color:"rgba(255,255,255,0.75)",marginBottom:48}}>
          That is why Energy Master pairs energy audits with crawlspace encapsulation and building science diagnostics -- because a new unit will not fix a leaky duct or a vented crawlspace. With 20+ years of customer relationships and a $199/yr service plan that keeps systems running right, Energy Master delivers whole-home performance, not just heating and cooling.
        </p>
        <div className="why-creds-row" style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:24}}>
          {["NATE Certified Technicians","BPI Certified Building Analysts","Authorized Trane Dealer","20+ Year Track Record","$199/yr Service Plan","24/7 Emergency Service"].map((c,i)=>(
    ...
    <section style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--white)",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
        <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",lineHeight:1.15,color:"var(--primary)",letterSpacing:-0.5,marginBottom:20}}>
          Need HVAC service?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Energy Master.</span>
        </h2>
        <p style={{fontSize:17,lineHeight:1.7,color:"var(--text-mid)",marginBottom:36}}>
          Honest diagnostics. Energy-efficient solutions. Whole-home performance backed by 20+ years of experience and dual NATE/BPI certification.
        </p>
        <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 40px",borderRadius:6,fontSize:16,fontWeight:700,fontFamily:"'DM Sans',sans-serif",boxShadow:"0 4px 20px rgba(46,125,82,0.3)"}}>{Icons.phone} {PHONE}</a>
      </div>
    </section>
    ...
    <footer className="mob-pad" style={{padding:"80px clamp(24px,5vw,64px) 40px",background:"var(--primary)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div className="footer-grid" style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:48,marginBottom:48}}>
          <div>
            <div className="serif" style={{fontSize:22,fontWeight:600,letterSpacing:-0.5,marginBottom:16}}><span style={{color:"#fff"}}>Energy Master</span> <span style={{color:"var(--accent)"}}>Home</span></div>
            <p style={{fontSize:14,lineHeight:1.7,color:"rgba(255,255,255,0.6)",marginBottom:16,maxWidth:320}}>HVAC service, energy audits, and building science diagnostics in North Little Rock and Central Arkansas. NATE Certified. BPI Certified. Trane dealer.</p>
            <p style={{fontSize:14,color:"rgba(255,255,255,0.6)",marginBottom:6}}>5 Collins Industrial Pl, North Little Rock, AR 72113</p>
            <a href={TEL} style={{fontSize:14,color:"var(--accent)",fontWeight:600}}>{PHONE}</a>
            <p style={{fontSize:14,color:"rgba(255,255,255,0.5)",marginTop:6}}>dallas@energymasterhomeinc.com</p>
            <p style={{fontSize:13,color:"rgba(255,255,255,0.4)",marginTop:8,display:"flex",alignItems:"center",gap:6}}><span style={{color:"var(--accent)",display:"inline-flex"}}>{Icons.clock}</span> Mon-Fri 8AM-5PM + 24/7 Emergency</p>
          </div>
          <div>
        ...
        <div style={{borderTop:"1px solid rgba(255,255,255,0.1)",paddingTop:24}}>
          <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <p style={{fontSize:13,color:"rgba(255,255,255,0.35)"}}>&#169; {yr} Energy Master Home Inc. All rights reserved.</p>
            <p style={{fontSize:13,color:"rgba(255,255,255,0.35)"}}>NATE Certified | BPI Certified | Trane Dealer</p>
          </div>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,width:"100%",padding:"16px",background:"var(--accent)",color:"#fff",fontSize:16,fontWeight:700,fontFamily:"'DM Sans',sans-serif",boxShadow:"0 -4px 20px rgba(0,0,0,0.15)"}}>{Icons.phone} Call Energy Master Now</a>
    </div>
  </>);
}