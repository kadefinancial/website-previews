...
const PHONE = "(501) 960-5030";
const TEL = "tel:+15019605030";
...
const services = [
  { name: "HVAC Repair", desc: "Fast, accurate diagnostics and repair for all makes and models. We fix it right the first time so your home stays comfortable year-round.", icon: Icons.wrench },
  { name: "HVAC Installation", desc: "Complete system replacement and new construction installs. As an authorized Heil dealer, we offer top-tier equipment with 0% financing for 60 months.", icon: Icons.wind },
  { name: "HVAC Maintenance", desc: "Preventative tune-ups that extend system life, lower energy bills, and catch small issues before they become costly repairs.", icon: Icons.clipboard },
  { name: "Ventilation", desc: "Duct inspection, repair, sealing, and new installation. Properly sealed ductwork improves airflow, efficiency, and indoor air quality.", icon: Icons.duct },
  { name: "Commercial Refrigeration", desc: "Professional refrigeration service for restaurants, convenience stores, and commercial kitchens. Trusted by businesses like Outback Steakhouse.", icon: Icons.snowflake },
  { name: "Emergency Service", desc: "When your system fails unexpectedly, we respond fast. Call us anytime for urgent HVAC repair and get your comfort restored.", icon: Icons.zap },
];
...
const areas = ["Little Rock", "North Little Rock", "Sherwood", "Jacksonville", "Bryant", "Benton", "Conway", "Hot Springs", "Sheridan", "Cabot"];

const reviews = [
  { text: "In February 2026, we made the decision to fully replace our HVAC system, including the condenser, furnace, coil, thermostats, zoning with automatic dampers, and an induct air purifier.", name: "Beau Turner", src: "Google" },
  { text: "One Call was suggested, so I reached out. When I called, I spoke to Stacy. She was so pleasant and helpful, she really set the tone.", name: "Michele Kulesa", src: "Google" },
];
...
export default function OneCallServiceGroup() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("ocsg-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("ocsg-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "HVAC Service North Little Rock AR | One Call Service Group";
    const els = [];
    const add = (tag, attrs) => {
    ...
    add("meta", { name: "description", content: "HVAC repair, installation & maintenance in North Little Rock, AR. 4.8 stars, 105+ reviews. Heil dealer. Call (501) 960-5030." });
    ...
    add("script", {
      ...
      textContent: JSON.stringify({
        ...
        "@type": "HVACBusiness",
        name: "One Call Service Group",
        telephone: "+15019605030",
        email: "lisa@onecallservice.net",
        address: { "@type": "PostalAddress", streetAddress: "3615 John F Kennedy Blvd", addressLocality: "North Little Rock", addressRegion: "AR", postalCode: "72116", addressCountry: "US" },
        geo: { "@type": "GeoCoordinates", latitude: 34.7695, longitude: -92.2671 },
        openingHoursSpecification: [{ "@type": "OpeningHoursSpecification", dayOfWeek: ["Monday", "Tuesday", "Wednesday", "Thursday", "Friday"], opens: "08:00", closes: "17:00" }],
        aggregateRating: { "@type": "AggregateRating", ratingValue: "4.8", reviewCount: "105", bestRating: "5" },
        founder: { "@type": "Person", name: "Sam Coulter" },
        description: "Women-owned HVAC company in North Little Rock, AR. 25+ years experience. Authorized Heil dealer. 0% financing for 60 months. Licensed, bonded, and insured.",
        ...
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#1C2B41" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, marginBottom: 8, fontWeight: 600, letterSpacing: -0.5 }}>
              <span style={{ color: "#1C2B41" }}>One Call</span>{" "}
              <span style={{ color: "#8B3A3A" }}>Service</span>
            </div>
    ...
    <>
      ...
      <section style={{ minHeight: "100vh", padding: "140px clamp(24px,5vw,64px) 100px", background: "var(--cream)", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 48, alignItems: "center" }}>
          <div>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(139,58,58,0.1)", padding: "8px 16px", borderRadius: 40, marginBottom: 24 }}>
              <Stars />
              <span style={{ fontSize: 13, fontWeight: 600, color: "var(--accent)" }}>4.8 Stars - 105 Reviews</span>
            </div>
            <h1 style={{ fontSize: "clamp(40px,5vw,68px)", lineHeight: 1.05, color: "var(--primary)", letterSpacing: -1.5, marginBottom: 20 }}>
              North Little Rock's<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>One Call</span><br />HVAC Experts
            </h1>
            <p style={{ fontSize: 18, lineHeight: 1.7, color: "var(--text-mid)", maxWidth: 480, marginBottom: 28 }}>
              Women-owned. 25+ years experience. Authorized Heil dealer with 0% financing for 60 months. Licensed, bonded, and insured.
            </p>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 32 }}>
              <Stars />
              <span style={{ fontSize: 14, color: "var(--text-light)", fontStyle: "italic", marginLeft: 4 }}>"Stacy was so pleasant and helpful, she really set the tone." — Michele Kulesa</span>
            </div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 14 }}>
              <a href={TEL} style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 36px", borderRadius: 6, fontSize: 16, fontWeight: 700, boxShadow: "0 4px 20px rgba(139,58,58,0.3)", fontFamily: "'DM Sans',sans-serif", transition: "transform 0.3s ease" }}>{Icons.phone} Call Now — {PHONE}</a>
      ...
      <section style={{ background: "var(--white)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", padding: "48px clamp(24px,5vw,64px)" }}>
        <div className="trust-row" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 32, textAlign: "center" }}>
          {[
            { num: "4.8", label: "Google Rating", desc: "105+ reviews" },
            { num: "105+", label: "Reviews", desc: "On Google" },
            { num: "Women", label: "Owned", desc: "Lisa Coulter, owner" },
            { num: "Heil", label: "Dealer", desc: "Authorized dealer" },
          ].map((s, i) => (
            <div key={i}>
      ...
      <section id="services" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <p style={{ textAlign: "center", marginTop: 40, fontSize: 15, color: "var(--text-mid)" }}>
            Questions? Call Lisa directly at <a href={TEL} style={{ color: "var(--accent)", fontWeight: 700 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--primary)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <div className="gold-line" style={{ margin: "0 auto 20px" }} />
            <div style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: 3, color: "var(--accent)", marginBottom: 14 }}>Why One Call Service Group</div>
            <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "#fff", letterSpacing: -0.5 }}>4.8 Stars on Google. 105 Reviews.</h2>
          </div>
          <div className="why-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "start" }}>
            <div>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "rgba(255,255,255,0.82)", marginBottom: 20 }}>
                One Call Service Group is a women-owned HVAC company founded by Sam Coulter and now led by Lisa Coulter. With over 25 years in the HVAC industry, Sam built this business on a simple promise: for comfort, all you need is one call. Lisa carries that same commitment forward every day.
              </p>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "rgba(255,255,255,0.82)", marginBottom: 32 }}>
                From Stacei in dispatch to Danny in the field to Jack handling estimates, the One Call team treats every customer like family. As an authorized Heil dealer, we offer premium equipment with 0% financing for 60 months. We are licensed, bonded, insured, and proudly LGBTQ+ friendly. Our commercial clients include Outback Steakhouse, and our residential customers trust us with their homes year after year.
              </p>
            ...
            <div className="badge-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {[
                { icon: Icons.heart, title: "Women-Owned", desc: "Led by Lisa Coulter, proudly women-owned and LGBTQ+ friendly" },
                { icon: Icons.award, title: "Heil Dealer", desc: "Authorized Heil dealer with premium equipment and warranties" },
                { icon: Icons.dollar, title: "0% Financing", desc: "0% interest financing available for up to 60 months" },
                { icon: Icons.shield, title: "25+ Years", desc: "Over 25 years of trusted HVAC experience in Central Arkansas" },
              ].map((b, i) => (
      ...
      <section style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)", borderTop: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "var(--primary)", letterSpacing: -0.5, lineHeight: 1.15, marginBottom: 20 }}>
            Need HVAC service?<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Call One Call.</span>
          </h2>
          <p style={{ fontSize: 17, lineHeight: 1.7, color: "var(--text-mid)", marginBottom: 36, maxWidth: 520, margin: "0 auto 36px" }}>
            Honest pricing. 0% financing for 60 months. Licensed, bonded, and insured. 100% satisfaction guaranteed. For comfort, all you need is one call.
          </p>
          <a href={TEL} style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "20px 44px", borderRadius: 6, fontSize: 18, fontWeight: 700, boxShadow: "0 4px 20px rgba(139,58,58,0.3)", fontFamily: "'DM Sans',sans-serif" }}>{Icons.phone} {PHONE}</a>
        </div>
      </section>
      ...
      <footer style={{ background: "var(--primary)", padding: "80px clamp(24px,5vw,64px) 40px" }}>
        <div className="footer-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: 48, marginBottom: 48 }}>
          <div>
            <div className="serif" style={{ fontSize: 22, fontWeight: 600, letterSpacing: -0.5, marginBottom: 16 }}>
              <span style={{ color: "#fff" }}>One Call</span>{" "}
              <span style={{ color: "var(--accent)" }}>Service</span>
            </div>
            <p style={{ fontSize: 14, lineHeight: 1.7, color: "rgba(255,255,255,0.6)", marginBottom: 16, maxWidth: 320 }}>
              Women-owned HVAC repair, installation, and maintenance in North Little Rock, AR and Central Arkansas. Authorized Heil dealer. 25+ years experience.
            </p>
            <p style={{ fontSize: 14, color: "rgba(255,255,255,0.6)", marginBottom: 6 }}>3615 John F Kennedy Blvd, North Little Rock, AR 72116</p>
            <a href={TEL} style={{ fontSize: 14, color: "var(--accent)", fontWeight: 700 }}>{PHONE}</a>
            <br />
            <a href="mailto:lisa@onecallservice.net" style={{ fontSize: 14, color: "rgba(255,255,255,0.6)", marginTop: 4, display: "inline-block" }}>lisa@onecallservice.net</a>
        ...
        <div className="footer-btm" style={{ maxWidth: 1200, margin: "0 auto", borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 24, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>&copy; {yr} One Call Service Group. All rights reserved.</p>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>Women-Owned | Authorized Heil Dealer</p>
        </div>
      </footer>
      ...
      <div className="mobile-only" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 998, display: "none" }}>
        <a href={TEL} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "16px 20px", fontSize: 16, fontWeight: 700, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 -4px 20px rgba(0,0,0,0.15)", textDecoration: "none" }}>
          {Icons.phone} Call One Call — {PHONE}
        </a>
      </div>
