// Layout: D | Industry: Roofing | City: Jacksonville
import { useState, useEffect, useRef } from "react";

const PHONE = "(501) 982-6483";
const TEL = "tel:+15019826483";
...
const Icons = {
  ...
  commercial: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="4" y="2" width="16" height="20" rx="2"/><path d="M9 22V12h6v10"/><path d="M8 6h.01"/><path d="M12 6h.01"/><path d="M16 6h.01"/><path d="M8 10h.01"/><path d="M12 10h.01"/><path d="M16 10h.01"/></svg>,
  ...
  interior: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><path d="M9 22V12h6v10"/></svg>,
...
const services = [
  { name: "Residential Roofing", desc: "Complete roof replacement and repair for homes across Central Arkansas. Thomas Roofing uses premium materials and handles everything from tear-off to final inspection.", icon: Icons.roof },
  { name: "Commercial Roofing", desc: "TPO, metal, and modified bitumen systems for commercial properties. Licensed General Contractor experience means your business is in qualified hands.", icon: Icons.commercial },
  { name: "Storm Damage", desc: "Rapid response after severe weather. We assess all damage, document findings thoroughly, and restore your roof to pre-storm condition.", icon: Icons.storm },
  { name: "Insurance Claims", desc: "Our Project Manager brings claims specialist experience to every job. We guide you through the entire insurance process from start to finish.", icon: Icons.clipboard },
  { name: "Interior Repairs", desc: "Water damage, framing, drywall, and interior restoration. When roof damage goes deeper than the surface, Thomas handles the full repair.", icon: Icons.interior },
  { name: "Gutters & Decks", desc: "Rain gutter installation, deck construction, and fence building. Protect your property top to bottom with one trusted contractor.", icon: Icons.gutter },
];

const allServiceNames = ["Residential Roof Replacement","Commercial Roofing","Roof Repair & Maintenance","Storm Damage Restoration","Insurance Claims Guidance","Interior Repairs & Framing","Rain Gutter Installation","Decks & Fences","Free Roof Inspections","Financing Available"];

const areas = ["Jacksonville","Little Rock","Sherwood","North Little Rock","Benton","Bryant","Cabot","Conway","Maumelle"];

const reviews = [
  { text: "Thomas Roofing and Restoration is one of the best Companies. Mr. Eric Thomas met with us and was very personable, fair on price, high quality work.", name: "Clarese Adams", src: "Google" },
  { text: "Erica Thomas Roofing replaced my roof after a hail storm. I cannot express enough how efficient he and his crew were.", name: "Kelly Martin", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

const trustStats = [
  { num: "4.9", label: "Google Rating", desc: "47 of 48 are 5-star" },
  { num: "48+", label: "Reviews", desc: "Verified on Google" },
  { num: "A+", label: "BBB Rating", desc: "Accredited business" },
  { num: "20", label: "Years", desc: "Nearly two decades" },
];

export default function ThomasRoofingAndRestoration() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("thomasrr-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if (pw === "2026") { sessionStorage.setItem("thomasrr-auth", "1"); setAuthed(true); } else { setPwBad(true); setPw(""); } };
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Roofing Jacksonville AR | Thomas Roofing & Restoration";
    const els = [];
    ...
    add("meta", { name: "description", content: "Top-rated roofing in Jacksonville, AR. BBB A+. Licensed General Contractor. Nearly 20 years. Call (501) 982-6483." });
    ...
    add("script", {
      type: "application/ld+json", textContent: JSON.stringify({
        ...
        name: "Thomas Roofing & Restoration", telephone: "+15019826483",
        email: "coverto@thomasrr.com",
        address: { "@type": "PostalAddress", streetAddress: "720 S First St", addressLocality: "Jacksonville", addressRegion: "AR", postalCode: "72076", addressCountry: "US" },
        ...
        founder: { "@type": "Person", name: "Eric Thomas" },
        description: "Thomas Roofing & Restoration provides residential and commercial roofing, storm damage restoration, and insurance claims guidance in Jacksonville, AR. BBB A+. Licensed General Contractor. Nearly 20 years in business.",
    ...
    return (<>
      ...
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#1E1E1E" }}>
        <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
          <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, marginBottom: 8, fontWeight: 600, letterSpacing: -0.5 }}><span style={{ color: "#1E1E1E" }}>Thomas</span>{" "}<span style={{ color: "#D4C5A9" }}>Roofing</span></div>
  ...
  return (<>
    ...
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled ? "rgba(30,30,30,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition: "all 0.4s ease" }}>
      <div className="serif" style={{ fontSize: 22, color: scrolled ? "#fff" : "var(--primary)", fontWeight: 600, letterSpacing: -0.5 }}><span>Thomas</span>{" "}<span style={{ color: "var(--accent)" }}>Roofing</span></div>
      ...
      <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1.8fr 1fr", gap: 48, alignItems: "center" }}>
        <div>
          <h1 style={{ fontSize: "clamp(48px, 6vw, 84px)", lineHeight: 1.0, color: "var(--primary)", letterSpacing: -2, marginBottom: 24 }}>
            Jacksonville's<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Most Trusted</span><br />Roofer
          </h1>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 32 }}>
            <Stars />
            <span style={{ fontSize: 14, color: "var(--text-light)", fontStyle: "italic", marginLeft: 4 }}>"One of the best companies. Very personable, fair on price, high quality work." — Clarese Adams</span>
          </div>
          <a href={TEL} style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 36px", borderRadius: 6, fontSize: 16, fontWeight: 700, boxShadow: "0 4px 20px rgba(212,197,169,0.3)", fontFamily: "'DM Sans',sans-serif" }}>{Icons.phone} Call Chad Overton — {PHONE}</a>
        </div>
        <div>
    ...
    <section id="services" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div style={{ textAlign: "center", marginBottom: 64 }}>
          ...
          <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "var(--primary)", letterSpacing: -0.5 }}>Roofing Services in Jacksonville</h2>
        </div>
        {/* Mobile trust */}
        ...
        <p style={{ textAlign: "center", marginTop: 48, fontSize: 15, color: "var(--text-light)" }}>Questions? Call Chad directly at <a href={TEL} style={{ color: "var(--accent)", fontWeight: 600 }}>{PHONE}</a></p>
      </div>
    </section>
    ...
    <section id="about" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--primary)", position: "relative", overflow: "hidden" }}>
      <div className="serif" style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", fontSize: "clamp(120px,20vw,280px)", color: "var(--accent)", opacity: 0.08, lineHeight: 1, whiteSpace: "nowrap", pointerEvents: "none" }}>20</div>
      <div style={{ maxWidth: 800, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <div className="gold-line" style={{ margin: "0 auto 20px" }} />
        <div style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: 3, color: "var(--accent)", marginBottom: 14, textAlign: "center", opacity: 0.7 }}>About Thomas Roofing & Restoration</div>
        <h2 style={{ fontSize: "clamp(30px,3.5vw,42px)", lineHeight: 1.15, marginBottom: 28, color: "#fff", textAlign: "center" }}>Nearly 20 Years.<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Built on Trust.</span></h2>
        <p style={{ fontSize: 16, lineHeight: 1.8, color: "rgba(255,255,255,0.6)", marginBottom: 24, textAlign: "center" }}>
          Thomas Roofing & Restoration was founded by Eric Thomas and has grown into one of Jacksonville's most respected contractors. Today, President Chad Overton leads the team from 720 S First St, carrying forward Eric's commitment to honest work and fair pricing. As a Licensed General Contractor, Thomas handles residential roofing, commercial systems including TPO, metal, and modified bitumen, plus interior repairs and framing when storm damage goes deeper than the roof.
        </p>
        <p style={{ fontSize: 16, lineHeight: 1.8, color: "rgba(255,255,255,0.6)", marginBottom: 36, textAlign: "center" }}>
          With a dedicated Project Manager who brings claims specialist experience, Thomas Roofing navigates insurance claims so you don't have to. They offer a $100 referral program for customers who spread the word, and 15-year financing to make major projects accessible. With 47 of 48 Google reviews at five stars and a BBB A+ rating, the reputation speaks for itself.
        </p>
        <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 20 }}>
          {["Licensed General Contractor", "BBB A+ Accredited", "Nearly 20 Years in Business", "$100 Referral Program", "15-Year Financing Available", "Claims Specialist on Staff"].map((c, i) => (
    ...
    <section style={{ padding: "100px clamp(24px,5vw,64px)", background: "var(--white)", textAlign: "center", borderTop: "1px solid var(--border)" }}>
      <div style={{ maxWidth: 600, margin: "0 auto" }}>
        <h2 className="serif" style={{ fontSize: "clamp(32px,4vw,50px)", color: "var(--primary)", lineHeight: 1.1, marginBottom: 20, letterSpacing: -1 }}>Need a new roof?<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Call Thomas Roofing.</span></h2>
        <p style={{ fontSize: 17, color: "var(--text-mid)", marginBottom: 36, lineHeight: 1.6 }}>Licensed General Contractor. BBB A+. Nearly 20 years serving Central Arkansas.</p>
    ...
    <footer style={{ background: "var(--primary)", color: "rgba(255,255,255,0.7)", padding: "64px clamp(24px,5vw,64px) 0" }}>
      <div className="footer-grid" style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "1.5fr 1fr 1fr", gap: 48, paddingBottom: 40 }}>
        <div>
          <div className="serif" style={{ fontSize: 22, color: "#fff", marginBottom: 16, fontWeight: 600, letterSpacing: -0.5 }}><span>Thomas</span>{" "}<span style={{ color: "var(--accent)" }}>Roofing</span></div>
          <p style={{ fontSize: 14, lineHeight: 1.7, opacity: 0.7, marginBottom: 16 }}>Founded by Eric Thomas, now led by Chad Overton. Nearly 20 years of trusted roofing and restoration across Central Arkansas. Licensed General Contractor. BBB A+.</p>
          <p style={{ fontSize: 14, lineHeight: 1.8, opacity: 0.6 }}>720 S First St<br />Jacksonville, AR 72076<br /><a href={TEL} style={{ color: "var(--accent)", fontWeight: 600 }}>{PHONE}</a><br /><a href="mailto:coverto@thomasrr.com" style={{ color: "var(--accent)", fontWeight: 600 }}>coverto@thomasrr.com</a></p>
        </div>
        <div>
      ...
      <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "20px 0", maxWidth: 1100, margin: "0 auto" }}>
        <div className="footer-btm-inner" style={{ display: "flex", justifyContent: "space-between", fontSize: 13, opacity: 0.4, flexWrap: "wrap", gap: 8 }}>
          <span>&copy; {yr} Thomas Roofing & Restoration. All rights reserved.</span>
          <span>Licensed General Contractor | BBB A+</span>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 998, display: "flex" }}>
      <a href={TEL} style={{ display: "flex", width: "100%", alignItems: "center", justifyContent: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px", fontSize: 17, fontWeight: 700, boxShadow: "0 -4px 20px rgba(0,0,0,0.15)", fontFamily: "'DM Sans',sans-serif" }}>{Icons.phone} Call Thomas Roofing</a>
    </div>
