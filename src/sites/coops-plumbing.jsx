// Layout: D | Industry: Plumber | City: North Little Rock
import { useState, useEffect, useRef } from "react";

const PHONE = "(501) 766-9573";
const TEL = "tel:+15017669573";
...
const Icons = {
  ...
  drain: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><path d="M12 8v8"/><path d="M8 12h8"/><path d="M12 2v2"/><path d="M12 20v2"/></svg>,
  toilet: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M6 3h12v5a8 8 0 01-4 6.93V18H10v-3.07A8 8 0 016 8V3z"/><path d="M9 18h6v2H9z"/><path d="M8 22h8"/></svg>,
  sewer: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 12h4l3-9 4 18 3-9h4"/></svg>,
...
const services = [
  { name: "Plumbing Repair", desc: "From leaky faucets to burst pipes, Chris handles every residential and commercial plumbing repair with speed and fair pricing.", icon: Icons.wrench },
  { name: "Water Line Service", desc: "Water line repair and installation for homes and businesses. Fast diagnosis and reliable fixes to restore your water supply.", icon: Icons.droplets },
  { name: "Sewer Line Service", desc: "Professional sewer line repair and replacement. Chris locates the problem quickly and gets your system flowing again.", icon: Icons.sewer },
  { name: "Pipe Repair", desc: "Frozen, burst, or corroded pipes repaired on-site. Chris had one customer back online in under 30 minutes.", icon: Icons.pipe },
  { name: "Drain Cleaning", desc: "Stubborn clogs and slow drains cleared fast. Kitchen, bathroom, and main line drain cleaning for homes and businesses.", icon: Icons.drain },
  { name: "Toilet Repair", desc: "Running, leaking, or clogged toilets fixed quickly without overcharging. Honest service you can count on.", icon: Icons.toilet },
];

const allServiceNames = ["Plumbing Repair & Service","Water Line Repair & Installation","Sewer Line Repair & Installation","Pipe Repair","Toilet Repair","Drain Cleaning","General Plumbing Maintenance"];

const areas = ["North Little Rock","Sherwood","Little Rock","Jacksonville","Maumelle","Cabot","Gravel Ridge","Camp Robinson"];

const reviews = [
  { text: "Chris is the best! Fixed the freeze-burst pipe and got us back online in no time (less than 30 minutes). Charged a very fair price, too.", name: "Bill Fitzgerald", src: "Google" },
  { text: "He fixed my toilet quickly and didn't charge a fortune. I'll definitely call him again if I need a plumber in the future.", name: "Ashley Mathis", src: "Google" },
  { text: "He did a fantastic job. I cant recommend him more.", name: "Sam Kirkpatrick", src: "Google" },
  { text: "Excellent customer service! Very knowledgeable and quick!", name: "Brandy Parker", src: "Google" },
  { text: "Fantastic work!", name: "Wes Kirtley", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function CoopsPlumbing() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("coops-auth") === "1");
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "Plumber North Little Rock AR | Coop's Plumbing";
    const els = [];
    ...
    add("meta",{name:"description",content:"24hr plumber in North Little Rock, AR. Residential & commercial. Call Chris at (501) 766-9573."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"Coop's Plumbing Service",telephone:"+15017669573",
      address:{"@type":"PostalAddress",streetAddress:"7821 Circle R Rd",addressLocality:"North Little Rock",addressRegion:"AR",postalCode:"72118",addressCountry:"US"},
      ...
      founder:{"@type":"Person",name:"Chris Cooper"},
      description:"Coop's Plumbing Service provides 24-hour residential and commercial plumbing in North Little Rock, AR and surrounding areas.",
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1C2B41"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8}}><span style={{fontWeight:700,color:"#1C2B41"}}>Coop's</span> <span style={{color:"#C8934F"}}>Plumbing</span></div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(28,43,65,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}><span>Coop's</span> <span style={{color:"var(--accent)"}}>Plumbing</span></div>
      ...
      <div className="hero-grid" style={{maxWidth:1200,width:"100%",display:"grid",gridTemplateColumns:"1.8fr 1fr",gap:48,alignItems:"center"}}>
        <div>
          <h1 style={{fontSize:"clamp(48px, 6vw, 84px)",lineHeight:1.0,color:"var(--primary)",letterSpacing:-2,marginBottom:24}}>
            The Plumber<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>North Little Rock</span><br/>Calls First
          </h1>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:32}}>
            <Stars />
            <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"Chris is the best! ...very fair price." — Bill Fitzgerald</span>
          </div>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        ...
        <div className="trust-mobile" style={{display:"none",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:48,textAlign:"center"}}>
          {[{num:"12+",label:"Reviews",desc:"Community recommended"},{num:"24hr",label:"Available",desc:"Day or night"},{num:"Res &",label:"Commercial",desc:"All property types"},{num:"BBB",label:"Listed",desc:"Verified business"}].map((s,i)=>(
            <div key={i}>
        ...
        <p style={{textAlign:"center",marginTop:48,fontSize:15,color:"var(--text-light)"}}>Questions? Call Chris directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{maxWidth:800,margin:"0 auto",position:"relative",zIndex:1}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,textAlign:"center",opacity:0.7}}>About Coop's Plumbing</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff",textAlign:"center"}}>Why North Little Rock Trusts<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Chris Cooper</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:24,textAlign:"center"}}>
          Coop's Plumbing is a solo operation run by Chris Cooper, serving North Little Rock and the surrounding communities. When you call, Chris answers. When someone shows up at your door, it's Chris. That direct accountability is what keeps customers coming back and recommending him in local Facebook groups.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:36,textAlign:"center"}}>
          Chris is known for fast response times — one customer had a freeze-burst pipe fixed and water running again in under 30 minutes. He handles residential and commercial work, charges fair prices, and is available around the clock. No call centers, no upsells, just honest plumbing from a tradesman who takes pride in his work.
        </p>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:20}}>
          {["Solo Operator — Direct Accountability","24-Hour Availability","Residential & Commercial","Fair & Transparent Pricing","BBB Listed Business","Community Recommended"].map((c,i)=>(
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>Got a plumbing problem?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Chris.</span></h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>Available 24 hours. Fair pricing, every time. Residential and commercial.</p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16}}><span>Coop's</span> <span style={{color:"var(--accent)"}}>Plumbing</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>24-hour plumbing service for residential and commercial properties in North Little Rock and Central Arkansas.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>7821 Circle R Rd<br/>North Little Rock, AR 72118<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} Coop's Plumbing Service. All rights reserved.</span>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Coop's Plumbing</a>
    </div>
