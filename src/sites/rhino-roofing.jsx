// Layout: B | Industry: Roofing | City: Benton
import { useState, useEffect } from "react";

const PHONE = "(501) 813-9009";
const TEL = "tel:+15018139009";
...
const Icons = {
  ...
  search: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/><path d="M8 11h6"/><path d="M11 8v6"/></svg>,
...
const services = [
  { name: "Roof Replacement", desc: "Full tear-off and replacement using Atlas shingles backed by a 20-year labor warranty and 60-year material warranty. Every job is handled with precision from start to finish.", icon: Icons.roof },
  { name: "Emergency Repair", desc: "Leaks and storm damage don't wait -- and neither does Rhino. Same-day response when your roof needs urgent attention, with honest assessments and fast solutions.", icon: Icons.emergency },
  { name: "Gutter Systems", desc: "Protect your home's foundation and landscaping with professionally installed gutter systems. Custom-fit to your roofline and built to handle Arkansas weather year-round.", icon: Icons.gutter },
  { name: "Storm Damage", desc: "Central Arkansas sees its share of severe weather. Rhino Roofing handles full storm damage restoration -- from initial assessment through completed repairs, fast and thorough.", icon: Icons.storm },
  { name: "Insurance Claims", desc: "Navigating insurance claims is stressful. Rhino works directly with your insurance company to document damage, file paperwork, and get your roof replaced with zero hassle.", icon: Icons.clipboard },
  { name: "Free Inspections", desc: "Not sure about your roof's condition? Schedule a free, no-obligation inspection. Rhino gives you an honest assessment -- no pressure, no surprise charges, just the facts.", icon: Icons.search },
];

const allServiceNames = ["Roof Replacement","Emergency Roof Repair","Gutter Systems","Storm Damage Restoration","Insurance Claims Assistance","Residential Roofing","Commercial Roofing","Free Roof Inspections"];

const areas = ["Benton","Little Rock","Conway","Bryant","Jacksonville","Sherwood","North Little Rock","Cabot","Maumelle","Hot Springs"];

const reviews = [
  { text: "I used Rhino Roofing for a roof repair last year and they were great! Honest and upfront about pricing.", name: "Emma Blosfelds", src: "Google" },
  { text: "Dylan and his team at Rhino Roofing did an amazing job on our house.", name: "Devin Simpson", src: "Google" },
  { text: "Dylan, Chris, Camila & the entire crew are absolutely wonderful!", name: "Tammi Mason", src: "Google" },
  { text: "I called Rhino Roofing because of a roof leak. Chris & Wyatt came out that same day.", name: "Shelia Gjertsen", src: "Google" },
  { text: "After meeting with four roofing companies, Rhino Roofing was far above all the others. Honesty, Integrity.", name: "Dee Callaham", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function RhinoRoofing() {
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="roof"){sessionStorage.setItem("rr-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "Roofing Contractor Benton AR | Rhino Roofing";
    const els = [];
    ...
    add("meta",{name:"description",content:"Trusted roofing contractor in Benton, AR. Perfect 5.0 stars, 99 reviews. Atlas shingles, 20-yr warranty. Call (501) 813-9009."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"Rhino Roofing",telephone:"+15018139009",
      email:"arrhinoroofing@gmail.com",
      address:{"@type":"PostalAddress",streetAddress:"303 N Market St",addressLocality:"Benton",addressRegion:"AR",postalCode:"72015",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:34.5645,longitude:-92.5868},
      openingHoursSpecification:[{"@type":"OpeningHoursSpecification",dayOfWeek:["Monday","Tuesday","Wednesday","Thursday","Friday"],opens:"08:00",closes:"17:00"}],
      aggregateRating:{"@type":"AggregateRating",ratingValue:"5.0",reviewCount:"99",bestRating:"5"},
      founder:[{"@type":"Person",name:"Chris Heredia"},{"@type":"Person",name:"Dylan Dodd"}],
      foundingDate:"2022",
      legalName:"Rock's Finest Contractors LLC dba Rhino Roofing",
      description:"Rhino Roofing is a trusted roofing contractor in Benton, AR serving Central Arkansas. Perfect 5.0-star rating with 99 reviews. Atlas shingles with 20-year labor and 60-year material warranty. BBB Listed.",
      ...
      memberOf:[{name:"BBB"},{name:"Atlas Certified"}],
      sameAs:[],
    })});
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1E1E1E"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8,fontWeight:600,letterSpacing:-0.5}}><span style={{color:"#1E1E1E"}}>Rhino</span> <span style={{color:"#8B3A3A"}}>Roofing</span></div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(30,30,30,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}><span>Rhino</span> <span style={{color:"var(--accent)"}}>Roofing</span></div>
    ...
    <section className="hero-pad" style={{padding:"180px clamp(24px,5vw,64px) 100px",background:"var(--cream)",textAlign:"center"}}>
      <div style={{maxWidth:800,margin:"0 auto"}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(139,58,58,0.15)",padding:"8px 18px",borderRadius:4,marginBottom:28}}>
          <div style={{width:6,height:6,borderRadius:"50%",background:"var(--accent)"}} />
          <span style={{fontSize:12,fontWeight:700,color:"var(--accent)",letterSpacing:2,textTransform:"uppercase"}}>Perfect 5.0 Stars -- All 99 Reviews Are 5-Star</span>
        </div>
        <h1 style={{fontSize:"clamp(44px, 6vw, 80px)",lineHeight:1.05,color:"var(--primary)",marginBottom:24,letterSpacing:-2}}>
          Professional Roofing<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Benton Trusts</span>
        </h1>
        <p style={{fontSize:18,lineHeight:1.7,color:"var(--text-mid)",marginBottom:32,maxWidth:620,margin:"0 auto 32px"}}>
          Rhino Roofing has built a perfect 5.0-star record since 2022. Atlas shingles with a 20-year labor warranty and 60-year material warranty, serving Benton and Central Arkansas.
        </p>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:36,flexWrap:"wrap"}}>
          <Stars />
          <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"After meeting with four companies, Rhino was far above all the others." -- Dee Callaham</span>
        </div>
        <div style={{display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap"}}>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(139,58,58,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Rhino Roofing</a>
      ...
      <div style={{maxWidth:480,margin:"0 auto"}}>
        <div style={{background:"var(--white)",borderRadius:12,padding:"44px 36px",boxShadow:"0 4px 40px rgba(0,0,0,0.08)",border:"1px solid var(--border)",marginBottom:56}}>
          {!formDone ? (
            ...
            <div style={{textAlign:"center",padding:"24px 0"}}>
              <div style={{width:56,height:56,borderRadius:"50%",background:"rgba(139,58,58,0.15)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px",color:"var(--accent)"}}>{Icons.check}</div>
        ...
        <div className="trust-row" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:24,textAlign:"center"}}>
          {[
            ...
            {num:"99+",label:"Reviews",desc:"All 5-star"},
            {num:"Atlas",label:"Certified",desc:"Premium shingles"},
            {num:"20yr",label:"Warranty",desc:"Labor guarantee"},
          ].map((s,i)=>(
            <div key={i}>
    ...
    <section id="services" className="section-pad" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:60}}>
          ...
          <h2 style={{fontSize:"clamp(30px, 3.5vw, 44px)",lineHeight:1.15,color:"var(--primary)",letterSpacing:-0.5}}>Roofing Services in Benton</h2>
        </div>
        ...
        <p style={{textAlign:"center",marginTop:40,fontSize:15,color:"var(--text-mid)"}}>Questions? Call Dylan directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:700}}>{PHONE}</a></p>
      </div>
    </section>
    ...
    <section id="about" className="section-pad" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)"}}>
      <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <p style={{fontSize:12,fontWeight:700,letterSpacing:3,textTransform:"uppercase",color:"rgba(255,255,255,0.5)",marginBottom:14}}>Why Rhino Roofing</p>
        <h2 style={{fontSize:"clamp(30px, 3.5vw, 44px)",lineHeight:1.15,color:"#fff",letterSpacing:-0.5,marginBottom:40}}>A Perfect Record Built on <span style={{fontStyle:"italic",color:"var(--accent)"}}>Honesty and Integrity</span></h2>
        <p style={{fontSize:17,lineHeight:1.8,color:"rgba(255,255,255,0.75)",marginBottom:24}}>
          Dylan Dodd runs customer operations at Rhino Roofing, and it shows. Since founding in 2022, Dylan, Chris Heredia, Wyatt, and Camila have built something rare in roofing -- a perfect 5.0-star rating across 99 reviews. Not 4.8. Not 4.9. Every single customer gave them five stars.
        </p>
        <p style={{fontSize:17,lineHeight:1.8,color:"rgba(255,255,255,0.75)",marginBottom:48}}>
          Rhino installs Atlas shingles backed by a 20-year labor warranty and a 60-year material warranty. They serve a 5-county area across Central Arkansas, from Benton and Little Rock to Conway and Hot Springs. BBB Listed with the kind of track record that speaks for itself.
        </p>
        <div className="why-creds-row" style={{display:"flex",justifyContent:"center",gap:28,flexWrap:"wrap"}}>
          {["Perfect 5.0 Rating","Atlas Certified Installer","BBB Listed","20-Year Labor Warranty","60-Year Material Warranty"].map((c,i)=>(
    ...
    <section id="reviews" className="section-pad" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--white)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:60}}>
          ...
          <h2 style={{fontSize:"clamp(30px, 3.5vw, 44px)",lineHeight:1.15,color:"var(--primary)",letterSpacing:-0.5}}>5.0 Stars on Google</h2>
        </div>
        <div className="rev-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:20}}>
          {[reviews[0], reviews[3], reviews[4]].map((r,i)=>(
    ...
    <section className="section-pad" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--white)",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
        <h2 style={{fontSize:"clamp(30px, 3.5vw, 44px)",lineHeight:1.15,color:"var(--primary)",letterSpacing:-0.5,marginBottom:20,fontFamily:"'Playfair Display',serif"}}>
          Need a new roof?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Rhino Roofing.</span>
        </h2>
        <p style={{fontSize:17,lineHeight:1.7,color:"var(--text-mid)",marginBottom:36}}>Honest assessments, Atlas shingles, and a 20-year labor warranty. Perfect 5.0 stars across 99 reviews -- every single one a five.</p>
        <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(139,58,58,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} {PHONE}</a>
      </div>
    </section>
    ...
    <footer style={{padding:"80px clamp(24px,5vw,64px) 40px",background:"var(--primary)"}}>
      <div className="footer-grid" style={{maxWidth:1200,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,marginBottom:48}}>
        <div>
          <div className="serif" style={{fontSize:22,fontWeight:600,letterSpacing:-0.5,marginBottom:16}}><span style={{color:"#fff"}}>Rhino</span> <span style={{color:"var(--accent)"}}>Roofing</span></div>
          <p style={{fontSize:14,lineHeight:1.7,color:"rgba(255,255,255,0.6)",marginBottom:16}}>Trusted roofing contractor serving Benton and Central Arkansas. Perfect 5.0 stars, Atlas Certified, BBB Listed.</p>
          <p style={{fontSize:14,color:"rgba(255,255,255,0.5)"}}>303 N Market St<br/>Benton, AR 72015</p>
          <a href={TEL} style={{display:"inline-block",marginTop:12,fontSize:15,fontWeight:700,color:"var(--accent)"}}>{PHONE}</a>
          <br/>
          <a href="mailto:arrhinoroofing@gmail.com" style={{display:"inline-block",marginTop:4,fontSize:14,color:"rgba(255,255,255,0.5)"}}>arrhinoroofing@gmail.com</a>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.1)",paddingTop:24}}>
        <div className="footer-btm-inner" style={{maxWidth:1200,margin:"0 auto",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <p style={{fontSize:13,color:"rgba(255,255,255,0.35)"}}>&copy; {yr} Rock's Finest Contractors LLC dba Rhino Roofing. All rights reserved.</p>
          <p style={{fontSize:13,color:"rgba(255,255,255,0.35)"}}>BBB Listed | Atlas Certified</p>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,width:"100%",padding:"18px 24px",background:"var(--accent)",color:"#fff",fontSize:16,fontWeight:700,fontFamily:"'DM Sans',sans-serif",boxShadow:"0 -4px 20px rgba(0,0,0,0.15)"}}>{Icons.phone} Call Rhino Roofing Now</a>
    </div>
