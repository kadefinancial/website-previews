...
const PHONE = "(501) 271-3652";
const TEL = "tel:+15012713652";
...
const services = [
  { name: "Air Conditioning Repair & Service", desc: "Fast, reliable AC diagnostics and repairs for all makes and models. When your system goes down in the Arkansas heat, Casey is there to get it running right.", icon: Icons.wrench },
  { name: "AC Installation", desc: "Complete air conditioning installation sized perfectly for your home. Quality equipment, expert setup, and built to handle Central Arkansas summers.", icon: Icons.wind },
  { name: "Heating Repair & Service", desc: "Keep your family warm through every Arkansas winter. Prompt heating repairs with honest assessments and transparent pricing on every call.", icon: Icons.flame },
  { name: "Heating Installation", desc: "New furnace and heating system installation tailored to your home's needs. Properly sized, professionally installed, and backed by the owner's guarantee.", icon: Icons.thermometer },
  { name: "HVAC System Replacement", desc: "When repair no longer makes sense, trust Casey to recommend the right replacement at a fair price. Full system changeouts with minimal disruption to your home.", icon: Icons.home },
  { name: "Seasonal Maintenance Plans", desc: "Twice-a-year maintenance keeps your system efficient and catches problems early. Affordable plans so you never forget a tune-up again.", icon: Icons.calendar },
];

const serviceNames = ["Air Conditioning Repair & Service","AC Installation","Heating Repair & Service","Heating Installation","HVAC System Replacement","Seasonal Maintenance Plans"];

const areas = ["North Little Rock","Central Arkansas"];

const reviews = [
  { text: "They were very professional and in and out in no time. Their 2X a year deal is great because it makes sure you don't forget! I highly recommend!", name: "Pattiann Smith-Mayabb", src: "Google" },
  { text: "The owner personally came out to make sure we were taken care of. He made sure to communicate constantly throughout the process and made sure I was always in the loop with what was going on. Will use them going forward.", name: "Matthew Holmes", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function AmericaFirstAirConditioningAndHeating() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("afac-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="roof"){sessionStorage.setItem("afac-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "HVAC Service North Little Rock AR | America First AC & Heating";
    const els = [];
    ...
    add("meta",{name:"description",content:"HVAC repair, installation & maintenance in North Little Rock, AR. 4.7 stars on Google. Owner on every job. Call (501) 271-3652."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"America First Air Conditioning and Heating",telephone:"+15012713652",
      address:{"@type":"PostalAddress",streetAddress:"1313 N Hills Blvd Suite 313",addressLocality:"North Little Rock",addressRegion:"AR",postalCode:"72114",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:34.7695,longitude:-92.2668},
      openingHoursSpecification:[],
      aggregateRating:{"@type":"AggregateRating",ratingValue:"4.7",reviewCount:"13",bestRating:"5"},
      founder:{"@type":"Person",name:"Casey Turbeville"},
      description:"America First Air Conditioning and Heating provides HVAC repair, installation, and maintenance services in North Little Rock and Central Arkansas. 4.7 stars, owner on every job.",
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1C2B41"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8,fontWeight:600,letterSpacing:-0.5}}><span style={{color:"#1C2B41"}}>America First</span> <span style={{color:"#C8934F"}}>AC & Heating</span></div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(28,43,65,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5,cursor:"pointer"}} onClick={()=>window.scrollTo({top:0,behavior:"smooth"})}><span>America First</span> <span style={{color:"var(--accent)"}}>AC & Heating</span></div>
    ...
    <section style={{padding:"180px clamp(24px,5vw,64px) 100px",background:"var(--cream)",textAlign:"center"}}>
      <div style={{maxWidth:800,margin:"0 auto"}}>
        ...
        <h1 style={{fontSize:"clamp(44px, 6vw, 80px)",lineHeight:1.05,color:"var(--primary)",marginBottom:24,letterSpacing:-2}}>
          North Little Rock's<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Trusted HVAC</span> Team
        </h1>
        <p style={{fontSize:18,lineHeight:1.7,color:"var(--text-mid)",marginBottom:32,maxWidth:600,margin:"0 auto 32px"}}>
          Air conditioning and heating service from an owner who personally shows up on every job. Honest work, clear communication, and results you can count on.
        </p>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:36}}>
          <Stars />
          <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"Very professional and in and out in no time. I highly recommend!" — Pattiann Smith-Mayabb</span>
        </div>
    ...
    <section id="form" style={{padding:"80px clamp(24px,5vw,64px)",background:"var(--white)",borderTop:"1px solid var(--border)",borderBottom:"1px solid var(--border)"}}>
      <div style={{maxWidth:1200,margin:"0 auto",display:"flex",flexDirection:"column",alignItems:"center"}}>
        ...
        <div className="trust-row" style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:32,marginTop:56,maxWidth:800,width:"100%",textAlign:"center"}}>
          {[
            {num:"4.7",label:"Google Rating",desc:"Based on 13 reviews"},
            {num:"13+",label:"Customer Reviews",desc:"Verified on Google"},
            {num:"2x/yr",label:"Maintenance Plans",desc:"Never miss a tune-up"},
            {num:"Owner",label:"On Every Job",desc:"Casey personally handles it"},
          ].map((s,i)=>(
            <div key={i}>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        ...
        <p style={{textAlign:"center",marginTop:48,fontSize:15,color:"var(--text-mid)"}}>
          Questions? Call Casey directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a>
        </p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)"}}>
      <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <p style={{fontSize:12,fontWeight:700,letterSpacing:3,textTransform:"uppercase",color:"var(--accent)",marginBottom:14}}>Why America First</p>
        <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",lineHeight:1.15,color:"#fff",letterSpacing:-0.5,marginBottom:40}}>The Owner Shows Up</h2>
        <p style={{fontSize:17,lineHeight:1.8,color:"rgba(255,255,255,0.75)",marginBottom:24}}>
          At America First Air Conditioning and Heating, you don't get a random technician — you get Casey Turbeville, the owner. Casey personally handles every job and every customer call because he believes that's how a small business should run. When he says he'll take care of it, he means he'll be the one doing the work.
        </p>
        <p style={{fontSize:17,lineHeight:1.8,color:"rgba(255,255,255,0.75)",marginBottom:48}}>
          Serving North Little Rock and Central Arkansas, America First is built on clear communication, honest pricing, and doing the job right the first time. From a simple AC repair to a full system replacement, Casey keeps you in the loop every step of the way. No surprises, no runaround — just dependable HVAC service from someone who takes personal pride in every installation, every repair, and every maintenance visit.
        </p>
        <div className="why-creds-row" style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:24}}>
          {["Owner On Every Job","Honest, Upfront Pricing","Clear Communication","Residential HVAC Specialist","Seasonal Maintenance Plans","Serving Central Arkansas"].map((c,i)=>(
    ...
    {/* REVIEWS -- Layout B: 2 cards side by side (only 2 reviews available) */}
    <section id="reviews" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--white)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        ...
        <div className="rev-grid" style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:20,maxWidth:880,margin:"0 auto"}}>
          {reviews.map((r,i)=>(
    ...
    <section style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--white)",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
        <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",lineHeight:1.15,color:"var(--primary)",letterSpacing:-0.5,marginBottom:20}}>
          HVAC not working right?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Casey.</span>
        </h2>
        <p style={{fontSize:17,lineHeight:1.7,color:"var(--text-mid)",marginBottom:36}}>
          Honest pricing, clear communication, and the owner on every job. North Little Rock trusts America First for a reason.
        </p>
    ...
    <footer className="mob-pad" style={{padding:"80px clamp(24px,5vw,64px) 40px",background:"var(--primary)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div className="footer-grid" style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:48,marginBottom:48}}>
          <div>
            <div className="serif" style={{fontSize:22,fontWeight:600,letterSpacing:-0.5,marginBottom:16}}><span style={{color:"#fff"}}>America First</span> <span style={{color:"var(--accent)"}}>AC & Heating</span></div>
            <p style={{fontSize:14,lineHeight:1.7,color:"rgba(255,255,255,0.6)",marginBottom:16,maxWidth:320}}>HVAC repair, installation, and maintenance in North Little Rock and Central Arkansas. Owner on every job.</p>
            <p style={{fontSize:14,color:"rgba(255,255,255,0.6)",marginBottom:6}}>1313 N Hills Blvd Suite 313, North Little Rock, AR 72114</p>
        ...
        <div style={{borderTop:"1px solid rgba(255,255,255,0.1)",paddingTop:24}}>
          <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <p style={{fontSize:13,color:"rgba(255,255,255,0.35)"}}>{"\u00A9"} {yr} America First Air Conditioning and Heating. All rights reserved.</p>
          </div>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",alignItems:"center",justifyContent:"center",gap:10,width:"100%",padding:"16px",background:"var(--accent)",color:"#fff",fontSize:16,fontWeight:700,fontFamily:"'DM Sans',sans-serif",boxShadow:"0 -4px 20px rgba(0,0,0,0.15)"}}>{Icons.phone} Call America First Now</a>
    </div>
  </>);
}
