...
const Icons = {
  ...
  duct: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="6" width="18" height="12" rx="2"/><path d="M3 10h18"/><path d="M3 14h18"/><path d="M7 6v12"/><path d="M17 6v12"/></svg>
  ),
  check: (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="20 6 9 17 4 12"/></svg>
  ),
  star: (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="#C17F5E" stroke="none"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
  ),
  mapPin: (
...
// --- Data ---
const PHONE = "(501) 397-1670";
const PHONE_TEL = "tel:+15013971670";

const services = [
  { name: "Air Conditioning Repair & Service", desc: "When Arkansas heat bears down, your AC needs to perform. Our team diagnoses problems fast, gets your system back up and running, and keeps it maintained so you stay cool all summer long.", icon: Icons.wind },
  { name: "Heating Repair & Service", desc: "From furnace failures to heat pump issues, we handle every heating problem with precision. Stay warm through Central Arkansas winters with reliable repairs and seasonal maintenance from a team that knows these systems inside and out.", icon: Icons.flame },
  { name: "HVAC Sales & Installation", desc: "Whether you're upgrading an outdated system or outfitting a new home, we help you choose the right equipment and install it properly. As an Elite Heil dealer, we offer top-tier systems built for Arkansas conditions.", icon: Icons.thermometer },
  { name: "Duct Work", desc: "Leaky or poorly designed ductwork wastes energy and money. We design, repair, and replace duct systems to ensure every room in your home gets the airflow it needs for maximum comfort and efficiency.", icon: Icons.duct },
  { name: "New Construction HVAC", desc: "Building a new home or commercial space? We work with builders and homeowners from blueprint to final inspection, designing and installing complete HVAC systems sized perfectly for the structure.", icon: Icons.home },
  { name: "24-Hour Emergency Service", desc: "Heating and cooling emergencies don't wait for business hours. Call Aaron or Matt any time -- day or night -- and our team responds fast when you need us most. That's the Ski Brothers difference.", icon: Icons.zap },
];

const allServices = [
  "Air Conditioning Repair & Service", "Heating Repair & Service", "HVAC Sales & Installation",
  "Duct Work", "New Construction HVAC", "24-Hour Emergency Service"
];

const areas = ["Little Rock", "North Little Rock", "Central Arkansas"];

// --- Main Component ---
export default function BrothersHeatAndAir() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("ski-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("ski-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "HVAC Service Little Rock AR | Ski Brothers Heat and Air";
    const els = [];
    ...
    add("meta", { name: "description", content: "Ski Brothers Heat and Air — 5.0 stars, 693+ reviews. Elite Heil dealer. 24-hour HVAC service in Little Rock, AR. Call (501) 397-1670." });
    ...
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      ...
      name: "Ski Brothers Heat and Air", telephone: "+15013971670",
      email: "skibrothersheatandair@gmail.com",
      address: { "@type": "PostalAddress", streetAddress: "12714 Clinton Rd", addressLocality: "North Little Rock", addressRegion: "AR", postalCode: "72118", addressCountry: "US" },
      geo: { "@type": "GeoCoordinates", latitude: 34.7465, longitude: -92.2896 },
      openingHoursSpecification: [
        { "@type": "OpeningHoursSpecification", dayOfWeek: ["Monday","Tuesday","Wednesday","Thursday","Friday"], opens: "07:00", closes: "17:00" },
        { "@type": "OpeningHoursSpecification", dayOfWeek: ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"], opens: "00:00", closes: "23:59", description: "Emergency service" }
      ],
      aggregateRating: { "@type": "AggregateRating", ratingValue: "5.0", reviewCount: "693", bestRating: "5" },
      founder: [
        { "@type": "Person", name: "Aaron Chwalinski" },
        { "@type": "Person", name: "Matt Chwalinski" }
      ],
      description: "Brothers Aaron and Matt Chwalinski — Ski Brothers Heat and Air. Elite Heil dealer, BBB Accredited, 24-hour emergency HVAC service in Little Rock and Central Arkansas.",
      ...
      memberOf: [
        ...
        { "@type": "Organization", name: "BNI Game Changers" }
      ],
      sameAs: [],
    })});
        ...
        <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#1C2B41" }}>
          <div style={{ background:"#fff", borderRadius:16, padding:"56px 44px", textAlign:"center", maxWidth:420, width:"92%", boxShadow:"0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:28, fontWeight:600, letterSpacing:-0.5, marginBottom:4 }}>
              <span style={{ color:"#1C2B41" }}>Ski</span>{" "}<span style={{ color:"#C17F5E" }}>Brothers</span>
            </div>
    ...
    <>
      ...
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display:"flex", alignItems:"center", justifyContent:"space-between", background: scrolled ? "rgba(28,43,65,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition:"all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        <div className="serif" style={{ fontSize:22, fontWeight:600, letterSpacing:-0.5, color:"#fff" }}>
          <span>Ski</span>{" "}<span style={{ color:"var(--accent)" }}>Brothers</span>
        </div>
        ...
        <div className="hero-grid" style={{ maxWidth:1200, width:"100%", display:"grid", gridTemplateColumns:"1fr 1fr", gap:64, alignItems:"center" }}>
          ...
          <div style={{ maxWidth:560 }}>
            ...
            <p style={{ fontSize:18, lineHeight:1.7, color:"rgba(255,255,255,0.75)", marginBottom:32, maxWidth:480 }}>
              Brothers Aaron and Matt Chwalinski built Ski Brothers Heat and Air on honest work and family values. Elite Heil dealer, BBB Accredited, and backed by 693+ five-star reviews. 24-hour emergency service for Little Rock and Central Arkansas.
            </p>
            {/* Star row with hero quote -- showing 5 stars for this specific 5-star review */}
            <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:32, flexWrap:"wrap" }}>
              <Stars />
              <span style={{ fontSize:14, color:"rgba(255,255,255,0.7)", fontStyle:"italic" }}>"They are the best and the cheapest" -- Tosha Hartman</span>
            </div>
            <div style={{ display:"flex", gap:16, flexWrap:"wrap" }}>
              <a href={PHONE_TEL} style={{ display:"inline-flex", alignItems:"center", gap:10, background:"var(--accent)", color:"#fff", padding:"18px 36px", borderRadius:6, fontSize:16, fontWeight:700, letterSpacing:0.3, fontFamily:"'DM Sans',sans-serif", boxShadow:"0 4px 20px rgba(193,127,94,0.3)", transition:"transform 0.2s" }}>
      ...
      <section style={{ background:"var(--white)", padding:"48px clamp(24px,5vw,64px)" }}>
        <div className="trust-grid" style={{ maxWidth:1200, margin:"0 auto", display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:32, textAlign:"center" }}>
          {[
            ...
            { num: "693+", label: "5-Star Reviews", desc: "Trusted by homeowners" },
            { num: "24hr", label: "Emergency Service", desc: "Day or night, every day" },
            { num: "Elite", label: "Heil Dealer", desc: "Factory authorized" },
          ].map((s, i) => (
      ...
      <section id="services" className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          <div style={{ textAlign:"center", marginBottom:64 }}>
            ...
            <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(30px,3.5vw,44px)", lineHeight:1.15, letterSpacing:-0.5, color:"var(--primary)" }}>HVAC Services in Little Rock, Arkansas</h2>
          </div>
          {services.map((svc, i) => {
            const isEven = i % 2 === 1;
            return (
          ...
          <p style={{ textAlign:"center", marginTop:48, fontSize:15, color:"var(--text-mid)" }}>
            Questions? Call Aaron or Matt directly at <a href={PHONE_TEL} style={{ color:"var(--accent)", fontWeight:600 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          <div style={{ textAlign:"center", marginBottom:64 }}>
            ...
            <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(30px,3.5vw,44px)", lineHeight:1.15, letterSpacing:-0.5, color:"var(--primary)" }}>The Ski Brothers Story</h2>
          </div>
          <div className="why-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:64, alignItems:"start" }}>
            ...
            <div>
              <p style={{ fontSize:17, lineHeight:1.8, color:"var(--text-mid)", marginBottom:20 }}>
                Brothers Aaron and Matt Chwalinski grew up in Central Arkansas, both attending Catholic High School before going into the trades together. The name "Ski Brothers" comes from their last name -- Chwalinski -- and it stuck. What started as two brothers who knew how to fix things has grown into one of the most reviewed HVAC companies in the state, with a perfect 5.0 Google rating backed by nearly 700 reviews.
              </p>
              <p style={{ fontSize:17, lineHeight:1.8, color:"var(--text-mid)", marginBottom:32 }}>
                As an Elite Heil dealer and BBB Accredited business, Ski Brothers brings both the credentials and the character that homeowners look for. Aaron and Matt still run every aspect of the operation -- from new construction installs to emergency calls at 2 AM. They're also proud members of BNI Game Changers, a testament to the relationships they've built across the Little Rock business community. When you call Ski Brothers, you're not getting a call center. You're getting Aaron or Matt.
              </p>
              ...
              <div style={{ position:"relative", borderLeft:"3px solid var(--accent)", paddingLeft:24, marginBottom:36 }}>
                ...
                <p style={{ fontSize:14, color:"var(--text-light)", marginTop:8, fontFamily:"'DM Sans',sans-serif" }}>-- Tosha Hartman</p>
              </div>
              {/* Credentials */}
              <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                {[
                  "Perfect 5.0 Google Rating — 693+ Reviews",
                  "Elite Heil Dealer — Factory Authorized",
                  "BBB Accredited Business",
                  "BNI Game Changers Member",
            ...
            <div className="badge-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
              {[
                { icon: Icons.users, title: "Brother-Owned", desc: "Aaron and Matt Chwalinski run every aspect of the business. When you call Ski Brothers, you're talking to a real person who cares." },
                { icon: Icons.award, title: "Elite Heil Dealer", desc: "Factory-authorized Heil dealer status means access to top-tier equipment and the training to install it right the first time." },
                { icon: Icons.shield, title: "BBB Accredited", desc: "Better Business Bureau accreditation reflects our commitment to honest business practices and customer satisfaction." },
                { icon: Icons.clock, title: "24-Hour Emergency", desc: "Heating and cooling emergencies don't keep business hours. Call any time, day or night, and our team responds fast." },
              ].map((b, i) => (
      ...
      {/* REVIEWS -- Layout C: white bg, single LARGE testimonial + second review */}
      <section id="reviews" className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--white)" }}>
        <div style={{ maxWidth:800, margin:"0 auto", textAlign:"center" }}>
          ...
          <div style={{ position:"relative", maxWidth:700, margin:"0 auto" }}>
            ...
            <p style={{ fontSize:17, fontWeight:700, color:"var(--primary)", marginBottom:4 }}>Tosha Hartman</p>
            <p style={{ fontSize:14, color:"var(--text-light)" }}>Google Review</p>
          </div>
          <p style={{ fontSize:14, color:"var(--text-light)", marginTop:40 }}>One of 693+ five-star reviews on Google</p>
          ...
          <div style={{ maxWidth:600, margin:"48px auto 0", padding:"36px 30px", background:"var(--cream)", borderRadius:12, border:"1px solid var(--border)" }}>
            ...
            <p style={{ fontSize:15, fontWeight:700, color:"var(--primary)", marginBottom:2 }}>Karen Ingram</p>
      ...
      <section className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--white)", borderTop:"1px solid var(--border)" }}>
        <div style={{ maxWidth:700, margin:"0 auto", textAlign:"center" }}>
          <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(30px,3.5vw,44px)", lineHeight:1.15, letterSpacing:-0.5, color:"var(--primary)", marginBottom:20 }}>
            ...
            <em style={{ color:"var(--accent)", fontStyle:"italic" }}>Call Aaron or Matt.</em>
          </h2>
          <p style={{ fontSize:17, lineHeight:1.7, color:"var(--text-mid)", marginBottom:36, maxWidth:520, margin:"0 auto 36px" }}>
            Elite Heil dealer, BBB Accredited, and backed by a perfect 5.0 Google rating with 693+ reviews. 24-hour emergency service for Little Rock and Central Arkansas. Honest pricing, expert work.
          </p>
          <a href={PHONE_TEL} style={{ display:"inline-flex", alignItems:"center", gap:10, background:"var(--accent)", color:"#fff", padding:"18px 42px", borderRadius:6, fontSize:18, fontWeight:700, letterSpacing:0.3, fontFamily:"'DM Sans',sans-serif", boxShadow:"0 4px 20px rgba(193,127,94,0.3)", transition:"transform 0.2s" }}>
            {Icons.phone} {PHONE}
          </a>
        </div>
      </section>
      ...
      <footer style={{ background:"var(--primary)", padding:"80px clamp(24px,5vw,64px) 40px", color:"rgba(255,255,255,0.75)" }}>
        <div className="footer-grid" style={{ maxWidth:1200, margin:"0 auto", display:"grid", gridTemplateColumns:"1.4fr 1fr 1fr", gap:48, marginBottom:56 }}>
          ...
          <div>
            <div className="serif" style={{ fontSize:22, fontWeight:600, letterSpacing:-0.5, marginBottom:16 }}>
              <span style={{ color:"#fff" }}>Ski</span>{" "}<span style={{ color:"var(--accent)" }}>Brothers</span>
            </div>
            <p style={{ fontSize:15, lineHeight:1.7, marginBottom:20, maxWidth:300 }}>
              Brothers Aaron and Matt Chwalinski — Elite Heil dealer and BBB Accredited HVAC service for Little Rock and Central Arkansas.
            </p>
            <p style={{ fontSize:14, lineHeight:1.8, marginBottom:8 }}>12714 Clinton Rd, North Little Rock, AR 72118</p>
            <a href={PHONE_TEL} style={{ fontSize:14, color:"var(--accent)", fontWeight:600 }}>{PHONE}</a>
            <p style={{ fontSize:13, marginTop:8, color:"rgba(255,255,255,0.55)" }}>skibrothersheatandair@gmail.com</p>
          </div>
          {/* Col 2 -- Services */}
          <div>
          ...
          <div>
            ...
            <div style={{ marginTop:24 }}>
              <h4 style={{ fontSize:14, fontWeight:700, letterSpacing:1, textTransform:"uppercase", color:"#fff", marginBottom:12, fontFamily:"'DM Sans',sans-serif" }}>Hours</h4>
              <p style={{ fontSize:14, color:"rgba(255,255,255,0.65)", lineHeight:1.7 }}>Office: Mon-Fri 7 AM - 5 PM</p>
              <p style={{ fontSize:14, color:"rgba(255,255,255,0.65)", lineHeight:1.7 }}>Emergency: 24 Hours</p>
            </div>
          </div>
        </div>
        {/* Bottom bar */}
        <div style={{ borderTop:"1px solid rgba(255,255,255,0.1)", paddingTop:28 }}>
          <div className="footer-btm-inner" style={{ maxWidth:1200, margin:"0 auto", display:"flex", justifyContent:"space-between", alignItems:"center", fontSize:13, color:"rgba(255,255,255,0.45)" }}>
            <span>Ski Brothers Heat and Air. All rights reserved.</span>
            <span>Elite Heil Dealer | BBB Accredited | BNI Game Changers</span>
          </div>
        </div>
      </footer>
