// Layout: B | Industry: Plumber | City: Little Rock
import { useState, useEffect } from "react";

const PHONE = "(501) 455-3146";
const TEL = "tel:+15014553146";
...
const Icons = {
  ...
  pipe: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 14h6v6H4z"/><path d="M14 4h6v6h-6z"/><path d="M10 17h4a3 3 0 003-3V7"/></svg>,
...
const services = [
  { name: "Residential Plumbing", desc: "Complete plumbing solutions for your home. From faucet repairs to full re-pipes, Oak Forest handles every residential plumbing need with 70 years of expertise.", icon: Icons.home },
  { name: "Commercial Plumbing", desc: "Reliable plumbing service for businesses and commercial properties throughout Little Rock. Licensed, insured, and built for your schedule.", icon: Icons.building },
  { name: "Plumbing Installation", desc: "New fixture installation done right the first time. Water heaters, toilets, sinks, and complete plumbing systems for new construction and remodels.", icon: Icons.pipe },
  { name: "Leak Detection & Repair", desc: "Find and fix hidden leaks before they become costly water damage. Thorough diagnosis and lasting repairs from a team you can trust.", icon: Icons.search },
  { name: "Drain Cleaning", desc: "Stubborn clogs and slow drains cleared with professional equipment. Kitchen, bathroom, and main line service for homes and businesses.", icon: Icons.droplets },
  { name: "General Plumbing Repair", desc: "No job too small. Running toilets, dripping faucets, broken pipes, and everything in between. Call Brad for honest, fair-priced repairs.", icon: Icons.wrench },
];

const allServiceNames = ["General Plumbing Service & Repair","Residential Plumbing","Commercial Plumbing","Plumbing Installation","Leak Detection & Repair","Drain Cleaning"];

const areas = ["Little Rock","Southwest Little Rock","Benton","Bryant","Bauxite","Alexander","Shannon Hills","Sheridan"];

const reviews = [
  { text: "Oak Forest Plumbing is reliable, and they provide excellent service. My family has been calling them for all our plumbing issues for many years", name: "Marti Haslauer", src: "Google" },
  { text: "Brad and andrew are the best! A great father and son team that take pride in getting the job done right the first time!", name: "Sandy Rhodes", src: "Google" },
  { text: "When no one else had time or the knowledge to attend to our needs Oak Forrest Plumbing came through!", name: "Mary LeFlore", src: "Google" },
  { text: "Great family business! Fair pricing and good work!", name: "Nathan Smith", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function OakForestPlumbing() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("oakforest-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="2026"){sessionStorage.setItem("oakforest-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "Plumber Little Rock AR | Oak Forest Plumbing";
    const els = [];
    ...
    add("meta",{name:"description",content:"Oak Forest Plumbing — Little Rock's trusted plumber since 1956. BBB A+ rated. Call (501) 455-3146."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"Oak Forest Plumbing",telephone:"+15014553146",
      address:{"@type":"PostalAddress",streetAddress:"8303 Stagecoach Rd",addressLocality:"Little Rock",addressRegion:"AR",postalCode:"72210",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:34.6868,longitude:-92.3942},
      ...
      founder:{"@type":"Person",name:"Brad Smith"},foundingDate:"1956",
      description:"Oak Forest Plumbing has served Little Rock, AR and surrounding communities since 1956. Family owned, BBB A+ rated since 1986. Call (501) 455-3146.",
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1C2B41"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8,fontWeight:600,letterSpacing:-0.5}}><span style={{color:"#1C2B41"}}>Oak Forest</span> <span style={{color:"#C8934F"}}>Plumbing</span></div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(28,43,65,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}><span>Oak Forest</span> <span style={{color:"var(--accent)"}}>Plumbing</span></div>
    ...
    <section style={{padding:"180px clamp(24px,5vw,64px) 100px",background:"var(--cream)",textAlign:"center"}}>
      <div style={{maxWidth:780,margin:"0 auto"}}>
        <div style={{display:"inline-flex",alignItems:"center",gap:8,background:"rgba(200,147,79,0.1)",padding:"8px 18px",borderRadius:4,marginBottom:28}}>
          ...
          <span style={{fontSize:12,fontWeight:700,color:"var(--accent)",letterSpacing:2,textTransform:"uppercase"}}>Serving Little Rock Since 1956</span>
        </div>
        <h1 style={{fontSize:"clamp(44px, 6vw, 80px)",lineHeight:1.05,color:"var(--primary)",marginBottom:24,letterSpacing:-2}}>
          Professional Plumbing<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Little Rock Trusts</span>
        </h1>
        <p style={{fontSize:18,lineHeight:1.7,color:"var(--text-mid)",marginBottom:32,maxWidth:560,margin:"0 auto 32px"}}>
          For 70 years, Oak Forest Plumbing has been the name Little Rock families call. Now under Brad and Andrew Smith, the tradition of honest, quality plumbing continues.
        </p>
        <div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:36}}>
          <Stars />
          <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"A great father and son team that take pride in getting the job done right!" — Sandy Rhodes</span>
        </div>
        <div style={{display:"flex",gap:14,justifyContent:"center",flexWrap:"wrap"}}>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(200,147,79,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Brad Now</a>
    ...
    {/* FORM + TRUST — Layout B: Combined section, no rating displayed */}
        ...
        <div style={{background:"var(--white)",borderRadius:12,padding:"44px 36px",boxShadow:"0 4px 40px rgba(0,0,0,0.08)",border:"1px solid var(--border)",maxWidth:480}}>
          {!formDone ? (
            ...
            <div style={{textAlign:"center",padding:"24px 0"}}>
              ...
              <p style={{fontSize:14,color:"var(--text-mid)"}}>Brad will call you back shortly.<br/>Call <a href={TEL} style={{color:"var(--accent)",fontWeight:700}}>{PHONE}</a> for emergencies.</p>
            </div>
          )}
        </div>
        <div className="trust-row" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:32,alignContent:"center",height:"100%"}}>
          {[
            {num:"70",label:"Years",desc:"Serving since 1956"},
            {num:"A+",label:"BBB Rating",desc:"Since 1986"},
            {num:"Father",label:"& Son",desc:"Brad & Andrew Smith"},
            {num:"Family",label:"Owned",desc:"Community trusted"}
          ].map((s,i)=>(
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        ...
        <p style={{textAlign:"center",marginTop:40,fontSize:15,color:"var(--text-light)"}}>Questions? Call Brad directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",color:"#fff"}}>
      <div style={{maxWidth:700,margin:"0 auto",textAlign:"center"}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,opacity:0.7}}>About Oak Forest Plumbing</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff"}}>70 Years of Trusted<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Little Rock Plumbing</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.65)",marginBottom:24,textAlign:"left"}}>
          Oak Forest Plumbing has been a Little Rock institution since 1956 — that is seven decades of keeping the city's water flowing and pipes in order. When Brad Smith took over ownership in 2023, he brought a renewed commitment to the same values that built the business: honest work, fair pricing, and treating every customer like a neighbor.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.65)",marginBottom:36,textAlign:"left"}}>
          Today, Brad and his son Andrew run Oak Forest as a true father-and-son operation. They are the ones who answer the phone, show up at your door, and do the work themselves. The company has held a BBB A+ rating since 1986 — over 40 years of verified trust — and is regularly recommended in the MySaline Facebook community groups. When Little Rock needs a plumber, they call Oak Forest.
        </p>
        <div className="why-creds-row" style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:24,marginTop:8}}>
          {["Est. 1956 — 70 Years","BBB A+ Since 1986","Father & Son Team","New Ownership 2023","Community Recommended"].map((c,i)=>(
    ...
    {/* REVIEWS — Layout B: 3 cards side by side, no rating in heading */}
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>Need a plumber?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Brad.</span></h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>70 years of trusted service. BBB A+ since 1986. Fair pricing, honest work.</p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16,fontWeight:600,letterSpacing:-0.5}}><span>Oak Forest</span> <span style={{color:"var(--accent)"}}>Plumbing</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>Family-owned plumbing serving Little Rock and Central Arkansas since 1956. BBB A+ rated since 1986.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>8303 Stagecoach Rd<br/>Little Rock, AR 72210<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
          <p style={{fontSize:13,opacity:0.5,marginTop:8}}>Mon-Fri 8:00 AM - 4:30 PM</p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} Oak Forest Plumbing. All rights reserved.</span>
          <span>BBB A+ Since 1986 &middot; Little Rock, AR</span>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Oak Forest Plumbing</a>
    </div>
