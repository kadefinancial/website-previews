// Layout: D | Industry: Plumber | City: Pocola
import { useState, useEffect, useRef } from "react";

const PHONE = "(479) 646-5585";
const TEL = "tel:+14796465585";
...
const Icons = {
  ...
  drain: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><circle cx="12" cy="12" r="10"/><circle cx="12" cy="12" r="3"/><path d="M12 2v7"/><path d="M12 15v7"/><path d="M2 12h7"/><path d="M15 12h7"/></svg>,
  ...
  septic: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="2" y="8" width="20" height="10" rx="3"/><path d="M6 8V5"/><path d="M18 8V5"/><path d="M10 18v3"/><path d="M14 18v3"/><path d="M6 13h12"/></svg>,
  sewer: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 20h18"/><path d="M6 20v-4a2 2 0 012-2h0"/><path d="M18 20v-4a2 2 0 00-2-2h0"/><path d="M8 14h8"/><path d="M12 14V4"/><path d="M9 7l3-3 3 3"/></svg>,
  ...
  star: <svg width="18" height="18" viewBox="0 0 24 24" fill="#2C5282" stroke="none"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>,
...
const services = [
  { name: "Plumbing Repairs", desc: "From leaky faucets to burst pipes, our experienced team handles all residential and commercial plumbing repairs with reliable, lasting results.", icon: Icons.wrench },
  { name: "Drain Cleaning", desc: "Professional drain clearing for stubborn clogs and recurring backups in kitchens, bathrooms, and main lines. Fast response, thorough results.", icon: Icons.droplets },
  { name: "Septic Tank Pumping & Service", desc: "Complete septic tank pumping, inspections, and maintenance to keep your system running efficiently and avoid costly failures.", icon: Icons.septic },
  { name: "Pipe Replacement", desc: "Full pipe replacement for aging, corroded, or damaged lines. We use quality materials and ensure code-compliant installation every time.", icon: Icons.pipe },
  { name: "Sewer Drain Line Repair", desc: "Expert sewer line diagnosis and repair using proven methods. We restore proper flow and prevent future backups and damage.", icon: Icons.sewer },
  { name: "24/7 Emergency Service", desc: "Plumbing emergencies don't wait, and neither do we. Call any time, day or night, for immediate response from our on-call team.", icon: Icons.zap },
];

const serviceNames = ["Plumbing Repairs","Drain Cleaning","Septic Tank Pumping & Service","Pipe Replacement","Sewer Drain Line Repair","24/7 Emergency Service"];

const areas = ["Fort Smith","Pocola","Van Buren","Alma","Greenwood","Barling","River Valley","Western Arkansas","Eastern Oklahoma"];

const reviews = [
  { text: "Excellent service all around. Called Monday morning and Cody was at my house by noon. He was friendly, knowledgeable, and got the work done quickly. It was affordable as well. They even called the next day to follow up. Will definitely recommend and use again.", name: "Carrie Miller", src: "Google" },
  { text: "Drain Masters did a wonderful job of repairing a sewer drain line. Jimmy and Michael were wonderful to work with and their company should consider them an asset to their team.", name: "Donald Woodworth", src: "Google" },
  { text: "Extremely pleased with every aspect of Drain Masters. We called Drain Master as a second quote as we felt the first company was too high. They were. After giving a much more reasonable quote they ended up fixing the problem for much less.", name: "Ray Morris", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function DrainMasters() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("drain-masters-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="roof"){sessionStorage.setItem("drain-masters-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "Plumber Fort Smith AR | Drain Masters";
    const els = [];
    ...
    add("meta",{name:"description",content:"Drain Masters — trusted plumber serving Fort Smith AR since 1986. 4.6 stars, 132+ reviews. Call (479) 646-5585."});
    add("meta",{name:"geo.region",content:"US-OK"});
    add("meta",{name:"geo.placename",content:"Pocola"});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"Drain Masters",telephone:"+14796465585",
      email:"service@drainmastersfortsmith.com",
      address:{"@type":"PostalAddress",streetAddress:"106 Folsom Blvd",addressLocality:"Pocola",addressRegion:"OK",postalCode:"74902",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:35.2334,longitude:-94.4932},
      openingHoursSpecification:[{"@type":"OpeningHoursSpecification",dayOfWeek:["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"],opens:"00:00",closes:"23:59"}],
      aggregateRating:{"@type":"AggregateRating",ratingValue:"4.6",reviewCount:"132",bestRating:"5"},
      founder:[{"@type":"Person",name:"Michael R. Newquest",jobTitle:"President"},{"@type":"Person",name:"May Newquest",jobTitle:"Vice President"}],
      foundingDate:"1986",
      description:"Drain Masters is a trusted plumbing and drain service company serving the Fort Smith, AR area since 1986. Specializing in drain cleaning, septic service, and plumbing repairs.",
      areaServed:areas.map(a=>({"@type":"City",name:a})),
      hasOfferCatalog:{"@type":"OfferCatalog",name:"Plumbing Services",itemListElement:serviceNames.map(s=>({"@type":"Offer",itemOffered:{"@type":"Service",name:s}}))},
      memberOf:[],
      sameAs:[],
    })});
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#4A1A2E"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8}}>
            <span style={{fontWeight:700,color:"#4A1A2E"}}>Drain</span> <span style={{color:"#2C5282"}}>Masters</span>
          </div>
  ...
  return (<>
    <style>{`
      @import url('https://fonts.googleapis.com/css2?family=Playfair+Display:ital,wght@0,400;0,500;0,600;0,700;1,400;1,500&family=DM+Sans:wght@400;500;600;700&display=swap');
      :root{--primary:#4A1A2E;--accent:#2C5282;--cream:#FAF9F6;--white:#FFFFFF;--text:#2A2A3C;--text-mid:#5A5A6E;--text-light:#8A8A9A;--border:rgba(0,0,0,0.06)}
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(74,26,46,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease",boxShadow:scrolled?"0 1px 0 rgba(255,255,255,0.06)":"none"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}>
        <span>Drain</span> <span style={{color:"var(--accent)"}}>Masters</span>
      </div>
      ...
      <div className="hero-grid" style={{maxWidth:1200,width:"100%",display:"grid",gridTemplateColumns:"1.8fr 1fr",gap:48,alignItems:"center"}}>
        <div>
          <h1 style={{fontSize:"clamp(48px, 6vw, 84px)",lineHeight:1.0,color:"var(--primary)",letterSpacing:-2,marginBottom:24}}>
            Fort Smith's<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Most Trusted</span><br/>Plumber
          </h1>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:32}}>
            <Stars />
            <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"Excellent service all around" — Carrie Miller</span>
          </div>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(44,82,130,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Michael — {PHONE}</a>
        </div>
        <div>
          <div style={{background:"#fff",borderRadius:12,padding:"32px 28px",boxShadow:"0 4px 40px rgba(0,0,0,0.08)",border:"1px solid var(--border)"}}>
            {!formDone ? (
              ...
              <div style={{textAlign:"center",padding:"20px 0"}}>
                <div style={{width:48,height:48,borderRadius:"50%",background:"rgba(44,82,130,0.12)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 14px",color:"var(--accent)"}}>{Icons.check}</div>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",color:"var(--primary)",letterSpacing:-0.5}}>Plumbing Services in Fort Smith, AR</h2>
        </div>

        {/* Mobile trust stats */}
        <div className="trust-mobile" style={{display:"none",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:48,textAlign:"center"}}>
          {[
            {num:"4.6",label:"Google Rating",desc:"132+ reviews"},
            {num:"132+",label:"Customer Reviews",desc:"5-star service"},
            {num:"1986",label:"Est. Founded",desc:"Nearly 40 years"},
            {num:"24/7",label:"Emergency Service",desc:"Always available"},
          ].map((s,i)=>(
            <div key={i}>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",position:"relative",overflow:"hidden"}}>
      <div className="serif" style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",fontSize:"clamp(120px,20vw,280px)",color:"var(--accent)",opacity:0.08,lineHeight:1,whiteSpace:"nowrap",pointerEvents:"none"}}>1986</div>
      <div style={{maxWidth:800,margin:"0 auto",position:"relative",zIndex:1}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,textAlign:"center",opacity:0.7}}>About Drain Masters</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff",textAlign:"center"}}>Serving Fort Smith<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Since 1986</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:24,textAlign:"center"}}>
          For nearly 40 years, Drain Masters has been the name Fort Smith and the River Valley trust for plumbing, drain cleaning, and septic service. Founded in 1986 by Michael R. Newquest and May Newquest, this family-led company has built its reputation one job at a time — with honest pricing, quality workmanship, and the kind of follow-up calls that show they genuinely care about every customer.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:36,textAlign:"center"}}>
          With a 4.6-star Google rating and over 132 reviews, Drain Masters has earned the trust of homeowners and businesses across Western Arkansas and Eastern Oklahoma. They were recognized with a 2024 Best of the Best award — a testament to decades of consistent, reliable service. When you call Drain Masters, you're not getting a call center. You're getting a team that treats your home like their own.
        </p>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:20}}>
          {["Founded 1986 — Nearly 40 Years","4.6 Stars on Google (132+ Reviews)","2024 Best of the Best Award","24/7 Emergency Response","Follow-Up Calls After Every Job","Serving Fort Smith & River Valley"].map((c,i)=>(
    ...
    <section id="reviews" style={{padding:"110px 0",background:"var(--white)"}}>
      <div style={{maxWidth:1200,margin:"0 auto",paddingLeft:"clamp(24px,5vw,64px)",paddingRight:"clamp(24px,5vw,64px)"}}>
        <div style={{textAlign:"center",marginBottom:56}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",color:"var(--primary)",letterSpacing:-0.5}}>4.6 Stars on Google</h2>
        </div>
      </div>
    ...
    <section id="areas" style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:12}}>
          {areas.map((a,i)=>(
            <span key={i} title={`Plumber in ${a}${a==="Western Arkansas"||a==="Eastern Oklahoma"||a==="River Valley"?"":", "+(a==="Pocola"?"OK":"AR")}`} style={{display:"inline-flex",alignItems:"center",gap:6,background:"#fff",border:"1px solid var(--border)",padding:"12px 24px",borderRadius:30,fontSize:15,fontWeight:500,color:"var(--text)",cursor:"default",transition:"all 0.25s"}}
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>
          Got a drain problem?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Michael.</span>
        </h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>
          Nearly 40 years of honest service, 132+ five-star reviews, and 24/7 emergency availability. Fair pricing, every time.
        </p>
        <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"22px 52px",borderRadius:8,fontSize:20,fontWeight:700,boxShadow:"0 6px 28px rgba(44,82,130,0.35)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} {PHONE}</a>
      </div>
    </section>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16}}><span>Drain</span> <span style={{color:"var(--accent)"}}>Masters</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>Trusted plumbing, drain cleaning, and septic service for Fort Smith and the River Valley since 1986. Nearly 40 years of honest, reliable work.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>106 Folsom Blvd<br/>Pocola, OK 74902<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a><br/><a href="mailto:service@drainmastersfortsmith.com" style={{color:"var(--accent)",fontWeight:600,fontSize:13}}>service@drainmastersfortsmith.com</a></p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} Drain Masters. All rights reserved.</span>
          <span>Serving the River Valley since 1986</span>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Drain Masters</a>
    </div>
