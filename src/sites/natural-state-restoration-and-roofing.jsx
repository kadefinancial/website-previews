...
const PHONE = "(501) 246-4422";
const TEL = "tel:+15012464422";
...
const Icons = {
  ...
  metal: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M2 20h20"/><path d="M4 20V8l8-5 8 5v12"/><path d="M4 8h16"/><path d="M8 8v12"/><path d="M12 8v12"/><path d="M16 8v12"/></svg>,
  siding: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 7h18"/><path d="M3 11h18"/><path d="M3 15h18"/><path d="M3 19h18"/></svg>,
  gutter: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v6"/><path d="M8 4v4"/><path d="M16 4v4"/><path d="M4 10h16l-2 12H6L4 10z"/><path d="M10 14v4"/><path d="M14 14v4"/></svg>,
...
const services = [
  { name: "Storm Damage Restoration", desc: "Fast response after hail, wind, and severe weather events. Brendan inspects your roof, documents damage, and coordinates the full repair process so you can focus on your family.", icon: Icons.storm },
  { name: "Roof Replacement", desc: "Complete tear-off and replacement using premium architectural shingles and materials. Atlas Pro GOLD certified installation backed by enhanced manufacturer warranties.", icon: Icons.roof },
  { name: "Insurance Claims", desc: "Brendan handles the insurance process from start to finish. Free inspections, detailed documentation, and direct communication with your adjuster to maximize your claim.", icon: Icons.clipboard },
  { name: "Metal Roofing", desc: "Durable, energy-efficient metal roof systems built to last decades. Standing seam and exposed fastener options for residential and commercial properties.", icon: Icons.metal },
  { name: "Siding", desc: "Professional siding installation and repair to protect your home and boost curb appeal. Vinyl, fiber cement, and engineered wood options available.", icon: Icons.siding },
  { name: "Gutter Installation", desc: "Seamless gutter systems that channel water away from your foundation. Proper drainage prevents costly water damage and erosion around your home.", icon: Icons.gutter },
];

const allServiceNames = ["Full Insurance Claims","Architectural Shingles","3-Tab Shingles","Metal Roofing","Slate and Tile","Siding","Gutter Installation","Roof Repair","Roof Maintenance","Storm Damage Restoration","Free Roof Inspections"];

const areas = ["North Little Rock","Little Rock","Maumelle","Sherwood","Jacksonville","Cabot","Benton","Bryant","Searcy"];

const reviews = [
  { text: "Brendan Lile of Natural State Restoration & Roofing was a Godsend! He handled everything with our insurance claim and made the entire process stress-free.", name: "Jeff Armstrong", src: "Google" },
  { text: "Brendan and Katie go above and beyond to assist with roof damage and give free inspections. They truly care about their customers and it shows in every interaction.", name: "Sharon Baker", src: "Google" },
  { text: "Natural State Roofing made the process easy. Their team of workers are very friendly and professional from start to finish.", name: "Julia Strickland", src: "Google" },
  { text: "We had a suspected leak. They immediately believed it was from storm damage and investigated. Brendan took care of everything with our insurance.", name: "Catherine Myers", src: "Google" },
  { text: "Natural State made the experience of getting a new roof easy and stress-free. Brendan handled the insurance claim and kept us informed every step of the way.", name: "Renata Halferty", src: "Google" },
];
...
const trustStats = [
  ...
  { num: "206+", label: "Reviews", desc: "Real customers" },
  { num: "GOLD", label: "Atlas Pro", desc: "Manufacturer certified" },
  { num: "A+", label: "BBB Rating", desc: "Accredited business" },
];

export default function NaturalStateRestorationAndRoofing() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("nsrr-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if (pw === "roof") { sessionStorage.setItem("nsrr-auth", "1"); setAuthed(true); } else { setPwBad(true); setPw(""); } };
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Roofing North Little Rock AR | Natural State Restoration";
    const els = [];
    ...
    add("meta", { name: "description", content: "Roofing contractor in North Little Rock, AR. 4.9 stars, 206 reviews. Atlas Pro GOLD. Call (501) 246-4422." });
    ...
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      ...
      name: "Natural State Restoration & Roofing", telephone: "+15012464422",
      address: { "@type": "PostalAddress", streetAddress: "6109 Remount Rd", addressLocality: "North Little Rock", addressRegion: "AR", postalCode: "72118", addressCountry: "US" },
      ...
      founder: { "@type": "Person", name: "Brendan Lile" },
      description: "Natural State Restoration & Roofing provides roofing, storm damage restoration, and insurance claims assistance in North Little Rock, AR and surrounding areas. Atlas Pro GOLD certified. 4.9 stars with 206 reviews.",
      ...
      memberOf: [{ "@type": "Organization", name: "BBB" }, { "@type": "Organization", name: "Atlas Pro GOLD" }], sameAs: [],
    }) });
    ...
    return (<>
      ...
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#1E1E1E" }}>
        <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
          <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, marginBottom: 8 }}><span style={{ fontWeight: 700, color: "#1E1E1E" }}>Natural State</span> <span style={{ color: "#8B3A3A" }}>Roofing</span></div>
  ...
  return (<>
    <style>{`
      ...
      :root{--primary:#1E1E1E;--accent:#8B3A3A;--cream:#FAF9F6;--white:#FFFFFF;--text:#2A2A3C;--text-mid:#5A5A6E;--text-light:#8A8A9A;--border:rgba(0,0,0,0.06)}
    ...
    <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled ? "rgba(30,30,30,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition: "all 0.4s ease" }}>
      <div className="serif" style={{ fontSize: 22, color: scrolled ? "#fff" : "var(--primary)", fontWeight: 600, letterSpacing: -0.5 }}><span>Natural State</span> <span style={{ color: "var(--accent)" }}>Roofing</span></div>
      ...
      <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1.8fr 1fr", gap: 48, alignItems: "center" }}>
        <div>
          <h1 style={{ fontSize: "clamp(48px, 6vw, 84px)", lineHeight: 1.0, color: "var(--primary)", letterSpacing: -2, marginBottom: 24 }}>
            North Little Rock's<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Most Trusted</span><br />Roofing Contractor
          </h1>
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 32 }}>
            <Stars />
            <span style={{ fontSize: 14, color: "var(--text-light)", fontStyle: "italic", marginLeft: 4 }}>"Brendan was a Godsend!" — Jeff Armstrong</span>
          </div>
          <a href={TEL} style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 36px", borderRadius: 6, fontSize: 16, fontWeight: 700, boxShadow: "0 4px 20px rgba(139,58,58,0.3)", fontFamily: "'DM Sans',sans-serif" }}>{Icons.phone} Call Brendan — {PHONE}</a>
        </div>
        <div>
          <div style={{ background: "#fff", borderRadius: 12, padding: "32px 28px", boxShadow: "0 4px 40px rgba(0,0,0,0.08)", border: "1px solid var(--border)" }}>
            {!formDone ? (
              ...
              <div style={{ textAlign: "center", padding: "20px 0" }}>
                <div style={{ width: 48, height: 48, borderRadius: "50%", background: "rgba(139,58,58,0.15)", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px", color: "var(--accent)" }}>{Icons.check}</div>
    ...
    <section id="services" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <p style={{ textAlign: "center", marginTop: 48, fontSize: 15, color: "var(--text-light)" }}>Questions? Call Brendan directly at <a href={TEL} style={{ color: "var(--accent)", fontWeight: 600 }}>{PHONE}</a></p>
      </div>
    </section>
    ...
    <section id="about" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--primary)", position: "relative", overflow: "hidden" }}>
      <div className="serif" style={{ position: "absolute", top: "50%", left: "50%", transform: "translate(-50%,-50%)", fontSize: "clamp(120px,20vw,280px)", color: "var(--accent)", opacity: 0.08, lineHeight: 1, whiteSpace: "nowrap", pointerEvents: "none" }}>206</div>
      <div style={{ maxWidth: 800, margin: "0 auto", position: "relative", zIndex: 1 }}>
        <div className="gold-line" style={{ margin: "0 auto 20px" }} />
        <div style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: 3, color: "var(--accent)", marginBottom: 14, textAlign: "center", opacity: 0.7 }}>About Natural State Roofing</div>
        <h2 style={{ fontSize: "clamp(30px,3.5vw,42px)", lineHeight: 1.15, marginBottom: 28, color: "#fff", textAlign: "center" }}>206 Reviews at 4.9 Stars.<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>A Free Wix Subdomain.</span></h2>
        <p style={{ fontSize: 16, lineHeight: 1.8, color: "rgba(255,255,255,0.6)", marginBottom: 24, textAlign: "center" }}>
          That's the gap. Brendan and Katie Lile have built one of the highest-rated roofing companies in Central Arkansas entirely on word-of-mouth and quality work. Their current website is a free Wix subdomain under someone else's username — the biggest disconnect between reputation and web presence on the entire roofing list. The work speaks for itself, but the internet doesn't know it yet.
        </p>
        <p style={{ fontSize: 16, lineHeight: 1.8, color: "rgba(255,255,255,0.6)", marginBottom: 36, textAlign: "center" }}>
          Brendan handles the roof inspections, insurance claims, and project management. Katie keeps the operations running and ensures every customer is taken care of. Together they've earned Atlas Pro GOLD Status — a manufacturer certification that less than 2% of roofing contractors receive — along with a BBB A+ rating and the 2018 Best of Little Rock award. When your roof takes storm damage, Brendan will be on it the same day.
        </p>
        <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 20 }}>
          {["Atlas Pro GOLD Certified", "BBB A+ Rated", "Best of Little Rock 2018", "206 Five-Star Reviews", "Free Roof Inspections", "Full Insurance Claims Support"].map((c, i) => (
    ...
    <section id="reviews" style={{ padding: "110px 0", background: "var(--white)" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto", paddingLeft: "clamp(24px,5vw,64px)", paddingRight: "clamp(24px,5vw,64px)" }}>
        <div style={{ textAlign: "center", marginBottom: 56 }}>
          ...
          <h2 style={{ fontSize: "clamp(30px,3.5vw,42px)", color: "var(--primary)", letterSpacing: -0.5 }}>4.9 Stars on Google</h2>
        </div>
      </div>
    ...
    {/* CTA — White background (contrasts with --primary footer below) */}
    <section style={{ padding: "100px clamp(24px,5vw,64px)", background: "var(--white)", textAlign: "center", borderTop: "1px solid var(--border)" }}>
      <div style={{ maxWidth: 600, margin: "0 auto" }}>
        <h2 className="serif" style={{ fontSize: "clamp(32px,4vw,50px)", color: "var(--primary)", lineHeight: 1.1, marginBottom: 20, letterSpacing: -1 }}>Need a new roof?<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Call Brendan.</span></h2>
        <p style={{ fontSize: 17, color: "var(--text-mid)", marginBottom: 36, lineHeight: 1.6 }}>Atlas Pro GOLD certified. BBB A+. 206 five-star reviews. Free inspections.</p>
    ...
    <footer style={{ background: "var(--primary)", color: "rgba(255,255,255,0.7)", padding: "64px clamp(24px,5vw,64px) 0" }}>
      <div className="footer-grid" style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gridTemplateColumns: "1.5fr 1fr 1fr", gap: 48, paddingBottom: 40 }}>
        <div>
          <div className="serif" style={{ fontSize: 22, color: "#fff", marginBottom: 16 }}><span>Natural State</span> <span style={{ color: "var(--accent)" }}>Roofing</span></div>
          <p style={{ fontSize: 14, lineHeight: 1.7, opacity: 0.7, marginBottom: 16 }}>Professional roofing, storm damage restoration, and insurance claims assistance for homes and businesses across Central Arkansas. Atlas Pro GOLD certified.</p>
          <p style={{ fontSize: 14, lineHeight: 1.8, opacity: 0.6 }}>6109 Remount Rd<br />North Little Rock, AR 72118<br /><a href={TEL} style={{ color: "var(--accent)", fontWeight: 600 }}>{PHONE}</a></p>
        </div>
        <div>
      ...
      <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", padding: "20px 0", maxWidth: 1100, margin: "0 auto" }}>
        <div className="footer-btm-inner" style={{ display: "flex", justifyContent: "space-between", fontSize: 13, opacity: 0.4, flexWrap: "wrap", gap: 8 }}>
          <span>&copy; {yr} Natural State Restoration & Roofing. All rights reserved.</span>
          <span>Atlas Pro GOLD | BBB A+ | Best of Little Rock 2018</span>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 998, display: "flex" }}>
      <a href={TEL} style={{ display: "flex", width: "100%", alignItems: "center", justifyContent: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px", fontSize: 17, fontWeight: 700, boxShadow: "0 -4px 20px rgba(0,0,0,0.15)", fontFamily: "'DM Sans',sans-serif" }}>{Icons.phone} Call Natural State Roofing</a>
    </div>
