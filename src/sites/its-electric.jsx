// Layout: C | Industry: Electrician | City: Rogers
...
const Icons = {
  ...
  lightbulb: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18h6"/><path d="M10 22h4"/><path d="M12 2a7 7 0 00-4 12.7V17h8v-2.3A7 7 0 0012 2z"/></svg>
  ),
  smartHome: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><circle cx="12" cy="13" r="2"/><path d="M12 9v1"/><path d="M12 17v-1"/><path d="M9.17 11.17l.71.71"/><path d="M14.12 14.12l.71.71"/><path d="M8 13h1"/><path d="M15 13h1"/><path d="M9.17 14.83l.71-.71"/><path d="M14.12 11.88l.71-.71"/></svg>
  ),
  panel: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><line x1="9" y1="3" x2="9" y2="21"/><line x1="9" y1="8" x2="21" y2="8"/><line x1="9" y1="13" x2="21" y2="13"/><line x1="9" y1="18" x2="21" y2="18"/></svg>
  ),
  shield: (
...
// --- Data ---
const PHONE = "(479) 866-6310";
const PHONE_TEL = "tel:+14798666310";
const EMAIL = "itsservicesnwa@gmail.com";

const services = [
  { name: "Residential Electrical", desc: "Complete home electrical services across Northwest Arkansas. From troubleshooting flickering lights to full rewiring, our licensed team delivers safe, code-compliant work every time.", icon: Icons.home },
  { name: "Lighting Design & Installation", desc: "Transform your home with custom lighting solutions. Chandeliers, recessed lighting, dimmers, landscape lighting, and everything in between -- installed with precision.", icon: Icons.lightbulb },
  { name: "Smart Home Installation", desc: "Bring your home into the future with smart switches, dimmers, thermostats, and whole-home automation installed correctly by experienced electricians.", icon: Icons.smartHome },
  { name: "Panel & Breaker Service", desc: "Outdated or overloaded electrical panel? We upgrade panels, replace breakers, and ensure your home's electrical system meets modern safety standards.", icon: Icons.panel },
  { name: "Emergency Electrical (24/7)", desc: "Sparking outlets, power outages, and electrical hazards don't wait for business hours. We're available 24/7 to restore safety and peace of mind.", icon: Icons.zap },
  { name: "Whole-House Remodel Electrical", desc: "Planning a major renovation? We handle all electrical work for whole-house remodels, from rough-in wiring to final fixture installation.", icon: Icons.wrench },
];

const allServices = [
  "Residential Electrical Repairs",
  "Lighting Design & Installation",
  "Electrical Installations & Replacements",
  "Smart Home Installation",
  "Panel & Circuit Breaker Service",
  "Ceiling Fan Installation",
  "Outlet & Switch Installation",
  "Whole-House Remodel Electrical",
  "Storm Damage Repair",
  "24/7 Emergency Service",
];

const reviews = [
  { text: "Light fixture was sparking and electricity was blown in adjacent rooms. Called It's and they sent Matt right away. So thankful for their willingness to fix the issue so quickly, and bring peace of mind that there would not be a fire.", name: "Shelly Rippee", src: "Google" },
  { text: "I can't say enough positive things about It's Electric, plumbing and HVAC. We have been using them for about 20 years and have never been let down.", name: "Nancy Weatherford", src: "Google" },
  { text: "We have used It's Electric twice and have been very satisfied. I needed them to check an electric stove outlet and circuit breaker and they were here within two hours.", name: "James Sawyer", src: "Google" },
  { text: "We needed a lighting chandelier replaced and dimmers installed. They were professional, courteous, and did an excellent job.", name: "Jean Gregesich", src: "Google" },
  { text: "We had large remodel project in our home. They handled everything professionally.", name: "Creston Mackey", src: "Google" },
];

const areas = ["Rogers", "Bentonville", "Springdale", "Fayetteville", "Bella Vista", "Centerton", "Lowell", "Pea Ridge", "Siloam Springs"];

// --- Main Component ---
export default function ItsElectric() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("its-electric-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "2026") { sessionStorage.setItem("its-electric-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Electrician Rogers AR | It's Electric";
    const els = [];
    ...
    add("meta", { name: "description", content: "Licensed electrician in Rogers, AR. 5.0 stars, 84+ reviews, 20+ years. Call (479) 866-6310 for residential electrical service." });
    add("meta", { name: "geo.region", content: "US-AR" });
    add("meta", { name: "geo.placename", content: "Rogers" });
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      "@context": "https://schema.org", "@type": "Electrician",
      name: "It's Electric", telephone: "+14798666310",
      email: EMAIL,
      address: { "@type": "PostalAddress", streetAddress: "14123 Pleasant Ridge Rd", addressLocality: "Rogers", addressRegion: "AR", postalCode: "72756", addressCountry: "US" },
      geo: { "@type": "GeoCoordinates", latitude: 36.332, longitude: -94.118 },
      openingHoursSpecification: [
        { "@type": "OpeningHoursSpecification", dayOfWeek: ["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"], opens: "00:00", closes: "23:59" },
      ],
      aggregateRating: { "@type": "AggregateRating", ratingValue: "5.0", reviewCount: "84", bestRating: "5" },
      description: "It's Electric is a licensed electrical contractor in Rogers, AR, serving Northwest Arkansas for 20+ years. Residential electrical, lighting, smart home, panel upgrades, and 24/7 emergency service.",
      areaServed: areas.map((a) => ({ "@type": "City", name: a })),
      hasOfferCatalog: { "@type": "OfferCatalog", name: "Electrical Services", itemListElement: services.map((s) => ({ "@type": "Offer", itemOffered: { "@type": "Service", name: s.name } })) },
      memberOf: [],
      sameAs: [],
    })});
        ...
        <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#2D2D2D" }}>
          <div style={{ background:"#fff", borderRadius:16, padding:"56px 44px", textAlign:"center", maxWidth:420, width:"92%", boxShadow:"0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:28, fontWeight:600, letterSpacing:-0.5, marginBottom:4 }}>
              <span style={{ color:"#2D2D2D" }}>It's</span> <span style={{ color:"#D4A03C" }}>Electric</span>
            </div>
    ...
    <>
      ...
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display:"flex", alignItems:"center", justifyContent:"space-between", background: scrolled ? "rgba(45,45,45,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition:"all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        <div className="serif" style={{ fontSize:22, fontWeight:600, letterSpacing:-0.5, color:"#fff" }}>
          <span>It's</span><span style={{ color:"var(--accent)", marginLeft:6 }}>Electric</span>
        </div>
        ...
        <div className="hero-grid" style={{ maxWidth:1200, width:"100%", display:"grid", gridTemplateColumns:"1fr 1fr", gap:64, alignItems:"center" }}>
          ...
          <div style={{ maxWidth:560 }}>
            ...
            <h1 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(40px,5vw,68px)", lineHeight:1.05, letterSpacing:-1.5, color:"#fff", marginBottom:24 }}>
              Rogers'{" "}<br/>
              <em style={{ color:"var(--accent)", fontStyle:"italic" }}>Trusted Electrician</em><br/>
              For 20+ Years
            </h1>
            <p style={{ fontSize:18, lineHeight:1.7, color:"rgba(255,255,255,0.75)", marginBottom:32, maxWidth:480 }}>
              Licensed electrical service for homes across Northwest Arkansas. From emergency repairs to whole-house remodels, It's Electric has delivered dependable work and a perfect 5.0-star reputation for over two decades.
            </p>
            {/* Star row with hero quote */}
            <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:32, flexWrap:"wrap" }}>
              <Stars />
              <span style={{ fontSize:14, color:"rgba(255,255,255,0.7)", fontStyle:"italic" }}>"We have been using them for about 20 years and have never been let down."</span>
              <span style={{ fontSize:13, color:"rgba(255,255,255,0.5)" }}>- Nancy Weatherford</span>
            </div>
            <div style={{ display:"flex", gap:16, flexWrap:"wrap" }}>
              <a href={PHONE_TEL} style={{ display:"inline-flex", alignItems:"center", gap:10, background:"var(--accent)", color:"#fff", padding:"18px 36px", borderRadius:6, fontSize:16, fontWeight:700, letterSpacing:0.3, fontFamily:"'DM Sans',sans-serif", boxShadow:"0 4px 20px rgba(212,160,60,0.3)", transition:"transform 0.2s" }}>
      ...
      <section style={{ background:"var(--white)", padding:"48px clamp(24px,5vw,64px)" }}>
        <div className="trust-grid" style={{ maxWidth:1200, margin:"0 auto", display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:32, textAlign:"center" }}>
          {[
            { num: "5.0", label: "Google Rating", desc: "Near-perfect reviews" },
            { num: "84+", label: "Reviews", desc: "Happy customers" },
            { num: "20+", label: "Years", desc: "Serving NWA" },
            { num: "24/7", label: "Emergency", desc: "Always available" },
          ].map((s, i) => (
      ...
      <section id="services" className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          <div style={{ textAlign:"center", marginBottom:64 }}>
            ...
            <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(30px,3.5vw,44px)", lineHeight:1.15, letterSpacing:-0.5, color:"var(--primary)" }}>Electrical Services in Rogers</h2>
          </div>
          {services.map((svc, i) => {
            const isEven = i % 2 === 1;
            return (
      ...
      <section id="about" className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          <div style={{ textAlign:"center", marginBottom:64 }}>
            <div className="gold-line" />
            <p style={{ fontSize:12, fontWeight:700, letterSpacing:3, textTransform:"uppercase", color:"var(--text-light)", marginBottom:14, fontFamily:"'DM Sans',sans-serif" }}>Why It's Electric</p>
            <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(30px,3.5vw,44px)", lineHeight:1.15, letterSpacing:-0.5, color:"var(--primary)" }}>5.0 Stars Across 84 Reviews</h2>
          </div>
          <div className="why-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:64, alignItems:"start" }}>
            ...
            <div>
              <p style={{ fontSize:17, lineHeight:1.8, color:"var(--text-mid)", marginBottom:20 }}>
                It's Electric has been the name Northwest Arkansas calls for over 20 years. With a perfect 5.0-star rating and 83 out of 84 reviews at five stars, this is a team that delivers consistently exceptional work across Rogers, Bentonville, Springdale, Fayetteville, and the entire NWA region.
              </p>
              <p style={{ fontSize:17, lineHeight:1.8, color:"var(--text-mid)", marginBottom:32 }}>
                Part of a multi-trade family that includes It's Plumbing and It's HVAC, the team brings deep home-service expertise to every job. Technicians like Matt and Hunter are praised by name for fast response times, professional work, and genuine care. Whether it's a sparking fixture at midnight or a whole-house remodel, It's Electric is the call NWA makes when the job has to be done right.
              </p>
              {/* Pull quote */}
              <div style={{ borderLeft:"3px solid var(--accent)", paddingLeft:24, marginBottom:36 }}>
                <p style={{ fontSize:18, fontStyle:"italic", lineHeight:1.7, color:"var(--primary)", fontFamily:"'Playfair Display',serif" }}>
                  "They were here within two hours."
                </p>
                <p style={{ fontSize:14, color:"var(--text-light)", marginTop:8, fontFamily:"'DM Sans',sans-serif" }}>- James Sawyer, Google Review</p>
              </div>
              {/* Credentials */}
              <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                {[
                  "Perfect 5.0-Star Google Rating",
                  "84+ Verified Customer Reviews",
                  "20+ Years Serving Northwest Arkansas",
                  "24/7 Emergency Electrical Service",
                  "Multi-Trade: Electric + Plumbing + HVAC",
                  "NWA Community Recommended",
                ].map((cred, i) => (
            ...
            <div className="badge-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
              {[
                { icon: Icons.award, title: "Near-Perfect Reviews", desc: "83 of 84 reviews are five stars. That kind of consistency across 20+ years speaks for itself." },
                { icon: Icons.users, title: "Multi-Trade Experts", desc: "It's Electric, It's Plumbing, It's HVAC -- one trusted team for every system in your home." },
                { icon: Icons.zap, title: "24/7 Emergency", desc: "Sparking fixtures, power outages, and electrical hazards don't wait. Neither does It's Electric." },
                { icon: Icons.clock, title: "20+ Year Track Record", desc: "Serving Northwest Arkansas families for over two decades with consistent, reliable electrical work." },
              ].map((b, i) => (
      ...
      <section id="reviews" className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--white)" }}>
        <div style={{ maxWidth:800, margin:"0 auto", textAlign:"center" }}>
          ...
          {/* Large featured review -- Shelly Rippee */}
          <div style={{ position:"relative", padding:"48px 40px", background:"var(--white)", borderRadius:12 }}>
            ...
            <p style={{ fontSize:20, lineHeight:1.8, color:"var(--text)", fontStyle:"italic", fontFamily:"'Playfair Display',serif", marginBottom:28 }}>
              "Light fixture was sparking and electricity was blown in adjacent rooms. Called It's and they sent Matt right away. So thankful for their willingness to fix the issue so quickly, and bring peace of mind that there would not be a fire."
            </p>
            <p style={{ fontSize:17, fontWeight:700, color:"var(--primary)" }}>Shelly Rippee</p>
            <p style={{ fontSize:13, color:"var(--text-light)", marginTop:4 }}>Google Review</p>
          </div>
          <p style={{ fontSize:14, color:"var(--text-light)", marginTop:32 }}>One of 84+ five-star reviews on Google</p>
      ...
      <section id="areas" className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:1200, margin:"0 auto", textAlign:"center" }}>
          ...
          <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(30px,3.5vw,44px)", lineHeight:1.15, letterSpacing:-0.5, color:"var(--primary)", marginBottom:56 }}>Serving Northwest Arkansas</h2>
          <div style={{ display:"flex", flexWrap:"wrap", justifyContent:"center", gap:12 }}>
            {areas.map((area, i) => (
              <span key={i} title={`Electrician in ${area}, AR`} style={{ display:"inline-flex", alignItems:"center", gap:8, padding:"12px 22px", background:"var(--white)", border:"1px solid var(--border)", borderRadius:50, fontSize:14, fontWeight:500, color:"var(--text)", cursor:"default", transition:"all 0.2s", fontFamily:"'DM Sans',sans-serif" }}
      ...
      <section className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--white)", borderTop:"1px solid var(--border)" }}>
        <div style={{ maxWidth:700, margin:"0 auto", textAlign:"center" }}>
          <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(30px,3.5vw,44px)", lineHeight:1.15, letterSpacing:-0.5, color:"var(--primary)", marginBottom:20 }}>
            Need an electrician?<br/><em style={{ color:"var(--accent)", fontStyle:"italic" }}>Call It's Electric.</em>
          </h2>
          <p style={{ fontSize:17, lineHeight:1.7, color:"var(--text-mid)", marginBottom:36, maxWidth:520, margin:"0 auto 36px" }}>
            A perfect 5.0-star rating from 84 customers. 20+ years of licensed electrical work, honest pricing, and 24/7 emergency availability across Northwest Arkansas.
          </p>
          <a href={PHONE_TEL} style={{ display:"inline-flex", alignItems:"center", gap:10, background:"var(--accent)", color:"#fff", padding:"20px 44px", borderRadius:6, fontSize:18, fontWeight:700, letterSpacing:0.3, fontFamily:"'DM Sans',sans-serif", boxShadow:"0 4px 20px rgba(212,160,60,0.3)", transition:"transform 0.2s" }}>
            {Icons.phone} {PHONE}
          </a>
          <p style={{ fontSize:14, color:"var(--text-light)", marginTop:20 }}>Or email <a href={`mailto:${EMAIL}`} style={{ color:"var(--accent)", fontWeight:600 }}>{EMAIL}</a></p>
        </div>
      </section>
      ...
      <footer style={{ background:"var(--primary)", padding:"80px clamp(24px,5vw,64px) 40px", color:"rgba(255,255,255,0.7)" }}>
        <div className="footer-grid" style={{ maxWidth:1200, margin:"0 auto", display:"grid", gridTemplateColumns:"1.5fr 1fr 1fr", gap:48, marginBottom:48 }}>
          ...
          <div>
            ...
            <p style={{ fontSize:14, lineHeight:1.7, marginBottom:20, maxWidth:300 }}>
              Licensed electrical contractor in Rogers, AR. Perfect 5.0-star rating from 84+ customers across Northwest Arkansas. Residential electrical, emergency service, and more.
            </p>
            <p style={{ fontSize:14, lineHeight:1.8 }}>
              14123 Pleasant Ridge Rd<br/>
              Rogers, AR 72756
            </p>
          ...
          <div>
            ...
            <ul style={{ listStyle:"none", display:"flex", flexDirection:"column", gap:10 }}>
              {["Residential Electrical","Lighting Installation","Smart Home Installation","Panel & Breaker Service","Emergency Electrical","Whole-House Remodel"].map((s, i) => (
        ...
        <div style={{ borderTop:"1px solid rgba(255,255,255,0.1)", paddingTop:24 }}>
          <div className="footer-btm-inner" style={{ maxWidth:1200, margin:"0 auto", display:"flex", justifyContent:"space-between", alignItems:"center", fontSize:13, color:"rgba(255,255,255,0.45)" }}>
            <span>&copy; {new Date().getFullYear()} It's Electric. All rights reserved.</span>
      ...
      <div className="mobile-only" style={{ position:"fixed", bottom:0, left:0, right:0, zIndex:998, display:"none" }}>
        <a href={PHONE_TEL} style={{ display:"flex", alignItems:"center", justifyContent:"center", gap:10, background:"var(--accent)", color:"#fff", padding:"18px 24px", fontSize:16, fontWeight:700, fontFamily:"'DM Sans',sans-serif", boxShadow:"0 -4px 20px rgba(0,0,0,0.15)", textDecoration:"none" }}>
          {Icons.phone} Call It's Electric
        </a>
      </div>
