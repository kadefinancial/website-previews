// Layout: C | Industry: Roofing | City: Maumelle
...
const Icons = {
  ...
  clipboard: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M16 4h2a2 2 0 012 2v14a2 2 0 01-2 2H6a2 2 0 01-2-2V6a2 2 0 012-2h2"/><rect x="8" y="2" width="8" height="4" rx="1" ry="1"/><path d="M9 14l2 2 4-4"/></svg>
  ),
  metal: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M2 20h20"/><path d="M4 20V8l8-5 8 5v12"/><path d="M4 8l8 5 8-5"/><path d="M12 13v7"/></svg>
  ),
  gutter: (
  ...
  star: (
    <svg width="18" height="18" viewBox="0 0 24 24" fill="#3A7D7E" stroke="none"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
  ),
  mapPin: (
...
// --- Data ---
const PHONE = "(501) 553-1025";
const PHONE_TEL = "tel:+15015531025";
const EMAIL = "jbryan@allyroofers.com";

const services = [
  { name: "Roof Replacement", desc: "Complete tear-off and replacement using high-quality materials built to last. Ally Roofing handles every step from permits to final inspection, ensuring your new roof protects your home for decades.", icon: Icons.roof },
  { name: "Storm Damage Repair", desc: "Fast response after hail, wind, and severe weather events. Ally's team assesses the damage, documents everything, and gets your roof restored before the next storm rolls through.", icon: Icons.storm },
  { name: "Insurance Claims", desc: "Navigating insurance claims can be overwhelming. Ally Roofing walks you through the entire process, handles the documentation, and works directly with your adjuster to maximize your coverage.", icon: Icons.clipboard },
  { name: "Metal Roofing", desc: "Durable, energy-efficient metal roofing systems designed for Arkansas weather. A long-term investment that reduces maintenance costs and adds lasting value to your property.", icon: Icons.metal },
  { name: "Gutter Installation", desc: "Professional gutter installation and repair to protect your foundation and landscaping. Properly installed gutters channel water away from your home, preventing costly damage.", icon: Icons.gutter },
  { name: "Free Inspections", desc: "Comprehensive roof inspections at no cost. Ally's team identifies potential issues before they become expensive problems, giving you peace of mind and a clear plan of action.", icon: Icons.shield },
];

const allServices = [
  "Roofing Restoration", "Roof Replacement", "Storm Damage Repair",
  "Insurance Claims Assistance", "Gutter Installation/Repair", "Metal Roofing",
  "Roof Deck", "Free Inspections",
];

const reviews = [
  { text: "Ethan Taylor and his crew were phenomenal!", name: "Michael Young", src: "Google" },
  { text: "Bo did a great job helping me navigate through my first roof replacement and dealing with the insurance.", name: "Brandy Goodson", src: "Google" },
  { text: "Bo and his crew were great. Very attentive, responded to all questions.", name: "Brian Jones", src: "Google" },
  { text: "Ethan Taylor worked on two separate jobs for me and both turned out just as we talked about.", name: "Kattie Parker", src: "Google" },
  { text: "Ethan and his crew did a great job. Ethan was very thorough with explaining the process and filing insurance.", name: "Douglas Koonst", src: "Google" },
];

const areas = ["Maumelle", "Little Rock", "North Little Rock", "Sherwood", "Jacksonville", "Conway", "Benton", "Bryant"];

// --- Main Component ---
export default function AllyRoofingAndConstructionLLC() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("ally-roofing-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("ally-roofing-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Roofing Maumelle AR | Ally Roofing & Construction";
    const els = [];
    const add = (tag, attrs) => {
    ...
    add("meta", { name: "description", content: "Ally Roofing & Construction \u2014 4.9 stars, 128+ reviews. BBB A+ since 2020. Call (501) 553-1025 for a free roof inspection in Maumelle, AR." });
    ...
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      ...
      name: "Ally Roofing & Construction LLC",
      telephone: "+15015531025",
      email: EMAIL,
      address: { "@type": "PostalAddress", streetAddress: "501 Millwood Cir # C", addressLocality: "Maumelle", addressRegion: "AR", postalCode: "72113", addressCountry: "US" },
      ...
      founder: { "@type": "Person", name: "Joshua Bryan" },
      description: "Ally Roofing & Construction LLC is a BBB A+ accredited roofing contractor serving Maumelle and central Arkansas. Specializing in roof replacement, storm damage repair, and insurance claims assistance.",
  ...
  if (!authed) {
    return (
      <>
        ...
        `}</style>
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#5C3A1E" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, fontWeight: 600, letterSpacing: -0.5, marginBottom: 4 }}>
              <span style={{ color: "#5C3A1E" }}>Ally</span> <span style={{ color: "#3A7D7E" }}>Roofing</span>
            </div>
            ...
            <form onSubmit={submitPw}>
              ...
              <button type="submit" style={{ width: "100%", padding: 16, marginTop: 20, fontSize: 15, fontWeight: 700, letterSpacing: .3, background: "#5C3A1E", color: "#fff", border: "none", borderRadius: 8, cursor: "pointer", fontFamily: "'DM Sans',sans-serif" }}>View Site</button>
            </form>
          </div>
        </div>
      </>
  ...
  return (
    <>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled ? "rgba(92,58,30,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition: "all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        <div className="serif" style={{ fontSize: 22, fontWeight: 600, letterSpacing: -0.5, color: "#fff" }}>
          <span>Ally</span><span style={{ color: "var(--accent)", marginLeft: 6 }}>Roofing</span>
        </div>
        ...
        <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
          ...
          <div style={{ maxWidth: 560 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.1)", padding: "8px 18px", borderRadius: 4, marginBottom: 28 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: "var(--accent)" }}>BBB A+ Since 2020</span>
            </div>
            ...
            <p style={{ fontSize: 18, lineHeight: 1.7, color: "rgba(255,255,255,0.75)", marginBottom: 32, maxWidth: 480 }}>
              Ally Roofing & Construction helps homeowners through every step of roof replacement, storm damage repair, and insurance claims. Licensed, insured, and rated 4.9 stars by 128 customers.
            </p>
            {/* Star row with hero quote */}
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 32, flexWrap: "wrap" }}>
              <Stars />
              <span style={{ fontSize: 14, color: "rgba(255,255,255,0.7)", fontStyle: "italic" }}>"Ethan Taylor and his crew were phenomenal!"</span>
              <span style={{ fontSize: 13, color: "rgba(255,255,255,0.5)" }}>- Michael Young</span>
            </div>
            <div style={{ display: "flex", gap: 16, flexWrap: "wrap" }}>
              <a href={PHONE_TEL} style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 36px", borderRadius: 6, fontSize: 16, fontWeight: 700, letterSpacing: 0.3, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 4px 20px rgba(58,125,126,0.3)", transition: "transform 0.2s" }}>
                {Icons.phone} Call Joshua Now
              </a>
      ...
      <section style={{ background: "var(--white)", padding: "48px clamp(24px,5vw,64px)" }}>
        <div className="trust-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 32, textAlign: "center" }}>
          {[
            ...
            { num: "128+", label: "Reviews", desc: "Happy customers" },
            { num: "A+", label: "BBB Rating", desc: "Since 2020" },
            { num: "#0126", label: "Licensed", desc: "AR Contractor" },
          ].map((s, i) => (
      ...
      <section id="services" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)" }}>Roofing Services in Maumelle</h2>
          </div>
          {services.map((svc, i) => {
            const isEven = i % 2 === 1;
            return (
          ...
          <p style={{ textAlign: "center", marginTop: 48, fontSize: 15, color: "var(--text-mid)" }}>
            Questions? Call Joshua directly at <a href={PHONE_TEL} style={{ color: "var(--accent)", fontWeight: 600 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <div className="gold-line" />
            <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: "var(--text-light)", marginBottom: 14, fontFamily: "'DM Sans',sans-serif" }}>Why Ally</p>
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)" }}>Roofing Done Right in Central Arkansas</h2>
          </div>
          <div className="why-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "start" }}>
            ...
            <div>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "var(--text-mid)", marginBottom: 20 }}>
                Joshua Bryan started Ally Roofing & Construction with a simple idea: treat every homeowner the way you'd want to be treated after a storm. No high-pressure sales, no confusing paperwork, no disappearing after the check clears. Just honest work from a team that shows up, communicates clearly, and stands behind every job.
              </p>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "var(--text-mid)", marginBottom: 32 }}>
                That approach has earned Ally a 4.9-star rating from 128 customers, with 124 of those giving a perfect five stars. Crew leaders like Ethan Taylor and Bo have become names homeowners across Maumelle and Little Rock trust with their biggest investment. When you call Ally, you get a real person who walks you through every step, from inspection to insurance to final cleanup.
              </p>
              {/* Pull quote */}
              <div style={{ borderLeft: "3px solid var(--accent)", paddingLeft: 24, marginBottom: 36 }}>
                <p style={{ fontSize: 18, fontStyle: "italic", lineHeight: 1.7, color: "var(--primary)", fontFamily: "'Playfair Display',serif" }}>
                  "Bo did a great job helping me navigate the insurance."
                </p>
                <p style={{ fontSize: 14, color: "var(--text-light)", marginTop: 8, fontFamily: "'DM Sans',sans-serif" }}>- Brandy Goodson, Google Review</p>
              </div>
              {/* Credentials */}
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {[
                  "BBB A+ Accredited Since 2020",
                  "Licensed Arkansas Contractor #0126",
                  "4.9 Stars from 128 Google Reviews",
                  "124 of 128 Reviews Are Five Stars",
                  "Full Insurance Claims Assistance",
            ...
            <div className="badge-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {[
                { icon: Icons.award, title: "BBB A+ Since 2020", desc: "Better Business Bureau accredited with an A+ rating. A track record of integrity, transparency, and customer satisfaction from day one." },
                { icon: Icons.shield, title: "Licensed #0126", desc: "Fully licensed and insured Arkansas roofing contractor. Every job meets state requirements and manufacturer specifications." },
                { icon: Icons.users, title: "Trusted Crews", desc: "Crew leaders like Ethan Taylor and Bo have earned a reputation for clear communication, quality work, and genuine care for every homeowner." },
                { icon: Icons.home, title: "Insurance Experts", desc: "Ally walks you through the entire insurance claims process. From filing paperwork to working with adjusters, they handle the hard part." },
              ].map((b, i) => (
      ...
      <section id="reviews" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
          ...
          {/* Large featured review — Douglas Koonst */}
          <div style={{ position: "relative", padding: "48px 40px", background: "var(--white)", borderRadius: 12 }}>
            ...
            <p style={{ fontSize: 20, lineHeight: 1.8, color: "var(--text)", fontStyle: "italic", fontFamily: "'Playfair Display',serif", marginBottom: 28 }}>
              "Ethan and his crew did a great job. Ethan was very thorough with explaining the process and filing insurance."
            </p>
            <p style={{ fontSize: 17, fontWeight: 700, color: "var(--primary)" }}>Douglas Koonst</p>
            <p style={{ fontSize: 13, color: "var(--text-light)", marginTop: 4 }}>Google Review</p>
          </div>
          <p style={{ fontSize: 14, color: "var(--text-light)", marginTop: 32 }}>One of 124 five-star reviews on Google</p>
      ...
      <section className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)", borderTop: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)", marginBottom: 20 }}>
            Need a new roof?<br/><em style={{ color: "var(--accent)", fontStyle: "italic" }}>Call Ally Roofing.</em>
          </h2>
          <p style={{ fontSize: 17, lineHeight: 1.7, color: "var(--text-mid)", marginBottom: 40, maxWidth: 520, margin: "0 auto 40px" }}>
            Honest pricing, clear communication, and a team that handles your insurance so you don't have to. From inspection to installation, Ally Roofing has your back.
          </p>
          <a href={PHONE_TEL} style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 48px", borderRadius: 6, fontSize: 18, fontWeight: 700, letterSpacing: 0.3, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 4px 20px rgba(58,125,126,0.3)", transition: "transform 0.2s" }}>
            {Icons.phone} {PHONE}
          </a>
        </div>
      </section>
      ...
      <footer style={{ background: "var(--primary)", padding: "80px clamp(24px,5vw,64px) 40px", color: "rgba(255,255,255,0.75)" }}>
        <div className="footer-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: 48, marginBottom: 48 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 15, lineHeight: 1.7, marginBottom: 20, maxWidth: 300 }}>
              BBB A+ accredited roofing contractor serving Maumelle and central Arkansas. Licensed, insured, and trusted by 128+ homeowners.
            </p>
            <p style={{ fontSize: 14, marginBottom: 8 }}>501 Millwood Cir # C, Maumelle, AR 72113</p>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 24 }}>
          <div className="footer-btm-inner" style={{ maxWidth: 1200, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>{"\u00A9"} {new Date().getFullYear()} Ally Roofing & Construction LLC. All rights reserved.</p>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>BBB A+ Accredited | License #0126</p>
          </div>
        </div>
      </footer>
      ...
      <div className="mobile-only" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 998, display: "none" }}>
        <a href={PHONE_TEL} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 24px", fontSize: 16, fontWeight: 700, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 -4px 20px rgba(0,0,0,0.15)", textAlign: "center" }}>
          {Icons.phone} Call Ally Roofing - {PHONE}
        </a>
      </div>
