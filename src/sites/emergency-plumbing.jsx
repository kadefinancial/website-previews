...
const PHONE = "(501) 455-4390";
const TEL = "tel:+15014554390";
...
const services = [
  { name: "Emergency Plumbing", desc: "Burst pipes, major leaks, and plumbing emergencies handled fast. Available 24 hours a day, 7 days a week.", icon: Icons.zap },
  { name: "Leak Detection & Repair", desc: "Find and fix hidden leaks before they cause costly water damage. Fast diagnosis, reliable repairs.", icon: Icons.search },
  { name: "Pipe Repair", desc: "Broken, corroded, or damaged pipes repaired and replaced by experienced technicians.", icon: Icons.wrench },
  { name: "Shut Off Valve Service", desc: "Shut off valve replacement and repair to protect your home from water damage and plumbing emergencies.", icon: Icons.shield },
  { name: "General Plumbing Repair", desc: "Faucets, toilets, drains, and fixtures — every residential plumbing repair handled professionally.", icon: Icons.home },
  { name: "Drain Service", desc: "Clogged drains and slow pipes cleared with professional equipment. Kitchens, bathrooms, and main lines.", icon: Icons.droplets },
];

const allServiceNames = ["Emergency Plumbing Repairs","Shut Off Valve Replacement","General Plumbing Service & Repair","Leak Detection & Repair","Pipe Repair","Drain Service"];

const areas = ["Little Rock","West Little Rock","North Little Rock","Maumelle","Sherwood","Benton","Bryant","Jacksonville"];

const reviews = [
  { text: "Absolutely recommend Emergency Plumbing! I had an issue that needed urgent attention, and they got back to me almost immediately. Their team was professional, super responsive, and explained everything clearly.", name: "Lulu Jones", src: "Google" },
  { text: "They are quick and prompt. Replaced my shut off valve within 2 hrs after calling. Great Service!!", name: "Alroy Avance", src: "Google" },
  { text: "Great people and very professional, don't cost an arm and both legs.", name: "Shirley Johnson", src: "Google" },
  { text: "Great technicians who really care. I will recommend them and will use them again.", name: "Barbie Morgan", src: "Google" },
  { text: "Does a great job!", name: "Frederick Howard", src: "Google" },
];
...
export default function EmergencyPlumbing() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("ep-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="2026"){sessionStorage.setItem("ep-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "Emergency Plumber Little Rock AR";
    const els = [];
    ...
    add("meta",{name:"description",content:"24/7 emergency plumber in Little Rock, AR. Fast response, fair pricing. Call (501) 455-4390."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"Emergency Plumbing",telephone:"+15014554390",
      address:{"@type":"PostalAddress",streetAddress:"16825 W Baseline Rd",addressLocality:"Little Rock",addressRegion:"AR",postalCode:"72210",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:34.7465,longitude:-92.2896},
      openingHoursSpecification:[{"@type":"OpeningHoursSpecification",dayOfWeek:["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"],opens:"00:00",closes:"23:59"}],
      description:"Emergency Plumbing provides 24/7 emergency plumbing services in Little Rock, AR.",
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1C2B41"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8}}><span style={{fontWeight:700,color:"#1C2B41"}}>Emergency</span> <span style={{color:"#C8934F"}}>Plumbing</span></div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(28,43,65,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}><span>Emergency</span> <span style={{color:"var(--accent)"}}>Plumbing</span></div>
      ...
      <div className="hero-grid" style={{maxWidth:1200,width:"100%",display:"grid",gridTemplateColumns:"1.2fr 1fr",gap:48,alignItems:"center"}}>
        <div style={{maxWidth:560}}>
          ...
          <h1 style={{fontSize:"clamp(40px, 5vw, 68px)",lineHeight:1.05,color:"var(--primary)",marginBottom:24,letterSpacing:-1.5}}>
            Little Rock's<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Emergency</span><br/>Plumber
          </h1>
          <p style={{fontSize:18,lineHeight:1.7,color:"var(--text-mid)",marginBottom:28,maxWidth:460}}>
            When plumbing emergencies strike, we respond fast. Professional service, fair pricing, and a team that picks up the phone around the clock.
          </p>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:32}}>
            <Stars />
            <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"Professional, super responsive, and explained everything clearly." — Lulu Jones</span>
          </div>
    ...
    {/* TRUST BAR — 4.0-4.4: No rating shown */}
    <section style={{padding:"72px clamp(24px,5vw,64px)",background:"var(--white)",borderTop:"1px solid var(--border)",borderBottom:"1px solid var(--border)"}}>
      <div className="trust-grid" style={{maxWidth:1000,margin:"0 auto",display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:32,textAlign:"center"}}>
        {[{num:"24/7",label:"Availability",desc:"Day and night"},{num:"18+",label:"Google Reviews",desc:"Real customers"},{num:"Fast",label:"Response",desc:"Same-day service"},{num:"Fair",label:"Pricing",desc:"No surprises"}].map((s,i)=>(
          <div key={i}>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",color:"var(--primary)",letterSpacing:-0.5}}>Emergency Plumbing in Little Rock, AR</h2>
        </div>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",color:"#fff"}}>
      <div className="why-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.2fr 0.8fr",gap:80,alignItems:"start"}}>
        <div>
          ...
          <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,opacity:0.7}}>About Emergency Plumbing</div>
          <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff"}}>When You Need a Plumber<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Right Now</span></h2>
          <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.65)",marginBottom:24}}>
            Plumbing emergencies don't wait for business hours — and neither does Emergency Plumbing. Available 24 hours a day, 7 days a week, the team responds fast when you need help most. Burst pipes, major leaks, shut off valve failures — they handle the urgent calls that can't wait until Monday.
          </p>
          <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.65)",marginBottom:36}}>
            Customers consistently praise their fast response times, professional technicians, and fair pricing. When your home is flooding at 2 AM, you want a team that picks up the phone, shows up quickly, and fixes the problem without overcharging. That's what Emergency Plumbing delivers.
          </p>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            {["Available 24 Hours, 7 Days a Week","Fast Emergency Response","Professional, Experienced Technicians","Fair and Transparent Pricing","Serving Central Arkansas"].map((c,i)=>(
        ...
        <div className="badge-grid" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,paddingTop:20}}>
          {[
            ...
            {icon:Icons.shield,title:"Professional",desc:"Experienced team"},
            {icon:Icons.home,title:"Residential",desc:"All home plumbing"},
            {icon:Icons.wrench,title:"Fast Repair",desc:"Same-day response"},
          ].map((b,i)=>(
    ...
    {/* REVIEWS — Layout A: Carousel, no rating in heading (4.0-4.4) */}
    ...
    <section id="areas" style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:900,margin:"0 auto",textAlign:"center"}}>
        ...
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:12}}>
          {areas.map((a,i)=>(
            <span key={i} title={`Emergency plumber in ${a}, AR`} style={{display:"inline-flex",alignItems:"center",gap:6,background:"#fff",border:"1px solid var(--border)",padding:"12px 24px",borderRadius:30,fontSize:15,fontWeight:500,color:"var(--text)",cursor:"default",transition:"all 0.25s"}}
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>Plumbing emergency?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>We're here.</span></h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>Available 24/7. Fast response. Fair pricing.</p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16}}><span>Emergency</span> <span style={{color:"var(--accent)"}}>Plumbing</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>24/7 emergency plumbing services in Little Rock and Central Arkansas. Fast response, professional technicians, fair pricing.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>16825 W Baseline Rd<br/>Little Rock, AR 72210<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} Emergency Plumbing. All rights reserved.</span>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Emergency Plumbing</a>
    </div>
