...
const PHONE = "(479) 424-1208";
const TEL = "tel:+14794241208";
...
const Icons = {
  ...
  residential: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>,
  construction: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M2 20h20"/><path d="M5 20V8l7-5 7 5v12"/><path d="M9 20v-6h6v6"/><path d="M9 10h6"/></svg>,
  drain: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v6"/><path d="M12 12v10"/><circle cx="12" cy="12" r="3"/><path d="M4 8h16"/><path d="M6 8v3a6 6 0 0012 0V8"/></svg>,
  gasLine: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 22c4-4 8-7.5 8-12a8 8 0 00-16 0c0 4.5 4 8 8 12z"/><path d="M12 22c-1.5-2-3-3.5-3-6a3 3 0 016 0c0 2.5-1.5 4-3 6z"/></svg>,
...
const services = [
  { name: "Commercial Plumbing", desc: "Full-service commercial plumbing for offices, restaurants, warehouses, and industrial facilities. From new builds to maintenance contracts, Aaron's team handles projects most plumbers can't.", icon: Icons.commercial },
  { name: "Residential Plumbing", desc: "Complete home plumbing services — from faucet repairs and water heater replacements to whole-house repiping. Honest pricing, clean work, and no surprises on the bill.", icon: Icons.residential },
  { name: "New Construction Plumbing", desc: "Ground-up plumbing design and installation for commercial and residential builds. We work directly with general contractors and builders throughout the River Valley.", icon: Icons.construction },
  { name: "Drain Cleaning & Camera Inspection", desc: "Advanced drain scoping and camera inspection to pinpoint problems before digging. Professional drain cleaning that solves the issue — not just the symptom.", icon: Icons.drain },
  { name: "Gas Line Service", desc: "Licensed gas line installation, repair, and leak detection for residential and commercial properties. Safety-first approach with thorough testing on every job.", icon: Icons.gasLine },
  { name: "24/7 Emergency Service", desc: "Burst pipes, sewer backups, gas leaks — emergencies don't wait, and neither do we. Call any time, day or night, and Aaron's team will be there.", icon: Icons.emergency },
];
...
const reviews = [
  { text: "Aaron Borengasser with Borengasser Plumbing is the best!", name: "Eric C Sweet", src: "Facebook" },
];
...
export default function BorengasserPlumbing() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("borengasser-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("borengasser-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Plumber Fort Smith AR | Borengasser Plumbing";
    const els = [];
    const add = (tag, attrs) => {
    ...
    add("meta", { name: "description", content: "BBB A+ rated plumber in Fort Smith, AR. Commercial, residential & emergency plumbing since 2006. Call (479) 424-1208." });
    ...
    add("script", {
      ...
      textContent: JSON.stringify({
        ...
        "@type": "Plumber",
        name: "Borengasser Plumbing",
        telephone: "+14794241208",
        address: { "@type": "PostalAddress", addressLocality: "Fort Smith", addressRegion: "AR", postalCode: "72903", addressCountry: "US" },
        ...
        founder: { "@type": "Person", name: "Aaron Borengasser", jobTitle: "President" },
        foundingDate: "2006",
        description: "Borengasser Plumbing provides commercial, residential, and new construction plumbing services in Fort Smith, AR and the River Valley. BBB A+ rated since 2006. 24/7 emergency service.",
        ...
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#4A1A2E" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, marginBottom: 8, fontWeight: 600, letterSpacing: -0.5 }}>
              <span style={{ color: "#4A1A2E" }}>Borengasser</span>{" "}
              <span style={{ color: "#C17F5E" }}>Plumbing</span>
            </div>
    ...
    <>
        ...
        <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1.2fr 1fr", gap: 48, alignItems: "center" }}>
          <div>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(193,127,94,0.1)", padding: "8px 16px", borderRadius: 40, marginBottom: 24 }}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#C17F5E" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="M9 12l2 2 4-4"/></svg>
              <span style={{ fontSize: 13, fontWeight: 600, color: "var(--accent)" }}>BBB A+ Rated Since 2006</span>
            </div>
            <h1 style={{ fontSize: "clamp(40px,5vw,68px)", lineHeight: 1.05, color: "var(--primary)", letterSpacing: -1.5, marginBottom: 20 }}>
              Fort Smith's<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Most Dependable</span><br />Plumber
            </h1>
            <p style={{ fontSize: 18, lineHeight: 1.7, color: "var(--text-mid)", maxWidth: 480, marginBottom: 28 }}>
              BBB A+ rated since 2006. Commercial, residential, and new construction plumbing across the River Valley. Owner Aaron Borengasser works every job personally. 24/7 emergency service.
            </p>
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 32 }}>
              <Stars />
              <span style={{ fontSize: 14, color: "var(--text-light)", fontStyle: "italic", marginLeft: 4 }}>"Aaron Borengasser with Borengasser Plumbing is the best!" — Eric C Sweet</span>
            </div>
      ...
      <section style={{ background: "var(--white)", borderTop: "1px solid var(--border)", borderBottom: "1px solid var(--border)", padding: "48px clamp(24px,5vw,64px)" }}>
        <div className="trust-row" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 32, textAlign: "center" }}>
          {[
            { num: "BBB", label: "A+ Rated", desc: "Since 2006" },
            { num: "20+", label: "Years Experience", desc: "Serving the River Valley" },
            { num: "24/7", label: "Emergency Service", desc: "Day or night, we're here" },
            { num: "3-Trade", label: "Full Coverage", desc: "Commercial / Residential / Industrial" },
          ].map((s, i) => (
            <div key={i}>
      ...
      <section id="services" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          ...
          <p style={{ textAlign: "center", marginTop: 40, fontSize: 15, color: "var(--text-mid)" }}>
            Questions? Call Aaron directly at <a href={TEL} style={{ color: "var(--accent)", fontWeight: 700 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--primary)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <div className="gold-line" style={{ margin: "0 auto 20px" }} />
            <div style={{ fontSize: 12, fontWeight: 700, textTransform: "uppercase", letterSpacing: 3, color: "var(--accent)", marginBottom: 14 }}>Why Borengasser</div>
            <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "#fff", letterSpacing: -0.5 }}>BBB A+ Rated. 20+ Years in the River Valley.</h2>
          </div>
          <div className="why-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "start" }}>
            <div>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "rgba(255,255,255,0.82)", marginBottom: 20 }}>
                Aaron Borengasser started Borengasser Plumbing in 2006 with a straightforward approach: do quality work, charge fair prices, and stand behind every job. Twenty years later, that BBB A+ rating proves the approach works. While most plumbers stick to residential or commercial — Aaron's team handles both, plus new construction and industrial work across the River Valley.
              </p>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "rgba(255,255,255,0.82)", marginBottom: 32 }}>
                What sets Borengasser apart is capability. Drain scoping with camera inspection means Aaron can show you exactly what's going on underground before any digging starts. Licensed gas line work, full commercial build-outs, 24/7 emergency response — this isn't a one-truck operation that handles faucets and toilet repairs. When Fort Smith's businesses and homeowners need a plumber who can handle the job right the first time, they call Aaron.
              </p>
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {[
                  "BBB A+ rated since 2006",
                  "Commercial, residential, and industrial plumbing",
                  "Camera inspection and drain scoping",
            ...
            <div className="badge-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {[
                { icon: Icons.shield, title: "BBB A+ Rated", desc: "Accredited with an A+ rating since 2006" },
                { icon: Icons.award, title: "Since 2006", desc: "Nearly two decades serving Fort Smith and the River Valley" },
                { icon: Icons.clock, title: "24/7 Emergency", desc: "Burst pipes and gas leaks don't wait — neither do we" },
                { icon: Icons.wrench, title: "3-Trade Plumber", desc: "Commercial, residential, and industrial under one roof" },
              ].map((b, i) => (
      ...
      {/* REVIEWS -- white, centered single-card (only 1 review available) */}
      ...
      <section id="areas" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "var(--primary)", letterSpacing: -0.5 }}>Serving the River Valley</h2>
          </div>
          <div style={{ display: "flex", flexWrap: "wrap", justifyContent: "center", gap: 12 }}>
            {areas.map((a, i) => (
              <span key={i} title={`Plumber in ${a}, AR`} style={{ display: "inline-flex", alignItems: "center", gap: 6, background: "var(--white)", border: "1px solid var(--border)", borderRadius: 40, padding: "12px 22px", fontSize: 14, fontWeight: 600, color: "var(--text)", cursor: "default", transition: "all 0.3s ease" }}
      ...
      <section style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)", borderTop: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontSize: "clamp(30px,3.5vw,44px)", color: "var(--primary)", letterSpacing: -0.5, lineHeight: 1.15, marginBottom: 20 }}>
            Got a plumbing problem?<br /><span style={{ fontStyle: "italic", color: "var(--accent)" }}>Call Aaron.</span>
          </h2>
          <p style={{ fontSize: 17, lineHeight: 1.7, color: "var(--text-mid)", marginBottom: 36, maxWidth: 520, margin: "0 auto 36px" }}>
            Honest pricing. BBB A+ rated since 2006. Available 24/7 for emergencies. Serving the entire River Valley.
          </p>
      ...
      <footer style={{ background: "var(--primary)", padding: "80px clamp(24px,5vw,64px) 40px" }}>
        <div className="footer-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: 48, marginBottom: 48 }}>
          <div>
            <div className="serif" style={{ fontSize: 22, fontWeight: 600, letterSpacing: -0.5, marginBottom: 16 }}>
              <span style={{ color: "#fff" }}>Borengasser</span>{" "}
              <span style={{ color: "var(--accent)" }}>Plumbing</span>
            </div>
            <p style={{ fontSize: 14, lineHeight: 1.7, color: "rgba(255,255,255,0.6)", marginBottom: 16, maxWidth: 320 }}>
              BBB A+ rated plumbing since 2006. Commercial, residential, and new construction services in Fort Smith and the River Valley. 24/7 emergency service.
            </p>
            <p style={{ fontSize: 14, color: "rgba(255,255,255,0.6)", marginBottom: 6 }}>Fort Smith, AR 72903</p>
        ...
        <div className="footer-btm" style={{ maxWidth: 1200, margin: "0 auto", borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 24, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>&copy; {yr} Borengasser Plumbing. All rights reserved.</p>
          <p style={{ fontSize: 13, color: "rgba(255,255,255,0.35)" }}>BBB A+ Accredited Business</p>
        </div>
      </footer>
      ...
      <div className="mobile-only" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 998, display: "none" }}>
        <a href={TEL} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "16px 20px", fontSize: 16, fontWeight: 700, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 -4px 20px rgba(0,0,0,0.15)", textDecoration: "none" }}>
          {Icons.phone} Call Borengasser — {PHONE}
        </a>
      </div>
