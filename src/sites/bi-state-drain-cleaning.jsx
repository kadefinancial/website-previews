...
const Icons = {
  ...
  hydro: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M12 2v6"/><path d="M8 4l4 4 4-4"/><path d="M12 14.69l4.24 4.24a6 6 0 11-8.49 0z"/></svg>
  ),
  camera: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M23 19a2 2 0 01-2 2H3a2 2 0 01-2-2V8a2 2 0 012-2h4l2-3h6l2 3h4a2 2 0 012 2z"/><circle cx="12" cy="13" r="4"/></svg>
  ),
  locate: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="3"/><path d="M12 2v4"/><path d="M12 18v4"/><path d="M2 12h4"/><path d="M18 12h4"/><circle cx="12" cy="12" r="9"/></svg>
  ),
  storm: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M20 16.58A5 5 0 0018 7h-1.26A8 8 0 104 15.25"/><polyline points="16 16 12 20 8 16"/></svg>
  ),
  pipe: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M4 6h6v12H4z"/><path d="M10 10h10v4H10"/><path d="M6 2v4"/><path d="M8 2v4"/><path d="M6 18v4"/><path d="M8 18v4"/></svg>
  ),
  check: (
  ...
  user: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M20 21v-2a4 4 0 00-4-4H8a4 4 0 00-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
  ),
};
...
// --- Data ---
const PHONE = "(479) 883-3170";
const PHONE_TEL = "tel:+14798833170";
const BIZ = "Bi-State Drain Cleaning";

const services = [
  { name: "Drain Cleaning", desc: "Clogged kitchen sinks, shower drains, and floor drains cleared fast. Lewis brings the right equipment to every job and gets your water flowing again — usually the same day you call.", icon: Icons.droplet },
  { name: "Hydro Jetting", desc: "High-pressure water jetting that blasts through grease, roots, and years of buildup. The most thorough way to clean a drain line — no chemicals, no shortcuts, just results.", icon: Icons.hydro },
  { name: "Video Inspection", desc: "A camera goes into your line so you can see exactly what's causing the problem. No guessing, no unnecessary work — Lewis shows you the footage and explains your options honestly.", icon: Icons.camera },
  { name: "Line Location", desc: "Precision electronic locating to find underground pipes and utilities before any digging starts. Saves you time, money, and the headache of damaged landscaping.", icon: Icons.locate },
  { name: "Storm Drain Service", desc: "Clearing and maintaining storm drains to prevent flooding and water damage on your property. Serving residential and commercial properties across the River Valley.", icon: Icons.storm },
  { name: "Sewer Line Cleaning", desc: "Complete sewer line cleaning from the house to the main. Lewis handles tough blockages and root intrusion with professional-grade equipment that gets the job done right.", icon: Icons.pipe },
];

const allServices = [
  "Drain Cleaning", "Hydro Jetting", "Video Inspection",
  "Line Location", "Storm Drain Service", "Sewer Line Cleaning",
];

const reviews = [
  { text: "Punctual, professional and reasonably priced — would highly recommend!", name: "Lisa Crispino", src: "Google" },
  { text: "Awesome service! Very punctual and cordial. I would definitely recommend if you are in need of this service.", name: "Angela Matthews", src: "Google" },
  { text: "Lewis is the man! He actually calls you back promptly when you call. His work and work ethic speaks for itself — very professional and reasonable on the wallet.", name: "Stephen Cox", src: "Google" },
];

const areas = ["Fort Smith", "Van Buren", "Alma", "Greenwood", "Barling", "River Valley", "Western Arkansas", "Eastern Oklahoma"];

// --- Main Component ---
export default function BiStateDrainCleaning() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("bi-state-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("bi-state-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Drain Cleaning Fort Smith AR | Bi-State Drain Cleaning";
    const els = [];
    const add = (tag, attrs) => {
    ...
    add("meta", { name: "description", content: "Bi-State Drain Cleaning — 4.6 stars on Google. Drain cleaning, hydro jetting & sewer service. Call (479) 883-3170 in Fort Smith, AR." });
    ...
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      ...
      "@type": "Plumber",
      name: "Bi-State Drain Cleaning",
      telephone: "+14798833170",
      address: { "@type": "PostalAddress", streetAddress: "10617 Jenny Lind Rd", addressLocality: "Fort Smith", addressRegion: "AR", postalCode: "72908", addressCountry: "US" },
      geo: { "@type": "GeoCoordinates", latitude: 35.3440, longitude: -94.3985 },
      openingHoursSpecification: [],
      aggregateRating: { "@type": "AggregateRating", ratingValue: "4.6", reviewCount: "18", bestRating: "5" },
      founder: { "@type": "Person", name: "Louis Nichols" },
      description: "Bi-State Drain Cleaning provides professional drain cleaning, hydro jetting, and sewer services in Fort Smith, AR and the River Valley. 4.6 stars on Google.",
      areaServed: areas.map((a) => ({ "@type": "City", name: a })),
      hasOfferCatalog: { "@type": "OfferCatalog", name: "Drain Cleaning Services", itemListElement: allServices.map((s) => ({ "@type": "Offer", itemOffered: { "@type": "Service", name: s } })) },
      memberOf: [],
      sameAs: [],
    })});
        ...
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#1C2B41" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, fontWeight: 600, letterSpacing: -0.5, marginBottom: 4 }}>
              <span style={{ color: "#1C2B41" }}>Bi-State</span> <span style={{ color: "#C8934F" }}>Drain Cleaning</span>
            </div>
    ...
    <>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled ? "rgba(28,43,65,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition: "all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        <div className="serif" style={{ fontSize: 22, fontWeight: 600, letterSpacing: -0.5, color: "#fff" }}>
          <span>Bi-State</span><span style={{ color: "var(--accent)", marginLeft: 6 }}>Drain Cleaning</span>
        </div>
        ...
        <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
          ...
          <div style={{ maxWidth: 560 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.1)", padding: "8px 18px", borderRadius: 4, marginBottom: 28 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: "var(--accent)" }}>Arkansas & Oklahoma Coverage</span>
            </div>
            <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(40px,5vw,68px)", lineHeight: 1.05, letterSpacing: -1.5, color: "#fff", marginBottom: 24 }}>
              Fort Smith's{" "}<br/>
              <em style={{ color: "var(--accent)", fontStyle: "italic" }}>Drain Cleaning Pro</em><br/>
              Two States Strong
            </h1>
            <p style={{ fontSize: 18, lineHeight: 1.7, color: "rgba(255,255,255,0.75)", marginBottom: 32, maxWidth: 480 }}>
              Lewis Nichols runs every job personally — from drain cleaning to hydro jetting across the River Valley. One call, one expert, both sides of the state line.
            </p>
            {/* Star row with hero quote */}
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 32, flexWrap: "wrap" }}>
              <Stars />
              <span style={{ fontSize: 14, color: "rgba(255,255,255,0.7)", fontStyle: "italic" }}>"Very professional and reasonable on the wallet."</span>
              <span style={{ fontSize: 13, color: "rgba(255,255,255,0.5)" }}>- Stephen Cox</span>
            </div>
      ...
      <section style={{ background: "var(--white)", padding: "48px clamp(24px,5vw,64px)" }}>
        <div className="trust-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 32, textAlign: "center" }}>
          {[
            ...
            { num: "18+", label: "Customer Reviews", desc: "And counting" },
            { num: "Bi-State", label: "AR & OK Coverage", desc: "Two states served" },
            { num: "Lewis", label: "On Every Job", desc: "One expert, every time" },
          ].map((s, i) => (
            <div key={i} style={{ padding: "16px 0" }}>
              <div style={{ fontFamily: "'Playfair Display',serif", fontSize: i >= 2 ? "clamp(22px,2.5vw,32px)" : "clamp(28px,3vw,40px)", fontWeight: 600, color: "var(--primary)", letterSpacing: -0.5, marginBottom: 6 }}>{s.num}</div>
      ...
      <section id="services" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)" }}>Drain Services in Fort Smith</h2>
          </div>
          {services.map((svc, i) => {
            const isEven = i % 2 === 1;
            return (
          ...
          <p style={{ textAlign: "center", marginTop: 48, fontSize: 15, color: "var(--text-mid)" }}>
            Questions? Call Lewis directly at <a href={PHONE_TEL} style={{ color: "var(--accent)", fontWeight: 600 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <div className="gold-line" />
            <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: "var(--text-light)", marginBottom: 14, fontFamily: "'DM Sans',sans-serif" }}>Why Bi-State</p>
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)" }}>One Man, Two States, Zero Excuses</h2>
          </div>
          <div className="why-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "start" }}>
            ...
            <div>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "var(--text-mid)", marginBottom: 20 }}>
                Bi-State Drain Cleaning is Lewis Nichols — one experienced professional who handles every job personally. Based in Fort Smith, Lewis serves the entire River Valley on both sides of the Arkansas-Oklahoma state line. When you call Bi-State, you're talking directly to the person who's going to do the work.
              </p>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "var(--text-mid)", marginBottom: 32 }}>
                With a 4.6-star Google rating and a reputation for calling back promptly, showing up on time, and pricing fairly, Lewis has built Bi-State on the things that matter most to homeowners — reliability, honesty, and getting the job done right the first time. No dispatchers, no runaround. Just a skilled drain technician who takes pride in his work.
              </p>
              {/* Pull quote */}
              <div style={{ borderLeft: "3px solid var(--accent)", paddingLeft: 24, marginBottom: 36 }}>
                <p style={{ fontSize: 18, fontStyle: "italic", lineHeight: 1.7, color: "var(--primary)", fontFamily: "'Playfair Display',serif" }}>
                  "Lewis is the man! He actually calls you back promptly when you call. His work and work ethic speaks for itself."
                </p>
                <p style={{ fontSize: 14, color: "var(--text-light)", marginTop: 8, fontFamily: "'DM Sans',sans-serif" }}>- Stephen Cox, Google Review</p>
              </div>
              {/* Credentials */}
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {[
                  ...
                  "Lewis on Every Job — No Subcontractors",
                  "Prompt Callbacks & On-Time Arrivals",
            ...
            <div className="badge-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {[
                { icon: Icons.user, title: "Owner-Operated", desc: "Lewis Nichols handles every single job personally. No technicians, no middlemen — just one experienced professional who takes pride in his craft." },
                { icon: Icons.shield, title: "Bi-State Coverage", desc: "Serving Fort Smith, the River Valley, western Arkansas, and eastern Oklahoma. One call covers both sides of the state line." },
                { icon: Icons.clock, title: "Prompt & Punctual", desc: "Lewis calls back quickly and shows up on time. Customers consistently praise his reliability and professionalism in every review." },
                { icon: Icons.award, title: "Fair Pricing", desc: "Professional drain cleaning at reasonable rates. No surprise charges, no upselling — just honest work at an honest price." },
              ].map((b, i) => (
      ...
      <section id="reviews" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
          ...
          {/* Large featured review — Stephen Cox */}
          <div style={{ position: "relative", padding: "48px 40px", background: "var(--white)", borderRadius: 12 }}>
            ...
            <p style={{ fontSize: 20, lineHeight: 1.8, color: "var(--text)", fontStyle: "italic", fontFamily: "'Playfair Display',serif", marginBottom: 28 }}>
              "Lewis is the man! He actually calls you back promptly when you call. His work and work ethic speaks for itself — very professional and reasonable on the wallet."
            </p>
            <p style={{ fontSize: 17, fontWeight: 700, color: "var(--primary)" }}>Stephen Cox</p>
            <p style={{ fontSize: 13, color: "var(--text-light)", marginTop: 4 }}>Google Review</p>
          </div>
          <p style={{ fontSize: 14, color: "var(--text-light)", marginTop: 32, marginBottom: 40 }}>One of 18+ five-star reviews on Google</p>

          {/* Secondary reviews */}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
            {[reviews[0], reviews[1]].map((rev, i) => (
      ...
      <section id="areas" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto", textAlign: "center" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 12, justifyContent: "center" }}>
            {areas.map((area, i) => (
              <span key={i} title={`Drain Cleaning in ${area}${area === "Eastern Oklahoma" ? "" : ", AR"}`} style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "12px 22px", background: "var(--white)", border: "1px solid var(--border)", borderRadius: 100, fontSize: 14, fontWeight: 500, color: "var(--text)", cursor: "default", transition: "all 0.2s ease", fontFamily: "'DM Sans',sans-serif" }}
      ...
      <section className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)", borderTop: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)", marginBottom: 20 }}>
            Got a drain problem?<br/><em style={{ color: "var(--accent)", fontStyle: "italic" }}>Call Lewis.</em>
          </h2>
          <p style={{ fontSize: 17, lineHeight: 1.7, color: "var(--text-mid)", marginBottom: 40, maxWidth: 520, margin: "0 auto 40px" }}>
            Professional drain cleaning, honest pricing, and an expert who shows up on time — every time. Bi-State Drain Cleaning serves both sides of the state line.
          </p>
      ...
      <footer style={{ background: "var(--primary)", padding: "80px clamp(24px,5vw,64px) 40px", color: "rgba(255,255,255,0.75)" }}>
        <div className="footer-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: 48, marginBottom: 48 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 15, lineHeight: 1.7, marginBottom: 20, maxWidth: 300 }}>
              Professional drain cleaning serving Fort Smith, the River Valley, western Arkansas, and eastern Oklahoma.
            </p>
            <p style={{ fontSize: 14, marginBottom: 8 }}>10617 Jenny Lind Rd, Fort Smith, AR 72908</p>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 24 }}>
          <div className="footer-btm-inner" style={{ maxWidth: 1200, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>{new Date().getFullYear()} Bi-State Drain Cleaning. All rights reserved.</p>
