...
const PHONE = "(501) 246-4936";
const TEL = "tel:+15012464936";
...
const services = [
  { name: "Air Conditioning Repair & Service", desc: "When your AC goes down in an Arkansas summer, M & I responds fast. Expert diagnostics and reliable repairs to restore your comfort the same day.", icon: Icons.wind },
  { name: "AC Installation", desc: "New air conditioning systems sized and installed correctly for your home. Energy-efficient units backed by manufacturer warranties and M & I's hands-on craftsmanship.", icon: Icons.home },
  { name: "Heating / Furnace Repair", desc: "Furnace not keeping up? Our team identifies the real issue quickly -- from ignition failures to blower problems -- and gets your heat running right.", icon: Icons.flame },
  { name: "Heating Installation", desc: "Complete heating system installation for new construction or replacement. Properly sized equipment ensures even warmth and lower utility bills all winter.", icon: Icons.wrench },
  { name: "AC Tune-Ups", desc: "Preventive maintenance that catches small problems before they become expensive repairs. Free AC tune-up offers available -- just call to schedule with Cindy.", icon: Icons.clipboard },
  { name: "Commercial HVAC", desc: "Reliable HVAC service for businesses across Central Arkansas. From office buildings to retail spaces, M & I keeps your commercial systems performing year-round.", icon: Icons.building },
];

const allServiceNames = ["Air Conditioning Repair & Service","AC Installation","Heating / Furnace Repair","Heating Installation","AC Tune-Ups","Commercial HVAC"];

const areas = ["Little Rock","North Little Rock","Sherwood","Jacksonville","Maumelle","Benton","Bryant","Conway","Cabot","Pine Bluff"];

const reviews = [
  { text: "M&I Heating and Air came through like a 'Champ'. I was without AC during this heat wave and in distress. I called M&I Heating and Air and they were ready and willing and able to diagnose and repair my AC system and I am soooo grateful.", name: "Lauri J Williams", src: "Google" },
  { text: "I spoke with Cindy on the telephone about the free AC tune-up. She was kind to schedule a day that was good for us. Victor, Luiz and Vincent were kind and competent. They got the job done for us.", name: "Larry Zinck", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function MAndIHeatingAndAir() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("mi-hvac-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="roof"){sessionStorage.setItem("mi-hvac-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "HVAC Service Little Rock AR | M & I Heating and Air";
    const els = [];
    ...
    add("meta",{name:"description",content:"HVAC service in Little Rock AR. 4.8 stars, 23 reviews. Family owned, BBB accredited. Free AC tune-ups. Call (501) 246-4936."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"M & I Heating and Air, LLC",telephone:"+15012464936",
      email:"miheatingandairus@gmail.com",
      address:{"@type":"PostalAddress",streetAddress:"9700 I-30",addressLocality:"Little Rock",addressRegion:"AR",postalCode:"72209",addressCountry:"US"},
      ...
      founder:{"@type":"Person",name:"Mike Ornelas",jobTitle:"President"},
      description:"M & I Heating and Air provides HVAC services in Little Rock and Central Arkansas. 4.8-star Google rating, BBB accredited, Entergy/ICF partner. Family owned and operated.",
      ...
      memberOf:[{"@type":"Organization",name:"Better Business Bureau"},{"@type":"Organization",name:"Entergy/ICF Partner"}],sameAs:[],
    })});
    ...
    return (<>
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1C2B41"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8,fontWeight:600,letterSpacing:-0.5}}><span style={{color:"#1C2B41"}}>M & I</span> <span style={{color:"#C8934F"}}>Heating and Air</span></div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(28,43,65,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}><span>M & I</span> <span style={{color:"var(--accent)"}}>Heating and Air</span></div>
      ...
      <div className="hero-grid" style={{maxWidth:1200,width:"100%",display:"grid",gridTemplateColumns:"1.8fr 1fr",gap:48,alignItems:"center"}}>
        <div>
          <h1 style={{fontSize:"clamp(48px, 6vw, 84px)",lineHeight:1.0,color:"var(--primary)",letterSpacing:-2,marginBottom:24}}>
            Little Rock's<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Trusted HVAC</span><br/>Team
          </h1>
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:32}}>
            <Stars />
            <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"They were ready and willing and able to diagnose and repair my AC system." — Lauri J Williams</span>
          </div>
    ...
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        <div style={{textAlign:"center",marginBottom:64}}>
          ...
          <h2 style={{fontSize:"clamp(30px,3.5vw,44px)",color:"var(--primary)",letterSpacing:-0.5}}>HVAC Services in Little Rock & Central Arkansas</h2>
        </div>
        {/* Mobile trust */}
        <div className="trust-mobile" style={{display:"none",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:48,textAlign:"center"}}>
          {[{num:"4.8",label:"Google Rating",desc:"23 reviews"},{num:"23+",label:"5-Star Reviews",desc:"On Google"},{num:"BBB",label:"Accredited Member",desc:"5+ years"},{num:"Free",label:"AC Tune-Ups",desc:"Call to schedule"}].map((s,i)=>(
            <div key={i}>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",position:"relative",overflow:"hidden"}}>
      <div className="serif" style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",fontSize:"clamp(120px,20vw,280px)",color:"var(--accent)",opacity:0.08,lineHeight:1,whiteSpace:"nowrap",pointerEvents:"none"}}>23+</div>
      <div style={{maxWidth:800,margin:"0 auto",position:"relative",zIndex:1}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,textAlign:"center",opacity:0.7}}>About M & I</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff",textAlign:"center"}}>Family Owned,<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Community Trusted</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:24,textAlign:"center"}}>
          M & I Heating and Air isn't a franchise or a call-center operation. It's a family-owned business led by President Mike Ornelas, where every customer gets treated like a neighbor. Connected to M & I Builders LLC, the company brings a contractor's eye for quality to every HVAC job -- whether it's a quick AC repair during an Arkansas heat wave or a full system installation for a new home. When you call M & I, you're calling people who live and work in the same community they serve.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:36,textAlign:"center"}}>
          The team customers praise by name -- Cindy, Victor, Luiz, and Vincent -- reflects the kind of business Mike runs: professional, personal, and proud of every job. As a BBB-accredited member for over five years and an Entergy/ICF partner, M & I has built a reputation across Little Rock and Central Arkansas the right way -- one satisfied customer at a time. That's how you earn a 4.8-star Google rating with 23 five-star reviews.
        </p>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:20}}>
          {["4.8-Star Google Rating","BBB Accredited (5+ Years)","Entergy/ICF Partner","Family Owned & Operated","Free AC Tune-Up Offers","Dedicated, Named Team"].map((c,i)=>(
    ...
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>AC down or heater trouble?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Mike.</span></h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>Family owned. BBB accredited. 4.8-star rated. Free AC tune-ups available.</p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16,fontWeight:600,letterSpacing:-0.5}}><span>M & I</span> <span style={{color:"var(--accent)"}}>Heating and Air</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>HVAC services for Little Rock and surrounding Central Arkansas. 4.8-star Google rating. BBB accredited. Entergy/ICF partner. Family owned and operated.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>9700 I-30<br/>Little Rock, AR 72209<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a><br/><a href="mailto:miheatingandairus@gmail.com" style={{color:"rgba(255,255,255,0.5)"}}>miheatingandairus@gmail.com</a></p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} M & I Heating and Air, LLC. All rights reserved.</span>
          <span>BBB Accredited | Mon-Fri 8 AM-4 PM</span>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call M & I — {PHONE}</a>
    </div>
