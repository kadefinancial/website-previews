// Layout: C | Industry: Roofing | City: Little Rock
...
const Icons = {
  ...
  metal: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><path d="M2 20h20"/><path d="M4 20V8l8-5 8 5v12"/><path d="M4 12h16"/><path d="M4 16h16"/><path d="M8 8v12"/><path d="M12 3v17"/><path d="M16 8v12"/></svg>
  ),
  flat: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true"><rect x="2" y="6" width="20" height="14" rx="2"/><path d="M2 10h20"/><path d="M6 6V4"/><path d="M18 6V4"/><path d="M10 14h4"/></svg>
  ),
  remodel: (
...
// --- Data ---
const PHONE = "(501) 519-4927";
const PHONE_TEL = "tel:+15015194927";
const EMAIL = "jose.rodriguezjz@gmail.com";

const services = [
  { name: "Roof Replacement", desc: "Complete tear-off and replacement using Atlas, Owens Corning, and CertainTeed materials. JZ Roofing handles every step from inspection to final cleanup, delivering a roof built to protect your home for decades.", icon: Icons.roof },
  { name: "Metal Roofing", desc: "Durable, long-lasting metal roofing systems installed by an experienced crew. Ideal for Arkansas weather, metal roofs offer superior wind and hail resistance with minimal maintenance over time.", icon: Icons.metal },
  { name: "Flat Roofing", desc: "Professional flat roof installation and repair for commercial and residential properties. JZ Roofing uses proven materials and techniques to eliminate leaks and extend the life of your flat roof system.", icon: Icons.flat },
  { name: "Remodeling", desc: "Full-service remodeling including bathroom additions, screened-in porches, and fencing. Jose and his crew bring the same precision and reliability to interior projects that they bring to every roof.", icon: Icons.remodel },
  { name: "Gutter Installation", desc: "Properly installed gutters channel water away from your foundation and landscaping. JZ Roofing ensures seamless integration with your roofline for maximum protection against water damage.", icon: Icons.gutter },
  { name: "Insurance Claims", desc: "Storm damage? JZ Roofing works directly with your insurance company, handling inspections, documentation, and follow-up so you can focus on getting your home back to normal.", icon: Icons.shield },
];

const allServices = [
  "Roof Replacement", "Roof Repair", "Asphalt Roofing", "Flat Roofing",
  "Metal Roofing", "Insurance Roof Replacement", "Remodeling",
  "Gutter Installation", "Bathroom Remodels", "Fencing", "Free Inspections",
];

const areas = ["Little Rock", "North Little Rock", "Sherwood", "Jacksonville", "Benton", "Bryant", "Maumelle", "Conway"];

// --- Main Component ---
export default function JZRoofingRemodeling() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("jz-roofing-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("jz-roofing-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "Roofing Little Rock AR | JZ Roofing Remodeling";
    const els = [];
    const add = (tag, attrs) => {
    ...
    add("meta", { name: "description", content: "JZ Roofing Remodeling \u2014 4.8 stars, 42+ reviews. BBB Accredited. Roofing and remodeling in Little Rock, AR. Call (501) 519-4927." });
    ...
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      ...
      name: "JZ Roofing Remodeling",
      telephone: "+15015194927",
      email: EMAIL,
      address: { "@type": "PostalAddress", streetAddress: "6827 Honeysuckle Ln", addressLocality: "Little Rock", addressRegion: "AR", postalCode: "72204", addressCountry: "US" },
      ...
      founder: { "@type": "Person", name: "Jose Rodriguez" },
      description: "JZ Roofing Remodeling is a BBB Accredited roofing and remodeling contractor serving Little Rock and central Arkansas. Specializing in roof replacement, metal roofing, flat roofing, bathroom remodels, and insurance claims.",
      areaServed: areas.map((a) => ({ "@type": "City", name: a })),
      hasOfferCatalog: { "@type": "OfferCatalog", name: "Roofing & Remodeling Services", itemListElement: allServices.map((s) => ({ "@type": "Offer", itemOffered: { "@type": "Service", name: s } })) },
      memberOf: [{ "@type": "Organization", name: "Better Business Bureau" }, { "@type": "Organization", name: "HomeAdvisor" }, { "@type": "Organization", name: "Angie's List" }],
      sameAs: [],
    })});
        ...
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#1E1E1E" }}>
          <div style={{ background: "#fff", borderRadius: 16, padding: "56px 44px", textAlign: "center", maxWidth: 420, width: "92%", boxShadow: "0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 28, fontWeight: 600, letterSpacing: -0.5, marginBottom: 4 }}>
              <span style={{ color: "#fff", background: "#1E1E1E", padding: "2px 8px", borderRadius: 4 }}>JZ</span> <span style={{ color: "#D4A03C" }}>Roofing</span>
            </div>
    ...
    <>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display: "flex", alignItems: "center", justifyContent: "space-between", background: scrolled ? "rgba(30,30,30,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition: "all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        <div className="serif" style={{ fontSize: 22, fontWeight: 600, letterSpacing: -0.5, color: "#fff" }}>
          <span>JZ</span><span style={{ color: "var(--accent)", marginLeft: 6 }}>Roofing</span>
        </div>
        ...
        <div className="hero-grid" style={{ maxWidth: 1200, width: "100%", display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "center" }}>
          ...
          <div style={{ maxWidth: 560 }}>
            <div style={{ display: "inline-flex", alignItems: "center", gap: 8, background: "rgba(255,255,255,0.1)", padding: "8px 18px", borderRadius: 4, marginBottom: 28 }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: "var(--accent)" }}>BBB Accredited</span>
            </div>
            <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(40px,5vw,68px)", lineHeight: 1.05, letterSpacing: -1.5, color: "#fff", marginBottom: 24 }}>
              Little Rock's{" "}<br/>
              <em style={{ color: "var(--accent)", fontStyle: "italic" }}>Roofing &amp; Remodeling</em><br/>
              Experts
            </h1>
            <p style={{ fontSize: 18, lineHeight: 1.7, color: "rgba(255,255,255,0.75)", marginBottom: 32, maxWidth: 480 }}>
              JZ Roofing Remodeling delivers full-service roofing and remodeling across Little Rock and central Arkansas. From roof replacements to bathroom remodels, Jose Rodriguez and his crew get it done right.
            </p>
            {/* Star row with hero quote */}
            <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 32, flexWrap: "wrap" }}>
              <Stars />
              <span style={{ fontSize: 14, color: "rgba(255,255,255,0.7)", fontStyle: "italic" }}>"The fastest workers I've ever seen. The work is beautiful!"</span>
              <span style={{ fontSize: 13, color: "rgba(255,255,255,0.5)" }}>- Kayla Buchanan</span>
            </div>
      ...
      {/* TRUST BAR — Layout C: white, no borders needed (dark-to-white transition is strong) */}
      <section style={{ background: "var(--white)", padding: "48px clamp(24px,5vw,64px)" }}>
        <div className="trust-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 32, textAlign: "center" }}>
          {[
            ...
            { num: "42+", label: "Reviews", desc: "Happy customers" },
            { num: "BBB", label: "Accredited", desc: "J Z Remodeling LLC" },
            { num: "7 Days", label: "A Week", desc: "8AM to 8PM" },
          ].map((s, i) => (
      ...
      <section id="services" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            ...
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)" }}>Roofing &amp; Remodeling in Little Rock</h2>
          </div>
          {services.map((svc, i) => {
            const isEven = i % 2 === 1;
            return (
          ...
          <p style={{ textAlign: "center", marginTop: 48, fontSize: 15, color: "var(--text-mid)" }}>
            Questions? Call Jose directly at <a href={PHONE_TEL} style={{ color: "var(--accent)", fontWeight: 600 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--cream)" }}>
        <div style={{ maxWidth: 1200, margin: "0 auto" }}>
          <div style={{ textAlign: "center", marginBottom: 64 }}>
            <div className="gold-line" />
            <p style={{ fontSize: 12, fontWeight: 700, letterSpacing: 3, textTransform: "uppercase", color: "var(--text-light)", marginBottom: 14, fontFamily: "'DM Sans',sans-serif" }}>Why JZ</p>
            <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)" }}>Roofing &amp; Remodeling Done Right in Little Rock</h2>
          </div>
          <div className="why-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 64, alignItems: "start" }}>
            ...
            <div>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "var(--text-mid)", marginBottom: 20 }}>
                Jose Rodriguez built JZ Roofing Remodeling on a simple idea: do quality work, show up when you say you will, and treat every home like your own. That approach has earned the trust of homeowners across Little Rock and central Arkansas, with 39 out of 42 Google reviews at five stars and a 4.8-star overall rating.
              </p>
              <p style={{ fontSize: 17, lineHeight: 1.8, color: "var(--text-mid)", marginBottom: 32 }}>
                What sets JZ apart is range. Most roofing companies only do roofing. Jose and his crew handle roof replacements, metal roofing, flat roofing, bathroom remodels, screened-in porches, fencing, and gutter installation. They use trusted materials from Atlas, Owens Corning, CertainTeed, and Lomanco. BBB Accredited, HomeAdvisor approved, and listed on Angie's List, JZ Roofing is available seven days a week, 8AM to 8PM.
              </p>
              {/* Pull quote */}
              <div style={{ borderLeft: "3px solid var(--accent)", paddingLeft: 24, marginBottom: 36 }}>
                <p style={{ fontSize: 18, fontStyle: "italic", lineHeight: 1.7, color: "var(--primary)", fontFamily: "'Playfair Display',serif" }}>
                  "They did an entire roof replacement. Great job."
                </p>
                <p style={{ fontSize: 14, color: "var(--text-light)", marginTop: 8, fontFamily: "'DM Sans',sans-serif" }}>- Dana Phelps, Google Review</p>
              </div>
              {/* Credentials */}
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
                {[
                  "BBB Accredited (J Z Remodeling LLC)",
                  "4.8 Stars from 42 Google Reviews",
                  "39 of 42 Reviews Are Five Stars",
                  "HomeAdvisor and Angie's List Approved",
                  "Atlas, Owens Corning, CertainTeed, Lomanco Materials",
            ...
            <div className="badge-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 20 }}>
              {[
                { icon: Icons.award, title: "BBB Accredited", desc: "J Z Remodeling LLC is accredited by the Better Business Bureau. A commitment to quality and accountability on every project." },
                { icon: Icons.shield, title: "Premium Materials", desc: "Atlas, Owens Corning, CertainTeed, and Lomanco. Every roof is built with manufacturer-backed materials that last." },
                { icon: Icons.home, title: "Full-Service Contractor", desc: "Roofing, remodeling, bathrooms, fencing, porches, and gutters. One crew for every project on your home." },
                { icon: Icons.clock, title: "Open 7 Days", desc: "Available every day of the week, 8AM to 8PM. When you need work done, JZ Roofing works around your schedule." },
              ].map((b, i) => (
      ...
      <section id="reviews" className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)" }}>
        <div style={{ maxWidth: 800, margin: "0 auto", textAlign: "center" }}>
          ...
          {/* Large featured review — DeLana Chance */}
          <div style={{ position: "relative", padding: "48px 40px", background: "var(--white)", borderRadius: 12 }}>
            ...
            <p style={{ fontSize: 20, lineHeight: 1.8, color: "var(--text)", fontStyle: "italic", fontFamily: "'Playfair Display',serif", marginBottom: 28 }}>
              "J &amp; Z has worked on many projects for my house to include bathroom remodels/additions, screened in porches."
            </p>
            <p style={{ fontSize: 17, fontWeight: 700, color: "var(--primary)" }}>DeLana Chance</p>
            <p style={{ fontSize: 13, color: "var(--text-light)", marginTop: 4 }}>Google Review</p>
          </div>
          <p style={{ fontSize: 14, color: "var(--text-light)", marginTop: 32 }}>One of 39 five-star reviews on Google</p>
      ...
      <section className="section-pad" style={{ padding: "110px clamp(24px,5vw,64px)", background: "var(--white)", borderTop: "1px solid var(--border)" }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <h2 style={{ fontFamily: "'Playfair Display',serif", fontSize: "clamp(30px,3.5vw,44px)", lineHeight: 1.15, letterSpacing: -0.5, color: "var(--primary)", marginBottom: 20 }}>
            Need a roof or remodel?<br/><em style={{ color: "var(--accent)", fontStyle: "italic" }}>Call JZ Roofing.</em>
          </h2>
          <p style={{ fontSize: 17, lineHeight: 1.7, color: "var(--text-mid)", marginBottom: 40, maxWidth: 520, margin: "0 auto 40px" }}>
            Honest pricing, premium materials, and a crew that shows up seven days a week. From your roof to your bathroom, Jose Rodriguez and the JZ team have you covered.
          </p>
          <a href={PHONE_TEL} style={{ display: "inline-flex", alignItems: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 48px", borderRadius: 6, fontSize: 18, fontWeight: 700, letterSpacing: 0.3, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 4px 20px rgba(212,160,60,0.3)", transition: "transform 0.2s" }}>
            {Icons.phone} {PHONE}
          </a>
        </div>
      </section>
      ...
      <footer style={{ background: "var(--primary)", padding: "80px clamp(24px,5vw,64px) 40px", color: "rgba(255,255,255,0.75)" }}>
        <div className="footer-grid" style={{ maxWidth: 1200, margin: "0 auto", display: "grid", gridTemplateColumns: "1.4fr 1fr 1fr", gap: 48, marginBottom: 48 }}>
          ...
          <div>
            ...
            <p style={{ fontSize: 15, lineHeight: 1.7, marginBottom: 20, maxWidth: 300 }}>
              BBB Accredited roofing and remodeling contractor serving Little Rock and central Arkansas. Open 7 days a week. Trusted by 42+ homeowners.
            </p>
            <p style={{ fontSize: 14, marginBottom: 8 }}>6827 Honeysuckle Ln, Little Rock, AR 72204</p>
        ...
        <div style={{ borderTop: "1px solid rgba(255,255,255,0.1)", paddingTop: 24 }}>
          <div className="footer-btm-inner" style={{ maxWidth: 1200, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>{"\u00A9"} {new Date().getFullYear()} JZ Roofing Remodeling. All rights reserved.</p>
            <p style={{ fontSize: 13, color: "rgba(255,255,255,0.4)" }}>BBB Accredited | HomeAdvisor | Angie's List</p>
          </div>
        </div>
      </footer>
      ...
      <div className="mobile-only" style={{ position: "fixed", bottom: 0, left: 0, right: 0, zIndex: 998, display: "none" }}>
        <a href={PHONE_TEL} style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: 10, background: "var(--accent)", color: "#fff", padding: "18px 24px", fontSize: 16, fontWeight: 700, fontFamily: "'DM Sans',sans-serif", boxShadow: "0 -4px 20px rgba(0,0,0,0.15)", textAlign: "center" }}>
          {Icons.phone} Call JZ Roofing - {PHONE}
        </a>
      </div>
