...
const PHONE = "(501) 388-8920";
const TEL = "tel:+15013888920";
...
const Icons = {
  ...
  pipe: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M4 6h4v12H4z"/><path d="M8 10h8"/><path d="M8 14h8"/><path d="M16 6h4v12h-4z"/></svg>,
  gasLine: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2v4"/><path d="M12 18v4"/><circle cx="12" cy="12" r="6"/><path d="M12 8v4l2 2"/><path d="M4 12H2"/><path d="M22 12h-2"/></svg>,
  pump: <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M3 20v-8a4 4 0 014-4h2"/><path d="M15 8h2a4 4 0 014 4v8"/><rect x="9" y="4" width="6" height="8" rx="1"/><path d="M12 12v4"/><path d="M8 20h8"/><path d="M10 16h4"/></svg>,
...
const services = [
  { name: "Water Heater Service", desc: "Repair, replacement, and new installation of residential water heaters. Justin diagnoses the issue and walks you through every option before any work begins.", icon: Icons.flame },
  { name: "Drain Cleaning", desc: "Professional clearing of stubborn clogs and recurring backups in kitchens, bathrooms, and main lines. Fast response, thorough results.", icon: Icons.droplets },
  { name: "Sewer & Water Line", desc: "Full repair and replacement of sewer and water lines. Justin handles the toughest underground jobs with precision and honest pricing.", icon: Icons.pipe },
  { name: "Gas Line Service", desc: "Safe, licensed repair and installation of gas lines for dryers, fireplaces, stoves, and more. Never cut corners on gas work.", icon: Icons.gasLine },
  { name: "Well & Sump Pumps", desc: "Installation, repair, and maintenance of well pumps and sump pumps. Keeping your water flowing and your basement dry.", icon: Icons.pump },
  { name: "Plumbing Repair", desc: "Faucets, garbage disposals, pipe installation, and comprehensive plumbing repairs. One call to Justin covers it all.", icon: Icons.wrench },
];

const allServiceNames = ["Water Heater Repair & Replacement","Drain Cleaning","Water/Sewer Line Repair & Replacement","Gas Line Repair & Installation","Sump Pump Installation & Repair","Well Pump Service","Faucet Installation & Repair","Garbage Disposal Repair","Pipe Installation","Plumbing Inspections","Comprehensive Plumbing Repairs","New Plumbing Installations","Routine Maintenance"];

const areas = ["Little Rock","North Little Rock","Conway","Maumelle","Mayflower","Sherwood","Jacksonville","Benton","Bryant"];

const reviews = [
  { text: "Justin knew right off that the problem with my toilet was the flange. He went the extra mile and checked the sink pipes that I had installed myself and made sure he explained what I needed to look for if I had any problems.", name: "Vickie Davis", src: "Google" },
  { text: "Justin was quick to respond to my initial request for service to install our shut off valve for a gas dryer. He was very kind and efficient getting the job done exactly the way we needed with a full install.", name: "Dana Lankford", src: "Google" },
  { text: "Return customer here and still couldn't suggest a plumber of higher quality. Justin does it all! When I felt a little unsafe dealing with an old, outdated gas line fitting for a new dryer, Justin came over and had it fixed.", name: "C.J. White", src: "Google" },
  { text: "Justin came out to repair an outdoor sink leak the same day I called him. He still checked everything out and even looked at my gas fireplace because we've had issues with it too.", name: "Christy Rasico", src: "Google" },
  { text: "Justin did an AMAZING job, he came out and helped us with our water heater and went above and beyond to help us understand the problem and how to prevent it from happening again in the future.", name: "Kristen Crabhammer", src: "Google" },
];

const navLinks = [{label:"Services",id:"services"},{label:"About",id:"about"},{label:"Reviews",id:"reviews"},{label:"Areas",id:"areas"}];

export default function DavisPlumbing() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("davis-plumbing-auth") === "1");
  ...
  const submitPw = (e) => { e.preventDefault(); if(pw==="2026"){sessionStorage.setItem("davis-plumbing-auth","1");setAuthed(true)}else{setPwBad(true);setPw("")} };
  ...
  useEffect(() => {
    if(!authed) return;
    document.title = "Plumber Little Rock AR | Davis Plumbing Solutions";
    const els = [];
    ...
    add("meta",{name:"description",content:"Licensed plumber in Little Rock, AR. 4.8 stars, 17+ years experience. Call (501) 388-8920 for service."});
    ...
    add("script",{type:"application/ld+json",textContent:JSON.stringify({
      ...
      name:"Davis Plumbing Solutions",telephone:"+15013888920",
      address:{"@type":"PostalAddress",streetAddress:"30 Windsong Way",addressLocality:"Mayflower",addressRegion:"AR",postalCode:"72106",addressCountry:"US"},
      geo:{"@type":"GeoCoordinates",latitude:34.9668,longitude:-92.4073},
      openingHoursSpecification:[{"@type":"OpeningHoursSpecification",dayOfWeek:["Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday"],opens:"00:00",closes:"23:59"}],
      aggregateRating:{"@type":"AggregateRating",ratingValue:"4.8",reviewCount:"48",bestRating:"5"},
      founder:{"@type":"Person",name:"Justin Davis"},
      foundingDate:"2008",
      description:"Davis Plumbing Solutions is a licensed, bonded, and insured plumbing company serving Little Rock and Central Arkansas since 2008.",
      ...
      <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:"#1C2B41"}}>
        <div style={{background:"#fff",borderRadius:16,padding:"56px 44px",textAlign:"center",maxWidth:420,width:"92%",boxShadow:"0 32px 80px rgba(0,0,0,.35)"}}>
          <div style={{fontFamily:"'Playfair Display',serif",fontSize:28,marginBottom:8,fontWeight:600,letterSpacing:-0.5}}>
            <span style={{color:"#1C2B41"}}>Davis</span> <span style={{color:"#C8934F"}}>Plumbing</span>
          </div>
  ...
  return (<>
    ...
    <nav style={{position:"fixed",top:0,left:0,right:0,zIndex:1000,padding:scrolled?"14px clamp(24px,5vw,64px)":"20px clamp(24px,5vw,64px)",display:"flex",alignItems:"center",justifyContent:"space-between",background:scrolled?"rgba(28,43,65,0.97)":"transparent",backdropFilter:scrolled?"blur(12px)":"none",transition:"all 0.4s ease",boxShadow:scrolled?"0 1px 0 rgba(255,255,255,0.06)":"none"}}>
      <div className="serif" style={{fontSize:22,color:scrolled?"#fff":"var(--primary)",fontWeight:600,letterSpacing:-0.5}}>
        <span>Davis</span> <span style={{color:"var(--accent)"}}>Plumbing</span>
      </div>
      ...
      <div className="hero-grid" style={{maxWidth:1200,width:"100%",display:"grid",gridTemplateColumns:"1.8fr 1fr",gap:48,alignItems:"center"}}>
        <div>
          ...
          <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:32}}>
            <Stars />
            <span style={{fontSize:14,color:"var(--text-light)",fontStyle:"italic",marginLeft:4}}>"Return customer here and still couldn't suggest a plumber of higher quality." — C.J. White</span>
          </div>
          <a href={TEL} style={{display:"inline-flex",alignItems:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px 36px",borderRadius:6,fontSize:16,fontWeight:700,boxShadow:"0 4px 20px rgba(200,147,79,0.3)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Justin — {PHONE}</a>
        </div>
        <div>
          <div style={{background:"#fff",borderRadius:12,padding:"32px 28px",boxShadow:"0 4px 40px rgba(0,0,0,0.08)",border:"1px solid var(--border)"}}>
            {!formDone ? (
              ...
              <div style={{textAlign:"center",padding:"20px 0"}}>
                ...
                <p style={{fontSize:13,color:"var(--text-mid)"}}>Justin will call you back shortly.<br/>Call <a href={TEL} style={{color:"var(--accent)",fontWeight:700}}>{PHONE}</a> for emergencies.</p>
              </div>
            )}
          </div>
        </div>
    ...
    {/* SERVICES + TRUST — Layout D: Trust vertical sidebar left, masonry service cards right */}
    <section id="services" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--cream)"}}>
      <div style={{maxWidth:1200,margin:"0 auto"}}>
        ...
        <div className="trust-mobile" style={{display:"none",gridTemplateColumns:"1fr 1fr",gap:24,marginBottom:48,textAlign:"center"}}>
          {[
            {num:"4.8",label:"Google Rating",desc:"Highly rated"},
            {num:"48+",label:"Reviews",desc:"Real customers"},
            {num:"17+",label:"Years Experience",desc:"Since 2008"},
            {num:"MP#6892",label:"Licensed",desc:"Bonded & insured"},
          ].map((s,i)=>(
            <div key={i}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"200px 1fr",gap:48}}>
          ...
          <div className="trust-sidebar" style={{display:"flex",flexDirection:"column",gap:32,paddingTop:8}}>
            {[
              {num:"4.8",label:"Google Rating",desc:"Highly rated"},
              {num:"48+",label:"Reviews",desc:"Real customers"},
              {num:"17+",label:"Years Exp.",desc:"Since 2008"},
              {num:"MP#6892",label:"Licensed",desc:"Bonded & insured"},
            ].map((s,i)=>(
        ...
        <p style={{textAlign:"center",marginTop:48,fontSize:15,color:"var(--text-light)"}}>
          Questions? Call Justin directly at <a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a>
        </p>
      </div>
    </section>
    ...
    <section id="about" style={{padding:"110px clamp(24px,5vw,64px)",background:"var(--primary)",position:"relative",overflow:"hidden"}}>
      <div className="serif" style={{position:"absolute",top:"50%",left:"50%",transform:"translate(-50%,-50%)",fontSize:"clamp(120px,20vw,280px)",color:"var(--accent)",opacity:0.08,lineHeight:1,whiteSpace:"nowrap",pointerEvents:"none"}}>17+</div>
      <div style={{maxWidth:800,margin:"0 auto",position:"relative",zIndex:1}}>
        <div className="gold-line" style={{margin:"0 auto 20px"}} />
        <div style={{fontSize:12,fontWeight:700,textTransform:"uppercase",letterSpacing:3,color:"var(--accent)",marginBottom:14,textAlign:"center",opacity:0.7}}>About Davis Plumbing</div>
        <h2 style={{fontSize:"clamp(30px,3.5vw,42px)",lineHeight:1.15,marginBottom:28,color:"#fff",textAlign:"center"}}>Why Little Rock Trusts<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Justin Davis</span></h2>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:24,textAlign:"center"}}>
          Davis Plumbing Solutions is a one-man operation, and that is exactly why customers keep calling back. When you call Justin, you get Justin — not a dispatcher, not a random technician, not a subcontractor. He has been personally handling every plumbing job across Central Arkansas since 2008, building a reputation one honest repair at a time.
        </p>
        <p style={{fontSize:16,lineHeight:1.8,color:"rgba(255,255,255,0.6)",marginBottom:36,textAlign:"center"}}>
          With a 4.8-star Google rating and 17 years of experience, customers consistently praise Justin for going the extra mile. He does not just fix the problem — he explains what went wrong, shows you what to watch for, and makes sure you understand every step. That is the kind of service you only get from someone who puts his own name on the truck.
        </p>
        <div style={{display:"flex",flexWrap:"wrap",justifyContent:"center",gap:20}}>
          {["Family Owned Since 2008","Licensed MP#6892","Bonded & Insured","Open 24/7","Solo Operator — Justin Personally","4.8 Stars on Google"].map((c,i)=>(
    ...
    {/* CTA — White background with top border */}
    <section style={{padding:"100px clamp(24px,5vw,64px)",background:"var(--white)",textAlign:"center",borderTop:"1px solid var(--border)"}}>
      <div style={{maxWidth:600,margin:"0 auto"}}>
        <h2 className="serif" style={{fontSize:"clamp(32px,4vw,50px)",color:"var(--primary)",lineHeight:1.1,marginBottom:20,letterSpacing:-1}}>
          Got a plumbing problem?<br/><span style={{fontStyle:"italic",color:"var(--accent)"}}>Call Justin.</span>
        </h2>
        <p style={{fontSize:17,color:"var(--text-mid)",marginBottom:36,lineHeight:1.6}}>
          Licensed, bonded, and insured. 17 years of honest work and a 4.8-star reputation. Available 24/7.
        </p>
    ...
    <footer style={{background:"var(--primary)",color:"rgba(255,255,255,0.7)",padding:"64px clamp(24px,5vw,64px) 0"}}>
      <div className="footer-grid" style={{maxWidth:1100,margin:"0 auto",display:"grid",gridTemplateColumns:"1.5fr 1fr 1fr",gap:48,paddingBottom:40}}>
        <div>
          <div className="serif" style={{fontSize:22,color:"#fff",marginBottom:16,fontWeight:600,letterSpacing:-0.5}}><span>Davis</span> <span style={{color:"var(--accent)"}}>Plumbing</span></div>
          <p style={{fontSize:14,lineHeight:1.7,opacity:0.7,marginBottom:16}}>Licensed plumbing company serving Little Rock and Central Arkansas since 2008. Justin Davis personally handles every job.</p>
          <p style={{fontSize:14,lineHeight:1.8,opacity:0.6}}>30 Windsong Way<br/>Mayflower, AR 72106<br/><a href={TEL} style={{color:"var(--accent)",fontWeight:600}}>{PHONE}</a></p>
        </div>
        <div>
      ...
      <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",padding:"20px 0",maxWidth:1100,margin:"0 auto"}}>
        <div className="footer-btm-inner" style={{display:"flex",justifyContent:"space-between",fontSize:13,opacity:0.4,flexWrap:"wrap",gap:8}}>
          <span>&copy; {yr} Davis Plumbing Solutions. All rights reserved.</span>
          <span>Licensed MP#6892 &middot; Bonded & Insured</span>
        </div>
      </div>
    </footer>
    ...
    <div className="mobile-only" style={{position:"fixed",bottom:0,left:0,right:0,zIndex:998,display:"flex"}}>
      <a href={TEL} style={{display:"flex",width:"100%",alignItems:"center",justifyContent:"center",gap:10,background:"var(--accent)",color:"#fff",padding:"18px",fontSize:17,fontWeight:700,boxShadow:"0 -4px 20px rgba(0,0,0,0.15)",fontFamily:"'DM Sans',sans-serif"}}>{Icons.phone} Call Davis Plumbing</a>
    </div>
