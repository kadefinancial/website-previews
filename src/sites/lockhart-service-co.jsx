...
const Icons = {
  ...
  snowflake: (
    <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><line x1="12" y1="2" x2="12" y2="22"/><path d="M20 12l-4 4-4-4"/><path d="M4 12l4-4 4 4"/><line x1="2" y1="12" x2="22" y2="12"/><path d="M12 2l4 4-4 4"/><path d="M12 22l-4-4 4-4"/></svg>
  ),
  check: (
...
// --- Data ---
const PHONE = "(501) 664-0132";
const PHONE_TEL = "tel:+15016640132";

const services = [
  { name: "Air Conditioning Repair & Service", desc: "When Little Rock summers push past 100 degrees, your AC system needs to perform. Mr. Lockhart diagnoses problems quickly and gets your cooling back up and running at a price that's fair and honest.", icon: Icons.wind },
  { name: "Heating Repair & Service", desc: "From furnace failures to heat pump issues, Lockhart Service Co handles every heating problem with precision. Stay warm through Arkansas winters with reliable repairs from a licensed contractor you can trust.", icon: Icons.flame },
  { name: "HVAC Installation", desc: "Whether you're upgrading an aging system or outfitting a new home, Reggie and his son help you choose the right equipment and install it properly. Licensed, insured, and built on decades of experience.", icon: Icons.thermometer },
  { name: "Thermostat Repair & Replacement", desc: "A faulty thermostat can waste energy and leave rooms uncomfortable. We diagnose and repair or replace thermostats to restore proper climate control throughout your home.", icon: Icons.shield },
  { name: "Refrigeration Services", desc: "Beyond residential comfort, Lockhart Service Co provides professional refrigeration services for commercial and residential applications. Reliable diagnostics, honest pricing, and expert repairs.", icon: Icons.snowflake },
  { name: "Residential HVAC", desc: "Complete heating, ventilation, and air conditioning service for homes across Little Rock and Central Arkansas. One call to Mr. Lockhart covers everything your home comfort system needs.", icon: Icons.home },
];

const allServices = [
  "Air Conditioning Repair & Service", "Heating Repair & Service", "HVAC Installation",
  "Thermostat Repair & Replacement", "Refrigeration Services", "Residential HVAC"
];

const areas = ["Little Rock", "Central Arkansas"];

// --- Main Component ---
export default function LockhartServiceCo() {
  const [authed, setAuthed] = useState(() => sessionStorage.getItem("lockhart-auth") === "1");
  ...
  const submitPw = (e) => {
    e.preventDefault();
    if (pw === "roof") { sessionStorage.setItem("lockhart-auth", "1"); setAuthed(true); }
  ...
  useEffect(() => {
    if (!authed) return;
    document.title = "HVAC Service Little Rock AR | Lockhart Service Co";
    const els = [];
    ...
    add("meta", { name: "description", content: "Lockhart Service Co — perfect 5.0 stars. Licensed HVAC contractor #0151247. BBB Accredited. Little Rock, AR. Call (501) 664-0132." });
    ...
    add("script", { type: "application/ld+json", textContent: JSON.stringify({
      ...
      name: "Lockhart Service Co", telephone: "+15016640132",
      address: { "@type": "PostalAddress", streetAddress: "8707 Kanis Rd", addressLocality: "Little Rock", addressRegion: "AR", postalCode: "72204", addressCountry: "US" },
      geo: { "@type": "GeoCoordinates", latitude: 34.7210, longitude: -92.3699 },
      openingHoursSpecification: [
        { "@type": "OpeningHoursSpecification", dayOfWeek: ["Monday","Tuesday","Wednesday","Thursday","Friday"], opens: "09:00", closes: "17:00" }
      ],
      aggregateRating: { "@type": "AggregateRating", ratingValue: "5.0", reviewCount: "6", bestRating: "5" },
      founder: { "@type": "Person", name: "Reginald Lockhart" },
      description: "Lockhart Service Co — father and son HVAC company in Little Rock, AR. Licensed contractor #0151247, BBB Accredited since 2014. Air conditioning, heating, refrigeration, and thermostat services.",
        ...
        <div style={{ minHeight:"100vh", display:"flex", alignItems:"center", justifyContent:"center", background:"#1C2B41" }}>
          <div style={{ background:"#fff", borderRadius:16, padding:"56px 44px", textAlign:"center", maxWidth:420, width:"92%", boxShadow:"0 32px 80px rgba(0,0,0,.35)" }}>
            <div style={{ fontFamily:"'Playfair Display',serif", fontSize:28, fontWeight:600, letterSpacing:-0.5, marginBottom:4 }}>
              <span style={{ color:"#1C2B41" }}>Lockhart</span>{" "}<span style={{ color:"#C8934F" }}>Service Co</span>
            </div>
    ...
    <>
      ...
      <nav style={{ position:"fixed", top:0, left:0, right:0, zIndex:1000, padding: scrolled ? "14px clamp(24px,5vw,64px)" : "20px clamp(24px,5vw,64px)", display:"flex", alignItems:"center", justifyContent:"space-between", background: scrolled ? "rgba(28,43,65,0.97)" : "transparent", backdropFilter: scrolled ? "blur(12px)" : "none", transition:"all 0.4s ease", boxShadow: scrolled ? "0 1px 0 rgba(255,255,255,0.06)" : "none" }}>
        <div className="serif" style={{ fontSize:22, fontWeight:600, letterSpacing:-0.5, color:"#fff" }}>
          <span>Lockhart</span>{" "}<span style={{ color:"var(--accent)" }}>Service Co</span>
        </div>
        ...
        <div className="hero-grid" style={{ maxWidth:1200, width:"100%", display:"grid", gridTemplateColumns:"1fr 1fr", gap:64, alignItems:"center" }}>
          ...
          <div style={{ maxWidth:560 }}>
            <div style={{ display:"inline-flex", alignItems:"center", gap:8, background:"rgba(255,255,255,0.1)", padding:"8px 18px", borderRadius:4, marginBottom:28 }}>
              ...
              <span style={{ fontSize:12, fontWeight:700, letterSpacing:3, textTransform:"uppercase", color:"var(--accent)" }}>Perfect 5.0 on Google</span>
            </div>
            ...
            <p style={{ fontSize:18, lineHeight:1.7, color:"rgba(255,255,255,0.75)", marginBottom:32, maxWidth:480 }}>
              Reginald "Reggie" Lockhart and his son have been keeping Little Rock comfortable for over a decade. Licensed contractor #0151247, BBB Accredited since 2014, and backed by a perfect 5.0 Google rating. Honest pricing, expert work.
            </p>
            {/* Star row with hero quote -- showing 5 stars for this specific 5-star review */}
            <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:32, flexWrap:"wrap" }}>
              <Stars />
              <span style={{ fontSize:14, color:"rgba(255,255,255,0.7)", fontStyle:"italic" }}>"He got it back running again...affordable" -- Blue Anderson</span>
            </div>
      ...
      <section style={{ background:"var(--white)", padding:"48px clamp(24px,5vw,64px)" }}>
        <div className="trust-grid" style={{ maxWidth:1200, margin:"0 auto", display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:32, textAlign:"center" }}>
          {[
            ...
            { num: "BBB", label: "Accredited", desc: "Since 2014" },
            { num: "Licensed", label: "Contractor", desc: "#0151247" },
            { num: "Family", label: "Father & Son", desc: "Team you can trust" },
          ].map((s, i) => (
      ...
      <section id="services" className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          ...
          <p style={{ textAlign:"center", marginTop:48, fontSize:15, color:"var(--text-mid)" }}>
            Questions? Call Mr. Lockhart directly at <a href={PHONE_TEL} style={{ color:"var(--accent)", fontWeight:600 }}>{PHONE}</a>
          </p>
        </div>
      </section>
      ...
      <section id="about" className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--cream)" }}>
        <div style={{ maxWidth:1200, margin:"0 auto" }}>
          <div style={{ textAlign:"center", marginBottom:64 }}>
            ...
            <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(30px,3.5vw,44px)", lineHeight:1.15, letterSpacing:-0.5, color:"var(--primary)" }}>The Lockhart Family Story</h2>
          </div>
          <div className="why-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:64, alignItems:"start" }}>
            ...
            <div>
              <p style={{ fontSize:17, lineHeight:1.8, color:"var(--text-mid)", marginBottom:20 }}>
                Reginald "Reggie" Lockhart built Lockhart Service Co on a simple principle: treat every home like your own. As a father and son company rooted in Little Rock, they bring a personal touch that larger outfits simply can't match. When you call Lockhart Service Co, you're not getting a dispatcher or a call center -- you're getting Reggie himself.
              </p>
              <p style={{ fontSize:17, lineHeight:1.8, color:"var(--text-mid)", marginBottom:32 }}>
                With Arkansas HVAC/Mechanical Contractor license #0151247 and BBB Accreditation since 2014, Lockhart Service Co has earned the trust of homeowners across Central Arkansas. From air conditioning repairs to full system installations, Reggie and his son deliver honest diagnostics, fair pricing, and work that's done right the first time. Over a decade of accredited service -- and a perfect 5.0 Google rating -- speaks for itself.
              </p>
              ...
              <div style={{ position:"relative", borderLeft:"3px solid var(--accent)", paddingLeft:24, marginBottom:36 }}>
                ...
                <p style={{ fontSize:18, fontStyle:"italic", lineHeight:1.7, color:"var(--primary)", fontFamily:"'Playfair Display',serif" }}>
                  "I have used this Father and son company for years. They are wonderful!"
                </p>
                <p style={{ fontSize:14, color:"var(--text-light)", marginTop:8, fontFamily:"'DM Sans',sans-serif" }}>-- Diana Atkinson</p>
              </div>
              {/* Credentials */}
              <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
                {[
                  ...
                  "Arkansas Contractor License #0151247",
                  "Father and Son Operated",
            ...
            <div className="badge-grid" style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:20 }}>
              {[
                { icon: Icons.users, title: "Father & Son Team", desc: "Reggie Lockhart and his son run every job personally. When you call Lockhart Service Co, you're talking to family -- not a call center." },
                { icon: Icons.award, title: "BBB Accredited", desc: "Better Business Bureau Accredited since 2014. Over a decade of verified trust and commitment to honest business practices." },
                { icon: Icons.shield, title: "Licensed Contractor", desc: "Arkansas HVAC/Mechanical Contractor license #0151247. Fully licensed, fully insured, fully accountable for every job." },
                { icon: Icons.heart, title: "Community Rooted", desc: "Born and raised in Little Rock. Lockhart Service Co is invested in the comfort and well-being of Central Arkansas homeowners." },
              ].map((b, i) => (
      ...
      <section id="reviews" className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--white)" }}>
        <div style={{ maxWidth:800, margin:"0 auto", textAlign:"center" }}>
          ...
          {/* Large featured review -- Blue Anderson */}
          <div style={{ position:"relative", maxWidth:700, margin:"0 auto" }}>
            ...
            <p style={{ fontSize:"clamp(18px,2.5vw,22px)", lineHeight:1.7, color:"var(--text)", fontStyle:"italic", fontFamily:"'Playfair Display',serif", marginBottom:28, maxWidth:600, margin:"0 auto 28px" }}>
              "Mr Lockhart was amazing my air conditioning went out and he got it back running again also fixed my thermostat all for a price that was affordable..I definitely recommend him on any air and heating problems."
            </p>
            <p style={{ fontSize:17, fontWeight:700, color:"var(--primary)", marginBottom:4 }}>Blue Anderson</p>
            <p style={{ fontSize:14, color:"var(--text-light)" }}>Google Review</p>
          </div>
          <p style={{ fontSize:14, color:"var(--text-light)", marginTop:40 }}>One of our five-star reviews</p>

          {/* Second review -- Diana Atkinson */}
          <div style={{ maxWidth:600, margin:"48px auto 0", padding:"36px 30px", background:"var(--cream)", borderRadius:12, border:"1px solid var(--border)" }}>
            ...
            <p style={{ fontSize:15, fontWeight:700, color:"var(--primary)", marginBottom:2 }}>Diana Atkinson</p>
      ...
      {/* CTA -- white with top border, "Call Mr. Lockhart" */}
      <section className="section-pad" style={{ padding:"110px clamp(24px,5vw,64px)", background:"var(--white)", borderTop:"1px solid var(--border)" }}>
        <div style={{ maxWidth:700, margin:"0 auto", textAlign:"center" }}>
          <h2 style={{ fontFamily:"'Playfair Display',serif", fontSize:"clamp(30px,3.5vw,44px)", lineHeight:1.15, letterSpacing:-0.5, color:"var(--primary)", marginBottom:20 }}>
            AC or heating trouble?{" "}<br/>
            <em style={{ color:"var(--accent)", fontStyle:"italic" }}>Call Mr. Lockhart.</em>
          </h2>
          <p style={{ fontSize:17, lineHeight:1.7, color:"var(--text-mid)", marginBottom:36, maxWidth:520, margin:"0 auto 36px" }}>
            Licensed contractor #0151247, BBB Accredited since 2014, and backed by a perfect 5.0 Google rating. Father and son service for Little Rock and Central Arkansas. Honest pricing, expert work.
          </p>
      ...
      <footer style={{ background:"var(--primary)", padding:"80px clamp(24px,5vw,64px) 40px", color:"rgba(255,255,255,0.75)" }}>
        <div className="footer-grid" style={{ maxWidth:1200, margin:"0 auto", display:"grid", gridTemplateColumns:"1.4fr 1fr 1fr", gap:48, marginBottom:56 }}>
          ...
          <div>
            <div className="serif" style={{ fontSize:22, fontWeight:600, letterSpacing:-0.5, marginBottom:16 }}>
              <span style={{ color:"#fff" }}>Lockhart</span>{" "}<span style={{ color:"var(--accent)" }}>Service Co</span>
            </div>
            <p style={{ fontSize:15, lineHeight:1.7, marginBottom:20, maxWidth:300 }}>
              Father and son HVAC company serving Little Rock and Central Arkansas. Licensed, BBB Accredited, and committed to honest service.
            </p>
            <p style={{ fontSize:14, lineHeight:1.8, marginBottom:8 }}>8707 Kanis Rd, Little Rock, AR 72204</p>
          ...
          <div>
            ...
            <div style={{ marginTop:24 }}>
              ...
              <p style={{ fontSize:14, color:"rgba(255,255,255,0.65)", lineHeight:1.7 }}>Mon-Fri: 9 AM - 5 PM</p>
              <p style={{ fontSize:14, color:"rgba(255,255,255,0.65)", lineHeight:1.7 }}>Sat-Sun: Closed</p>
            </div>
          </div>
        </div>
        {/* Bottom bar */}
        <div style={{ borderTop:"1px solid rgba(255,255,255,0.1)", paddingTop:28 }}>
          <div className="footer-btm-inner" style={{ maxWidth:1200, margin:"0 auto", display:"flex", justifyContent:"space-between", alignItems:"center", fontSize:13, color:"rgba(255,255,255,0.45)" }}>
            <span>Lockhart Service Co. All rights reserved.</span>
            <span>BBB Accredited | License #0151247</span>
          </div>
        </div>
      </footer>
